...
// same rows the browser fixture serves): 94 distinct eligible listings over
// 4 pages, cross-marketplace copies that disagree, distinct grades,
// auctions, unknown shipping, plain listings, and rows that must never show
// (quarantined, slab-titled raw, inactive, stale, Japanese, ended auction).
...
test("AD-1. every eligible listing appears exactly once across all pages, and the count is exact", () => {
  ...
  assert.equal(ALL_DEALS_ELIGIBLE_LISTING_IDS.length, 94);
  assert.equal(first.totalCount, 94);
  assert.equal(first.totalPages, 4);
  assert.equal(first.exact, true);
  assert.deepEqual(pages.map((p) => p.deals.length), [24, 24, 24, 22]);
  ...
  for (const p of pages) assert.equal(p.totalCount, 94, "every page reports the same total");
});

test("AD-2. quarantined, slab-titled raw, inactive, stale, Japanese and ended rows never appear or count", () => {
...
test("AD-5. distinct grades of the same card stay separate listings, and grading filters count exactly", () => {
  ...
  assert.equal(count({ cardType: "raw" }), 91);
...
test("AD-6. marketplace scope counts a multi-marketplace listing once in each of its marketplaces", () => {
  ...
  assert.equal(sum, 94 + 2 + 1 + 1, "3-copy listing adds 2, the two 2-copy listings add 1 each");
  // an invalid country is ignored, not an empty result
  assert.equal(inv.queryAllDeals(CHUNKS, { country: "EBAY_XX" }).totalCount, 94);
...
test("AD-7. buying format, price and search filters count exactly across pages", () => {
  ...
  check({ listingType: "BIN" }, 90);
  check({ listingType: "FIXED_PRICE" }, 90);
  const { deals } = allPages();
  ...
  check({ q: "zekrom" }, 14);
  check({ q: "ZEKROM" }, 14);
  check({ q: "z" }, 94); // under 2 characters: ignored, never a silent partial match
...
test("AD-8. sorts change order only: same identities and count, deterministic, ordered across page boundaries", () => {
  ...
  for (const sort of ["newest", "discount", "price_asc", "price_desc", "ending"]) {
    ...
    assert.equal(a.first.totalCount, 94, sort);
...
test("AD-9. out-of-range and malformed pages", () => {
  ...
  assert.equal(past.totalCount, 94);
...
test("AD-14. loader, API and grid wiring", () => {
  ...
  assert.match(deals, /unstable_cache\(fetchAllDealsMarketplaceUncached, \["all-deals-inventory-v1"\], \{\s*revalidate: POOL_REVALIDATE_SECONDS/);
  assert.match(deals, /\.eq\("is_active", true\)\s*\.eq\("marketplace", marketplace\)\s*\.eq\("card_language", "english"\)/);
