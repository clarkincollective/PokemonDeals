...
test("C2-9. the page: pilot set swaps the plain index for the checklist; every other set keeps the index; routes/canonical/robots untouched", () => {
  ...
  assert.match(page, /\{checklistPilot \? \(\s*<SetChecklist setName=\{resolved\.set\} cards=\{checklistCards\} headingId="full-set-index" compact \/>\s*\) : \(\s*<CatalogueLinkIndex label=\{setLabel\} cards=\{catalogueIndexItems\} headingId="full-set-index" \/>\s*\)\}/);
