...
- Not done: `pokemon-booster-box-prices` has no card subject; a sealed-product figure would need a verified sealed identity registry (the `sealed_catalog` table has TCGplayer product images) and a tile that links `/sealed-deals`. Deferred; small, but a new component rather than a reuse.
...
## Blockers
- Search Console API token is read-only (no sitemap resubmission / removal, no indexing API); UI actions done by hand when authorised. Open owner action: remove the direct `/sitemaps/cards-bulk.xml` submission (Sitemaps → ⋮ → Remove) so the crawl-focus change takes full effect.
...
|---|---|---|---|
| 1 | Crawlability & technical baseline | Verified working; one open owner action | robots (explicit AI agents), sitemap index (`lib/sitemap.js`), canonicals, pagination `nofollow` beyond page 5 (`9d99d4a`), bulk shard unadvertised (`b2f3272`, 2026-09-19 22:06 UTC). Crawl stats read 20 Sep (above). Open: direct bulk sitemap submission. |
