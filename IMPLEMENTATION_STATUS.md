...
### Deployment (2026-09-15): Browse budget pacing live in OBSERVE mode at `8b21599`
...
## PokemonPriceTracker request telemetry (PPT-TELEMETRY-R1), 2026-09-15
**Purpose.** Measure outbound PokemonPriceTracker request attempts by consumer before any release that could raise them. Records requests and attempts only, not credits, cost or billing headroom. The documented Business-tier 200,000 credits/day allowance remains unverified. No cap, refusal or behaviour change.

**Instrumented path (`lib/pokemonPriceTracker.js`):** every outbound `fetch` in the shared client: `fetchPPT` (all 16 call sites name their lib function), `fetchPPTPaced` (each retry is its own attempt) and `downloadPrintingsExport`. Per attempt, after it settles:
- consumer, endpoint family (`cards`, `cards+ebay`, `cards+ebay+history`, `sets`, `sealed-products`, `export`, ...), lib function
- outcome `ok` / `r429` (with the 429 body's `limitType` when named) / `failed` / `network_error`; retries marked
- standard `x-ratelimit-*` / `retry-after` values only if returned as numbers
- never the API key, query values, response bodies or personal data

A missing API key still throws before any request and is not counted. Results, thrown errors, retries, backoff and caching are unchanged. Cache hits make no outbound request and are not counted.

**Consumer attribution:** explicit tag, else the eBay job run's `job`, else `unknown`. Tags live in an AsyncLocalStorage store (`withPptConsumer` = scoped run, `setPptConsumer` = rest of the current request): request-local, not module state. A concurrent check with differently tagged requests (scoped, and from separate request roots) shows no mixing.
...
- From the eBay job context: `refresh-deals:sweep`, `refresh-deals:allocated`, `refresh-sealed-deals`, other `beginJobRun` routes.
- `unknown`: any other caller of the shared client (e.g. manual scripts using it).

**Storage and response latency (existing `catalog_snapshot`, no migration, insert-only):**
- Inside an eBay job run (cron routes): counts stay on the run context; `finishJobRun` inserts ONE row after the `ebay_job_runs` row, in its own try/catch.
- Elsewhere (pages, card search, catalogue syncs): one row per attempt, **persisted after the response**. `scheduleAfterResponse` hands the write to Next's `after()` (on Vercel backed by `waitUntil`, which keeps the invocation alive until the task settles). Where Next runs work without a request scope (e.g. background cache revalidation) it uses the request context's `waitUntil` directly (`globalThis[Symbol.for("@next/request-context")]`, the primitive `after()` uses). The page or API response never waits for the insert. Only where neither exists (scripts, tests) is the write awaited, bounded at 1.5 s. No untracked fire-and-forget promise.
- kind `ppt_requests:<UTC day>:<ISO time>:<random>`; no shared read-modify-write counter. Pruning deletes only rows with the `ppt_requests:` prefix older than 35 days (at most hourly per instance, from job-run flushes).
- Recording failures (database down, insert error, hung write, failed scheduled task) are swallowed.

**Reading daily totals:** `node scripts/pptRequestReport.mjs [YYYY-MM-DD] [--json]` (read-only) sums the day's rows by consumer / endpoint / function as **recorded attempts** with ok / 429 / failed / network / retries, and prints these limitations with every report:
- Excluded: `scripts/ppt-history-backfill.mjs` and `scripts/auditMissingIds.js` call the provider with their own `fetch` (not changed in this release).
- Can under-count: a process that dies before its job-run row or scheduled write persists, or a write that fails or exceeds 1.5 s, leaves no record.
- A job run spanning UTC midnight is assigned to its finish day. Today's figure is partial.
...
## Cache propagation of confirmed retirements (CACHE-RETIRE-R1), 2026-09-15
**Gap (traced).** Detection delay is unchanged: a sale is only known after a normal availability check. The cache delay after a confirmed database change came from two independent layers:
- Data caches (`unstable_cache`): pools and All deals 180 s, set / species catalogues 900 s. A regenerating ISR page waits for fresh data only when the data entry is already stale; an entry still inside its window is reused.
- Page ISR: `/deals` 600 s, set and species pages 3600 s, `/cards/[slug]` 3600 s. `/deals/[id]` renders per request over its own data caches.
- Deal 38057 (02:49:34): `/sets/ex-unseen-forces` regenerated at ~02:55 from set-catalogue data built before the quarantine and cached that HTML for another hour (worst case about data window + page window, ~75 min).

**Existing invalidation (kept):** verify-deals and ingest-feed already expire `card-offers:*` (card offers data + `/cards/[slug]` HTML) and `deal-detail:<id>` for rows they retire.

**Correction.** Surface tags on the cached loaders, expired after successful visibility-changing writes, once per tag per invocation:
...
- `all-deals:<marketplace>`: `fetchAllDealsMarketplace`.
- `set-deals:<set>`: `fetchSetCatalog`, `fetchSetDealsPage`, the single-set `fetchDealsPage` sample in `/sets/[slug]` metadata.
- `species-deals:<species>`: `fetchSpeciesCatalog`, `fetchSpeciesDealsPage`, `fetchSpeciesDealStats`.
- Not tagged: `fetchRelatedActiveDeals`, hubs / aggregates, sealed surfaces, market-data pages.

**Covered paths (this release only):**
...
| `scripts/remediation/unownIdentityQuarantine.mjs` apply / rollback (queued, drained by sweep-stale-deals) | new | new | new | new (`deal-detail`) | not included |

**Not covered (normal cache windows still apply):** freshness-TTL and per-card grace expiries, ingest-feed feed-only grace expiry, image and visual-authenticity screening verdicts, all other quarantine / review-hold / remediation scripts (integrity-r1, language mismatch, review hold), any manual database edit, sealed deals, and card pages for ended auctions and script quarantines. This release does not cover all quarantine, screening, freshness or sealed-item changes.

**Safeguards.** Tags come only from rows whose write succeeded; UNKNOWN verdicts, failed provider requests and failed writes add nothing. Quarantine reasons and guarded writes are unchanged. `RETIRE_RETURN_COLS` also returns `marketplace, card_name, card_set, card_language`. Invalidation makes no eBay or PPT request.

**Actual Next cache semantics (local production build, 2026-09-15).** `next build` + `next start` of this branch against a local PostgREST stand-in with fixture rows (no provider keys, clean `.next`), journey on `/sets/ex-unseen-forces`:
1. Warm: first request `MISS` (renders, 10 database reads), second `HIT` (0 reads); `/deals`, both card pages `HIT` after warm-up.
2. Database-only quarantine of the Unown (M) row: set page and `/deals` still `HIT` with the offer (the 38057 gap, reproduced).
3. Real helper `queueCacheInvalidation` (5 tags), then the real `/api/sweep-stale-deals` cron: `queuedInvalidation { rows: 1, expired: 5, acknowledged: 1, retained: 0 }`.
4. First set-page request after the cron response: `MISS`, rendered synchronously from fresh reads (set catalogue and deal queries re-run), offer gone, same-set Unown (R) kept; second request `HIT` with the fresh page. `/deals`: `MISS` re-reading only the `EBAY_US` chunk, then `HIT`.
5. Both card pages stayed `HIT` with 0 database reads and 0 pricing attempts (the quarantined card's page still shows the offer, as documented). The active deal page made 0 pricing attempts; its only read was its own 60 s deal-data window elapsing. The quarantined deal page rendered "cannot currently be shown" (noindex) with 0 pricing attempts.
- API and options: `revalidateTag(tag, { expire: 0 })` via `expireTags`. Next records the tag (`pendingRevalidatedTags`) and applies it when the route handler finishes (`executeRevalidates` in `pendingWaitUntil`); no completion result reaches the handler. After that, the first page request blocks for fresh data rather than receiving a stale copy. A page request racing the cron in the moment before the tag is applied can still get the previous copy; Vercel's edge propagation was not reproduced locally.
- Indirect dependencies: a static call graph from every loader `/cards/[slug]` and `/deals/[id]` import reaches none of the newly tagged loaders; nested `unstable_cache` tags do not attach to the outer page. Confirmed at runtime by step 5.

**Queue reliability (`drainCacheInvalidationQueue`, called on every sweep-stale-deals run, whether or not anything was retired):**
- Only rows read in that drain are considered; a row queued during the drain survives for the next run.
- Each distinct tag is expired once; a row is deleted only when all of its tags were accepted by `revalidateTag`; a row with any failed tag is retained and retried next run; a failed delete retains the rows (their tags are expired again next run).
- A failed queue read reports the error and removes nothing (seen locally when the stand-in was down: `errors: ["TypeError: fetch failed"]`).
- The script reports a failed enqueue: after a successful database write it prints `cache invalidation was NOT queued` and exits 3, instead of implying prompt propagation.
- "Accepted" is the strongest signal available: Next applies the expiry after the cron response and does not report its outcome to the handler.

**Expected visibility delay after a confirmed write:**
- verify-deals / ingest-feed / sweep-stale-deals ended auctions: at the end of that invocation; the next request to an affected page regenerates it from fresh data.
- Unown script quarantine / rollback: up to the next successful sweep-stale-deals run (every 30 min), **assuming that cron succeeds**; if it fails or is skipped, the queued rows wait for the next successful run, and pages fall back to their normal cache windows.
- Card pages for ended auctions and script quarantines: unchanged (card offers data 180 s, page ISR 3600 s).

**Rendering and pricing-request implications:** lists, All deals, set and species regeneration reads only the database. A retired deal's page renders its unavailable state without pricing. Card pages re-request pricing whenever they regenerate; this release does not add card-page expiry anywhere new, and card-page pricing behaviour is unchanged.

**Offline scenario** (`tests/harness/cache/retirementScenario.mjs`, tag-aware model of Next's cache with the real routes and loaders): database-only change still served; verify-deals SOLD removed; UNKNOWN and failed write unchanged with no tags; queued quarantine removed only after the drain; ended auction propagated, freshness expiry not; no provider call from invalidation.

**Checks:** `cache-retire-r1` 9 / 9 (incl. queue acknowledgement: failed expiry retained, late row survives, failed delete retained, drain unconditional, failed enqueue reported) and `unown-identity-quarantine` 4 / 4; related suites (sold-item freshness, all-deals-r1, r6 category currency, integrity follow-up r2, language mismatch quarantine) pass apart from the pre-existing baseline; `next build` exit 0.

## Homepage caching r1 + Search Console actions, 2026-09-16

**Homepage static again (`77cde84`, production `dpl_ASGnhQy5g8Z9m2G6ZHVXyRMoz64m`).** `app/page.js` read `searchParams` (country/sort/filters/page), which made every homepage request a dynamic render - and RegionRedirect writes `?country=` into the URL for almost every real visitor right after hydration. The feed, mode row, explore section and analytics mount moved to `components/HomeFeed.js` (client; reads `window.location.search` through `useSyncExternalStore`, the DealGrid shape) and fetch non-default states from a new `kind=home` branch of `/api/deals-page`, which runs the exact server logic the page used to run. `app/page.js` renders only the default view; `next build` lists `/` as static (3 m revalidate). Every `/?…` variant canonicalises to `/` and gets `X-Robots-Tag: noindex, follow` from `next.config.mjs` (`HOME_VARIANT_PARAMS`, the `/deals` mechanism) - the earlier per-page self-canonical + `<meta robots>` for `?page=N` is gone. A cold navigation to a variant URL shows a skeleton until hydration (same guard as DealGrid). Verified live: `/?page=2` and `/?country=EBAY_GB` carry the header, canonical `/`, `X-Vercel-Cache: HIT` on the country variant. Follow-up in the same day: the trust line ("N live deals · checked X ago") is built in `app/page.js` and passed to HomeFeed as an element - a client module must not call `timeAgo()` (relative-time-hydration test 7).

Tests updated for the split (homepage-hierarchy, homepage-browse-conversion, homepage-variety, analytics-homepage, claims-consistency, deal-first-review-fixes, email-return-loop-crm1, marketplace-broaden-r1, seo-17b-foundation, gsc2-crawl-hubs; seo2-title-lifecycle test 17 rewritten for the canonical-to-`/` policy plus a config test for the header rule). Scanner suite 3,521 / 3,561 - the 37 failures are the pre-existing baseline (AuctionPrice, /search guide, browse-budget-postgres, social pipeline, P043 reserve, GitHub Actions runner…), none in the homepage files.

**Deploy note.** The GitHub push of `77cde84` produced no Vercel deployment (integration confirmed connected; the next push built normally), so it was deployed with `npx vercel deploy --prod --yes`.

**Search Console (UI, via Chrome, 2026-09-15 21:00–22:30 UTC).** Overview: 5 clicks; Pages report 1,801 indexed / 24,964 not indexed, of which 24,683 "Discovered – currently not indexed", 238 alternate-canonical, 29 noindex, 13 crawled-not-indexed, 1 redirect - a crawl-scheduling picture, nothing blocked. All nine child sitemaps submitted individually (previously only the index): `pages` 36, `sets` 208, `pokemon` 906, `cards-high`, `cards-mid` 2,261, `cards-low` 4,576, `cards-bulk` 15,624, `deals` 1,239, `sealed-deals` 241 discovered. Indexing requested for `/cards`, `/pokemon`, `/deals`, `/sets/base-set`, `/pokemon/charizard`, `/cards/charizard-base-set`, `/deals/graded`, `/latest-releases`, `/guides/pokemon-card-grading-scale`, `/guides/how-to-check-pokemon-card-condition` (the first three and `/sets/base-set` and the grading guide were "URL is unknown to Google" beforehand; the grading guide read "URL is on Google" within the hour). Weekly reading and next Monday's check list: `docs/seo/gsc-weekly-2026-09-15.md`. Impressions fell from ~90/day to ~25/day from 4 Sep while average position improved from ~50 to ~9 - the trim after a new domain's first-crawl phase, starting before the 11–13 Sep releases.

**Price-drop lane (merged `1e5108e` after the owner ran the migration in the Supabase SQL editor on 16 Sep; columns verified present read-only before the merge).** Originally held on branch `price-drop-lane`: `supabase/price_drop_migration.sql`: `deals.previous_price`, `deals.price_dropped_at`, a `BEFORE UPDATE OF price` trigger that records a material drop (≥ 2 % and ≥ 0.50 in the listing currency, both rows active) and clears it on a rise, and a partial index. `/deals/price-drops` (`lib/dealCategories`, `filter: { priceDrop: true }`, default sort `price_drop` = newest drop first, 7-day window in `lib/deals` `PRICE_DROP_WINDOW_DAYS`); DealCard states "Seller reduced the item price from X" - plain text, no strikethrough (deal-first R1). DealCategoryPage now passes the category's `defaultSort` to DealGrid, so a filtered/paged fetch on `/deals/auctions` keeps "ending" instead of falling back to newest. The Supabase dashboard step was blocked by the session's permission classifier; the category must not go live before the SQL has run (the query would fail on the missing column and the sitemap would list the route).

**Same evening, later.** Second URL-Inspection batch (all confirmed): `/market-data`, `/deals/uk`, `/deals/australia`, `/deals/canada`, `/deals/usa`, `/pokemon/pikachu`, `/pokemon/umbreon`, `/best-finds`, `/deals/auctions`; `/sets`, `/guides`, `/methodology`, `/sets/sv10-destined-rivals`, `/deals/under-25` were already on Google. IndexNow (`7280650`): `public/7a55ee1154e991b2366b89fcc47ed41e.txt` + `scripts/indexnowSubmit.mjs` (`npm run seo:indexnow`); all 26,319 sitemap URLs accepted by api.indexnow.org (three batches, HTTP 200) - Bing / DuckDuckGo / Yandex only, Google does not participate. Enhancement reports: Breadcrumbs 3 valid / 0 errors; Product snippets and Merchant listings 0 items (card pages emit Product+Offer only with a live listing - expected); no Core Web Vitals field data yet. Production homepage QA in Chrome after `ef43893`: region redirect to `?country=EBAY_AU`, country-scoped feed, "checked 3 min ago" trust line, mode-chip navigation to `?listing=FIXED_PRICE` renders the filtered grid with the flagship row hidden. Deal detail pages already link their species and set hubs in-body (audit §18 #4 - no change needed).

**Bing Webmaster Tools (owner signed in, 16 Sep).** The property already existed (imported from GSC 26 Aug; 3 clicks / 194 impressions on Bing), but Bing had not re-read the sitemap index since 26 Aug - before the SEO-3 card shards. Re-submitted the index and submitted all nine child sitemaps; `cards-high.xml` crawled within minutes (1.2K URLs). IndexNow submissions are attributed to the property (32.6K URLs in 23 h). BWT's own recommendation is the same as Google's: not enough inbound links.

**Still open from audit-r1.** Graded scanner change (observe window ends 17 Sep 07:00 UTC); digest (2 subscribers). Backlinks / outreach remain the actual crawl-budget lever and stay parked (Instantly trial ended 16 Sep).

## 30th Celebration collector's guide, 2026-09-16

**`/guides/pokemon-30th-celebration-guide`** (branch `article-30th-celebration`, merged to main the same day). An illustrated guide to the Pokemon TCG 30th-anniversary expansion, written against five official pages and this site's own catalogue, in the existing guide system (`lib/guides.js` registry, `GuideLayout` BreadcrumbList + Article JSON-LD, `/guides` hub under a new "New releases" group, `/latest-releases` identity section links it). What it states as fact and where each fact comes from: worldwide release 16 Sep 2026, all-foil packs, one of thirty Pikachu rares per pack, the Futuristic rare (YOSHIROTTEN) and the Classic Collection reprints (not Standard-legal) from the official expansion page and the UK announcement of 1 Jun 2026; the 14-product contents / release-wave table (September / October / November) from the UK product showcase of 30 Jun 2026 and, for the Elite Trainer Box, the US product page; collector numbers, the 023–052 Pikachu range, the illustration-rare and special-illustration-rare numbers and the Classic Collection list from the catalogue, labelled as such and never called complete. Sections: what the anniversary is and how the set differs from Celebrations (2021); release dates; the 30 Pikachu rares; Futuristic rares; illustration rares / special illustration rares / Pokemon ex; the Classic Collection (reprint vs the 1999 card, linked to the Base Set Charizard page); how main set / Classic Collection / product promos relate; every announced product; singles or sealed (no return or savings claims); identifying the printing in a listing (anniversary vs 2021, main set vs reprint, reprint vs vintage, language, box type, promos); checklist coverage (explicitly not complete); collector questions; sources and what is still unconfirmed.

**Artwork.** 24 catalogue card scans (`catalogImageUrl`, the TCGplayer product image every card page already uses - an approved source, complete card faces, no generated or reconstructed art), each `next/image` with fixed 717:1000 dimensions, `sizes`, lazy below the fold, descriptive alt (`<name> #<number> — <set>, complete card face`), linked to the card's own page through `lib/guideLinks` GUIDE_CARDS (24 new `c30*` entries; the guide-card-links test checks every href against the live catalogue). Illustrators are credited in the copy only where the official pages name them (YOSHIROTTEN; OKACHEKE, Yuu Nishida and Atsuko Nishida for the announcement's examples), never per card from memory. Two fixes after the Vercel preview QA (desktop and 390 px): adjacent citations rendered "source source" - now "(source: expansion page)" / "(sources: …)"; and the tile link was a centred block, so before a lazy image loaded the tile collapsed to zero and the card popped in - the link is full tile width so the width/height attributes reserve the box, and tiles cap at half a row on phones (two per row instead of a single column).

**Not confirmed on any official page at the time of writing** (the guide says so in its last section): the total number of cards in the set, the number of Classic Collection cards, the illustrator of each individual Pikachu rare, pull rates, any price, and an Australian product-wave schedule (the worldwide date is official; the local product calendar is not). No pricing-provider request was made for the article; every number on the page is from the catalogue table the card pages already render.

**Tests.** `tests/scanner/guide-card-links.test.mjs` (24 new verified hrefs, per-guide minimum, gallery count 20–30), `tests/seo/pages.test.mjs` (route added), `tests/seo/gsc3-grading-cluster.test.mjs` test 1 (guide count is now "at least 6" - later guides are legitimate). Scanner suite: same 37-failure baseline as before the branch, none new; SEO suite 21-failure baseline, none new.

**Reciprocal linking (`f6b25b9`, production `dpl_4YNdUftyDS9hpzSvPp9eKNYnKzVD`).** The guide sent 25 links into card pages and got none back, so the two highest-intent surfaces - the set page and the card pages themselves - never mentioned it; discovery was the guides hub and `/latest-releases` only. `lib/guides` entries gained an optional `sets` (the catalogue set names a guide is genuinely about) with `guideForSet` / `guideLinkLabel`; matching is on the exact set name, case-insensitive but never partial, so the anniversary guide cannot attach itself to the 2021 "Celebrations" set. `/sets/[slug]` shows a one-line link under the subtitle when its set has a guide, and `lib/cardNextSteps` adds a `guide` step above the price/era categories (Explore row and the no-live-deal module), with no live count and a label that cannot read as a deal; still capped at 4 links. Verified live: the 30th set page and its card pages (main set and Classic Collection) carry the link; Base Set, Celebrations and Base Set Charizard carry none. Four tests added to guide-card-links (exact-name matching, every claimed set is a real catalogue set name, the bounded no-claim next step, and the set page building the href from the registry rather than hand-typing it). Note: "ME: 30th Celebration Classic Collection" has no set page of its own, so that registry entry only drives its card pages - no dead link is produced either way.

## 30th Celebration release day: cluster, news, RGB Mew, and two gates that held real deals back (2026-09-16, later)

**Coverage cluster (`06140c0`, `0f6fe45`, `5e4b359`, `16d7d0a`).** Nine companion guides beside the anniversary guide (Pikachu checklist, Pikachu artwork picks, Classic Collection reprint-vs-original, ETB vs Pokemon Center ETB, promos by product, UK/US release-date comparison, Mew & Mewtwo + Futuristic rares, best cards beyond Pikachu, organising a collection) and `/news` (`bea0f49`: registry `lib/news.js`, hub, `NewsArticle` item pages, "News & Guides" header submenu keeping deal-first's three destinations; `scripts/newsWeeklyReport.mjs` ships fail-closed - 0 of 190 probed printings cleared the movement gate because provenance-complete price history only began 2026-09-11). `lib/guideLinks` carries 99 verified 30th identities generated from `card_catalog`; `components/guides/CardArt.js` + `Src.js` are the shared gallery/table/citation. Research findings that shaped copy: no official Classic Collection checklist exists; the official gallery is a selection with no illustrator credits; the UK and US showcases disagree on the Tech Sticker (Nov vs 16 Sep) and Binder (Sep vs 4 Dec) collections. The RGB Mew story (`/news/rgb-mew-30th-celebration-unconfirmed`, `d6ea26c` → `f37f70f`) runs owner-supplied photographs cropped to the cards and credited; the copies in circulation are AI-sharpened (the same green card reads "Psychic"/English body in one and "Psyche"/pseudo-script in another), which the article states and tells readers not to authenticate from. eBay's August removals under its Stolen Property Policy after TPCi's notice are verified across PokeBeach, Dexerto and Wargamer.

**Set display name (`a593986`).** `setDisplayName()` strips a leading all-caps release code for headings/title/breadcrumb/meta only; slugs and canonicals unchanged.

**Two gates that hid 60 real 30th Celebration deals on release day, and what changed:**
1. `RELEASE_TIME_ZONE` (Los Angeles) made a worldwide release "upcoming" for everyone east of California on the 16th. `4eb25cc`: an official record flagged `worldwide` is judged in `WORLDWIDE_RELEASE_TIME_ZONE` (Australia/Sydney) via `releaseZoneFor/releasedAt/releaseStartFor`; staggered releases keep LA. The early-listing evidence rule is unchanged. `/latest-releases` featured CTA is now "See <set> deals →" (`#deals`), "Checklist" secondary; lineup links go to `#deals`.
2. Latent since 17C.7: the `catalog_snapshot` refresh and `AGGREGATE_SELECT` never loaded the reference-evidence columns `savingsClaimTrusted` reads, so every row of a set with an official release record failed the snapshot filter and the set could never become deal-backed (older sets have no record and skipped the check). `ec12db8`: `SAVINGS_EVIDENCE_COLUMNS` in `lib/deals`, carried by both aggregate selects and the route's two selects; `tests/scanner/aggregate-evidence-columns` pins them. First refresh under the fix: 82 → 87 deal-backed sets, 30th Celebration in at count 36 (24 of its 60 active deals still have a reference observed before the worldwide release start and stay held until the verifier refreshes them - the normal pipeline).

**Price-intent guides and full imagery (`fbc81ce`, `a2843a3`).** Search Console (19 Aug–14 Sep: 203 queries, 255 impressions, 0 clicks) shows price/value/worth as the dominant intent and "shadowless arcanine" as the most-seen query. Three guides answer it: `/guides/how-much-is-my-pokemon-card-worth` (the procedure, links the price checker), `/guides/base-set-shadowless-unlimited-first-edition` (every tell checked against our own scans — the ", 99" copyright line is Shadowless/1st Edition, Unlimited lacks it; the catalogue's Shadowless Charizard scan is a stamped copy and the page says so), `/guides/pokemon-promo-card-numbers` (prefixes read from how the catalogue files each era's promo set; eleven verified examples). Every guide and news story now carries real card imagery — the six original guides and the launch story had none. Rule learned: a catalogue label containing "Secret" trips the grading spokes' clickbait pin. Mobile menu's help group renamed "About this site" (was "Guides & help", a duplicate beside "News & Guides"). Delta Reign (6 Nov, official) is NOT yet in OFFICIAL_RELEASES: the catalogue has no rows for it, so there is no verifiable `set` string to record; add it when PPT lists the set. `/news/mega-evolution-delta-reign-what-is-known` (`3d41a33` → `9522dff`) covers it in three labelled tiers - official English facts (date; "over 135" / "more than 20" / "more than 35" in the official wording; the four named Mega ex), the Japanese source set M6 Storm Emeralda (on sale since 31 Jul; 76 main-set cards per PokeBeach, 113 total per collector sites), and the 28 Jul leak of 14 cards, which the Japanese release has since made moot. Asserts no English number, rarity, pull rate or price; the gallery shows the Rayquaza cards we hold and says they are not Delta Reign. Update on: English list published, catalogue listing, release day. Lesson: news titles must clear the 65-char core cap (`tests/seo/pages.test.mjs`).

**Indexing sweep, 16 Sep.** 28 indexing requests via the GSC UI, no quota wall. Batch 1 (17): every new guide/news URL plus a `/guides` re-request. Batch 2 (11), chosen from a read-only `scripts/_gscAudit.mjs inspect-file` sweep rather than guessed: `/latest-releases`, all four `/market-data/*` sub-pages, `/guides/how-to-find-pokemon-card-set-and-number`, `/deals/price-drops`, `/deals/under-100`, and the three near-miss card pages (`arcanine-base-set-shadowless`, `lickitung-aquapolis`, `glaceon-171-cosmos-holo…`). The 30th guide, requested that morning, already read "URL is on Google" — requests land within hours.

**Index-coverage baseline from that sweep (16 Sep).** Static pages 47 PASS / 11 NEUTRAL. Sampled: sets 1/11, species 1/16, cards 1/12, deals 0/6, sealed 0/4 — so the 208 set and 925 species pages, the internal-link hubs feeding 23.7k card pages, are ~6–9% indexed. Manual requests cannot cover 1,133 pages; inbound links remain the only lever on crawl rationing. Notable: `/cards/arcanine-base-set-shadowless` ranks pos 13 for the site's most-seen query while reading "URL is not on Google".

**Integrity: deal 38589 (`760c837`).** Owner reported eBay AU 366670093596 as fake. It was live at 67% below a US$869 reference: a Base Set Charizard whose photo shows a 1st Edition stamp on a Shadowless frame, matched to our Unlimited card (42382) — a printing mismatch either way, since no genuine 1st Ed Shadowless sells for A$403. The screener had cleared it MATCH on "matching HP, attacks, weakness/resistance, artwork, copyright line", all of which agree across every 1999 print run. The vision prompt's IDENTITY_MISMATCH definition never named the two markers that separate them, so it now names the 1st Edition stamp and the illustration drop shadow, states image 1 is Unlimited unless its set name says otherwise, and says the shared details agreeing does NOT establish MATCH. Row held under `review:owner_reported_counterfeit` (review hold, not an authenticity finding; sticky; propagates to marketplace copies). Worth a sweep of other high-discount WOTC rows the screener cleared as MATCH.

Operational notes: `/api/refresh-catalog` is unauthenticated and DB-only (cron every 30 min); `fetchSets` is `unstable_cache` 900 s and set pages ISR 3600 s, so a snapshot change reaches the page within ~15–75 min. Owner-supplied social slides from @dropalertshq were not reused as site imagery (third-party design); only the card photographs, cropped, with credit.

**Deployed.** Merge `68d98f4` → production `dpl_FGfkaiVB39MpTexCZAsWeB6BciTK` (16 Sep, ~23:59 UTC). Live checks: 200 / `X-Vercel-Cache: PRERENDER`, self-canonical, Article `datePublished`/`dateModified` 2026-09-16, 26 card images (22 lazy), linked from `/guides` and `/latest-releases`, present in `sitemaps/pages`. IndexNow accepted the article, `/guides` and `/latest-releases` (HTTP 200); Search Console URL Inspection read "URL is not on Google" and indexing was requested (confirmed "Indexing requested"). Note for the GSC UI: the property is the URL-prefix `https://pokemondealfinder.com/` (the `sc-domain:` id has no access) and the `/search-console/inspect?…&id=<url>` deep link 404s - type the URL into the overview's inspection box instead.
