...
const TITLE = "Browse by Pokémon";
const DESCRIPTION =
  "Every Pokémon with active below-market card deals on eBay right now, one Pokémon at a time — cards across sets, price ranges, and current listings.";
...
export default async function PokemonIndexPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-paper">
      <JsonLd
        data={[
          breadcrumbList([{ name: "Deals", href: "/" }, { name: "Pokémon" }]),
      ...
      <header className="border-b border-zinc-200 dark:border-zinc-800">
        <div className="mx-auto max-w-7xl px-6 py-10">
          ...
          <p className="mt-3 max-w-xl text-base text-zinc-600 dark:text-zinc-400">
            Every Pokémon with real, active below-market deals right now - pick one to see every
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 px-6 py-10">
        {error && (
          <p className="rounded-lg bg-red-50 p-4 text-red-700">Couldn&apos;t load Pokémon: {error}</p>
        )}
