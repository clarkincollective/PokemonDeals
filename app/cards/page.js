            ...
            <ul className="mt-4 flex flex-wrap gap-2">
              {topSpecies.map((s) => (
                <li key={s.slug}>
                  <Link
                    href={`/pokemon/${s.slug}`}
                    className="inline-flex min-h-11 items-center rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-700 transition-colors hover:border-red-400 hover:text-red-600 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-300 dark:hover:text-red-500"
                  >
                    {s.species}
                  </Link>
                </li>
