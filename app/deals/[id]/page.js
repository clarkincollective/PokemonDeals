...
import { getPriceHistory } from "@/lib/justtcg";
...
async function loadHistory(deal, watchlist) {
  try {
    if (deal.is_graded) {
      ...
      return graded?.history ?? [];
    }
    return await getPriceHistory(watchlist.justtcg_tcgplayer_id, deal.condition ?? "Near Mint");
  } catch (err) {
...
export default async function DealDetailPage({ params }) {
  ...
  const history = await loadHistory(deal, deal.watchlist);
      ...
      <div className="mx-auto max-w-3xl px-6 py-10">
        ...
        <div className="mt-6 rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
          ...
          <p className="text-xs text-zinc-400">
            {deal.is_graded ? "Real graded sold comps" : "JustTCG market pricing"}, fetched fresh for this
            page.
          </p>
          <div className="mt-4">
