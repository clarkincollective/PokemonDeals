...
export default function HomeQuickFilters({ species = [], categories = [] }) {
  ...
  return (
    <div className="mt-4 flex flex-wrap items-center gap-x-2 gap-y-2">
      <span className="text-[13px] font-medium text-zinc-500 dark:text-zinc-400">Most listed:</span>
      {pills.map((p, i) => (
        <Link
          key={p.key}
          href={p.href}
