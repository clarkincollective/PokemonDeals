...
try{
  ...
  for(const width of [390,320]){
    ...
    for(const [name,props] of Object.entries({unavailable:{priceLabel:'Listing total',priceNote:'Includes recorded shipping',priceNative:{amount:null,currency:'USD'},priceUsd:null},unknown:{priceLabel:'Recorded auction price',priceNote:'Shipping breakdown not recorded'},unconfirmed:{priceLabel:'Listing price',priceNote:'Shipping not confirmed'},large:{priceLabel:'Listing total',priceNote:'Includes recorded shipping',priceNative:{amount:123456.78,currency:'AUD'}}})){
      await ev('window.__renderSticky('+JSON.stringify(props)+');window.scrollTo(0,700)');await sleep(400);
      if(name==='unavailable')await check('unavailable sticky has no total/shipping claim '+width,`(()=>{const bar=document.querySelector('a[href*="customid=deal_page"]').closest('.fixed');return bar.textContent.includes('Price unavailable')&&!bar.textContent.includes('Listing total')&&!bar.textContent.includes('Includes recorded shipping');})()`);
