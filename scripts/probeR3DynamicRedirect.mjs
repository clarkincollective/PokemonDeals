// Verify the approved dynamic listing route in the provider-isolated fixture.
import fs from 'node:fs';
import path from 'node:path';
import http from 'node:http';
const root=path.resolve(import.meta.dirname,'..');
const manifest=JSON.parse(fs.readFileSync(path.join(root,'.next/r3-runtime/manifest.json'),'utf8'));
if(manifest.listingRendering!=='dynamic')throw Error('Requires the approved dynamic listing fixture');
...
const result={checks,responses,followed:{status:followed.status,url:followed.url},limitation:'Approved dynamic listing route on fixture inputs. Data-cache source preserved; provider-call counts, production costs and category rendering not measured.'};
