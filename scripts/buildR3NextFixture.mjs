...
write('app/search/page.js',source('app/search/page.js'));
write('app/search/SearchClient.js',source('app/search/SearchClient.js'));
...
write('app/api/deals-page/route.js',source('app/api/deals-page/route.js'));
write('app/api/card-search/route.js',source('app/api/card-search/route.js'));
write('package.json',JSON.stringify({name:'r3-next-runtime-fixture',private:true},null,2));
const aliases=Object.fromEntries(['@/lib/deals','@/lib/supabaseAdmin','@/lib/supabaseClient','@/lib/pokemonPriceTracker','@/lib/email','@/lib/searchEngine'].map(name=>[name+'$',resolve(fixtures,'data.js')]));
...
export default {...base,devIndicators:false,images:{...base.images,unoptimized:true},experimental:{cpus:1,externalDir:true},webpack(config){
 ...
 const forbidden=resources.filter(r=>/[/\\\\]lib[/\\\\](deals|supabaseAdmin|supabaseClient|pokemonPriceTracker|email|searchEngine)\\.js$|posthog-js|@supabase|[/\\\\]lib[/\\\\]analytics[/\\\\]client\\.js$/.test(r));
