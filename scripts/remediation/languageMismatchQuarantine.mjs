// Language-mismatch QUARANTINE - PROPOSED, NOT APPROVED.
//
//   node scripts/remediation/languageMismatchQuarantine.mjs                 # dry run (read-only, default)
//   node scripts/remediation/languageMismatchQuarantine.mjs --out=plan.json # dry run + write the plan
//   node scripts/remediation/languageMismatchQuarantine.mjs --apply --confirm=3 --prior-out=prior.json
//   node scripts/remediation/languageMismatchQuarantine.mjs --rollback=prior.json --confirm=3
//
// What it does: for the HIGH-confidence rows in
// language-mismatch-quarantine-manifest.json - active EBAY_IT listings whose
// title states Japanese in Italian ("giapponese"), with corroborating
// Japanese-print evidence, that were matched to an ENGLISH catalogue
// identity and English market reference - it sets the EXISTING exclusion
// column deals.disqualified_reason to "identity:language_conflict". Any
// non-null reason already fails isDisplayableDeal and
// isVerificationCandidate. Scanner sightings do not clear it (their row
// payload carries no disqualified_reason).
//
// Why a write: the shared language classifier does not recognise localized
// language words, so these rows pass the display gate. Fixing the classifier
// is a scanner/matcher change tracked as a separate follow-up.
//
...
//
// Safety (same contract as integrityR1Quarantine.mjs):
...
//   also requires the title to be unchanged and still state Japanese;
...
export const QUARANTINE_REASON = "identity:language_conflict";
export const MANIFEST_PATH = join(HERE, "language-mismatch-quarantine-manifest.json");
// Reviewed marker only - this script's own recheck, not a matcher rule.
export const STATED_JAPANESE_IT = /\bgiappones[ei]\b/i;
const ROW_COLS =
...
export async function planQuarantine(db, manifest, libs = loadLibs()) {
  ...
  return manifest.candidates.map((c) => {
    ...
    const base = {
      ...
      title: c.title,
      storedAs: `${c.storedIdentity.cardName} | ${c.storedIdentity.cardSet} | ${c.storedIdentity.cardLanguage} | ${c.storedIdentity.cardTcgplayerId}`,
      evidence: c.evidence,
    };
    ...
    const current = {
      ...
      titleStatesJapanese: STATED_JAPANESE_IT.test(row.title ?? ""),
    };
    ...
    if (!current.titleStatesJapanese) changed.push("title no longer states Japanese");
  ...
  for (const p of plan) {
    console.log(
      [
        ...
        `statesJapanese=${p.current?.titleStatesJapanese}`,
        "|",
        cut(p.storedAs, 60),
        "|",
        cut(p.title, 70),
