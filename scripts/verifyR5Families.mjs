...
try{
 ...
 for(const [width,scheme] of [[1280,'light'],[390,'light'],[320,'dark']]){
  ...
  for(const route of routes){
   ...
   if(route==='/sealed-deals')check(prefix+' first sealed action within first screen',await ev(`(()=>{const a=[...document.querySelectorAll('main a')].find(a=>a.textContent.includes('View listing on eBay'));return !!a&&a.getBoundingClientRect().bottom<900;})()`));
   if(route==='/latest-releases')check(prefix+' release navigation before long identity note',await ev(`(()=>{const lineup=document.querySelector('[data-analytics-section="latest_lineup"]'),note=document.querySelector('#release-identity');return !!lineup&&!!note&&lineup.getBoundingClientRect().top<note.getBoundingClientRect().top&&!!document.querySelector('a[href="#release-identity"]');})()`));
   if(width===1280){await ev('document.body.tabIndex=-1;document.body.focus();document.body.removeAttribute("tabindex")');await key('Tab',9);await key('Enter',13);check(prefix+' keyboard skip',await ev(`document.activeElement.id==='main-content'`));await ev('document.activeElement?.blur();scrollTo(0,0)');}
 ...
 await go('/pokemon');await click(button('With deals'));check('species deal mode exposes qualifying species',await ev(`document.querySelector('button[aria-pressed="true"]').textContent.trim()==='With deals'&&document.querySelectorAll('main a[href^="/pokemon/"]').length===1&&document.querySelector('main a[href="/pokemon/dragonite"]').getBoundingClientRect().height>=44`));await shot('pokemon-with-deals-320-dark');
 await type('input[aria-label="Filter Pokemon"]','Cleffa');check('species deal mode combines name filter honestly',await ev(`!document.querySelector('main a[href^="/pokemon/"]')&&document.querySelector('main').innerText.includes('No Pokemon match these filters')`));await click(button('All Pokemon'));check('species all mode restores catalogue match',await ev(`!!document.querySelector('main a[href="/pokemon/cleffa"]')`));await click(`document.querySelector('button[aria-label="Clear filter"]')`);check('species clearing restores full crawlable inventory',await ev(`document.querySelectorAll('main a[href^="/pokemon/"]').length===1025`));
 await go('/cards');check('card directory early browse destinations',await ev(`(()=>{const links=[...document.querySelectorAll('nav[aria-label="Browse card catalogue"] a')];return links.length===2&&links.every(a=>a.getBoundingClientRect().bottom<900)&&links.map(a=>a.getAttribute('href')).join(',')==='/sets,/pokemon';})()`));
 await go('/sets');check('set row explains deal count and card list',await ev(`document.querySelector('main a[href="/sets/jungle"]').innerText.includes('3 deals')&&document.querySelector('main a[href="/sets/neo-destiny"]').innerText.includes('Explore card list')`));
