...
// `lanes` (deal-first review fix, P3): the lane keys the caller will
// actually RENDER, in LANES() precedence order. A lane not listed is
// returned empty and takes part in no dedupe, so a page that renders only
// the flagship row and the grid does not reserve printings for lanes a
// visitor never sees. Omitted -> every lane, exactly as before (the other
// callers and the P0.4.1 contract are unchanged). Pools are already
// eligibility-gated upstream; this changes which lanes CLAIM printings,
// never which rows qualify.
function buildHomepageLanes(pools = {}, { bucket = rotationBucket(), limits = {}, lanes = null } = {}) {
  const result = {};
  const render = Array.isArray(lanes) ? new Set(lanes) : null;
