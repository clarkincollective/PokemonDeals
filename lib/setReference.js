...
//
// PROVENANCE (phase record, 2026-09-12). Two kinds of statement appear in
// the copy, and only the first is protected by the render-time checks:
//
// A. ROW-DERIVED - read from the checklist rows and re-verified on every
//    render (a mismatch withholds the block):
//      Jungle: 64 entries numbered /64, none above the total; rows 1-16
//        are "Holo Rare" and rows 17-32 carry the same sixteen names as
//        "Rare" (example pair taken from the rows).
//      Neo Destiny: 113 entries numbered /105; the eight rows above 105
//        are 106-113, named "Shining ...", rarity "Secret Rare"; 46 rows
//        carry a Dark/Light prefix; the Dark/Light and Dark/Shining
//        example pairs are found in the rows.
//      Boundaries Crossed: 153 entries numbered /149; rows 150-153 are
//        "Secret Rare"; rows 137-140 are "Rare Ace"; rows 141-149 are
//        named "... (<n> Full Art)"; the same-name pairs, the secret
//        Rocky Helmet twin and the Celebi EX 9/141 example are found in
//        the rows.
//
// B. EXTERNALLY SOURCED - publication history and printing variations
//    that are NOT in the rows and NOT validated by the render-time checks.
//    Each is supported only by the public set article it cites, read on
//    2026-09-12; no other investigation was made:
//      Jungle - printed in 1st Edition and Unlimited; the Unlimited
//        holofoil rares were also printed without the set symbol (error
//        cards):
//        https://bulbapedia.bulbagarden.net/wiki/Jungle_(TCG)
//      Neo Destiny - the last English expansion with a 1st Edition
//        symbol (hence 1st Edition and Unlimited printings); 106-113 are
//        the Shining Pokemon:
//        https://bulbapedia.bulbagarden.net/wiki/Neo_Destiny_(TCG)
//      Boundaries Crossed - introduced ACE SPEC Item cards; 150-153 are
//        the set's Secret cards:
//        https://bulbapedia.bulbagarden.net/wiki/Boundaries_Crossed_(TCG)
//    (pokemontcg.io's API answered 403/502 to the same checks and is not a
//    source here.)
//
// Observation, recorded without conclusion: the Boundaries Crossed rows
// list a regular Skyla (134/149) but no regular Bianca or Cheren under
// those names, while Full Art Bianca (147) and Cheren (148) are listed. A
// Full Art card does not establish that a regular counterpart belongs to
// the same set, so the copy claims only the pairs the rows show and this
// is NOT recorded as a catalogue defect.
//
