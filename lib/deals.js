...
// Every active deal row (matching the given select + the english/lang
// `!inner` watchlist join) that fetchSets / fetchCardHubs / fetchSpeciesHubs
// group over. Supabase caps any single request at 1,000 rows, so the scan
// has to be paged - but the pages don't depend on each other, so this
// fires them concurrently instead of in a sequential await loop, turning
// a ~4s scan of the ~8k active rows into ~1s. An estimated count sizes
// the parallel batch; a short sequential tail covers an undercount.
async function scanActiveDealRows(selectStr, { language = "english" } = {}) {
  const PAGE_SIZE = 1000;
  // Fire a fixed batch of pages at once (no count query - an estimated
  // count on this join filter undershoots badly, and PostgREST then
  // rejects any range past the estimate). A range past the real end
  // just comes back empty, so an over-wide batch is safe; the current
  // active pool is ~8-9k rows, BATCH*PAGE_SIZE gives comfortable headroom.
  const BATCH = 14;
  const q = () =>
    supabase
      .from("deals")
  ...
  const batch = await Promise.all(
    Array.from({ length: BATCH }, (_, i) => q().range(i * PAGE_SIZE, (i + 1) * PAGE_SIZE - 1))
  );
  const rows = [];
  for (const r of batch) {
    if (r.error) return { rows: null, error: r.error.message };
    if (r.data?.length) rows.push(...r.data);
  }

  // If even the last page of the batch was full, the pool has outgrown
  // BATCH*PAGE_SIZE - keep paging sequentially until it doesn't.
  if (batch[BATCH - 1].data?.length === PAGE_SIZE) {
    for (let from = BATCH * PAGE_SIZE; ; from += PAGE_SIZE) {
      const more = await q().range(from, from + PAGE_SIZE - 1);
      if (more.error) return { rows: null, error: more.error.message };
      if (!more.data?.length) break;
      rows.push(...more.data);
      if (more.data.length < PAGE_SIZE) break;
    }
  }
...
async function fetchSetsUncached({ language = "english" } = {}) {
  const { rows, error } = await scanActiveDealRows("watchlist:watchlist_id!inner (set, language)", {
    language,
  });
...
async function fetchCardHubsUncached({ language = "english" } = {}) {
  const { rows, error } = await scanActiveDealRows(
...
// the two can't drift.
async function fetchFreshFindsUncached({ language = "english", limit = 12 } = {}) {
...
async function fetchSpeciesHubsUncached({ language = "english" } = {}) {
  const { rows, error } = await scanActiveDealRows(
    "total_price, image_url, watchlist:watchlist_id!inner (id, name, set, language)",
    { language }
  );
  if (error) return { species: [], error };
