...
async function fetchHomepageLanesUncached({ country = null } = {}) {
  ...
  const flagshipPromise = selectFlagshipDeals({ limit: 60, language, country }).then((r) => r.deals ?? []);
  ...
  const freshPromise = fetchFreshFindsUncached({ limit: 40, language, country }).then((r) => r.deals ?? []);
