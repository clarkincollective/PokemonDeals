...
async function fetchSetCatalogUncached(setName, language = "english") {
  if (!setName) return { cards: [], dealCount: 0, totalCards: 0, truncated: false, stats: null, indexable: false, error: null };
  ...
  if (catErr && !/card_catalog/.test(catErr.message || "")) {
    return { cards: [], dealCount: 0, totalCards: 0, truncated: false, stats: null, indexable: false, error: catErr.message };
  }
  ...
  // Set-level stats for the catalogue-backed hub (SEO Phase 4A). All
  // computed from card_catalog, nothing fabricated. `eligibleCount` =
  // real + imaged + stable-id + non-sentinel price = a card with an
  // INDEXABLE permanent /cards/[slug]; that count is the set-hub
  // indexability basis (setIndexable).
  const eligibleRows = (catRows ?? []).filter(setEligibleCard);
  const pricedRows = (catRows ?? []).filter((r) => catalogPriceOk(r.market_price));
  const stats = {
    cardCount: trimmed.length,
