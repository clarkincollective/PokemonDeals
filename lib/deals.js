...
// "Today's Best Finds" - the flagship "great card I can buy NOW below a
// trustworthy market reference" lane (homepage "Best deals right now" +
// /best-finds). minMarketPrice keeps it off $15-30 bargain-bin cards;
...
//
// 13C.2 - this lane is now BUY IT NOW / FIXED PRICE ONLY. An auction's
// price is a current bid that can rise, so it is not a "buy now below
// market" result; auctions keep their own "Auctions ending soon" lane
// (fetchAuctionsEndingSoon). Ordering is the deterministic composite in
// lib/flagshipRanking (discount x absolute saving x reference confidence,
// freshness as a tie-break) - never a raw discount_pct sort, and never an
// exposed score.
async function fetchBestFindsUncached({
  ...
  // Every filter a visitor picks on the homepage applies to every section
  // shown there, not just the main "All Deals" grid below it.
  if (country) query = query.eq("marketplace", country);

  const { data, error } = await query
    .order("discount_pct", { ascending: false })
    .limit(BEST_FINDS_POOL_LIMIT);

  if (error || !data) return { deals: [], error: error ? error.message : null };

  // PREMIUM gate (drops a high-risk raw card with no completed MATCH
  // visual verdict), then the deterministic flagship composite +
  // one-tile-per-canonical-card dedupe. `preferLocal` in a country view
  // keeps in-country listings first (no shipping wait), matching the
  // pre-13C.2 `is_local` ordering.
  const deals = rankFlagshipDeals(premiumDisplayable(data), {
    freshnessOf: (row) => dealFreshness(row),
    preferLocal: Boolean(country),
    limit,
  });
