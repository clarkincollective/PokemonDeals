...
async function fetchMostExpensiveCardsUncached({ language = "english", limit = 100 } = {}) {
  ...
  // SEO Phase 4B: the user's intent on this evergreen page is the CARD,
  // not one ephemeral listing - so link the permanent /cards/[slug] when
  // it resolves (hub or catalogue), falling back to /deals/[id] only when
  // it doesn't. Both lists are already cached.
  const [{ slugs: catalogSlugs }, { hubs }] = await Promise.all([
    fetchCatalogCardSlugs(),
    fetchCardHubs({ language }),
  ]);
  const cardPageSlugs = new Set([...(catalogSlugs ?? []), ...hubs.map((h) => h.slug)]);
  ...
  for (const row of data ?? []) {
    ...
    seen.add(w.id);
    const slug = catalogCardSlug(w.name, w.set);
    cards.push({
      id: w.id,
      name: w.name,
      set: w.set,
      ...
      dealId: row.id,
      slug,
      cardHasPage: cardPageSlugs.has(slug),
    });
