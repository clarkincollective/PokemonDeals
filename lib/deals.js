...
import { isChecklistPilotSet, SET_CHECKLIST_MAX_ROWS } from "@/lib/setChecklist";
...
async function fetchSetCatalogUncached(setName, language = "english") {
  ...
  // Phase 17C.2 pilot: the COMPLETE set (every catalogue row, priced or
  // not) for the readable checklist table - pilot sets only, and only when
  // small enough to be readable, so no other set's cached payload grows.
  const checklistCards =
    isChecklistPilotSet(setName) && cards.length <= SET_CHECKLIST_MAX_ROWS ? cards : null;
  ...
  return {
    ...
    checklistCards, // pilot sets only - the <SetChecklist> table (else null)
