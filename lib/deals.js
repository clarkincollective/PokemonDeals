...
// The tracked-catalogue view of ONE species, for the /pokemon/<slug>
// fallback a species with no active deal still gets. Every distinct
// tracked English card of the species (name + set + last known market
// price), most-valuable first, each linked to its /cards/<slug> hub when
// one exists. Sourced from `watchlist` (the priced catalogue), not
// `deals` - so it works for a species that currently has zero deals.
//
// A substring `ilike` on the species name narrows the scan before
// extractSpecies confirms the exact species in JS (the base species name
// is always a substring of a matching card name - "Alolan Vulpix",
// "Rocket's Meowth" - so the prefilter is safe). Cached per species.
async function fetchSpeciesCatalogUncached(speciesName, language = "english") {
  if (!speciesName) return { cards: [], error: null };
  // Punctuation-tolerant prefilter: every non-alphanumeric run becomes a
  // wildcard so "Farfetch'd" matches "Farfetch’d"/"Farfetchd", "Mr. Mime"
  // matches "Mr Mime", etc. extractSpecies does the exact confirmation.
  ...
  const hubSlugByName = new Map(hubs.map((h) => [`${h.name}|${h.set}`, h.slug]));
  const seen = new Set();
  const cards = [];
  for (const row of data ?? []) {
    ...
    cards.push({
      name: row.name,
      set: row.set,
      price: row.last_known_price != null ? Number(row.last_known_price) : null,
      tcgplayerId: row.justtcg_tcgplayer_id ?? null,
      hubSlug: hubSlugByName.get(key) ?? null,
    });
  }
  return { cards, error: null };
}

export const fetchSpeciesCatalog = unstable_cache(fetchSpeciesCatalogUncached, ["species-catalog"], {
  revalidate: CARD_HUB_REVALIDATE_SECONDS,
});
