...
async function fetchCardPriceHistoryUncached(tcgplayerId) {
  const empty = { chartPoints: [], trends: null, signal: null, coverage: null, hasBackfill: false, firstDate: null, lastDate: null, points: 0 };
  if (!tcgplayerId) return empty;
  const { series, error } = await getCanonicalPriceHistory(supabaseAdmin(), String(tcgplayerId), {
    condition: "Near Mint",
    language: "english",
    maxPoints: 800,
  });
  ...
  const trends = trendWindows(series);
  return {
