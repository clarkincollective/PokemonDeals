...
import { catalogCardSlug, splitCardSlug, isRealCardName } from "@/lib/cardSlug";
import { SPECIES_CATALOG_MIN_CARDS, speciesIndexable } from "@/lib/speciesHub";
...
async function fetchDealsPoolUncached({ language, country, cardType, listingType, maxPrice, minPrice }) {
  ...
  return { data: (data ?? []).map(withCard), error: error ? error.message : null };
}
...
// Repdigit sentinel prices PPT returns as "no real data" - inlined (not
// imported from lib/pokemonPriceTracker) to keep this file's import graph
// free of the PPT API client, matching lib/sealedCatalog.js. Keep in sync
// with SENTINEL_PRICES there.
const CATALOG_SENTINEL_PRICES = new Set([999, 999.99, 9999, 9999.99, 99999, 99999.99]);
function catalogPriceOk(value) {
...
// resolveCardSlug (the deal hub) misses. Returns the card, or null when
// the slug matches no catalog card OR the matched card fails the
// data-quality bar (needs a non-sentinel price AND an image - the same
// bar the page render and the sitemap use, so a thin/priceless card never
...
async function resolveCatalogCardUncached(slug, language = "english") {
  ...
  const matches = (data ?? []).filter((r) => slugifySet(r.name) === split.nameSlug);
  if (matches.length === 0) return null;

  // Deterministic pick: usable (real card + non-sentinel price + image)
  // first, highest price first. If nothing in the match set is usable,
  // return null so the page 404s rather than rendering a thin / non-card
  // page.
  ...
  return {
    ...
    refPrice: Number(pick.market_price),
    slug,
  };
}
...
async function fetchCatalogSpeciesUncached() {
  ...
  for (let from = 0; ; from += 1000) {
    const { data, error } = await supabase
      .from("card_catalog")
      .select("species, name, market_price, image_url")
      .eq("language", "english")
      .not("species", "is", null)
      .not("image_url", "is", null)
      .not("market_price", "is", null)
...
async function fetchSpeciesCatalogUncached(speciesName, language = "english") {
  ...
  if ((catRows ?? []).length > 0) {
    ...
    cards = catRows.map((r) => {
      ...
      const refPrice = r.market_price != null ? Number(r.market_price) : null;
      // The permanent /cards/[slug] created in P0 exists only when the card
      // is a real card (not a code card / box) with a non-sentinel price
      // and an image. Only then do we link to it - otherwise there's no
      // page and the tile keeps its eBay-search CTA.
      const catalogEligible = Boolean(isRealCardName(r.name) && r.image_url && catalogPriceOk(refPrice));
      return {
        tcgplayerId: id,
        name: r.name,
        set: r.set,
        ...
        catalogSlug: catalogEligible ? catalogCardSlug(r.name, r.set) : null,
  ...
  // card_catalog, nothing fabricated. `eligibleCount` (real card + price +
  // image, i.e. cards that have a permanent /cards/[slug]) is the
  // indexability basis.
  const priced = cards.filter((c) => catalogPriceOk(c.refPrice));
  const eligible = cards.filter((c) => c.catalogSlug);
...
async function fetchSetCatalogUncached(setName, language = "english") {
  ...
  const [{ rows: catRows, error: catErr }, { hubs }, { data: dealRows }] = await Promise.all([
    ...
    supabase
      .from("deals")
      .select(
        "total_price, total_price_usd, marketplace, discount_pct, watchlist:watchlist_id!inner (justtcg_tcgplayer_id, set, language)"
      )
      .eq("is_active", true)
  ...
  // Per-card active-deal aggregate for this set (same shape SpeciesCard
  // expects: cheapest listing wins, rows arrive cheapest-first).
  const dealByTcgId = new Map();
  for (const d of dealRows ?? []) {
    const id =
...
function sealedCardFrom(r, deal) {
  return {
    ...
    searchQuery: r.name, // sealed product names are self-contained
    refPrice: r.market_price != null ? Number(r.market_price) : null,
    image: r.image_url ?? null,
    hubSlug: null,
    deal: deal ?? null,
...
async function fetchSetSealedCatalogUncached(setName, language = "english") {
  ...
  const [{ data: catRows, error: catErr }, { data: dealRows }] = await Promise.all([
    ...
    supabase
      .from("sealed_deals")
      .select(
        "total_price, total_price_usd, marketplace, discount_pct, sealed_watchlist:sealed_watchlist_id!inner (tcgplayer_id, set)"
      )
      .eq("is_active", true)
