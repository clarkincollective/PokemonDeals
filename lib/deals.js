...
// Every active deal row (matching `selectStr` + the english/lang `!inner`
// watchlist join) that fetchSets / fetchCardHubs / fetchSpeciesHubs group
// over. Supabase caps a single request at 1,000 rows so the scan is paged.
//
// Deliberately SEQUENTIAL. Parallelising the pages (all at once, or in
// waves) cut the isolated wall time but, with several page renders in
// flight (build workers, concurrent visitors), opened enough connections
// to saturate the Supabase pooler - later requests then hung for tens of
// seconds. One request at a time is ~4s for the current ~8k active rows;
// each result is unstable_cache'd for 900s, so only the first hit in a
// window pays it. The real fix if this gets slow is a precomputed
// aggregate table refreshed by the existing scan cron.
