...
async function fetchSetSealedCatalogUncached(setName, language = "english") {
  ...
  // Deals first, then browse by descending reference price (a set's
  // headline sealed products - booster box, ETB - are its priciest, so
  // that ordering surfaces them).
  const priced = (c) => (c.refPrice != null ? c.refPrice : -1);
  const deals = all.filter((c) => c.deal).sort((a, b) => a.deal.cheapestUsd - b.deal.cheapestUsd);
  const browse = all.filter((c) => !c.deal).sort((a, b) => priced(b) - priced(a));
...
export const fetchSealedCatalog = unstable_cache(fetchSealedCatalogUncached, ["sealed-catalog-v1"], {
  revalidate: CARD_HUB_REVALIDATE_SECONDS,
});
