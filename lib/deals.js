...
import { catalogCardSlug, splitCardSlug, isRealCardName, catalogPriceOk, pickCatalogMatch } from "@/lib/cardSlug";
...
async function resolveCatalogCardUncached(slug, language = "english") {
  ...
  // Deterministic pick across the FULLY-paginated set (real card +
  // non-sentinel price + image; highest price wins a tie). null -> 404.
...
async function fetchSpeciesCatalogUncached(speciesName, language = "english") {
  ...
  if (hubEntry?.watchlistIds?.length) {
    const { data: dealRows } = await supabase
      .from("deals")
      .select(
        "total_price, total_price_usd, marketplace, discount_pct, affiliate_url, listing_url, watchlist:watchlist_id!inner (justtcg_tcgplayer_id)"
      )
      .eq("is_active", true)
    ...
    for (const d of dealRows ?? []) {
      ...
      dealByTcgId.set(id, {
        ...
        // The cheapest active listing's own affiliate URL - rows are
        // total_price_usd asc, so the first per card is the cheapest, and
        // this is the "View Deal on eBay" target (a real verified listing,
        // not a search).
        affiliateUrl: d.affiliate_url ?? d.listing_url ?? null,
      });
    }
  }
