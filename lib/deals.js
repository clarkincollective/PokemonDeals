...
import { buildSetAliases } from "@/lib/pokemonSets";
...
// ("151", "evolving skies").
//
// Each entry: { name, set_id, slug, phrases } where `phrases` is
// buildSetAliases(name) with any alias that is ambiguous ACROSS the
// vocabulary removed (so "base set" is never claimed by
// "Base Set (Shadowless)"). Longest phrase wins at match time.
// Cached 6h - the catalogue only changes on the daily provider sync.
async function fetchSetSearchVocabularyUncached() {
  ...
  for (const [name, setId] of bySet) {
    ...
    // the exact full name is always unambiguous (distinct set strings);
    // guarantee it survives even if buildSetAliases returned nothing.
