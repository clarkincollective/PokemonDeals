...
async function fetchCardHubsUncached({ language = "english" } = {}) {
  ...
  for (let from = 0; ; from += PAGE_SIZE) {
    const { data, error } = await supabase
      .from("deals")
      .select("watchlist:watchlist_id!inner (id, name, set, language, justtcg_tcgplayer_id)")
      .eq("is_active", true)
      .eq("watchlist.language", language)
      .range(from, from + PAGE_SIZE - 1);
    ...
    for (const row of data) {
      ...
      else grouped.set(w.id, { id: w.id, name: w.name, set: w.set, tcgplayerId: w.justtcg_tcgplayer_id, count: 1 });
    }
    if (data.length < PAGE_SIZE) break;
  }
