...
import { isDisplayableDeal, savingsClaimTrusted } from "@/lib/dealQuality";
    ...
    return {
      ...
      deal: deal
        ? {
            ...
            // Same trust gate as every other catalogue projection (lib/deals.js
            // fetchSetCatalog/fetchSpeciesCatalog): a plain (untrusted) listing
            // must never carry a "% below market" claim into this reference
            // grid - null here means "no savings claim", not "no deal".
            discountPct: deal.discount_pct != null && savingsClaimTrusted(deal) ? Number(deal.discount_pct) : null,
...
export async function runCardSearch({
  ...
  db = null,
  debug = false,
  // 13B.6.3 - the caller's precomputed valid /sets/[slug] list (an array
  // OR a promise for one, so the caller can start it in parallel). When
  // present, the "Browse <set> deals" link is a pure lookup - no
  // resolveSetSlug -> fetchSets + fetchCatalogSets round trip.
  validSetSlugs = null,
} = {}) {
  ...
  timing.vocab_ms = Math.round(performance.now() - mv);
  try {
    ...
    // 13B.6.3 - when the caller already has the valid /sets slug list
    // (app/search/page.js does), this is a pure array check - no
    // resolveSetSlug -> fetchSets + fetchCatalogSets round trip.
    let mSetLink = performance.now();
    let setSlug = null;
    let setLink = null;
    if (intent.subject.set) {
      setSlug = slugifySet(intent.subject.set);
      const slugList =
        validSetSlugs && typeof validSetSlugs.then === "function"
          ? await validSetSlugs.catch(() => null)
          : validSetSlugs;
      if (Array.isArray(slugList)) {
        if (slugList.includes(setSlug)) setLink = `/sets/${setSlug}`;
      } else {
        try {
    ...
    timing.set_link_ms = Math.round(performance.now() - mSetLink);
    ...
    // 13B.6.3 - structural cold-path timing (labels + ms + mode only,
    // NEVER the query text). Safe in production; readable via
    // `vercel logs`. Kept as an ongoing latency signal.
    console.log(
      "[searchEngine]",
      JSON.stringify({
        mode: resolution.mode,
