...
// Same transform as lib/slugify.js's slugifySet - inlined (not imported)
// so this stays a dependency-free CommonJS module. Keep in sync.
...
// URL slug for a species. baseSlug() is unique across the whole dex
// EXCEPT the Nidoran pair (both -> "nidoran"), so those two get an
// explicit -f / -m suffix. Everything else (Farfetch'd -> farfetch-d,
// Ho-Oh -> ho-oh, Type: Null -> type-null, ...) matches the slug
// computeAggregates writes for a deal-having species.
const SPECIES_SLUG_OVERRIDES = { "Nidoran♀": "nidoran-f", "Nidoran♂": "nidoran-m" };

function speciesSlug(name) {
  return SPECIES_SLUG_OVERRIDES[name] ?? baseSlug(name);
}

const SLUG_TO_SPECIES = new Map(SPECIES.map((name) => [speciesSlug(name), name]));

// Canonical species name for a /pokemon/<slug>, or null if the slug isn't
// a real dex species. Used for the catalog fallback page a species with
// no active deal still gets.
function speciesForSlug(slug) {
  return SLUG_TO_SPECIES.get(String(slug || "").toLowerCase()) ?? null;
}
...
// Non-Pokemon card families whose NAME can legitimately start with or
// contain a Pokemon name: Energy cards ("Fire Energy", "Aurora Energy",
// "Twin Energy"), the "<Pokemon> Spirit Link" Trainer-Tool family, and
// Stadiums. Used as a fallback when the catalogue has no structured
// card_type (the /export backfill leaves it null on ~29k rows).
const NON_SPECIES_CARD_NAME = /\benerg(?:y|ies)\b|\bspirit link\b|\bstadium\b/i;

// True only when `species` is the card's OWN identity - it leads the card
// name (token 0, or within the first three tokens after an owner
// possessive and stacked form/variant modifiers), not a mid-name mention.
// This is what extractSpecies gets wrong for Trainer/Energy cards:
//   "Fire Energy (#9 Charizard Stamped)"  -> extractSpecies => "Charizard"
//                                            speciesLeadsCardName => false
//   "Houndoom Spirit Link"                -> extractSpecies => "Houndoom"
//                                            speciesLeadsCardName => false
// while keeping every real variant:
//   "Dark Charizard", "Blaine's Charizard", "M Charizard EX",
//   "Rocket's Sneasel ex", "Team Aqua's Kyogre ex", "Team Plasma Deoxys",
//   "Dawn Wings Necrozma", "Radiant Charizard", "Charizard & Reshiram GX".
function speciesLeadsCardName(name, species) {
  if (!name || !species) return false;
  if (NON_SPECIES_CARD_NAME.test(name)) return false;
  // Strip a leading owner possessive ("Blaine's ", "Team Aqua's ") before
  // normalize (which turns "'" into a space and would split the owner).
  const stripped = String(name).replace(/^\s*(?:team\s+[a-z]+'s|[a-z][a-z.'’-]*'s)\s+/i, "");
  const norm = normalize(stripped);
  if (!norm) return false;
  const tokens = norm.split(" ");
  const limit = Math.min(2, tokens.length - 1); // first 3 leading tokens
  for (let start = 0; start <= limit; start++) {
    const bucket = BY_FIRST_TOKEN.get(tokens[start]);
    if (!bucket) continue;
    const hit = matchAt(tokens, start, bucket);
    if (hit) return hit === species; // the first species in the leading window must BE this one
  }
  return false;
}
...
const SPECIES_WITH_GENERATION = SPECIES.map((name, i) => ({
  ...
  slug: speciesSlug(name),
}));

module.exports = {
  SPECIES,
  extractSpecies,
  speciesLeadsCardName,
  speciesSlug,
  speciesForSlug,
