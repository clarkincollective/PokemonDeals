...
import { listingIdentityKey } from "./listingIdentity.js";
...
// 13B.4.2 - PostgREST `.or()` scoping `deals` to ONE exact card by
// canonical card identity (card_tcgplayer_id) OR its legacy watchlist id.
// The card id pulls in feed-discovered listings (watchlist_id NULL) for
// the exact card that a watchlist-only scope misses. Single-value `.eq`
// terms (one card), unlike speciesScopeOrClause's `.in` lists. Returns
// null when neither id is usable.
export function cardScopeOrClause({ tcgplayerId, watchlistId } = {}) {
  const parts = [];
  const tcg = tcgplayerId != null ? String(tcgplayerId).trim() : "";
  if (tcg) parts.push(`card_tcgplayer_id.eq.${tcg}`);
  if (watchlistId != null && String(watchlistId).trim() !== "") parts.push(`watchlist_id.eq.${watchlistId}`);
  return parts.length ? parts.join(",") : null;
}
...
// A distinct eBay listing (not a card, not a seller - Phase 12C
// terminology). Two rows for the same eBay item collapse; two different
// listings of the same card do NOT.
//
// AUDIT FINDING 6 (2026-09-25). This used to be
// `${deal.marketplace}:${deal.listing_id}`, which did NOT do what the
// sentence above says: the same eBay item is discoverable from several
// regional eBay sites and is stored once per site, so the marketplace in
// the key turned one buying option into up to six. Measured on live
// records: 1,103 displayable rows behind 998 listings, and 75 of 637 card
// hubs overstating their option count (the Magneton hub said "12 active
// listings" over 6 real listings). lib/allDealsInventory.js already had
// the correct rule - this now uses the same shared one, variation
// component included so two variations of one listing stay apart.
export function listingKey(deal) {
  return listingIdentityKey(deal);
}
