...
const { resolveAttribution } = require("./affiliateAttribution");
...
// `short` is the two-letter form the deal cards pair with the flag ("🇬🇧
// eBay UK") so a flag is never the only statement of the marketplace.
const MARKETPLACES = {
  EBAY_US: { label: "United States", short: "US", flag: "🇺🇸", currency: "USD" },
  EBAY_GB: { label: "United Kingdom", short: "UK", flag: "🇬🇧", currency: "GBP" },
  EBAY_AU: { label: "Australia", short: "AU", flag: "🇦🇺", currency: "AUD" },
  EBAY_CA: { label: "Canada", short: "CA", flag: "🇨🇦", currency: "CAD" },
  EBAY_DE: { label: "Germany", short: "DE", flag: "🇩🇪", currency: "EUR" },
  EBAY_IT: { label: "Italy", short: "IT", flag: "🇮🇹", currency: "EUR" },
};
...
// (see lib/affiliateAttribution.js) - "<page>-<placement>", e.g.
// "sealed-selected", "card-offer", "deal-sticky" - never a card/listing/
// deal/user identity, never a search query, and never the visitor's
// acquisition source. This is
...
// FINDING 5 (2026-09-25). `customid` now carries "<page>-<placement>" from
// lib/affiliateAttribution.js instead of a single fused token. Callers may
// pass { page, placement } (preferred, and the only way to distinguish two
// controls on the same page) or the legacy { surface }, which is
// translated by the documented migration table. Everything else about this
// function is unchanged: the destination, campid, mkevt, mkcid, mkrid and
// toolid are all preserved exactly as before, and .set() (never .append())
// still guarantees re-wrapping is idempotent.
function wrapEbayAffiliateUrl(url, { surface, page, placement } = {}) {
  if (!url) return url;
  ...
  try {
    ...
    wrapped.searchParams.set("customid", resolveAttribution({ surface, page, placement }));
...
// `attribution` is either a legacy surface string (translated) or a
// { page, placement } object — see wrapEbayAffiliateUrl above.
function buildEbaySearchLink(query, marketplace, attribution) {
  ...
  const opts = typeof attribution === "string" || attribution == null ? { surface: attribution } : attribution;
  return wrapEbayAffiliateUrl(url.toString(), opts);
}
