...
const { resolveAttribution, IMPACT_SUBID_PARAM } = require("./affiliateAttribution");
...
// every result for that Pokemon. Only falls back to the old
...
// FINDING 5 (2026-09-25) — source attribution on the Impact link.
//
// TCGPlayer's programme is impact.com, NOT eBay Partner Network, and the
// two contracts are NOT the same. eBay's sub-ID rides in `customid`;
// Impact's rides in `subId1`. This function therefore builds its own
// parameter rather than sharing eBay's builder — only the identifier
// VALUE is shared (lib/affiliateAttribution.js), because that is what
// makes one PostHog row joinable to either network's report.
//
// `attribution` is { page, placement } (or a legacy surface string).
// Everything else is byte-identical to before: the `u=` destination, its
// encoding, and the plain non-affiliate fallback when
// TCGPLAYER_AFFILIATE_LINK is unset. The extra parameter is additive and
// cannot alter where the link goes.
//
// VERIFIED STRUCTURALLY ONLY. `subId1` is Impact's documented partner
// sub-ID parameter, and the tests below pin that the destination, its
// encoding and the tracking base are unchanged. Whether the value appears
// in THIS account's Impact reporting can only be confirmed by a real
// click, which this work deliberately does not generate.
function buildTcgplayerLink(cardName, tcgplayerId, attribution) {
  ...
  const subId = resolveAttribution(opts);
  return `${affiliateLink}?u=${encodeURIComponent(destination)}&${IMPACT_SUBID_PARAM}=${subId}`;
}
