...
async function sendEmail({ to, subject, html, text, replyTo }) {
  ...
  try {
    ...
    // the Resend message id lets a caller verify delivery afterwards
...
async function sendBatch(messages) {
  ...
  for (let i = 0; i < messages.length; i += 100) {
    const chunk = messages.slice(i, i + 100).map((m) => ({
      ...
      html: m.html,
      text: m.text,
      // AUTO-2: forward per-message headers when supplied (RFC 8058
      // List-Unsubscribe / List-Unsubscribe-Post for the digest). Absent
      // for the price-alert path, which passes no headers.
