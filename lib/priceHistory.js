...
// (D) Price-condition provenance (2026-09-11): the two points a window
// compares must be observations of the SAME reference. When both points
// carry a recorded condition (or printing) and they differ - e.g. the
// provider starts pricing Near Mint for a printing that was only ever
// Lightly Played - the step between them is a change of reference, not a
// price movement, and the window is withheld. A point with nothing
// recorded (null: pre-migration rows) cannot be judged and is compared as
// before - it is never treated as Near Mint.
...
// the market signal's basis is gone, so the confidence carries
// "reference-changed" for the UI's restrained wording.
function confidentTrendWindows(series, { rawRows = null, cfg = TREND_CONFIDENCE } = {}) {
  ...
  for (const w of TREND_WINDOWS) {
    ...
    const compared = series.find((p) => p.date === t.comparedOn);
    if (referenceChanged(latest, compared)) {
      referenceChangedWindows.push(w.key);
      windows[w.key] = null;
      continue;
    }
    windows[w.key] = t;
  }
  const d30Withheld = referenceChangedWindows.includes("d30");
  return {
    windows,
    confidence: {
      level: cardLow || d30Withheld ? "low" : "ok",
      reason: endpoint ? "endpoint-anomaly" : sourceBad ? "source-disagreement" : d30Withheld ? "reference-changed" : null,
      referenceChangedWindows,
    },
  };
}
