...
const { isMissingProvenanceColumnError } = require("./referenceProvenanceDb");
...
// --- merge read path (§11) -------------------------------------------
//
// `rows` = raw price_history rows for ONE card + ONE series key (the
// legacy `condition` column), any sources, unsorted. Produces one canonical
// point per calendar day, oldest -> newest, sentinels dropped. When both a
// first-party ('catalog') and a provider ('ppt_backfill') observation exist
// for the same day the FIRST-PARTY one wins (§11). Provenance is retained
// on each point: `source`, plus - price-condition provenance (2026-09-11) -
// the observed reference's recorded `referenceCondition` /
// `referencePrinting` (null = not recorded, which is NOT "Near Mint").
function mergeHistoryRows(rows, { maxPoints = 800 } = {}) {
  ...
  for (const r of rows ?? []) {
    ...
    // Only rows that recorded a provenance carry it on the point - the
    // canonical { date, price, source } shape of every other point (all
    // pre-migration rows, the whole ppt_backfill prefix) is unchanged.
    if (r.reference_condition != null || r.reference_printing != null) {
      cand.referenceCondition = r.reference_condition ?? null;
      cand.referencePrinting = r.reference_printing ?? null;
    }
...
async function getCanonicalPriceHistory(
  ...
  // `condition` here is the SERIES KEY (legacy column name), not a claim
  // about the observed condition - that is reference_condition, read
  // alongside so trend windows can refuse to compare across a change of
  // reference. Until the provenance migration has run, selecting the two
  // extra columns fails with the specific missing-column schema error and
  // ONLY that error falls back to the base columns; anything else is
  // returned as the error it is.
  const BASE_COLS = "observed_on, price, source";
  const build = (cols) => {
    let q = db
      .from("price_history")
      .select(cols)
  ...
  let { data, error } = await build(`${BASE_COLS}, reference_condition, reference_printing`);
  if (error && isMissingProvenanceColumnError(error)) ({ data, error } = await build(BASE_COLS));
...
//    94% of pairs, < 10% for 96%, and >= 25% for only ~3% - so a >= 25%
//    same-day disagreement (≈97th percentile) means the two independent
//    measurements of that card are far enough apart that we should not
//    publish a trend % for it. Withholding is the safe outcome; it
//    self-resolves as first-party history accrues past each window.
const TREND_CONFIDENCE = {
  ...
  sourceDisagreePct: 0.25, // same-day catalog-vs-ppt_backfill gap that condemns the card
};
...
// (D) Price-condition provenance (2026-09-11). A trend is a claim that two
// observations of the SAME reference moved. That claim is only made when
// EVERY point from the comparison date to the latest is a VERIFIED,
// COMPARABLE observation:
//   * verified   - the row recorded both the condition and the printing
//                  the price was for (pre-migration rows, the ppt_backfill
//                  prefix and any headline-derived row have nothing / only
//                  part of it recorded -> "provenance-unknown");
//   * comparable - condition AND printing equal the latest point's; a
//                  change anywhere in the span - including NM -> LP -> NM,
//                  whose endpoints happen to match - is a change of
//                  reference, not a price movement ("reference-changed").
// Nothing here ever reads a missing provenance as Near Mint.
function pointProvenanceComplete(p) {
  return p != null && p.referenceCondition != null && p.referencePrinting != null;
}
function provenanceKey(p) {
  return pointProvenanceComplete(p) ? `${p.referenceCondition}|${p.referencePrinting}` : null;
}

// Are the points in [fromDate, toDate] (inclusive, `series` oldest ->
// newest) all verified and all the same reference as the point at toDate?
// -> { ok: true } | { ok: false, reason: "provenance-unknown" | "reference-changed" }
function spanComparability(series, fromDate, toDate) {
  const pts = (series ?? []).filter((p) => p.date >= fromDate && p.date <= toDate);
  const end = pts.length ? pts[pts.length - 1] : null;
  if (!end || !pointProvenanceComplete(end)) return { ok: false, reason: "provenance-unknown" };
  const key = provenanceKey(end);
  for (const p of pts) {
    if (!pointProvenanceComplete(p)) return { ok: false, reason: "provenance-unknown" };
    if (provenanceKey(p) !== key) return { ok: false, reason: "reference-changed" };
  }
  return { ok: true, reason: null };
}

// The trailing run of verified points that share the latest point's
// reference - the only history that is "comparable" to the latest value.
// Empty when the latest point itself is unverified. Used for the chart's
// comparable range and its legend; never derived from the live reference.
function comparableTail(series) {
  const pts = series ?? [];
  if (!pts.length || !pointProvenanceComplete(pts[pts.length - 1])) return [];
  const key = provenanceKey(pts[pts.length - 1]);
  let i = pts.length - 1;
  while (i >= 0 && provenanceKey(pts[i]) === key) i--;
  return pts.slice(i + 1);
}

// Kept for callers that compare two individual points.
function referenceChanged(a, b) {
  if (!a || !b) return false;
  const condDiffers = a.referenceCondition != null && b.referenceCondition != null && a.referenceCondition !== b.referenceCondition;
  const printDiffers = a.referencePrinting != null && b.referencePrinting != null && a.referencePrinting !== b.referencePrinting;
  return condDiffers || printDiffers;
}

// Trend windows with the anomaly gate applied. SAME shape as
// trendWindows() (each key null or a trend object) plus a `confidence`
// descriptor. (A) or (B) suppress every window for the card; (C) and (D)
// suppress just the affected window. When (D) withholds the 30-day window
// the market signal's basis is gone, so the confidence carries that
// window's reason ("provenance-unknown" / "reference-changed") for the
// UI's restrained wording. `withheld` lists every window (D) withheld and
// why.
function confidentTrendWindows(series, { rawRows = null, cfg = TREND_CONFIDENCE } = {}) {
  ...
  for (const w of TREND_WINDOWS) {
    const t = base[w.key];
    if (!t || cardLow || comparePointAnomaly(series, t.comparedOn, cfg)) {
      windows[w.key] = null;
      continue;
    }
    const span = spanComparability(series, t.comparedOn, latest.date);
    if (!span.ok) {
      withheld[w.key] = span.reason;
      windows[w.key] = null;
      continue;
    }
    windows[w.key] = t;
  }
  const d30Reason = withheld.d30 ?? null;
  return {
    windows,
    confidence: {
      level: cardLow || d30Reason ? "low" : "ok",
      reason: endpoint ? "endpoint-anomaly" : sourceBad ? "source-disagreement" : d30Reason,
      withheld,
    },
  };
}
