...
import { UTM_KEYS, sanitizeUtmValue } from "./props.js";

// ---- attribution allowlist on the FINAL payload (17C.0) -------------
// The only attribution values allowed to leave the browser are:
//   traffic_source                  - our derived category
//   utm_source / utm_medium /       - only when the value is exactly what
//   utm_campaign / utm_content        sanitizeUtmValue() approves
// Everything else of this kind is removed, whoever added it (the SDK's
// campaign capture - disabled in config.js, removed here too in case a
// future SDK re-enables it - or a careless call site):
//   * ad-platform click IDs (identifiers tied to an ad click)
//   * utm_term (free-text keyword) and any unapproved utm value
//   * the SDK's search-engine keyword (ph_keyword - can carry the text a
//     visitor typed into a search engine)
//   * the $initial_* copies of all of the above
export const CLICK_ID_KEYS = Object.freeze([
  "gclid", "gclsrc", "dclid", "gbraid", "wbraid", "fbclid", "msclkid", "twclid", "li_fat_id",
  "igshid", "ttclid", "rdt_cid", "epik", "qclid", "sccid", "irclid", "_kx", "gad_source", "mc_cid",
]);
const REMOVED_ATTRIBUTION_KEYS = new Set([...CLICK_ID_KEYS, "utm_term", "ph_keyword", "$search_keyword"]);
const APPROVED_UTM_KEYS = new Set(UTM_KEYS);

function enforceAttributionAllowlistOn(p) {
  if (!p || typeof p !== "object") return;
  for (const key of Object.keys(p)) {
    const bare = key.startsWith("$initial_") ? key.slice("$initial_".length) : key;
    if (REMOVED_ATTRIBUTION_KEYS.has(bare)) {
      delete p[key];
      continue;
    }
    if (APPROVED_UTM_KEYS.has(bare)) {
      const v = p[key];
      if (v === null || v === undefined) {
        delete p[key]; // the SDK attaches unset campaign keys as null - no value, no key
        continue;
      }
      const clean = sanitizeUtmValue(v);
      if (clean === undefined || clean !== v) delete p[key];
    }
  }
}
...
export function buildBeforeSend({ allowed = ALLOWED_EVENTS } = {}) {
  ...
  // 17C.0 - attribution allowlist, last: runs on the fully assembled event
  // (our props + everything the SDK attached). Never touches token /
  // distinct_id; only removes the attribution keys listed above.
  const enforceAttributionAllowlist = (event) => {
    if (!event) return null;
    enforceAttributionAllowlistOn(event.properties);
    return event;
  };

  return [dropDisallowedEvents, scrubProperties, enforceAttributionAllowlist];
}
