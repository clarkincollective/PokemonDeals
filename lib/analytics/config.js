...
export function buildPostHogConfig({ beforeSend } = {}) {
  return {
    ...
    // --- attribution comes from OUR sanitised landing context only ---
    // (17C.0) With the default save_campaign_params:true, posthog-js copies
    // EVERY campaign parameter from the landing URL - raw utm_* (incl.
    // utm_term), gad_source, mc_cid and 17 ad click IDs (gclid, fbclid,
    // msclkid, ...) - into memory and attaches them to every event,
    // bypassing sanitizeUtmValue. Off: the only attribution that leaves the
    // browser is traffic_source + the approved utm_* set by
    // AnalyticsBootstrap. before_send (sanitize.js) enforces the same
    // allowlist on the final payload as a second layer.
