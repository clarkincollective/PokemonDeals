...
import { UTM_KEYS, classifyTrafficSource, aiAssistantSource, deviceClassFromWidth, sanitizeUtmValue } from "./props.js";
...
export function readLandingAttribution() {
  ...
  // Which assistant, when it IS one. Only set for ai_assistant traffic, so
  // every other visit carries no extra property at all, and only from the
  // fixed vocabulary in props.js - never a host or a URL.
  const ai_source = traffic_source === "ai_assistant" ? aiAssistantSource({ referrer, utmSource: utm.utm_source }) : null;
  return {
    traffic_source,
    ...(ai_source ? { ai_source } : {}),
