...
// WHICH assistant, not just "an assistant" (2026-09-21 GEO audit).
// `traffic_source` collapses every one of them into "ai_assistant", so the
// question "which AI systems actually send visitors that click through to
// a listing" could not be answered at all. This returns a small fixed
// vocabulary - never a host, never a URL, never free text - so it carries
// no more information about a visitor than the category already did.
//
// Deliberately conservative: anything not recognised returns null rather
// than guessing, so the figure is always "assistants we can name", not an
// estimate.
const AI_SOURCE_BY_HOST = Object.freeze({
  "chatgpt.com": "chatgpt",
...
const AI_SOURCE_BY_UTM = Object.freeze({
  "chatgpt.com": "chatgpt",
  chatgpt: "chatgpt",
  openai: "chatgpt",
  "chat.openai.com": "chatgpt",
...
export function aiAssistantSource({ referrer = "", utmSource = "" } = {}) {
  const fromUtm = AI_SOURCE_BY_UTM[String(utmSource ?? "").trim().toLowerCase()];
  if (fromUtm) return fromUtm;
  try {
    const host = new URL(referrer).hostname.toLowerCase();
    return AI_SOURCE_BY_HOST[host] ?? null;
  } catch {
    return null;
  }
}
