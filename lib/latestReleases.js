...
export function latestReleaseLineup(clock, { releasedCount = LATEST_RELEASED_COUNT } = {}) {
  ...
  const upcoming = OFFICIAL_RELEASES.filter((r) => r.released > day).map(entry).sort(byDateDesc);
  const released = OFFICIAL_RELEASES.filter((r) => r.released <= day).map(entry).sort(byDateDesc).slice(0, releasedCount);
...
// date; after release it says it is out. Neither implies availability of
// any particular listing.
export function featuredReleaseLine(entry) {
  if (!entry) return null;
  return entry.status === "upcoming"
    ? `Releases ${entry.releaseDateText} · listings appear here only once eBay confirms them`
