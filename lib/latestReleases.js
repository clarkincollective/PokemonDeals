...
export function latestReleaseLineup(clock, { releasedCount = LATEST_RELEASED_COUNT, releases = OFFICIAL_RELEASES } = {}) {
  ...
  // Judged in each record's own release zone (a worldwide release flips in
  // Sydney, a staggered one in Los Angeles) - the same rule as
  // expansionReleaseStatus, so the lineup and the badge never disagree.
  const upcoming = releases.filter((r) => !releasedAt(r, clock)).map(entry).sort(byDateDesc);
  const released = releases.filter((r) => releasedAt(r, clock)).map(entry).sort(byDateDesc).slice(0, releasedCount);
