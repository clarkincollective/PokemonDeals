...
// audit-r1 (verify what people are about to buy) - the FEATURED tier of the
// general slots. Premium placement (homepage flagship row, Best Finds,
// Top 10) requires an exact verification within
// PREMIUM_EXACT_VERIFICATION_MAX_AGE_HOURS; on 15 Sep only 60 of 1,145 shown
// deals had one, so the lanes that earn most clicks were starved for want
// of a check. A fixed-price deal that would qualify for those lanes once
// verified - inside the Best Finds band (lib/deals BEST_FINDS_MIN_MARKET_PRICE
// / BEST_FINDS_MAX_DISCOUNT_PCT, mirrored here), savings evidenced, no
// visual match still pending, and not verified inside the premium window -
// is checked ahead of the other high-value rows, highest savings first.
// Auctions and never-verified new listings keep their precedence; no
// budget, batch or cadence changes.
export const FEATURED_MIN_MARKET_PRICE = 75;
export const FEATURED_MAX_DISCOUNT_PCT = 0.65;
export function featuredCandidate(r, now = Date.now()) {
  if (!r || r.listing_type === "AUCTION") return false;
  const market = Number(r.market_price);
  const discount = Number(r.discount_pct);
  if (!(market >= FEATURED_MIN_MARKET_PRICE)) return false;
  if (!(discount >= DEAL_DISCOUNT_THRESHOLD && discount <= FEATURED_MAX_DISCOUNT_PCT)) return false;
  if (!savingsClaimTrusted(r, now)) return false;
  if (!r.is_graded && premiumNeedsVisualMatch(r) && r.visual_authenticity_status !== "MATCH") return false;
  const verifiedHours = hoursSinceExactVerification(r, now);
  return !(Number.isFinite(verifiedHours) && verifiedHours <= PREMIUM_EXACT_VERIFICATION_MAX_AGE_HOURS);
}
...
// out. Before any provider call, the route reserves each lane pick durably
// and atomically, keyed by the exact eBay item (legacy id - every
// marketplace copy of one listing shares it), in lib/gradedRetentionAttempts
// (existing catalog_snapshot table; separate from availability evidence).
// This module stays pure: it only honours `retentionBlocked` (items with a
// live reservation, or whose reservation failed this run) and
// `retentionAllowed` (the items this run reserved). The cooldown applies to
// this lane only; the general ranking can still pick such an item.
//
...
// A listing exactly checked, or reserved by this lane, this recently is not
...
// `blocked` = item keys in cooldown / not reservable this run.
export function gradedRetentionCandidate(row, now = Date.now(), { conflicts = new Set(), blocked = null } = {}) {
  ...
  if (blocked && blocked.has(retentionAttemptKey(row))) return false; // UNKNOWN cooldown, per exact item
  return true;
}
...
function generalRank(r, now) {
  ...
  if (featuredCandidate(r, now)) return 2; // audit-r1: see featuredCandidate
  if (highValue) return 3;
  if (midValue) return 4;
  return 5;
}
function generalTieBreak(r, now) {
  ...
  if (featuredCandidate(r, now)) return -(Number(r.discount_pct) || 0); // highest savings first
...
export function allocateVerifyBatch({ pool = [], batch = 20, now = Date.now(), quotaRemaining = Infinity, reserve = 0, gradedSlots = GRADED_RETENTION_SLOTS, retentionBlocked = null, retentionAllowed = null } = {}) {
  const seen = new Set();
  const take = (rows, cap) => {
    const out = [];
    for (const r of rows) {
  ...
  if (gradedSize > 0) {
    ...
    gradedSubpool = bins
      .filter((r) => !seen.has(r.id) && gradedRetentionCandidate(r, now, { conflicts, blocked: retentionBlocked }))
      // when the route passes the items it reserved, only those may use the lane
      .filter((r) => !retentionAllowed || retentionAllowed.has(retentionAttemptKey(r)))
