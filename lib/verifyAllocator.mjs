...
//
// COST: slots are taken OUT of the batch, never added; unused slots fall
// through to the BIN reserve and the general ranking. When used, they
...
// Set GRADED_RETENTION_SLOTS = 0 to disable.
export const GRADED_RETENTION_SLOTS = 2;
// A row exactly checked this recently is not re-picked by this lane.
...
export function gradedRetentionCandidate(row, now = Date.now(), { conflicts = new Set() } = {}) {
...
export function allocateVerifyBatch({ pool = [], batch = 20, now = Date.now(), quotaRemaining = Infinity, reserve = 0, gradedSlots = GRADED_RETENTION_SLOTS } = {}) {
  const seen = new Set();
  const take = (rows, cap) => {
    const out = [];
    for (const r of rows) {
  ...
  // 1b) graded-retention-r1: graded fixed-price rows nearest their real
  //     freshness cutoff, inside the same batch.
  ...
  if (gradedSize > 0) {
    ...
    gradedSubpool = bins
      .filter((r) => gradedRetentionCandidate(r, now, { conflicts }))
  ...
  const gradedPicked = take(gradedSubpool, Math.max(0, Math.min(gradedSize, batch - criticalPicked.length)));
  ...
  const binPicked = take(binSubpool, Math.max(0, Math.min(reserveSize, batch - criticalPicked.length - gradedPicked.length)));

  // 3) everything else, by the preserved P0.2 rank/tie-break, but with
  //    cooldown'd auctions dropped from the auction tier.
  const remainingSlots = batch - criticalPicked.length - gradedPicked.length - binPicked.length;
  const general = pool
  ...
  const finalBatch = [...criticalPicked, ...gradedPicked, ...binPicked, ...generalPicked];
