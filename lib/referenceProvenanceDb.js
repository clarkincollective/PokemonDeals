...
// 17C.10 - THE COLUMN PROBE, three-valued on purpose.
//
// A boolean probe ("did the select error?") conflates two very different
// situations, and the conflation is dangerous once the columns exist: a
// permission error, an expired JWT, a network blip or a 5xx would all read
// as "columns missing", the writer would omit the reference columns, and
// the result is a COMPARISON-ONLY write - a new market_price wearing the
// previous row's evidence. That is precisely the staleness the atomic
// write exists to prevent, reintroduced through the probe.
//
//   "present"  no error                      -> write comparison + evidence
//   "missing"  ONLY isMissingProvenanceColumnError
//                                            -> omit them; safe, because if
//                                               the columns do not exist
//                                               there is no old evidence to
//                                               retain
//   "unknown"  any other failure             -> STILL write the evidence
//
// The asymmetry is deliberate. Writing evidence when the columns turn out
// to be absent fails the whole statement with 42703: nothing is written,
// the row is untouched, and the error is visible. Omitting evidence when
// the columns are present silently corrupts the row's meaning. Only the
// first failure mode is acceptable.
async function probeReferenceColumns(db, table, column = "reference_source") {
  try {
    const { error } = await db.from(table).select(column).limit(1);
    if (!error) return "present";
    return isMissingProvenanceColumnError(error) ? "missing" : "unknown";
  } catch {
    return "unknown";
  }
}

// May this writer put the reference columns in its write? Everything
// except a DEFINITIVE "missing" says yes.
function writesReferenceColumns(probeResult) {
...
module.exports = {
  ...
  probeReferenceColumns,
  writesReferenceColumns,
};
