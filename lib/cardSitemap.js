...
// A real calendar day in "YYYY-MM-DD" form. DAY_RE alone is only a SHAPE
// check - it happily matches "2026-13-01" and "2026-02-30", which V8 then
// rolls forward rather than rejecting - so the parse is round-tripped.
// Shared by sanitizeLastmodMap and the substance gate so both agree on
...
export function sanitizeLastmodMap(raw) {
  ...
  for (const [id, v] of Object.entries(raw)) {
    if (!id || !isCalendarDay(v)) continue;
    map[String(id)] = v;
  }
  return map;
}
...
// SITEMAP SUBSTANCE GATE (SEO-4, 16 Sep 2026).
//
// A sitemap is a RECRAWL-PRIORITY signal, not a site inventory. Listing
// every indexable card spends a rationed crawl budget on pages that have
// nothing new to fetch. Measured on 2026-09-16: of 24,691 catalogue cards
// eligible for the card shards, only 10,506 had a <lastmod> - i.e. a
// first-party-proven material change to the price the page displays (the
// cardReferenceLastmod contract above) - and 620 had a live listing. The
// other ~13.9k advertise a URL whose displayed content has never provably
// changed since we began recording it.
//
// So a card is ADVERTISED when at least one of these holds:
//
//   1. it is a live-deal hub (its listings change constantly, and it is
//      indexable by its own rule - CARD_HUB_MIN_LISTINGS - so it must be
//      in a sitemap at all times);
//   2. it carries a real <lastmod> (its displayed price has provably
//      moved, so there is something to recrawl);
//   3. it is linked from a guide or news article (editorial cards we want
//      discovered because we point readers at them).
//
// This NARROWS the sitemap only. It changes NOTHING about the pages: a
// dropped card keeps its URL, its canonical, its indexability and its
// internal links from the set / species / cards hubs, and Google may index
// it exactly as before. Nothing is deleted, hidden or noindex'd - the
// invariant sitemap-parity.test.mjs pins (sitemap membership is a SUBSET
// of what renders indexable) is preserved, since a subset of a subset is
// still a subset.
//
// FAIL OPEN. The lastmod map comes from an RPC that may be absent or time
// out, in which case fetchCardLastmodMap returns an empty map rather than
// throwing. Enforcing the gate against an empty map would silently cut the
// card sitemaps down to the few hundred live hubs. `enforce` is therefore
// passed false whenever the lastmod source is not "rpc", and the full
// membership is emitted exactly as before.
export function selectSitemapCards(cards, { lastmodByTcg = new Map(), editorialSlugs = new Set(), enforce = true } = {}) {
  const list = cards ?? [];
  if (!enforce) return { kept: list, dropped: [], enforced: false };
  const kept = [];
  const dropped = [];
  for (const c of list) {
    if (!c?.slug) continue;
    const lastmod = c.tcgplayerId != null ? lastmodByTcg.get(String(c.tcgplayerId)) : null;
    if (c.hub || isCalendarDay(lastmod) || editorialSlugs.has(c.slug)) kept.push(c);
    else dropped.push(c.slug);
  }
...
//
// On 2026-09-19 the sitemap index stopped listing cards-bulk to steer
// crawl toward higher-value pages (b2f3272). Inspected on 2026-09-20 that
// left the shard in a contradictory state: /sitemaps/cards-bulk.xml still
// served 6,050 URLs (all passing the substance gate), Search Console still
// held a DIRECT submission of it from 15 Sep, and six live-deal hubs plus
// 41 guide-linked cards sat in an unadvertised file - against the rule
// above that a hub "must be in a sitemap at all times".
//
// Resolution: the bulk band is advertised again, but for the lowest-value
// band a proven price change alone is not enough to spend crawl on. A
// bulk-band card is listed only when it is a live-deal hub or a card an
// article points readers at; sub-$5 cards whose only signal is a lastmod
// leave the sitemaps (they stay live, canonical, indexable and internally
// linked - nothing about the page changes). Still a subset of what renders
// indexable, so sitemap parity holds. Pure; the caller passes the shard's
// entries as assignCardShards produced them plus the editorial slug set.
export function selectBulkShardCards(entries, { editorialSlugs = new Set() } = {}) {
  const kept = [];
  const dropped = [];
  for (const e of entries ?? []) {
    if (!e?.slug) continue;
    if (e.hub || editorialSlugs.has(e.slug)) kept.push(e);
    else dropped.push(e.slug);
  }
  return { kept, dropped };
}
...
export function assignCardShards(cards, lastmodByTcg = new Map()) {
  ...
  for (const c of cards ?? []) {
    ...
    shards.get(band).push({ slug: c.slug, lastmod: isCalendarDay(lastmod) ? lastmod : null, noReference: !cardValueBand(c.marketPrice) });
  }
