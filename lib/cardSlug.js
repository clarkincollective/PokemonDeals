...
// "4/102"), so it is kept ahead of the set and the " Price & Value" tail;
// those are dropped in that order before the card name is ever truncated.
export function catalogCardTitle(name, set, cardNumber = null) {
  const num = cardNumber ? String(cardNumber).trim() : "";
  const withNum = num ? `${name} ${num}` : name;
