...
// imaged; a same-slug tie is broken on a STABLE key (lowest tcgplayer_id),
// NEVER on price - price must not decide which printing a permanent URL
// resolves to. Returns null when nothing in the set matches or nothing
// that matches passes the data-quality bar (-> the page 404s instead of a
// thin render). Pure, so the >1,000-row set case is unit-testable without
// a DB.
export function pickCatalogMatch(rows, nameSlug) {
  ...
  // (In practice ~28,345 of 28,348 catalogue slugs are already unique; the
