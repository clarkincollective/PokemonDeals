...
// misses these. Used ONLY to keep a graded eBay SOLD listing out of a
// raw-only recent-sales list (SEO Phase 3 closeout): PokemonPriceTracker's
// `soldListings.ungraded` bucket occasionally contains sales whose title
// clearly names a slab grader. Covers the six the rest of the site
// recognises (PSA / CGC / BGS+Beckett / SGC / ACE / TAG) PLUS the common
// off-brand slab graders that show up in eBay Pokemon sold titles
// (GMA, MGC, HGA, TQG, RCG, MNT, DSG) - every one is only ever on a
// graded slab, so a raw-only list is better off omitting them (see the
// "prefer omitting over misclassifying" rule). This is NOT a second
// grade classifier - GRADED_CARD_PATTERN and the deal-side grader logic
// are untouched.
const GRADER_MENTION_PATTERN =
  /\b(psa|cgc|bgs|beckett|sgc|ace grading|ace\s*\d|tag\s*(grad|\d)|gma|mgc|hga|tqg|rcg|mnt\s*\d|dsg\s*\d)\b/i;

// True when a listing/sale TITLE indicates a graded slab (either the
// number-adjacent form or a bare grader mention). Conservative on
// purpose: a raw-only list would rather omit an ambiguous "PSA" sale than
// mislabel it as a raw sale.
function titleLooksGraded(title) {
  return (
    typeof title === "string" &&
    (GRADED_CARD_PATTERN.test(title) || GRADER_MENTION_PATTERN.test(title))
  );
}
