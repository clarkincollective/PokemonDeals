...
// as opposed to a seller typo / dropped digit / truncated title. Only a
// real set-size clash: both look like real set totals (>= 40), neither is
// a substring of the other ("32" vs "132" is a dropped digit), and they
// are not trivially close.
...
// "mega" or "m" immediately before a 3+ letter word (a species), or the
// historical "M <Species> EX" shorthand. Does NOT fire on set names like
// "Mega Evolution" followed by a rule word, or a stray initial.
function megaFormAsserted(text) {
  const s = String(text ?? "");
  return (
    /\bmega\s+[a-z]{3,}/i.test(s) ||
    /\bm\s*-?\s*[a-z]{3,}\s+(?:ex|gx)\b/i.test(s)
  );
}
