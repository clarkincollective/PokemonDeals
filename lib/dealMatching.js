...
// ---------------------------------------------------------------------------
// P0.3.1 - MULTI-CARD / LOT DETECTION
// ---------------------------------------------------------------------------
//
// EXCLUDED_TITLE_PATTERN above only catches the words "lot" / "bundle" /
// "playset". Production (deal 31721) proved that a title can sell several
// distinct cards at one price WITHOUT any of those words - "3 CARDS: 2025
// THROH #050, 2000 ITALIAN FOSSIL #8 HYPNO, 2001 ITALIAN #8 GUARADOS" -
// and still be matched to, and priced against, ONE of the named cards.
// The listing price is for the whole group, so any "% below market" on a
// single catalogue card is fiction, and a graded slab can never hold more
// than one card.
//
// Conservative and evidence-backed (per the phase brief): a Pokemon name
// or number appearing more than once is NOT enough. This fires only on:
//   1. an explicit count of CARDS (plural): "2 cards", "3 CARDS:",
//      "10 pokemon cards", "50 holo cards"
//   2. an explicit lot / set / pair phrase: "card lot", "job lot",
//      "bulk lot", "lot of 4", "set of 3", "pair of cards", "bundle of 5",
//      "x3 pokemon", "cards x4"
//   3. >=2 DISTINCT full "N/M" collector numbers (M>=10) joined by a
//      separator ( , & + "and" ) - the "sold together" evidence - and
//      NOT a DP/HGSS-era LEGEND card (one card printed as two numbered
//      halves, "89/90 & 90/90").
const MULTI_CARD_COUNT_RE =
  /\b(?:[2-9]|[1-9]\d)\s*(?:pok[eé]mon\s+|holo\s+|graded\s+)?cards\b/i;
const MULTI_CARD_PHRASE_RE =
  /\bcards?\s+lot\b|\bjob\s?lot\b|\bbulk\s+lot\b|\blot\s+of\s+\d|\bset\s+of\s+(?:[2-9]|[1-9]\d)\b|\bpair\s+of\s+(?:cards?|pok[eé]mon|holos?)\b|\bbundle\s+of\s+\d|\b(?:[2-9]|[1-9]\d)\s?x\s?(?:pok[eé]mon|holo|cards?|singles?)\b|\b(?:pok[eé]mon|holo|card)s?\s?x\s?(?:[2-9]|[1-9]\d)\b|\bx\s?(?:[2-9]|[1-9]\d)\s+(?:pok[eé]mon|holo|cards?|singles?)\b/i;
// A LEGEND (DP/HGSS) is ONE card in two numbered halves - never a lot.
const LEGEND_HALVES_RE =
  /\blegend\b|\btop\s*(?:&|and|\+|\/|-)\s*bottom\b|\bbottom\s*(?:&|and|\+|\/|-)\s*top\b|\bhalf\s*cards?\b/i;

function distinctCollectorNumerators(title) {
  const nums = [];
  for (const m of String(title ?? "").matchAll(/\b(\d{1,3})\s*\/\s*(\d{1,3})\b/g)) {
    if (parseInt(m[2], 10) >= 10) nums.push(parseInt(m[1], 10));
  }
  return new Set(nums);
}

function multipleNamedCardsSoldTogether(title) {
  const s = String(title ?? "");
  if (distinctCollectorNumerators(s).size < 2) return false;
  // needs a joining token between the card references
  if (!/[,]|&|\+|\band\b/i.test(s)) return false;
  // a LEGEND card legitimately carries two consecutive numbered halves
  if (LEGEND_HALVES_RE.test(s)) return false;
  return true;
}

// True = the listing plainly sells more than one card at one price, so it
// can never be reconciled apples-to-apples with a single catalogue card.
function looksLikeMultiCardListing(title) {
  const s = String(title ?? "").trim();
  if (!s) return false;
  if (MULTI_CARD_COUNT_RE.test(s)) return true;
  if (MULTI_CARD_PHRASE_RE.test(s)) return true;
  if (multipleNamedCardsSoldTogether(s)) return true;
  return false;
}

// ---------------------------------------------------------------------------
// P0.3.1 - LISTING LANGUAGE (generalised reverse guard)
// ---------------------------------------------------------------------------
//
// Moved here (was in lib/dealQuality.js) so listingMatchesCard - the ONE
// gate every ingestion path runs, INCLUDING the graded branch that used
// to skip the old Japanese-only check - can enforce it. dealQuality.js
// re-exports these unchanged.
//
// Markers extended per the phase brief to cover the vintage-European
// printings that were matching English references live (Italian
// "Edizione", German "Deutsche", French "Francaise", Portuguese
// "Portugues", ...). A listing's language is only set when the title /
// item-specific STATES one - never guessed from seller location.
const LANGUAGE_PATTERNS = [
  ...
  ["german", /\bgerman(?:y)?\b|\bdeutsch(?:e|land)?\b/i],
  ["french", /\bfrench\b|\bfran[çc]aise?\b/i],
  ["spanish", /\bspanish\b|\bespa[ñn]ol(?:a)?\b/i],
  ["italian", /\bitalian(?:o|a)?\b|\bedizione\b/i],
...
// Is a listing in `listingLang` allowed to be a deal for a catalogue row
// in `cardLang`? MATCH, not "english only": a Japanese listing is a
// legitimate deal for a Japanese catalogue row. "unknown" listing
// language is allowed (never invent a foreign status with no evidence).
...
// The eBay "Language" item-specific, if the listing object carries a
// localizedAspects array (getItem / feed paths only - null on a plain
// search result). Kept tiny so dealMatching stays dependency-free.
function listingLanguageAspect(listing) {
  const a = (listing?.localizedAspects ?? []).find((x) => /^language$/i.test(x?.name || ""));
  return a?.value ?? null;
}

// ---------------------------------------------------------------------------
// P0.3.1 - GRADED-REFERENCE EVIDENCE INTEGRITY
// ---------------------------------------------------------------------------
//
// The scanner's graded branch fires when eBay's search result carries
// `condition === "Graded"`, then trusts a "Grade" / "Professional Grader"
// item-specific from getItem to look up a slab-price reference. Deal
// 31721 proved a raw 3-card lot can carry those item-specifics (a sloppy
// seller filled them in) and get priced against a $1,389 English PSA 10
// single. A graded market reference is only trustworthy when the listing
// itself credibly shows a SINGLE graded slab whose grade evidence is
// mutually consistent:
//   - not a multi-card lot (a slab holds exactly one card)
//   - a recognised grader (matching lib/ebay.js normalizeGrader output)
//   - a valid numeric grade (1..10, optional .5)
//   - the TITLE independently shows slab-grade evidence
//     (mentionsSlabGrader) - a genuine graded seller always says so
//   - if the title also STATES a grade number, it agrees with the
//     structured grade
// Anything short of that -> the caller must NOT use a graded reference
// (fail closed - never silently fall back to graded pricing).
const RECOGNISED_SLAB_GRADERS = /^(?:PSA|CGC|BGS|SGC|TAG|ACE)$/i;
const VALID_SLAB_GRADE = /^(?:10(?:\.0)?|[1-9](?:\.5)?)$/;

function isRecognisedGrader(g) {
  return RECOGNISED_SLAB_GRADERS.test(String(g ?? "").trim());
}
function isValidGradeValue(v) {
  return VALID_SLAB_GRADE.test(String(v ?? "").trim());
}

// A slab grade number explicitly stated in the title next to a grader /
// "grade(d)", or null. Used only to catch a title<->structured mismatch.
function titleStatedSlabGrade(title) {
  const t = String(title ?? "").replace(/&amp;/gi, "&");
  const m =
    t.match(/\b(?:psa|cgc|bgs|beckett|sgc|csg|ace|tag)\s*-?\s*(10(?:\.0)?|[1-9](?:\.5)?)\b/i) ||
    t.match(/\b(?:psa|cgc|bgs|sgc|csg)(10|[1-9](?:\.5)?)\b/i) ||
    t.match(/\bgrade[sd]?\s*(10(?:\.0)?|[1-9](?:\.5)?)\b/i) ||
    t.match(/\b(10(?:\.0)?|[1-9](?:\.5)?)\s*grade[sd]?\b/i);
  return m ? m[1] : null;
}

// The TITLE-ONLY half of gradedReferenceAllowed: true unless the title
// already guarantees the graded reference will be refused (empty, a
// multi-card lot, or no slab-grade evidence). It needs no structured
// grading, so the scanner checks it BEFORE spending a getGradingDetails()
// call - a listing that fails it could never receive a graded reference
// whatever eBay's item-specifics say (graded-growth-r1). Never a reason to
// ACCEPT anything: gradedReferenceAllowed still applies every rule.
function gradedLookupWorthwhile(listing) {
  const title = String(listing?.title ?? "");
  if (!title) return false;
  if (looksLikeMultiCardListing(title)) return false;
  return mentionsSlabGrader(title);
}

function gradedReferenceAllowed(listing, grading) {
  const title = String(listing?.title ?? "");
  if (!gradedLookupWorthwhile(listing)) return false;
  if (!isRecognisedGrader(grading?.grader)) return false;
  if (!isValidGradeValue(grading?.grade)) return false;
  const stated = titleStatedSlabGrade(title);
...
function qualifiesAsTradingCard(listing) {
  ...
  // P0.3.1 - a multi-card lot is not "one card we can price". This runs
  // here (STAGE 0) so it covers the raw path, the graded path, the feed,
  // AND the read-time display gate (lib/dealQuality.listingIsTradingCard).
  if (looksLikeMultiCardListing(title)) return false;
...
// paper, counts.
// ALTERED cards (recoloured / repainted / "alter art") are NOT handled here:
// lib/dealQuality's DAMAGE_TITLE_PATTERNS already classes "altered" as a
// condition the reference can never be advertised against, and the same
// list now carries the other alteration words (integrity-2026-09-19, deal
// 40228). One home for that vocabulary, not two.
...
// integrity-2026-09-19 - the REVERSE direction of the two lists above: the
// LISTING asserts a product form the catalogue card does not carry. A
// Jumbo/oversized promo, a World Championships reprint (the grey-backed,
// non-tournament-legal decks), a Prerelease or Staff stamp are each a
// distinct, separately-priced product, and a title that says so is
// decisive evidence that this is not the standard card the row was priced
// against. Verified live: 35 active "JUMBO"/"Oversized" rows shown 54-70%
// under standard-size references (deals 39405, 35640, 38874, 40117 ...), 3
// "World Championship(s)" rows (19552, 38260, 24911). Checked
// unconditionally - a matching collector number proves nothing here, the
// jumbo carries the same number as the standard print. Positive
// contradiction only: a title that says nothing is left to the other gates.
const TITLE_ASSERTED_PRODUCT_FORMS = [
  { form: "jumbo", title: /\bjumbo\b|\bover-?sized?\b/i, catalog: /\bjumbo\b|\boversized\b/i },
  { form: "world_championship", title: /\bworld\s*championships?\b|\bworlds\s*(?:20\d\d|\d\d|edition|deck)\b/i, catalog: /\bworld\s*championships?\b/i },
  { form: "prerelease", title: /\bpre[\s-]?release\b/i, catalog: /\bprerelease\b|\bpre-release\b/i },
  { form: "staff", title: /\bstaff\b/i, catalog: /\bstaff\b/i },
];

// The product form a listing title asserts that the catalogue card does not
// carry, or null. Exported for the audit trail (disqualificationReason).
function titleAssertedProductForm(title, card) {
  const t = String(title ?? "");
  if (!t) return null;
  const catalogText = `${card?.name ?? ""} ${card?.set ?? ""}`;
  for (const { form, title: re, catalog } of TITLE_ASSERTED_PRODUCT_FORMS) {
    if (re.test(t) && !catalog.test(catalogText)) return form;
  }
  return null;
}

// integrity-2026-09-19 - a title that POSITIVELY names a different English
// expansion. Verified live on deal 38959: "PSA 8 SHAYMIN EX 77 ROARING
// SKIES XY" was priced against the "XY Promos" Shaymin EX because the set's
// only non-stopword token is the era code "xy", which the title carries as
// an era word. An era code (xy / sm / bw / dp / sv / me / ex / swsh) alone
// is weak evidence; when the catalogue set has NO stronger token in the
// title, a distinctive expansion name in the title decides against the
// match. When the title does carry the catalogue set's own strong tokens,
// a second set name is not decisive (a reprint listing that names both the
// original and the reprint set is left to the reprint-family rule above).
//
// Phrases are the mainline English expansion names (lib/pokemonSets.js
// SET_RELEASE_ORDER, prefix codes stripped), minus names that are also
// Pokemon / generic words (Arceus, Platinum, Generations, Base Set) and
// minus the reprint families REPRINT_SET_MARKERS already handles.
const ERA_CODE_TOKENS = new Set(["xy", "sm", "bw", "dp", "sv", "me", "ex", "swsh", "hgss", "svp", "smp"]);
const KNOWN_EXPANSION_PHRASES = [
  "jungle", "fossil", "team rocket", "gym heroes", "gym challenge", "neo genesis", "neo discovery",
  "neo revelation", "neo destiny", "expedition", "aquapolis", "skyridge",
  "ruby and sapphire", "ruby sapphire", "sandstorm", "team magma vs team aqua", "hidden legends",
  "firered leafgreen", "firered and leafgreen", "team rocket returns", "deoxys", "emerald", "unseen forces",
  "delta species", "legend maker", "holon phantoms", "crystal guardians", "dragon frontiers", "power keepers",
  "diamond and pearl", "diamond pearl", "mysterious treasures", "secret wonders", "great encounters",
  "majestic dawn", "legends awakened", "stormfront", "rising rivals", "supreme victors",
  "heartgold soulsilver", "unleashed", "undaunted", "triumphant", "call of legends",
  "black and white", "black white", "emerging powers", "noble victories", "next destinies",
  "dark explorers", "dragons exalted", "boundaries crossed", "plasma storm", "plasma freeze",
  ...
  "celestial storm", "lost thunder", "team up", "unbroken bonds", "unified minds", "hidden fates",
  "cosmic eclipse",
  "rebel clash", "darkness ablaze", "champion s path", "champions path", "vivid voltage", "shining fates",
  ...
  "paldea evolved", "obsidian flames", "scarlet violet 151", "paradox rift", "paldean fates",
  ...
  "mega evolution", "phantasmal flames", "ascended heroes", "perfect order", "chaos rising",
  "pitch black", "30th celebration",
];
// "&" and a standalone "and" are dropped on both sides so "Scarlet &
// Violet 151", "Scarlet and Violet 151" and "Scarlet Violet 151" all
// normalise to the same words.
const normaliseWords = (s) =>
  ` ${String(s ?? "").toLowerCase().replace(/&/g, " ").replace(/[^a-z0-9]+/g, " ").replace(/\band\b/g, " ").replace(/\s+/g, " ").trim()} `;
const EXPANSION_PHRASES_NORMALISED = [...new Set(KNOWN_EXPANSION_PHRASES.map((p) => normaliseWords(p).trim()).filter(Boolean))];
// A set CODE token ("sv01", "swsh07", "me01", "xy") is not strong evidence
// of a set on its own - sellers omit them and reuse them across eras.
const isSetCodeToken = (tok) => ERA_CODE_TOKENS.has(tok) || /^\d+$/.test(tok) || /^[a-z]{1,4}\d{1,3}[a-z]?$/.test(tok);

// The different expansion the title names, or null.
function titleNamesDifferentExpansion(cardSet, title) {
  const setNorm = normaliseWords(cardSet);
  if (setNorm.trim() === "") return null;
  const titleNorm = normaliseWords(title);
  // Catalogue set carries a strong (non-code) token that the title also
  // has -> the title evidences THIS set; another set name is not decisive.
  const strong = setTokensFor(cardSet).filter((tok) => !isSetCodeToken(tok) && tok !== "and");
  if (strong.length > 0 && strong.every((tok) => titleNorm.includes(` ${tok} `))) return null;
  // A PROMO catalogue set: sellers legitimately name the expansion the promo
  // shipped with ("SM210 Hidden Fates ETB Promo" IS the SM Promos card), so
  // another expansion's name only decides when the title carries NO promo
  // evidence at all - no promo code (XY77, SM210, SWSH039, SVP 053) and no
  // "promo" word. Deal 38959's "...77 ROARING SKIES XY" had neither.
  if (/\bpromos?\b/i.test(String(cardSet))) {
    const t = String(title ?? "");
    if (/\bpromos?\b/i.test(t) || /\b(?:xy|sm|swsh|bw|dp|hgss|svp|smp|xyp|bwp)\s?-?\s?\d{1,3}[a-z]?\b/i.test(t)) return null;
  }
  for (const phrase of EXPANSION_PHRASES_NORMALISED) {
    if (setNorm.includes(` ${phrase} `)) continue; // the catalogue set itself
    if (titleNorm.includes(` ${phrase} `)) return phrase;
  }
  return null;
}
...
function titleCollectorNumbers(title) {
  ...
  // integrity-r1 - letter-prefixed subset numbers. Same shape the search
  // parser accepts (lib/searchIntent PREFIXED_SLASH_NUMBER_RE): the SAME
  // alphabetic prefix on both sides ("TG05/TG30", "SV49/SV94", "H31/H32",
  // "RC3/RC32", "GG34/GG70"); differing prefixes are not one number.
  const prefixedPairs = [...t.matchAll(/(?:^|[^A-Za-z0-9])#?\s*([A-Za-z]{1,5})(\d{1,4})\s*\/\s*([A-Za-z]{1,5})(\d{1,4})\b/g)]
    .filter((m) => m[1].toLowerCase() === m[3].toLowerCase())
    .map((m) => ({ prefix: m[1].toLowerCase(), num: parseInt(m[2], 10), den: parseInt(m[4], 10) }));
  // "#SV64", "#TG05", "#SM81" - a hash-marked letter-prefixed number.
  const prefixedHashes = [...t.matchAll(/#\s*([A-Za-z]{1,5})(\d{1,4})\b/g)].map((m) => ({
    prefix: m[1].toLowerCase(),
    num: parseInt(m[2], 10),
  }));
  // Japanese promo numbering "122/XY-P", "001/SM-P": a card number over a
  // promo series rather than a set size.
  const promoSeriesNumbers = [...t.matchAll(/(\d{1,3})\s*\/\s*([A-Za-z]{2,4})-P\b/gi)].map((m) => ({
    num: parseInt(m[1], 10),
  ...
  return {
    ...
    prefixedHashes,
    promoSeriesNumbers,
    any: pairs.length + hashes.length + promos.length + prefixedPairs.length + prefixedHashes.length + promoSeriesNumbers.length > 0,
  };
}

// Black Star promo families whose code+number names ONE promo card
// ("SM210", "SWSH050", "XY124"). Deliberately excludes "sv"/"s", which are
// also expansion codes and the Shiny Vault prefix.
const PROMO_CARD_FAMILIES = new Set(["sm", "swsh", "xy", "bw", "dp", "hgss", "svp", "smp", "xyp", "swshp", "bwp"]);

// A title promo token that is an explicit promo CARD number rather than an
// expansion code: the family's codes and set codes share letters ("SWSH11"
// Lost Origin vs "SWSH110" a promo), so require 3+ digits or a zero-padded
// number ("SWSH050", "XY01") or a number above any expansion code (>= 13).
function explicitPromoCardNumbers(promoTokens) {
  return (promoTokens ?? []).filter((code) => {
    const m = String(code).match(/^([a-z]+)(\d+)$/);
    if (!m || !PROMO_CARD_FAMILIES.has(m[1])) return false;
    return m[2].length >= 3 || /^0/.test(m[2]) || parseInt(m[2], 10) >= 13;
  });
}
...
function collectorNumberConflict(title, catalogNumber) {
  ...
  if (cat.kind === "pair") {
    // integrity-r1 - a letter-prefixed catalogue number ("TG17/TG30",
    // "SV22/SV94") is contradicted by the same subset's numbering naming
    // a different card ("TG05/TG30", "#SV64", "TG05"). Agreement on this
    // prefix settles it; no same-prefix token falls through unchanged.
    if (cat.prefix) {
      let samePrefix = [
        ...t.prefixedPairs.filter((p) => p.prefix === cat.prefix),
        ...t.prefixedHashes.filter((p) => p.prefix === cat.prefix),
      ];
      // A bare "TG05" (no slash, no #) counts only when the title has no
      // prefixed pair - a pair's own "TG30" denominator is not a card
      // number - and never for "sv", which is also a Scarlet & Violet
      // expansion code ("SV3", "SV5").
      if (samePrefix.length === 0 && cat.prefix !== "sv") {
        samePrefix = t.promos
          .map((code) => code.match(/^([a-z]+)(\d+)$/))
          .filter((m) => m && m[1] === cat.prefix)
          .map((m) => ({ prefix: m[1], num: parseInt(m[2], 10) }));
      }
      if (samePrefix.some((p) => p.num === cat.num && (p.den == null || p.den === cat.den))) return false;
      if (samePrefix.length > 0) return true;
    }
    ...
    if (!cat.prefix) {
      // integrity-r1 - the title's only explicit numbering belongs to a
      // different numbering scheme than this regular set card: a subset
      // pair ("H31/H32" vs "33/144"), a Japanese promo number
      // ("122/XY-P" vs "9/106"), or a Black Star promo card number
      // ("SM210" vs Hidden Fates "44/68"). Set codes ("SWSH11", "SV5")
      // and bare numbers stay no-opinion.
      if (t.prefixedPairs.length > 0) return true;
      if (t.promoSeriesNumbers.length > 0 && t.promoSeriesNumbers.every((p) => p.num !== cat.num)) return true;
      if (explicitPromoCardNumbers(t.promos).length > 0) return true;
    }
    return false; // only set codes / unrelated tokens -> no safe opinion
  }
  ...
  if (cat.kind === "promo") {
    ...
    // integrity-r1 - no agreeing promo code, and the title states a set
    // collector number instead ("#69/68", "#69", "H31/H32"): that is a set
    // printing, not this promo.
    if (t.pairs.some((p) => p.den >= 10) || t.hashes.length > 0 || t.prefixedPairs.length > 0) return true;
    return false;
  }
...
// unown-identity-r1 - UNOWN LETTER IDENTITY. Unown cards that differ only by
// letter (or "!" / "?") share name tokens, set and set size, so name + set +
// "/28" evidence matched every letter: "Unown R/28" was stored as Unown (M)
// and "#W/28 ... PSA 7" as Unown (!). The letter is the card's identity.
//
// Catalogue identity, in order of strength:
//   - card_number is a letter pair ("M/28", "!/28")        -> letter required
//   - name "Unown (M)" (the letter-subset naming, which the
//     read-time display gate can see without card_number)  -> letter required
//   - name "Unown [X]" (numbered sets: Neo, promos)        -> reject a
//     contradicting letter only; the collector number still decides
// Title evidence is EXPLICIT only: a letter collector number ("R/28",
// "#W/28", "? / 28"), "Unown (R)" / "Unown [R]", or "Unown R" as its own
// token. A bare "Unown V" never counts (the V card mechanic), and nothing is
// inferred from other tokens ("A28", "1/28"). No evidence where a letter is
// required, or two different letters, is not a match - never a guessed
// letter. REJECT-only.
const UNOWN_LETTER_CHARS = "[a-z!?]";

function cardUnownLetter(card) {
  const name = String(card?.name ?? "");
  if (!/\bunown\b/i.test(name)) return null;
  const pair = String(card?.card_number ?? "").trim().match(new RegExp(`^(${UNOWN_LETTER_CHARS})\\s*/\\s*\\d{1,3}$`, "i"));
  if (pair) return { letter: pair[1].toUpperCase(), required: true };
  const paren = name.match(new RegExp(`\\bunown\\s*\\(\\s*(${UNOWN_LETTER_CHARS})\\s*\\)\\s*$`, "i"));
  if (paren) return { letter: paren[1].toUpperCase(), required: true };
  const square = name.match(new RegExp(`\\bunown\\s*\\[\\s*(${UNOWN_LETTER_CHARS})\\s*\\]`, "i"));
  if (square) return { letter: square[1].toUpperCase(), required: false };
  return null;
}

function titleUnownLetters(title) {
  const t = String(title ?? "");
  const letters = new Set();
  // letter collector number: a lone letter/!/? (not the end of a word) before "/<digits>"
  for (const m of t.matchAll(new RegExp(`(?:^|[^a-z0-9])#?(${UNOWN_LETTER_CHARS})\\s*/\\s*\\d{1,3}(?!\\d)`, "gi"))) letters.add(m[1].toUpperCase());
  if (/\bunown\b/i.test(t)) {
    for (const m of t.matchAll(new RegExp(`\\bunown\\s*[(\\[]\\s*(${UNOWN_LETTER_CHARS})\\s*[)\\]]`, "gi"))) letters.add(m[1].toUpperCase());
    for (const m of t.matchAll(new RegExp(`\\bunown\\s*[-:]?\\s*(${UNOWN_LETTER_CHARS})(?=$|[^a-z0-9])`, "gi"))) {
      const l = m[1].toUpperCase();
      if (l !== "V") letters.add(l);
    }
  }
  return letters;
}

function unownLetterConflict(title, card) {
  const id = cardUnownLetter(card);
  if (!id) return false;
  const letters = titleUnownLetters(title);
  if (letters.size === 0) return id.required;
  if (letters.size > 1) return true;
  return !letters.has(id.letter);
}
...
function listingMatchesCard(listing, card) {
  ...
  // integrity-2026-09-19 - the listing asserts a product form (Jumbo,
  // World Championships, Prerelease, Staff) the catalogue card does not
  // carry. Unconditional; see TITLE_ASSERTED_PRODUCT_FORMS.
  if (titleAssertedProductForm(listing.title, card)) return false;
  ...
  // unown-identity-r1 - Unown letter identity (see unownLetterConflict).
  // Works from the name alone too, so the read-time display gate applies it.
  if (unownLetterConflict(listing.title, card)) return false;
  ...
  // integrity-2026-09-19 - the title positively names a DIFFERENT expansion
  // and the catalogue set has no stronger evidence in it than an era code
  // (deal 38959: "...77 ROARING SKIES XY" vs "XY Promos").
  if (titleNamesDifferentExpansion(card.set, listing.title)) return false;
  ...
  // showing up as) an English one. (Kept as its own stricter rule: a
  // Japanese row needs POSITIVE "japanese" evidence, not merely the
  // absence of another language.)
  if (card.language === "japanese" && !/\bjapan(?:ese)?\b/i.test(listing.title)) return false;

  // P0.3.1 - GENERALISED LANGUAGE REVERSE GUARD. Was Japanese-only; now
  // any explicit non-<cardLang> signal in the title (or the "Language"
  // item-specific, when present) fails the match closed. This runs INSIDE
  // listingMatchesCard so it also covers the scanner's graded branch and
  // the read-time display gate (listingStillMatchesCatalogue), which both
  // skipped the old check. Verified live: deal 31721 (Italian Fossil
  // Hypno lot) + deal 31556 (Italian PSA 1 Aerodactyl) + 2 more active
  // rows priced against ENGLISH references. "unknown" (no stated
  // language) is still allowed - never invent a foreign status.
  const listingLang = classifyListingLanguage({
    title: listing.title,
    itemSpecificLanguage: listingLanguageAspect(listing),
  });
  if (!languageCompatible(listingLang, card.language)) return false;
...
// integrity-r1 - does a title CREDIBLY claim a slab grade? Stricter than
// mentionsSlabGrader (which also fires on a bare "PSA" / "BGS Mint" /
// "Gem Mint 10"): it needs a recognised grader (same tiers as
// mentionsSlabGrader) with a numeric grade attached, or an explicit
// "Grade 7.5", and NO speculative / raw wording. Used to refuse a RAW price
// comparison for a listing the title says is a graded slab - "PSA 9
// Charizard GX", "Ace 9", "(CGA 8.5)" - while leaving genuine raw copies
// that merely talk about grading ("PSA 10 Contender", "PSA ready",
// "likely PSA 9", "PSA 6 or 7", "for PSA CGC BGS") alone.
const SLAB_GRADE_VALUE_SRC = "(?:10|[1-9](?:\\.5)?)(?!\\s*\\/)(?!\\d)";
const SLAB_GRADE_CLAIM_RE = new RegExp(
  `\\b(?:${SLAB_GRADERS_STRONG.join("|")}|${SLAB_GRADERS_WEAK.join("|")}|ace|tag)[\\s:_-]*${SLAB_GRADE_VALUE_SRC}`,
  "i"
);
const GRADE_WORD_CLAIM_RE = new RegExp(`(?<!un)(?<!not )\\bgraded?[\\s:_-]*${SLAB_GRADE_VALUE_SRC}`, "i");
const SPECULATIVE_GRADE_RE =
  /\b(?:contender|candidate|worthy|ready|potential(?:ly)?|possibl[ey]|likely|prospect|hopeful|gem\s*able|pre-?grad\w*|could|would|should|maybe|might|ungraded|not\s+graded|raw|for\s+grading|to\s+grade|send\s+(?:in|off|to))\b|\bor\s+(?:10|[1-9](?:\.5)?)\b|\?/i;

function titleClaimsSlabGrade(title) {
  if (typeof title !== "string" || !title) return false;
  const t = title
    .replace(/&amp;/gi, "&")
    .replace(/\bace\s+(?:spec|trainer)\b/gi, " ")
    .replace(/\btag\s+team\b/gi, " ");
  if (!SLAB_GRADE_CLAIM_RE.test(t) && !GRADE_WORD_CLAIM_RE.test(t)) return false;
  return !SPECULATIVE_GRADE_RE.test(t);
}
...
// 17C.10 - selectConditionPrice's decision, returning the REFERENCE that
// produced the number instead of the number. ADDED ALONGSIDE: the function
// above is untouched and still returns a price, so no caller's figure
// changes. The branch structure is deliberately identical, so
// selectConditionReference(refs, tier, fallbackRef)?.price ===
// selectConditionPrice(prices, tier, fallbackRef?.price) for the same input.
// Returns null wherever selectConditionPrice returns null.
function selectConditionReference(byConditionReference, detectedTier, fallbackReference) {
  const fallback = fallbackReference?.price == null ? null : fallbackReference;
  const detectedIdx = CONDITION_TIERS.indexOf(detectedTier);
  if (detectedIdx === -1) return fallback;

  if (byConditionReference?.[detectedTier] != null) return byConditionReference[detectedTier];

  if (detectedTier !== "Near Mint") {
    for (let i = detectedIdx + 1; i < CONDITION_TIERS.length; i++) {
      const ref = byConditionReference?.[CONDITION_TIERS[i]];
...
// graded-growth-r1 - the ONE graded listing a per-card scan spends its
// getGradingDetails() call on: the first (search results are price-sorted,
// so the cheapest) eBay-"Graded" listing that clears every check the graded
// branch applies before that call, plus the title-only half of
// gradedReferenceAllowed. It used to be the first graded listing
// unconditionally, so a lot / proxy / other-card / no-slab-evidence listing
// at the top either spent the call on a certain rejection or hid every
// other graded listing for that card. Selection only: it never accepts a
// listing the branch would refuse, and the caller still re-checks.
function pickGradedLookupCandidate(listings, card) {
  for (const l of listings ?? []) {
    if (!l?.isGraded) continue;
    if (!gradedLookupWorthwhile(l)) continue;
    if (!qualifiesAsTradingCard(l)) continue;
    if (admitsProxyOrCounterfeit(l, { name: card?.name, set: card?.set })) continue;
    if (!isTrustworthyListing(l)) continue;
    if (!listingMatchesCard(l, card)) continue;
    return l;
  }
  return null;
}
