...
// Every real, whole word/number in a title, as a Set for O(1) membership
// checks. NOT the same as substring containment - see listingMatchesCard's
...
function listingMatchesCard(listing, card) {
  const titleWords = titleWordSet(listing.title);

  // Real, severe bug found live: this used to check
  // normalizedTitle.includes(token) - plain SUBSTRING containment, not
  // whole-word matching. A watchlist card in the "Pokemon GO" set
  // requires the token "go" to appear - "go" is a substring of
  // "dra-GO-nite", so ANY listing merely mentioning "Dragonite" (a
  // completely different card, different set, different rarity) matched
  // and got shown as a "44% below market" deal against the Pokemon GO
  // card's price. Now checks whole-word Set membership, so "go" only
  // matches an actual standalone "go" token in the title, never a
  // substring inside an unrelated word.
  const nameTokens = coreTokens(card.name);
  if (nameTokens.length > 0 && !nameTokens.every((token) => titleWords.has(token))) return false;
  ...
  if (setTokens.length > 0 && !setTokens.every((token) => titleWords.has(token))) return false;
...
// coreTokens and the same whole-word titleWordSet matching, not substring
// containment - see listingMatchesCard's comment for the real bug that
// distinction fixes) - "Twilight Masquerade Booster Box" requires
// "booster" AND "box" in the title, so it can't cross-match an Elite
// Trainer Box (or a different set's box) listing.
...
const CONDITION_SIGNALS = [
  { tier: "Damaged", pattern: /\b(damaged?|dmg|water[\s-]?damage|bent|creased?|torn|stained)\b/i },
  { tier: "Heavily Played", pattern: /heavily played|\[\s*hp\s*\]|\(\s*hp\s*\)/i },
  { tier: "Moderately Played", pattern: /moderately played|\[\s*mp\s*\]|\(\s*mp\s*\)/i },
  { tier: "Lightly Played", pattern: /lightly played|\[\s*lp\s*\]|\(\s*lp\s*\)/i },
  { tier: "Near Mint", pattern: /near mint|\bnm\b/i },
];
