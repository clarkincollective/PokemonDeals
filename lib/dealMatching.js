...
// set by appending the collector number in parens - "Dark Blastoise (3)"
// (the Team Rocket holo, ~$450) vs "Dark Blastoise (20)" (the non-holo,
// ~$15). coreTokens() drops anything under 2 chars, so a single-digit
// number like "(3)" was silently discarded and never required in the
// title - verified live: watchlist "Dark Blastoise (3)" matched an eBay
// listing for "Dark Blastoise 20/82" and priced it against the holo, a
// fake 71%-below "deal". Returns the bare number ("3", "20", "9a") or null.
function cardNumberFromName(name) {
  const m = String(name ?? "").trim().match(/\((\d{1,3}[a-z]?)\)\s*$/i);
...
function listingMatchesCard(listing, card) {
  ...
  if (cardNumber && !titleHasCardNumber(title, cardNumber)) return false;

  // Premium-variant row -> the listing has to name the variant too.
