...
// real cards). Prefers the nearest tier AT OR WORSE than detected (never
// a better tier - that would overstate the listing's condition and
// therefore its "discount"); only falls back to a better tier, nearest
// first, when nothing at-or-worse is priced at all. Returns null if
// byCondition has nothing usable - callers should fall back to
// getConditionPrices' fallbackPrice (Near Mint-ish) in that case, same
// as the pre-existing behavior everywhere before this.
function selectConditionPrice(byCondition, detectedTier) {
  const detectedIdx = CONDITION_TIERS.indexOf(detectedTier);
  if (detectedIdx === -1 || !byCondition) return null;

  for (let i = detectedIdx; i < CONDITION_TIERS.length; i++) {
    const price = byCondition[CONDITION_TIERS[i]];
    if (price != null) return price;
  }
  for (let i = detectedIdx - 1; i >= 0; i--) {
    const price = byCondition[CONDITION_TIERS[i]];
    if (price != null) return price;
  }
  return null;
