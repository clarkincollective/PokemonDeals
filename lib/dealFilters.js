...
// 2026-09-19 - auction ending windows. `ending` narrows to AUCTION rows
// whose stored end time falls inside the window from now. Hours, not a
// calendar "today": the six marketplaces span nine time zones, and "today"
// would mean six different things. "24h" is what a visitor reading "ending
// today" actually wants. Values are the URL vocabulary.
export const ENDING_VALUES = Object.freeze(["1h", "6h", "24h"]);
export const ENDING_HOURS = Object.freeze({ "1h": 1, "6h": 6, "24h": 24 });
export const ENDING_LABELS = Object.freeze({ "1h": "Ending within 1 hour", "6h": "Ending within 6 hours", "24h": "Ending within 24 hours" });
...
export function normalizeDealFilters(raw = {}) {
  ...
  // --- ending window (auctions only) --------------------------------
  // An ending window is an AUCTION filter by definition; like grader ->
  // graded, a contradictory ?listing=BIN is resolved by keeping the more
  // specific window and flipping the listing type, with a note.
  let ending = cleanStr(raw.ending);
  if (ending) ending = ending.toLowerCase();
  if (ending && !ENDING_VALUES.includes(ending)) {
    notes.push({ code: "ending_invalid", message: `Ignored unknown ending window "${raw.ending}".` });
    ending = null;
  }
  if (ending) {
    if (listing === "BIN") {
      notes.push({ code: "bin_vs_ending", message: `"${ENDING_LABELS[ending]}" is an auction filter — showing auctions.` });
    }
    listing = "AUCTION";
  }
  ...
  return { type, grader, grade, listing, ending: ending ?? null, minPrice, maxPrice, notes };
}
...
export function planDealFilters(raw = {}) {
  ...
  // ending window: AUCTION rows whose real stored end time falls in
  // (now, now + hours]. `now` is injectable so the plan is testable; a
  // caller inside a short-lived cache entry gets a window that is at most
  // that entry's age stale, which the display gate (auctionEnded) and the
  // countdown both correct for.
  if (norm.ending) {
    const now = Number.isFinite(Number(raw.now)) ? Number(raw.now) : Date.now();
    eq.listing_type = "AUCTION";
    gte.auction_end_at = new Date(now).toISOString();
    lte.auction_end_at = new Date(now + ENDING_HOURS[norm.ending] * 3_600_000).toISOString();
  }
...
export function appliedFilterChips(raw = {}) {
  ...
  if (n.listing === "AUCTION") chips.push({ key: "listing", label: "Auction", clears: ["listing", "ending"] });
  if (n.ending) chips.push({ key: "ending", label: ENDING_LABELS[n.ending], clears: ["ending"] });
  return chips;
}
...
//
// marketplace-broaden-r1: when raw.country names ONE marketplace, "Browse all
// marketplaces" is offered first (always before "Clear all filters"). It keeps every
// filter and SETS country=all (an explicit choice that a stored or
// geo-detected region can't override); `set` holds params to set.
export function relaxationSteps(raw = {}) {
  const n = normalizeDealFilters(raw);
  const country = typeof raw.country === "string" && /^EBAY_[A-Z]{2}$/.test(raw.country) ? raw.country : null;
  const steps = [];
  // First: it is the only step that keeps every filter the collector chose.
  if (country) steps.push({ label: "Browse all marketplaces", drop: [], set: { country: "all" } });
  ...
  // the ending window is the narrowest auction filter, so it relaxes first
  if (n.ending) steps.push({ label: "Auctions ending any time", drop: ["ending"] });
  if (n.listing !== "all") steps.push({ label: "Buy It Now and auctions", drop: ["listing", "ending"] });
  if (n.type === "graded" && !n.grader && n.grade == null)
    steps.push({ label: "Raw and graded", drop: ["type"] });
  // always last: clear everything
  steps.push({
    label: "Clear all filters",
    drop: ["type", "grader", "grade", "listing", "ending", "minPrice", "maxPrice"],
  });
  return steps;
}

// The href for an auction ending-window pill. Selecting a window sets BOTH
// listing=AUCTION and ending=<window> (a window is an auction filter by
// definition - normalizeDealFilters resolves the pair the same way);
// clicking the active window clears just `ending`, leaving Auction
// selected. Pagination resets. Pure, so the pill logic is unit-testable
// without JSX.
export function endingHref(currentParams, value, basePath = "/") {
  const sp = new URLSearchParams(currentParams ?? {});
  sp.delete("page");
  if (sp.get("ending") === value) {
    sp.delete("ending");
  } else {
    sp.set("listing", "AUCTION");
    sp.set("ending", value);
  }
