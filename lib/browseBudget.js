...
// sealed-rev1 (16 Sep 2026) - SEALED IS NOW FUNDED, at 200/day, taken from
// sweep (1,730 -> 1,530; the whole 200 comes off sweep:EBAY_US, 990 -> 790).
//
// Why sealed needed funding at all: refresh-sealed-deals had been skipped on
// every attempt for five consecutive days, always "ebay_rate_limited". It
// fired at 06:00 UTC - an hour before the 07:00 reset, the emptiest moment of
// the day - and was additionally capped at 0 here, so it could never run even
// with quota to spare. Sealed inventory went 43h+ stale (median last-seen 12
// days) and no sealed row had ever been written with reference provenance,
// which is why no sealed product could evidence a saving.
//
// Why the 200 comes off sweep:EBAY_US specifically. The envelope is strictly
// zero-sum (the assertion below enforces it), so this had to come from
// somewhere, and the US sweep is the most redundant consumer we have: it runs
// 96 times a day over the same query set, so its cap is depth-per-run (~10
// pages) rather than coverage. At 790 it still gets ~8 pages per run, and a
// listing missed by one pass is picked up by the next one 15 minutes later.
// The allocated passes were deliberately NOT touched - they run twice daily,
// so capacity removed there delays discovery of a new card by up to 12 hours.
// verify (integrity) and ingest are untouched.
//
// 200 covers one marketplace pass over the ~195-product sealed watchlist at
// one Browse call per product. The cron passes ?country=EBAY_US for exactly
// that reason; an unscoped run would attempt all six marketplaces (~1,170
// calls) and could never fit.
const CONSUMER_GROUP_CAPS = Object.freeze({ verify: 450, allocated: 2350, sweep: 1530, ingest: 40, images: 10, sealed: 200, manual: 0 });
...
// sealed-rev1: US 990 -> 790, the 200 that now funds sealed. Every other
// country is unchanged - they run 2-hourly rather than every 15 minutes, so
// they have far less redundancy to give.
const SWEEP_COUNTRY_CAPS = Object.freeze({ EBAY_US: 790, EBAY_GB: 160, EBAY_CA: 160, EBAY_AU: 150, EBAY_DE: 130, EBAY_IT: 140 });
const CONSUMER_CAPS = Object.freeze({
  ...
  sealed: CONSUMER_GROUP_CAPS.sealed,
  manual: 0,
});
// Pacing bursts, borrowed from each consumer's own cap.
// sealed-rev1: sealed's burst is its WHOLE cap, deliberately. Pacing exists to
// stop a CONTINUOUS consumer spending its day's capacity in the first hour;
// sealed is a single daily batch that runs at 07:20, twenty minutes after the
// 07:00 reset. Paced like a continuous consumer it would be granted
// pace(200, 20min/24h) ~= 3 calls and the run would be pointless. The burst is
// clamped to the cap by evaluateGrant, so this lets one daily pass take what
// it is funded for and nothing more.
const PACE_BURST = Object.freeze({ verify: 40, sweep: 30, ingest: 40, sealed: 200 });
