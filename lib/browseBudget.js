...
//                      confirming reading net of everything the ledger records.
...
function confirmReset(ledger, outcome, reading) {
  ...
  // everything the ledger records (settled + open, counted in full) is known
  // consumption; only what eBay shows beyond it is unattributed
  const unattributed = reading.limit - reading.remaining - (sumUsed(ledger) + sumOpen(ledger));
  if (unattributed > BIRTH_MAX_CONSUMED) {
    ledger.state = "pending";
    ledger.stateReason = "prior_consumption_unaccounted";
    return;
  }
  ...
  if (unattributed > 0) ledger.used.unattributed_at_birth = (ledger.used.unattributed_at_birth ?? 0) + unattributed;
}
