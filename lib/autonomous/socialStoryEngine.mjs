...
import { hookArchetypeFor } from "../social/analytics/core.mjs";
  ...
  for (const [platform, e] of Object.entries(elig)) {
    ...
    if (built.ok) {
      ...
      const discoveryFields = platformOut ? {
        ...
        // SOCIAL-ANALYTICS-1 §8 - hook type persisted at creation time,
        // never guessed retroactively from performance.
        hook_archetype: hookArchetypeFor(pkg.family, pkg.discovery?.platform?.instagram?.hook ?? null),
