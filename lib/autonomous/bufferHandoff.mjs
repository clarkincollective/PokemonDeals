...
export async function submitBufferPlacementLive(placement, pkg, { env = process.env, provider = null, db = realDb, mediaPackage = null } = {}) {
  ...
  // SOCIAL-DISCOVERY-2 §4/§5/§13 - the real provider-bound text is the
  // discovery-computed, already-audited final_provider_text (caption +
  // budget-fit hashtags) when a discovery manifest was built for this
  // package; falls back to the bare 5B caption for any pre-DISCOVERY-2
  // package (backward compatible - discovery is additive, never required
  // to submit at all).
  const caption = pkg.discovery?.platform?.[placement.platform]?.final_provider_text ?? pkg.captions?.[captionKey]?.caption_text ?? "";
