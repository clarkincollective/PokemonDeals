...
// SOCIAL-LIVE-2: only a WRITE creates the directory. Reading used to
// mkdir too, which threw ENOENT on Vercel's read-only /var/task and turned
// every hourly /api/social-auto run into a 500.
function dirFor(surface, { create = false } = {}) {
...
export function appendRun(surface, run) {
  ...
  rows.push(run);
  writeJson(path.join(dirFor(surface, { create: true }), "runs.json"), rows.slice(-KEEP_RUNS));
  return run;
}
...
export function saveCircuit(surface, c) {
  writeJson(path.join(dirFor(surface, { create: true }), "circuit.json"), c);
  return c;
}
...
export function appendDigestSend(entry) {
  ...
  writeJson(path.join(dirFor("email", { create: true }), "digests.json"), rows.slice(-KEEP_RUNS));
