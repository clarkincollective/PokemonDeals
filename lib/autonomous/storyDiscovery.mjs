  ...
  if (deal?.ok) {
    candidates.push(familyCandidate({
      ...
      facts: {
        ...
        // SOCIAL-CAPTION-5B.1 root-cause fix: keyed by tcgplayer_id, matching
        // three_under_25/printing_compare below - resolvedShapeFor() (in
        // socialStoryEngine.mjs) reads `Object.values(canonical_card_metadata)[0]`
        // and assumes an OBJECT value; a flat {name,set} shape here made that
        // resolve to the STRING "Clefairy" (canonical_card_metadata.name's
        // own value), so `.name` on a string was undefined -> null card_name
        // flowed into the semantic manifest -> a null-interpolated caption
        // prompt -> the caption either invented a card ("Pikachu") or echoed
        // the literal word "null".
        canonical_card_metadata: { [deal.data.tcgplayerId]: { name: deal.data.card_name, set: deal.data.card_set } },
  ...
  if (shape?.ok) {
    candidates.push(familyCandidate({
      ...
      facts: {
        ...
        // SOCIAL-CAPTION-5B.1 - keyed by tcgplayer_id (see the deal_drop
        // comment above for why this exact shape matters).
        canonical_card_metadata: shape.data.featured ? { [shape.data.featured.tcgplayerId]: { name: shape.data.featured.card_name } } : {},
  ...
  if (askEdu?.ok) {
    candidates.push(familyCandidate({
      ...
      facts: {
        ...
        // SOCIAL-CAPTION-5B.1 - keyed by tcgplayer_id.
        canonical_card_metadata: { [askEdu.data.tcgplayerId]: { name: askEdu.data.card_name, set: askEdu.data.card_set } },
