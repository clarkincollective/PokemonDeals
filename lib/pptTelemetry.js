...
//   - anywhere else (pages, card search, catalogue syncs): one row per
//     attempt, persisted AFTER the response via Next's after() (see
//     scheduleAfterResponse), so a visitor's page or API response never waits
//     for it; where no request lifecycle exists (scripts, tests) the bounded
//     write is awaited instead.
...
// Runs task after the current response through a TRACKED lifecycle hook,
// never an untracked fire-and-forget promise. Returns false when no request
// lifecycle is available. Never throws.
//   1. next/server after(): Next's supported API, backed on Vercel by
//      waitUntil, which keeps the invocation alive until the task settles.
//   2. The request context's waitUntil (globalThis[Symbol.for("@next/request-context")],
//      the primitive after() itself uses) - for work Next runs without a
//      request scope, e.g. a background cache revalidation.
function scheduleAfterResponse(task) {
  const safeTask = () => Promise.resolve().then(task).catch(() => {});
  if (afterOverride) return afterOverride(safeTask);
  try {
    require("next/server").after(safeTask);
    return true;
  } catch {
  ...
  try {
    ...
    /* no platform request context */
  }
  return false;
}
let afterOverride = null; // tests only: (task) => boolean
function setPptAfterResponse(fn) {
  afterOverride = fn;
}
...
async function recordPptAttempt({ url, op, res = null, error = null, attempt = 0, bodyText = null }) {
  try {
    ...
    const persist = () => insertRow(sink(), data);
    // Off the response path: scheduled to run after the response through the
    // framework's request lifecycle. Only where no request lifecycle exists
    // (scripts, tests) is the bounded write awaited here.
    if (!scheduleAfterResponse(persist)) await persist();
  } catch {
...
let lastPruneAt = 0;
// Deletes ONLY this telemetry's own rows (kind prefix "ppt_requests:") older
// than the retention window. Never throws.
async function prunePptTelemetry(db, now = Date.now()) {
  try {
    const { error } = await db.from("catalog_snapshot").delete().like("kind", `${KIND_PREFIX}%`).lt("updated_at", new Date(now - RETENTION_MS).toISOString());
    return !error;
  } catch {
    return false;
  }
