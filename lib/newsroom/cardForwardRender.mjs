...
import { runHybridPipeline, hybridEnabled, reviseOnce } from "./hybrid/pipeline.mjs";
...
export async function renderCardForwardStory(story, platform, { renderer, db, renderDir, sha256, env = process.env, hybrid = null } = {}) {
  ...
  // SS4 - every card that is shown must have canonical art
  ...
  const html = renderCardEditorialHtml(spec.layout, { ...facts.props, target, cardArt: art.map, ...(bgDataUrl ? { backgroundDataUrl: bgDataUrl } : {}) });
