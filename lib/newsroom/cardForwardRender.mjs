...
export async function renderCardForwardStory(story, platform, { renderer, db, renderDir, sha256, env = process.env, hybrid = null } = {}) {
  ...
  if (useFullGenerative) {
    ...
    if (!fg.ok) {
      return { ok: false, withheld: { reason: fg.state, detail: fg.reason }, full_generative: { state: fg.state, regenerated: fg.regenerated ?? false, assessed: fg.assessed ?? null, printing_proof: fg.printing_proof ?? null, fact_manifest: fg.factManifest ?? null } };
    }
