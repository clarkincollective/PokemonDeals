...
function gateInputsFor(series, facts, story) {
  ...
  if (series === "MARKET_SNAPSHOT") {
    ...
    Object.assign(base, { card_name: p.card?.name ?? null, card_set: p.card?.set ?? null, card_tcgplayer_id: p.card?.tcgplayerId ?? null, listed_price: p.askingUsd ?? null, market_price: p.marketRefUsd ?? null, sold_price: Array.isArray(p.soldPoints) && p.soldPoints.length ? p.soldPoints[1] ?? p.soldPoints[0] : null });
...
async function resolveFacts(series, { db, cap = 25 } = {}) {
  ...
  if (S === "MARKET_SNAPSHOT") {
    ...
    return {
      ...
      numeric: [r.data.priced_cards, r.data.under_25_pct, r.data.over_100_pct, r.data.featured?.asking_usd ?? r.data.featured?.market_ref_usd].filter(Boolean),
...
export async function renderCardForwardStory(story, platform, { renderer, db, renderDir, sha256, env = process.env, hybrid = null } = {}) {
  ...
  if (useFullGenerative) {
    // 5A.1 - the canonical card_catalog record for the hero card, for the
    // CARD_METADATA_LOCK (rarity / number / set / edition may be shown
    // ONLY when they are in this record).
    let cardCatalogRow = null;
    const heroId = facts.cardIds[0];
    if (heroId != null && /^\d+$/.test(String(heroId))) {
      try {
        const { data } = await db.from("card_catalog").select("tcgplayer_id,name,set,card_number,rarity,language,image_url").eq("tcgplayer_id", String(heroId)).maybeSingle();
        cardCatalogRow = data ?? null;
      } catch { /* metadata lock degrades to name-only */ }
    }
    const fg = await runFullGenerativeSocial({
      ...
      // SOCIAL-LIVE-1: real filesystem paths - the full-generative modules
      // only read these as files, and stripping "file://" off a Windows URL
      // ("file:///C:/...") left "/C:/...", so every card image read as missing
      cardImagePaths: facts.cardIds.map((id) => art.map[String(id)]).filter(Boolean).map((u) => (String(u).startsWith("file:") ? fileURLToPath(u) : u)),
      cardCatalogRow,
      candidates: hybrid?.canvasCandidates,
      budget: hybrid?.budget, env, fetchImpl: hybrid?.fetchImpl ?? fetch,
    });
    if (!fg.ok) {
      return { ok: false, withheld: { reason: fg.state, detail: fg.detail ? `${fg.reason}: ${String(fg.detail).slice(0, 300)}` : fg.reason }, full_generative: { state: fg.state, regenerated: fg.regenerated ?? false, assessed: fg.assessed ?? null, printing_proof: fg.printing_proof ?? null, fact_manifest: fg.factManifest ?? null } };
    }
    ...
    // 5A - the final artifact is the brand-composited HTML (approved
    // deterministic PokemonDealFinder asset over the generated design).
    if (fg.finalHtml) await renderer.renderToPng(fg.finalHtml, localPath);
    ...
    return {
      ...
      full_generative: {
        state: "BUFFER_READY", repaired: fg.repaired, regenerated: fg.regenerated ?? false,
        brand_locked: fg.brand_locked ?? true, model: fg.model,
        fact_lock_hash: fg.factLockHash?.short ?? null,
        verification: fg.verification, selected: fg.selected,
        printing_proof: fg.printing_proof ?? null,
        // 5A.1 - the fact-source locks the auditor enforced
        card_metadata_lock: fg.semanticManifest?.card_metadata_lock ?? null,
        visualization_data_manifest: fg.semanticManifest?.visualization_data_manifest ?? null,
