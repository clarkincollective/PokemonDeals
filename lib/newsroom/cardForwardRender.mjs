...
import { runHybridPipeline, hybridEnabled, freeformEnabled, reviseOnce } from "./hybrid/pipeline.mjs";
...
export async function renderCardForwardStory(story, platform, { renderer, db, renderDir, sha256, env = process.env, hybrid = null } = {}) {
  ...
  // §4B.1 - a freeform blueprint replaces the fixed template entirely.
  const html = hybridResult?.freeform?.html
