...
import { manualPlatforms } from "../social/autopilot/slots.mjs";
...
// SOCIAL-LIVE-1 (2026-09-14, owner-approved cadence): about two posts a day
// per platform, Australia/Brisbane. `launchAt` (ISO) - an owner-authorised
// launch run - puts ONE near-term slot first; every other slot is unchanged.
export const REFILL_SLOT_HOURS_BRISBANE = Object.freeze({ instagram: [10, 18], x: [8, 16] });
export function slotsFor(platform, count, now = Date.now(), { launchAt = null } = {}) {
  const hours = REFILL_SLOT_HOURS_BRISBANE[platform] ?? [10];
  const out = [];
  if (launchAt && Date.parse(launchAt) > now) out.push(new Date(Date.parse(launchAt)).toISOString());
  let day = 1;
...
// SOCIAL-LIVE-3: "<platform>|<epoch ms>" for every placement that already
// occupies a posting time (queued, mid-submit or published).
// AUTOPILOT_READY = a rendered autopilot post reserving its slot (planned_for)
export const SLOT_OCCUPYING_STATUSES = Object.freeze(["BUFFER_QUEUED", "BUFFER_SUBMITTING", "PUBLISHED", "AUTOPILOT_READY"]);
export function slotsBooked(placements = []) {
  const out = new Set();
  for (const p of placements) {
    const t = Date.parse(p.scheduled_for ?? (p.status === "AUTOPILOT_READY" ? p.planned_for : "") ?? "");
    if (!Number.isFinite(t)) continue;
    if (p.buffer_provider_ref || SLOT_OCCUPYING_STATUSES.includes(p.status)) out.add(`${p.platform}|${t}`);
  }
  return out;
}
...
export async function refillQueueReconcile(opts = {}) {
  const {
    ...
    platforms: requestedPlatforms = EDITORIAL_REFILL_PLATFORMS,
    env = process.env, now = Date.now(), provider = null,
    launchAt = null,
  } = opts;

  // A manually produced feed is out of scope for EVERY automatic path, not
  // just the autopilot one. Applied to the resolved list, so it holds for the
  // Saturday/Tuesday cron and for an explicit --platforms argument alike: no
  // channel is resolved for it, no BUFFER_READY row of its is curated, and no
  // depth cap is computed. Its rows, assets and posts are left untouched -
  // they simply stop being candidates. Reversed by the same single control
  // (MANUAL_PLATFORMS_DEFAULT / SOCIAL_AUTOPILOT_MANUAL_PLATFORMS).
  const manual = manualPlatforms(env);
  const platforms = requestedPlatforms.filter((p) => !manual.has(p));
  ...
  for (const p of placements) {
    ...
    if (manual.has(p.platform)) { report.skipped.push({ placement_id: p.placement_id, platform: p.platform, reason: "manually produced feed - left for the owner, not queued" }); continue; }
  ...
  // SOCIAL-LIVE-1: diversity is a per-FEED property. Tracking it across the
  // whole batch meant one story could never reach both IG and X in the same
  // run (found live: the Instagram copy was dropped for the X copy).
  const pickedSeries = {};
  const pickedLayout = {};
  const perPlatform = {};
  let dealDrops = 0;
  ...
  for (const e of order) {
    ...
    const ps = (pickedSeries[e.p.platform] ??= new Set());
    const pl = (pickedLayout[e.p.platform] ??= new Set());
    if (ps.has(e.st.series) && pl.has(lf)) continue;
  ...
  const slotIdx = {};
  const toQueue = [];
  // SOCIAL-LIVE-3: a slot already holding a post (from an earlier run or the
  // daily autopilot) is taken - never double-book a feed slot.
  const bookedSlots = slotsBooked(allPlacements);
  for (const e of curated) {
    const slots = (slotIdx[e.p.platform] ??= slotsFor(e.p.platform, 8, now, { launchAt }));
    let due = null;
    for (let i = 0; i < slots.length; i++) {
      const cand = slots[i];
      if (slots[`_used_${i}`]) continue;
      if (bookedSlots.has(`${e.p.platform}|${Date.parse(cand)}`)) continue;
