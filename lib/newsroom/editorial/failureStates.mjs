...
export const FAILURE_STATES = Object.freeze([
  ...
  // --- SOCIAL-MEDIA-SAFEAREA-1 - PLATFORM CROP / SAFE-ZONE AUDIT ---
  // critical content (text/labels/CTA/brush-stroke callout) sits inside
  // the platform-unsafe margin near an image edge (bottom, by default)
  // and risks being clipped by Instagram/X's own display/crop handling
  "PLATFORM_SAFEAREA_FAIL",
  // a static creative's baked-in fact text (e.g. tracked_population) does
  // not match the current immutable story snapshot it is paired with
  "STATIC_CAPTION_FACT_MISALIGNMENT_FAIL",
  ...
  // --- SOCIAL-AUTOPILOT-3 - production media hosting ---
  // the hosted object's sha256 no longer matches the package's own
  // creative/video sha - never silently re-host or swap the asset (§4)
  "HOSTED_ASSET_DRIFT_FAIL",
  // the hosted public URL did not respond successfully when checked the
  // way the provider would fetch it (§5)
  "MEDIA_URL_UNREACHABLE_FAIL",
  // the hosted object's Content-Type is not an allowed image/video mime (§5)
  "MEDIA_MIME_FAIL",
  // the hosted image/video's decoded dimensions don't match what the
  // platform expects (§5)
  "MEDIA_DIMENSION_FAIL",
  // the hosted video failed an ffprobe-compatible decode/format check (§5)
  "MEDIA_VIDEO_PROBE_FAIL",
  // the hosted bytes' own sha256 does not match the record's sha256 -
  // storage-level identity failure, distinct from a package-level drift (§5)
  "MEDIA_IDENTITY_FAIL",
  // a placement that requires media was about to submit with assets:[] -
  // hard-refused, never a silent text-only fallback (§8)
  "MEDIA_REQUIRED_FAIL",
  // --- SOCIAL-AUTOPILOT-4 - live pilot wiring + caption encoding ---
  // a caption string failed UTF-8/NFC round-trip verification (a
  // replacement character, an unstable normalization, or a corrupted
  // control character) - never posted as-is (§2/§4)
  "CAPTION_ENCODING_FAIL",
  // --- SOCIAL-CAPTION-5B.1 - caption entity lock + placeholder hardening ---
  // the caption names a specific Pokemon/card/set/printing that does not
  // match the story's locked identity (e.g. "Pikachu" when the locked
  // example card is Clefairy) - never a fuzzy same-species pass (§2)
  "CAPTION_ENTITY_MISMATCH",
  // a literal production placeholder (null/undefined/NaN/[object Object]/
  // an unresolved template token) leaked into the generated caption (§5)
  "CAPTION_PLACEHOLDER_FAIL",
  // --- SOCIAL-AUTOPILOT-5.2 - real production failure: an X caption sent ---
  // to Buffer's createPost exceeded X's real 280-character limit
  // (buffer_InvalidInputError, provider-confirmed) - no deterministic
  // check had ever enforced this before
  "X_CAPTION_LENGTH_FAIL",
]);
...
export const REVISABLE_STATES = Object.freeze([
  ...
  "CAPTION_ENTITY_MISMATCH", "CAPTION_PLACEHOLDER_FAIL", "X_CAPTION_LENGTH_FAIL",
  ...
  // SAFEAREA-1 - one bounded regeneration for a platform-crop-unsafe slip
  "PLATFORM_SAFEAREA_FAIL",
]);
