...
function prompt({ platform, family, series, hookText, editorial = false, ctaIntensity = null }) {
  const editorialNote = editorial
    ? `\nThis is a TEXT-FORWARD EDITORIAL / DATA creative (a market snapshot, a ranking, a price story, an explainer). ` +
      `There is DELIBERATELY no card image. Do NOT penalise the absence of a card or "imagery": CARD_DOMINANCE is not applicable here - score it 100. ` +
      `Judge it as editorial design: FACT_HIERARCHY, EDITORIAL_VALUE, TYPOGRAPHY, SPACING, PREMIUM_FEEL, AI_SPAM_RISK, THUMBNAIL_READABILITY.\n`
    : "";
  const ctaNote =
    ctaIntensity === "NONE"
      ? `\nCTA intensity is NONE by design - there is intentionally no call to action. A subtle wordmark only is CORRECT: score CTA_CLARITY 100 and do not flag a missing CTA.\n`
      : ctaIntensity === "BRAND_ONLY"
        ? `\nCTA intensity is BRAND_ONLY by design - a plain domain mention, no hard pitch. That restraint is CORRECT: do not flag it as a weak or missing CTA.\n`
        : "";
  return (
