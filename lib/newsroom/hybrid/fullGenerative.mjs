...
import { classifyImageApiError } from "./openaiError.mjs";
...
const FAMILY_INTENT = Object.freeze({
  ...
  market_shape: (m) =>
    ...
    `KEY FACTS: ${m.required_numeric_facts.under_25_pct}% of ${Number(m.required_numeric_facts.tracked_count).toLocaleString("en-US")} tracked singles have a market price under $25` +
...
export async function generateFullSocial({ prompt, cardImagePaths = [], size = OPENAI_IMAGE_REQUEST_SIZE, budget = null, env = process.env, fetchImpl = fetch } = {}) {
  ...
  const attempt = async (model) => {
    ...
    if (!res.ok) {
      ...
      const error = classifyImageApiError({ status: res.status, body, requestId: res.headers?.get?.("x-request-id") });
      return { ok: false, status: res.status, error, detail: `${error.kind}: ${error.message}`, latency };
    }
  ...
  // the previous model is tried only for a genuinely invalid request - never
  // after a safety refusal (re-sending refused content is not allowed)
  if (!r.ok && r.error?.kind === "invalid_request" && OPENAI_IMAGE_MODEL_PREVIOUS) r = await attempt(OPENAI_IMAGE_MODEL_PREVIOUS);
  if (budget) recordCall(budget, "background_generation", { ok: r.ok, latencyMs: r.latency, detail: r.ok ? "full_social" : `${r.error?.kind ?? "http"}_${r.status}` });
  if (!r.ok) return { ok: false, availability: r.error?.kind === "safety_refusal" ? "safety_refusal" : `http_${r.status ?? "err"}`, error: r.error ?? null, detail: r.detail };
