...
export async function extractImageClaims({ b64, layout, env = process.env, fetchImpl = fetch, budget = null } = {}) {
  ...
  const prompt =
    ...
    `- card_metadata_shown: ["ONLY CARD-DESCRIPTIVE attributes the poster DESIGN adds as its own text - a caption, spec panel, label, badge, or callout ABOUT the card: rarity (Common/Rare/Holo Rare/...), edition (1st Edition/Shadowless/Unlimited), collector number (5/102, #34, SWSH241), grade (PSA 10), printing, variant - verbatim. EXCLUDE anything that is part of the trading-card artwork itself (the card's own printed HP, collector number, level, set symbol, illustrator credit, copyright line). If in doubt whether text is on the card face or added by the poster, leave it out."]\n` +
    `- card_set_shown: "the SET name the poster DESIGN prints as its own label (NOT the set symbol / name on the card art), or null"\n` +
    `- chart_values: [{ "label": "the bucket / category / axis label, verbatim", "value": "the number or % shown for it, verbatim" }]  (every value in any chart, graph, bar, donut, ladder, or stat table)\n` +
    `- chart_population: "any 'based on N ...' / 'sample: N' figure attached to a chart, verbatim, or null"\n` +
    `- source_attribution_text: "any source / methodology line ('eBay Sold Listings', 'across major marketplaces', 'recent sales', 'live market data'), verbatim, or null"\n` +
    `- timeframe_text: "any time window ('last 60 days', 'this week', a date range, 'current'), verbatim, or null"\n` +
    `- footnotes: ["every small footnote / caption / disclosure line, verbatim"]\n` +
    `- brand_safe_zone_content: "describe EXACTLY what occupies the top ~8% strip of the image (the top ~110px). Say 'empty' if it is calm/blank/texture-only. Otherwise name the element (headline, card, chart, logo, icon, large text, ...)"\n` +
    `- brand_safe_zone_occupied: <bool - true if a headline, card, chart, logo, icon, price, or other important visual element is inside that top ~8% strip>\n` +
