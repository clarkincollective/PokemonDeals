...
// The clause added to every generation prompt (§12).
export const BRAND_SAFE_ZONE_CLAUSE =
  "BRAND: do NOT draw the PokemonDealFinder logo or wordmark yourself. Leave a clean, calm, empty horizontal strip across the very TOP ~90px " +
  "(and optionally a matching one at the very BOTTOM) with nothing in it - the real approved brand mark is composited there afterward. " +
  "Never draw a Poke Ball, a red-and-white ball/circle icon, or any official-Pokemon / Nintendo-style brand mark anywhere in the image.";
...
export function compositeBrandAssetHtml({ generatedDataUrl, where = "top", ctaText = null, canvasW = 1080, canvasH = 1350 } = {}) {
  ...
  const bar = (pos) => `<div style="position:absolute;left:0;right:0;${pos}:0;height:${pos === "top" ? 92 : 84}px;z-index:3;` +
    `background:linear-gradient(${pos === "top" ? "180deg" : "0deg"},rgba(11,11,13,0.92),rgba(11,11,13,0.72) 55%,rgba(11,11,13,0));` +
