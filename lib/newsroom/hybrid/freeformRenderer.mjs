...
export function buildSlots(layout, { factLock = {}, resolved = null, cardArt = {} } = {}) {
  ...
  if (layout === "market_shape") {
    ...
    return {
      ...
      hero_stat: { value: under25 != null ? `${under25}%` : "—", caption: tracked != null ? `of ${tracked.toLocaleString("en-US")} tracked singles have a market price under $25` : "have a market price under $25" },
      distribution_bar: {
        segments: [
  ...
  if (layout === "asking_vs_sold") {
    ...
    return {
      ...
      comparison_axis: { left: asking != null ? usd(asking) : "—", right: market != null ? usd(market) : "—", leftLabel: "This listing asks", rightLabel: "Market reference" },
      market_range: { min: soldMin, max: soldMax, mid: market, rangeLabel: sold.length ? "Recent sold range" : "Market reference" },
      hero_stat: premium != null ? { value: `${premium > 0 ? "+" : ""}${premium}%`, caption: "vs the market reference" } : { value: "—", caption: "" },
