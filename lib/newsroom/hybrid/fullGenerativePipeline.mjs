...
export const FULL_GENERATIVE_PIPELINE_VERSION = "5a1.1";
...
export async function runFullGenerativeSocial(opts = {}) {
  ...
  const assess = async (cands) => {
    ...
    for (const cand of cands) {
      ...
      out.push({ cand, fidelity, semantic, factSources, facts, pair, quality });
    }
    return out;
  };
  ...
  const rankKey = (a) => {
    ...
    return semOk * 1e7 + srcOk * 1e6 + factOk * 1e5 + fidOk * 1e4 + pairOk * 1e3 + qOk * 100 + Math.round(qScore(a) / 12);
  };
  const cleanOf = (list) => list.filter((a) =>
    a.semantic.ok && a.factSources.ok && a.fidelity.ok && a.pair.ok && a.quality.verdict !== "FAIL" &&
  ...
  if (!clean.length) {
    ...
    const bySeverity = [
      ...
      "BRAND_SAFE_ZONE_OCCUPIED", "GENERATED_BRAND_RISK",
      "CARD_FIDELITY_FAIL", "FACT_VERIFY_FAIL", "VISUAL_QUALITY_HOLD", "SAFE_REPAIR_REQUIRED",
    ];
    const states = assessed.flatMap((a) => [a.semantic.state, a.factSources.state, a.pair.state, a.fidelity.ok ? null : "CARD_FIDELITY_FAIL", a.facts.ok ? null : a.facts.state, a.quality.verdict === "FAIL" ? "VISUAL_QUALITY_HOLD" : null]).filter(Boolean);
    const state = bySeverity.find((s) => states.includes(s)) ?? "VISUAL_QUALITY_HOLD";
    const why = assessed.map((a) => a.semantic.reason || a.factSources.reason || a.pair.reason || a.fidelity.reason || (!a.facts.ok ? a.facts.reason : null)).filter(Boolean)[0] ?? "no candidate cleared the audit";
