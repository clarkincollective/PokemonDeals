...
import { compositeBrandAssetHtml, BRAND_SAFE_ZONE_CLAUSE } from "./brandLock.mjs";

export const FULL_GENERATIVE_PIPELINE_VERSION = "5a.1";
...
const REGENERABLE = new Set(["SEMANTIC_FACT_FAIL", "CLAIM_SCOPE_FAIL", "SEMANTIC_CONTRADICTION_FAIL", "GENERATED_BRAND_RISK", "CARD_PAIR_FIDELITY_FAIL"]);
...
export async function runFullGenerativeSocial(opts = {}) {
  ...
  // ---- SEMANTIC manifest (§2) --------------------------
  const semanticManifest = buildSemanticManifest({ layout, factLock, resolved, contract, printingProof: printingProof?.proof ?? null });
  ...
  const assess = async (cands) => {
    ...
    for (const cand of cands) {
      ...
      out.push({ cand, fidelity, semantic, facts, pair, quality });
    }
    return out;
  };
  ...
  const rankKey = (a) => {
    ...
    return semOk * 100000 + factOk * 10000 + fidOk * 1000 + pairOk * 100 + qOk * 10 + Math.round(qScore(a) / 12);
  };
  const cleanOf = (list) => list.filter((a) => a.semantic.ok && a.fidelity.ok && a.pair.ok && a.quality.verdict !== "FAIL" && (a.facts.ok || (a.facts.state === "SAFE_REPAIR_REQUIRED" && assessRepair(a.facts.repairable ?? {}).patchable)));
  ...
  if (!clean.length) {
    const worstSemantic = assessed.map((a) => a.semantic).find((s) => !s.ok);
    const worstPair = assessed.map((a) => a.pair).find((p) => !p.ok);
    const worst = worstSemantic ?? worstPair;
  ...
  if (!clean.length) {
    ...
    const bySeverity = ["PRINTING_IDENTITY_FAIL", "CARD_PAIR_FIDELITY_FAIL", "CLAIM_SCOPE_FAIL", "SEMANTIC_CONTRADICTION_FAIL", "SEMANTIC_FACT_FAIL", "GENERATED_BRAND_RISK", "CARD_FIDELITY_FAIL", "FACT_VERIFY_FAIL", "VISUAL_QUALITY_HOLD", "SAFE_REPAIR_REQUIRED"];
    const states = assessed.flatMap((a) => [a.semantic.state, a.pair.state, a.fidelity.ok ? null : "CARD_FIDELITY_FAIL", a.facts.ok ? null : a.facts.state, a.quality.verdict === "FAIL" ? "VISUAL_QUALITY_HOLD" : null]).filter(Boolean);
    const state = bySeverity.find((s) => states.includes(s)) ?? "VISUAL_QUALITY_HOLD";
    const why = assessed.map((a) => a.semantic.reason || a.pair.reason || a.fidelity.reason || (!a.facts.ok ? a.facts.reason : null)).filter(Boolean)[0] ?? "no candidate cleared the semantic audit";
