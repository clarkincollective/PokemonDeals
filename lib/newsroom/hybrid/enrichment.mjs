...
export function planEnrichments({ factLock = {}, contract = null, resolved = null, layout = null } = {}) {
  ...
  if (soldPts && soldPts.length >= 2) {
    add("market_range_bar", "resolver.sold_points", { min: Math.min(...soldPts), max: Math.max(...soldPts), mid: market, rangeLabel: "Recent sold range" }, "recent sold range");
  } else if (num(R.high?.price_usd) != null && num(R.low?.price_usd) != null) {
    add("market_range_bar", "resolver.high/low", { min: num(R.low.price_usd), max: num(R.high.price_usd), rangeLabel: "Printing price spread" }, "printing price spread");
  }
