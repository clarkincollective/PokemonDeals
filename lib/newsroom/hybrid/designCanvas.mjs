...
export async function generateDesignCanvas({ spec, budget = null, env = process.env, fetchImpl = fetch } = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (budget) recordCall(budget, "background_generation", { ok: false, latencyMs: Date.now() - t0, detail: `${error.kind} ${res.status}` });
      return { ok: false, availability: error.kind === "safety_refusal" ? "safety_refusal" : `http_${res.status}`, error, detail: `${error.kind}: ${error.message}` };
    }
