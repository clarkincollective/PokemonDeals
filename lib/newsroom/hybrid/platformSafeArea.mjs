...
export const PLATFORM_SAFEAREA_VERSION = "safearea1.2";
...
function bilinearSample(data, width, height, xf, yf) {
  const x0 = Math.floor(xf), y0 = Math.floor(yf);
  const x1 = Math.min(width - 1, x0 + 1), y1 = Math.min(height - 1, y0 + 1);
  const cx0 = Math.max(0, Math.min(width - 1, x0)), cy0 = Math.max(0, Math.min(height - 1, y0));
  const fx = xf - x0, fy = yf - y0;
  const at = (x, y) => { const i = (y * width + x) * 4; return [data[i], data[i + 1], data[i + 2]]; };
  const [r00, g00, b00] = at(cx0, cy0), [r10, g10, b10] = at(x1, cy0), [r01, g01, b01] = at(cx0, y1), [r11, g11, b11] = at(x1, y1);
  const lerp = (a, b, t) => a + (b - a) * t;
  return {
    r: Math.round(lerp(lerp(r00, r10, fx), lerp(r01, r11, fx), fy)),
    g: Math.round(lerp(lerp(g00, g10, fx), lerp(g01, g11, fx), fy)),
    b: Math.round(lerp(lerp(b00, b10, fx), lerp(b01, b11, fx), fy)),
  };
}

/**
 * applySafeAreaReflow({ pngBuffer, headerZoneHeight, scale }) ->
 *   { png_buffer, scale, header_zone_height, dest_bounds, background_color }
 *
 * SAFEAREA-1.1 §2/§3 - a deterministic derivative transform, not a redraw.
 * The top `headerZoneHeight` rows (the deterministically-composited brand
 * strip) are copied byte-for-byte unchanged - brand design is untouched.
 * Everything below is uniformly scaled (same factor on both axes - never a
 * non-uniform stretch) and re-anchored at the TOP of its own zone,
 * horizontally re-centered, via bilinear resampling (no AI, no redraw, no
 * text/chart/card changes - every source pixel below the header is either
 * copied through the scale or left as sampled background fill). The
 * exposed space this opens up at the bottom (and the sides) is filled with
 * a color sampled from the source image's own existing clean background -
 * "safe padding" / "background extension", never a hardcoded constant,
 * never new content.
 */
export function applySafeAreaReflow({ pngBuffer, headerZoneHeight = 110, scale = 0.92, bgSample = { x: 20, y: null } }) {
  const png = decodePng(pngBuffer);
  const { width, height, data } = png;
  const bgY = bgSample.y ?? Math.max(headerZoneHeight + 5, height - 25);
  const bg = sampleBackgroundColor(png, { x: bgSample.x, y: bgY, size: 12 });
  ...
  // header zone: byte-for-byte copy, untouched.
  for (let y = 0; y < headerZoneHeight; y++) {
    for (let x = 0; x < width; x++) {
      const i = (y * width + x) * 4;
      out.data[i] = data[i]; out.data[i + 1] = data[i + 1]; out.data[i + 2] = data[i + 2]; out.data[i + 3] = 255;
    }
  }

  // content zone: uniform scale, top-anchored + horizontally centered.
  const scaledW = Math.round(width * scale);
  const scaledH = Math.round((height - headerZoneHeight) * scale);
  const destX0 = Math.round((width - scaledW) / 2);
  const destY0 = headerZoneHeight;
  for (let dy = 0; dy < scaledH; dy++) {
    const srcYf = headerZoneHeight + dy / scale;
    const oy = destY0 + dy;
    ...
    for (let dx = 0; dx < scaledW; dx++) {
      ...
      const { r, g, b } = bilinearSample(data, width, height, srcXf, Math.min(height - 1, srcYf));
  ...
  return {
    png_buffer: encodePng(out), scale, header_zone_height: headerZoneHeight,
    dest_bounds: { x0: destX0, y0: destY0, w: scaledW, h: scaledH },
    background_color: bg,
  };
}
