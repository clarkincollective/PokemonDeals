...
import {
  buildCardMetadataLock, buildVisualizationManifest, buildSourceAttributionManifest, buildTimeframeManifest, FACT_SOURCE_CLASSES,
} from "./factSource.mjs";
 ...
 */
export function buildSemanticManifest({ layout, factLock = {}, resolved = null, contract = null, printingProof = null, cardCatalogRow = null } = {}) {
  ...
  // 5A.1 - the fact-source locks (data the model may NOT invent)
  const cardMetadataLock = buildCardMetadataLock({ catalogRow: cardCatalogRow, factLock: F });
  const visualizationManifest = buildVisualizationManifest({ layout, resolved });
  const sourceAttributionManifest = buildSourceAttributionManifest({ resolved, contract });
  const timeframeManifest = buildTimeframeManifest({ resolved });
  ...
  const sem = {
    ...
    // 5A.1
    card_metadata_lock: cardMetadataLock,
    visualization_data_manifest: visualizationManifest,
    source_attribution_manifest: sourceAttributionManifest,
    timeframe_manifest: timeframeManifest,
    fact_source_classes: FACT_SOURCE_CLASSES,
  ...
  if (layout === "market_shape") {
    ...
    Object.assign(sem, {
      ...
      claim_metric: "share with a market price under $25",
      claim_value: u25,
      claim_unit: "percent",
