...
export const CAPTION_AUDIT_VERSION = "5b2.1";
...
// Phase SOCIAL-AUTOPILOT-5.2 - a real production submit failed with
// Buffer's own "Twitter / X posts cannot exceed 280 characters" error;
// no deterministic check in this pipeline had ever enforced that limit
// (a scoring-only 280 reference existed in scoreCaption() below, but it
// only nudged the quality score - it never hard-failed). X's real limit
// is 280 characters; counted with the same [...string] codepoint
// iteration used elsewhere in this file (not .length, which
// mis-counts multi-byte characters like curly quotes/em-dashes).
export const X_CAPTION_HARD_LIMIT = 280;
export function checkXLength(text, platform) {
  if (platform !== "x") return [];
  const len = [...String(text ?? "")].length;
  if (len > X_CAPTION_HARD_LIMIT) {
    return [{ code: "X_CAPTION_LENGTH_FAIL", detail: `X caption is ${len} characters, exceeding the ${X_CAPTION_HARD_LIMIT}-character limit by ${len - X_CAPTION_HARD_LIMIT}` }];
  }
  return [];
}
...
export function auditCaption({
  ...
  const findings = [
    ...
    // SOCIAL-AUTOPILOT-5.2 - platform-specific hard length limit
    ...checkXLength(full, platform),
  ];
  ...
  const verification = {
    ...
    x_length: findings.some((f) => f.code === "X_CAPTION_LENGTH_FAIL") ? "FAIL" : "PASS",
