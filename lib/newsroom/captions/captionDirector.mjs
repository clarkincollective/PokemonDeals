...
export function buildCaptionPrompt({ brief }) {
  ...
  return [
    ...
    b.example_card_is_not_population ? `SCOPE: the headline stat is for the WHOLE tracked-single population (${b.allowed_facts.population ?? "all tracked singles"}), NOT for ${b.example_card}. ${b.example_card} is only ONE example - label it as an example or leave it out. Never write "${b.example_card} singles".` : ``,
    ``,
    `YOU MAY invent wording, structure, tone and hook style.`,
    `YOU MUST NOT invent: prices, percentages, card metadata, grades, variants, dates, timeframes, source claims, market movement, rarity, urgency, scarcity, investment advice, seller behaviour, buyer behaviour, or historical facts.`,
