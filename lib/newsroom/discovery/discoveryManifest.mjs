...
export const DISCOVERY_MANIFEST_VERSION = "discovery1.1";

/**
 * buildSocialDiscoveryManifest(pkg, { recentTags = {}, now } = {}) ->
 *   { manifest, platform, audit, growth }
 *
 * recentTags: optional { instagram:[], x:[], tiktok:[], youtube_shorts:[] }
 * real recent-usage history for hashtag rotation (SS22) - defaults to
 * empty (no rotation penalty) rather than inventing history that doesn't
 * exist yet; a caller with real placement history should supply it.
 */
export function buildSocialDiscoveryManifest(pkg, { recentTags = {}, now = Date.now() } = {}) {
  ...
  const instagram = packageInstagram(pkg, { keywords, route, audience, recentTags: recentTags.instagram ?? [] });
  const x = packageX(pkg, { keywords, route, audience, recentTags: recentTags.x ?? [] });
  const tiktok = packageTikTok(pkg, { keywords, route, audience, hook, recentTags: recentTags.tiktok ?? [], onScreenAlignment });
  const youtube_shorts = packageYouTubeShorts(pkg, { keywords, route, audience, hook, recentTags: recentTags.youtube_shorts ?? [] });
  ...
  const keywordScores = [keywords.primary_search_query, ...keywords.secondary_search_queries, ...keywords.entity_keywords]
    .filter(Boolean)
    .map((k) => scoreKeyword(k, { family: pkg.family, lockedEntityTerms: allLockedTerms, recentKeywords: [] }));
