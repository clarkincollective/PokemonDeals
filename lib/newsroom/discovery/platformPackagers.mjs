...
export function packageInstagram(pkg, { keywords, route, audience, recentTags = [], recentCombos = [] } = {}) {
  const captionText = pkg?.captions?.instagram?.caption_text ?? "";
  const hook = captionText.split(/\n/)[0] ?? "";
  const { card_entities } = entityArrays(pkg);
  const entityTerm = card_entities[0] ?? null;
  const hashtags = selectHashtags("instagram", pkg?.family, { entityTerm, recentTags, lockedEntityTerms: card_entities, recentCombos });
  const isEmpty = captionText.trim().length === 0;
  // SS4/SS13 - audit the FINAL combined string (caption + hashtags), not
  // the bare caption - the exact bytes a real submit would send.
  const finalText = buildFinalPlatformText("instagram", captionText, hashtags.tags);
  const emojiAudit = auditEmojiUsage(finalText.text, "instagram");
  const finalAudit = auditFinalPlatformText("instagram", finalText.text);
  return {
    hook, caption: captionText,
    final_provider_text: finalText.text,
    ...
    audit: {
      ok: !isEmpty && hashtags.ok && emojiAudit.ok && finalAudit.ok,
      hashtag_result: hashtags, emoji_result: emojiAudit, caption_length: captionText.length, empty_caption: isEmpty,
      final_text_length: finalText.length, final_text_findings: finalAudit.findings,
    },
  };
}

// ================================== X (SS8/SS9/SS10) ==================================
export function packageX(pkg, { keywords, route, audience, recentTags = [], recentCombos = [] } = {}) {
  const captionText = pkg?.captions?.x?.caption_text ?? "";
  const { card_entities } = entityArrays(pkg);
  const entityTerm = card_entities[0] ?? null;
  const hashtags = selectHashtags("x", pkg?.family, { entityTerm, recentTags, lockedEntityTerms: card_entities, recentCombos });
  const bareLengthFindings = checkXLength(captionText, "x");
  const len = [...captionText].length;
  const preferredMax = 240;
  const isEmpty = captionText.trim().length === 0;
  if (isEmpty) bareLengthFindings.push({ code: "CAPTION_GENERATION_HOLD", detail: "X caption is empty - the underlying caption pipeline HELD this platform (a rare, pre-existing, platform-independent outcome, not something this discovery layer can fix); never package/submit an empty caption as ready" });

  // SS4/SS5/SS13/SS18 - the REAL gate: build + audit the FINAL combined
  // string (caption + hashtags), budget-aware so appending a hashtag can
  // NEVER recreate the AUTOPILOT-5.1 buffer_InvalidInputError class -
  // hashtags are only included while the running total still fits 280.
  const finalText = buildFinalPlatformText("x", captionText, hashtags.tags);
  const emojiAudit = auditEmojiUsage(finalText.text, "x");
  const finalAudit = auditFinalPlatformText("x", finalText.text);
  const lengthFindings = [...bareLengthFindings, ...finalAudit.findings];

  return {
    caption: captionText,
    final_provider_text: finalText.text,
    caption_keywords: keywords?.caption_keywords ?? [],
    hashtags: finalText.hashtags_included, // ONLY the hashtags that actually fit within 280 - never the full unaudited set
    hashtags_dropped_for_length: finalText.hashtags_dropped,
    ...
    audit: {
      ...
      final_text_length: finalText.length, final_text_within_hard_limit: finalText.length <= X_CAPTION_HARD_LIMIT,
...
// ================================ TIKTOK (SS11-14) ================================
export function packageTikTok(pkg, { keywords, route, audience, hook, recentTags = [], recentCombos = [], onScreenAlignment = null } = {}) {
  const f = factsOf(pkg);
  const { card_entities } = entityArrays(pkg);
  const entityTerm = card_entities[0] ?? null;
  ...
  const hashtags = selectHashtags("tiktok", pkg?.family, { entityTerm, recentTags, lockedEntityTerms: card_entities, recentCombos });
  const finalText = buildFinalPlatformText("tiktok", caption, hashtags.tags);
  const emojiAudit = auditEmojiUsage(finalText.text, "tiktok");
  const finalAudit = auditFinalPlatformText("tiktok", finalText.text);
  const len = caption.length;
  return {
    primary_search_query: keywords?.primary_search_query ?? null,
    secondary_search_queries: keywords?.secondary_search_queries ?? [],
    caption, final_provider_text: finalText.text, caption_keywords: keywords?.caption_keywords ?? [],
    ...
    related_site_route: route?.route ?? null,
    audit: {
      ok: len >= 40 && len <= 500 && hashtags.ok && emojiAudit.ok && finalAudit.ok && (!onScreenAlignment || onScreenAlignment.verdict !== "FAIL"),
...
export function packageYouTubeShorts(pkg, { keywords, route, audience, hook, recentTags = [], recentCombos = [] } = {}) {
  const f = factsOf(pkg);
  const { card_entities } = entityArrays(pkg);
  const entityTerm = card_entities[0] ?? null;
  const title = youtubeTitle(pkg, hook).slice(0, 100);
  const summary = f.pct != null
    ? `${f.pct}% of the ${f.population?.toLocaleString("en-US")} Pokemon singles tracked in this market snapshot have a market price under $25.${entityTerm ? ` ${entityTerm} is one example of how much of the hobby remains affordable.` : ""}`
    : f.asking != null
  ...
  const hashtags = selectHashtags("youtube_shorts", pkg?.family, { entityTerm, recentTags, lockedEntityTerms: card_entities, recentCombos });
