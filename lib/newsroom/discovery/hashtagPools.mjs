 ...
 */
// §10 - SOCIAL-DISCOVERY-2: penalize an EXACT repeated combination, an
// identical entity tag repeated every time, or the same family/tag combo
// repeatedly - but RELEVANCE ALWAYS BEATS VARIETY (the phase's own
// explicit rule). This never swaps in an irrelevant tag just to look
// different; when the relevant pool is small enough that no safe
// alternative exists, an exact repeat is correctly allowed and reported,
// not hidden.
function comboKey(tags) {
  return tags.map((t) => String(t).replace(/^#/, "").toLowerCase()).sort().join(",");
}

export function selectHashtags(platform, family, { entityTerm = null, recentTags = [], lockedEntityTerms = [], recentCombos = [] } = {}) {
  ...
  const recentComboKeys = recentCombos.map((c) => comboKey(c));
  const exactRepeatCount = recentComboKeys.filter((k) => k === key).length;
  // is there a relevant, non-spam candidate NOT already in `tags` that
  // could safely replace the least-relevant member? Only note it - never
  // auto-swap in something less relevant than what it would replace.
  const unusedRelevant = sorted.slice(targetCount).filter((t) => !isSpamTag(t));
  ...
  return {
    ...
    rotation: {
      exact_combo_repeated_recently: exactRepeatCount > 0,
      exact_repeat_count: exactRepeatCount,
      safe_alternative_available: unusedRelevant.length > 0,
      note: exactRepeatCount > 0 && unusedRelevant.length === 0
        ? "combination repeats a recent one, but the relevant pool has no safe alternative tag - relevance beats variety, repeat is correct"
        : null,
    },
  };
}
