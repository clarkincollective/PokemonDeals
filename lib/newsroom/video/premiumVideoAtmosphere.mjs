...
export function buildAtmosphere({
  ...
  const poolCss = pools
    .map((p, i) => {
      const color = p.kind === "red"
        ? `radial-gradient(circle at center, ${hexA(accent, 0.20)} 0%, ${hexA(accent, 0.05)} 42%, transparent 72%)`
        : `radial-gradient(circle at center, rgba(255,255,255,0.09) 0%, rgba(255,255,255,0.03) 45%, transparent 74%)`;
  ...
  .atmo::after{content:"";position:absolute;inset:0;
    box-shadow:inset 0 0 260px 90px rgba(0,0,0,0.55), inset 0 0 90px rgba(0,0,0,0.4)}
  ...
  return Object.freeze({ css, html, className: cls, version: PREMIUM_VIDEO_ATMOSPHERE_VERSION, variant, spotlights: pools, grain, dust });
}
