...
export const UNIVERSAL_CTA_END_SCREEN_VERSION = "4c4.1";
export const CTA_DOMAIN = APPROVED_BRAND.domain; // "pokemondealfinder.com" - a constant, never generated
export const CTA_END_SCREEN_DEFAULT_MS = 1500;
export const CTA_END_SCREEN_MIN_MS = 1300;
export const CTA_END_SCREEN_MAX_MS = 1800;
export const CTA_BRAND_ASSET_CACHE = path.join(".social-preview", "cta-brand-asset");

// §30 - three deterministic brand-value items. Safe language only - no
// "always save money" / no overpromise.
export const CTA_VALUE_POINTS = Object.freeze(["REAL CARD DEALS", "COMPARE PRICES", "FIND BETTER VALUE"]);
...
export function buildUniversalCtaEndScreen({
  ...
  const endScreen = {
    ...
    duration_ms: dur,
    // §36 - restrained resolve-in, no bounce / spin / confetti / flashing
    animation: [
      { at_ms: 0, end_ms: 300, what: "screen resolves in; card fan lifts ~2%" },
      { at_ms: 300, end_ms: 700, what: "three value points settle" },
      { at_ms: 700, end_ms: 1100, what: `"${(variant.line1 || CTA_UNIVERSAL_FALLBACK.line1)}" reads in` },
      { at_ms: 1100, end_ms: dur, what: `${CTA_DOMAIN} gets a subtle glow / underline` },
    ],
    background: bgApproved
      ...
      : { kind: "deterministic", note: "premium radial black with a single restrained red backlight - no AI" },
    reusable: true,
