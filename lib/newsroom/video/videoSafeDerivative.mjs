...
function blocksFor({ family, semanticManifest: S = {}, factLock: F = {}, cardAssets = [] }) {
  ...
  if (family === "deal_hero" || family === "asking_vs_sold") {
    ...
    b.push({
      ...
      stat_lines: [
        u25 != null ? `${u25}%` : "—",
        // SOCIAL-LIVE-2: a market reference is not a sale - never "sell"
        "OF TRACKED SINGLES:",
    ...
    b.push({
      ...
      items: [
        ...
        // SOCIAL-LIVE-2: only what the price distribution shows (no activity claim)
        { icon: "bulb", label: "WHY IT MATTERS", body: "Big-ticket singles are the exception, not the rule." },
      ],
