...
export const PROFESSIONAL_VIDEO_QA_VERSION = "4c6.1";
...
export function auditContentComprehension({ derivative = {}, timeline = {} } = {}) {
  ...
  const hasSubject = p1.some((b) => ["card", "card_row", "chart", "hero_row"].includes(b.role));
  const hasFigure = p1.some((b) => ["primary_stat", "hero_stat", "comparison", "value_ladder", "hero_row"].includes(b.role) && (String(b.text ?? "").trim() || (b.points ?? []).length || (b.rows ?? []).length || (b.spine ?? []).length));
...
export function auditStaticMasterParity({ derivative = {}, endScreen = {} } = {}) {
  ...
  const heroStat = blocks.find((b) => ["hero_stat", "hero_row"].includes(b.role));
  const heroFont = heroStat?.font ?? (heroStat?.role === "hero_row" ? 46 : 0);
  const bigStat = blocks.some((b) => b.role === "hero_stat" && (b.font ?? 0) >= 120);
  const cardRow = blocks.some((b) => b.role === "card_row");
  ...
  const takeaway = blocks.find((b) => b.role === "takeaway");
  ...
  const dim = {
    ...
    collector_appeal: clamp((blocks.some((b) => ["card", "card_row", "hero_row"].includes(b.role)) ? 42 : 0) + Math.min(hero, 0.5) * 80 + (cardRow ? 12 : 0) + (derivative.atmosphere?.enabled ? 12 : 0)),
...
export function auditMobileHierarchy({ derivative = {}, endScreen = {} } = {}) {
  ...
  if (family === "asking_vs_sold" || family === "deal_hero") {
    ...
    if (!readable("hero_stat", 26)) missing.push("85.7% hero legible");
    if (!has("chart")) missing.push("distribution chart");
    if (!has("card")) missing.push("real example card");
...
export function scoreOwnerTaste({ derivative = {}, endScreen = {} } = {}) {
  ...
  const heroStat = blocks.find((b) => b.role === "hero_stat");
  if (heroStat && (heroStat.font ?? 0) >= 120) { delta += 6; bonuses.push("large decisive key stat"); }
  const strongValue = blocks.some((b) => (b.role === "hero_row" && b.style?.spine) || (b.role === "chart" && b.premium));
  ...
  if (blocks.some((b) => b.role === "card" && (b.zone?.h ?? 0) < 340) && derivative.family === "market_shape") { delta -= 8; penalties.push("tiny example card"); }
  // template symmetry: only full-width centred rows, no hero_row / no chart
  if (!blocks.some((b) => ["hero_row", "chart", "card_row"].includes(b.role))) { delta -= 6; penalties.push("template-like symmetric stack"); }
...
export function auditMobilePreview({ derivative = {}, endScreen = {} } = {}) {
  ...
  const findings = [];
  const need = { label: 10, hook: 14, hero_stat: 24, primary_stat: 17, comparison: 14, value_ladder: 13, value_spine: 13, hero_row: 13, context: 11, takeaway: 10, domain: 10 };
  for (const b of derivative.blocks ?? []) {
    if (b.font == null) continue;
  ...
  const cardOk = (derivative.blocks ?? []).some((b) => ["card", "card_row", "hero_row"].includes(b.role) && b.zone && b.zone.h * scale >= 70);
...
export function runProfessionalVideoQa({ derivative = {}, timeline = {}, endScreen = {}, semanticManifest = {}, captionHandoff = null, cardAssets = [] } = {}) {
  ...
  const audits = { comprehension, cutoff, readability, motion, realCards, cta, exact, ctaHold, ctaReadability, deadBlack, frameDensity, mobilePreview, replay, parity, mobileHierarchy };
