...
export function buildChoreography({ family = "deal_hero", semanticManifest = {}, durationMs = null, masterAr = 1350 / 1080 } = {}) {
  ...
  return Object.freeze({
    ...
    // 24fps keeps the local Chrome+ffmpeg render tractable (~20% fewer
    // frames) and is fine for short-form; motion is eased, not stroboscopic.
    family, width: 1080, height: 1920, fps: 24, duration_ms: dur,
    master_ar: masterAr,
...
export async function renderMasterLayeredMotionToMp4(result, outPath, { posterPath = null, keepFrames = null } = {}) {
  ...
  const planShim = { duration: result.choreography.duration_ms, fps: result.choreography.fps || 24, width: 1080, height: 1920, poster_frame: { at_ms: Math.round(posterBeat.at_ms + Math.min(500, (posterBeat.end_ms - posterBeat.at_ms) * 0.35)) } };
