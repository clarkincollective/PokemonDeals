...
export const PROFESSIONAL_SOCIAL_LOOP_VERSION = "4c5.1";
...
export function buildProfessionalTimeline({ family = "deal_hero", derivative = {}, durationMs = null } = {}) {
  ...
  if (family === "deal_hero" || family === "asking_vs_sold") {
    // §12 - HOOK already populated; ladder illuminates downward; % impact; market lift; card sweep; settle
    add({ id: "ladder_reveal", kind: "illuminate", target: "value_ladder", at_ms: 700, end_ms: 2000, opacity_from: 0.15, opacity_to: 1, ease: EASE.outQuart, reveal: ["value_ladder"], note: "the ASK -> % -> MARKET ladder illuminates downward" });
    add({ id: "gap_impact", kind: "stat_pulse", target: "value_ladder", at_ms: 2000, end_ms: 3300, scale_from: 1, scale_to: 1.035, ease: EASE.inOutCubic, reveal: ["value_ladder"], note: "the % row gets a controlled impact" });
    add({ id: "card_sweep", kind: "card_sweep", target: "hero_card", at_ms: 3400, end_ms: 5200, translate_pct: 1.4, ease: EASE.outQuart, reveal: ["hero_card"], note: "card gets a soft light sweep + gentle lift with shadow" });
    add({ id: "settle", kind: "settle", target: "takeaway", at_ms: 5200, end_ms: storyMs, opacity_from: 0.55, opacity_to: 1, ease: EASE.outCubic, note: "short lesson settles" });
  } else if (family === "market_shape") {
    add({ id: "stat_reveal", kind: "stat_pulse", target: "hero_stat", at_ms: 300, end_ms: 1700, scale_from: 1.02, scale_to: 1.04, ease: EASE.outQuart, reveal: ["hero_stat"], note: "85.7% reveal + controlled emphasis" });
    add({ id: "chart_fill", kind: "chart_fill", target: "chart", at_ms: 1700, end_ms: 3600, opacity_from: 0, opacity_to: 1, ease: EASE.outCubic, reveal: ["chart"], note: "distribution chart builds; population resolves under it" });
    add({ id: "card_sweep", kind: "card_sweep", target: "example_card", at_ms: 3800, end_ms: 5300, translate_pct: 1.3, ease: EASE.outQuart, reveal: ["example_card"], note: "real example card light sweep / depth lift" });
    add({ id: "settle", kind: "settle", target: "takeaway", at_ms: 5300, end_ms: storyMs, opacity_from: 0.55, opacity_to: 1, ease: EASE.outCubic, note: "short takeaway settles" });
...
export function runProfessionalSocialLoop(opts = {}) {
  ...
  let revised = false;
  if (!qa.ok && ["PROFESSIONAL_BRAND_FAIL", "CONTENT_PURPOSE_UNCLEAR_FAIL", "MOTION_TOO_SUBTLE_FAIL", "MOTION_TOO_AGGRESSIVE_FAIL", "PARTIAL_PANEL_FAIL", "SAFE_ZONE_VIOLATION_FAIL", "MOBILE_TEXT_TOO_SMALL_FAIL", "FRAME_DENSITY_TOO_HIGH_FAIL", "DEAD_BLACK_SPACE_FAIL"].includes(qa.state)) {
    revised = true;
