...
export const PROFESSIONAL_SOCIAL_LOOP_VERSION = "4c7.1";
...
export function buildProfessionalTimeline({ family = "deal_hero", derivative = {}, durationMs = null } = {}) {
  ...
  if (family === "deal_hero" || family === "asking_vs_sold") {
    ...
    add({ id: "settle", kind: "settle", target: "lesson", at_ms: 5600, end_ms: storyMs, opacity_from: 0.5, opacity_to: 1, ease: EASE.outCubic, note: "the LESSON panel resolves" });
  } else if (family === "market_shape") {
    // 4C.7 §5/§15 - 85.7% lands with restrained scale; the donut grows;
    // Clefairy (now a real hero in the split) lifts with a light sweep;
    // the bottom band resolves.
    add({ id: "stat_reveal", kind: "stat_pulse", target: "market_split", at_ms: 200, end_ms: 1400, scale_from: 1.02, scale_to: 1.045, ease: EASE.outQuart, reveal: ["market_split"], note: "85.7% lands with a restrained scale" });
    add({ id: "chart_fill", kind: "chart_fill", target: "market_split", at_ms: 1500, end_ms: 3400, opacity_from: 0, opacity_to: 1, ease: EASE.outCubic, reveal: ["market_split"], note: "the donut grows; bucket values resolve" });
    add({ id: "card_sweep", kind: "card_sweep", target: "market_split", at_ms: 3800, end_ms: 5200, translate_pct: 1.5, ease: EASE.outQuart, note: "Clefairy lifts / light sweep" });
    add({ id: "settle", kind: "settle", target: "market_band", at_ms: 5200, end_ms: storyMs, opacity_from: 0.5, opacity_to: 1, ease: EASE.outCubic, note: "the takeaway / why-it-matters band resolves" });
...
export function runProfessionalSocialLoop(opts = {}) {
  ...
  return {
    ...
    // 4C.7 final visual-parity fields
    content_occupancy_audit: qa.verification?.content_occupancy ?? null,
    card_prominence_audit: { verdict: qa.verification?.card_prominence ?? null, hero_fraction: qa.verification?.card_hero_fraction ?? null },
    lower_dead_space_audit: qa.verification?.lower_dead_space ?? null,
    cta_luminance_audit: qa.verification?.cta_luminance ?? null,
    static_parity_rubric: {
      static_master_visual_score: qa.verification?.static_master_visual_score ?? null,
      video_derivative_visual_score: qa.verification?.video_derivative_visual_score ?? null,
      parity_gap: qa.verification?.parity_gap ?? null,
    },
