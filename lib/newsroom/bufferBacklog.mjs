...
export function preflightPlacement({ story, placement, dueAtUtc, professionalResult, now = Date.now() }) {
  const blockers = [];
...
//                 NOT published).
export async function scheduleOne({ story, placement, channelId, caption, dueAtUtc, professionalResult, mode = "draft", env = process.env, now = Date.now(), provider = null }) {
  const pre = preflightPlacement({ story, placement, dueAtUtc, professionalResult, now });
