...
//   No cap, allowance or schedule is raised.
//
// WHAT STOPS PILOT BROWSE REQUESTS (independent of any lease or pacing mode):
//   - a HARD per-substitution attempt ceiling (lib/ebayTelemetry
//     setBrowseAttemptGuard = pilotCardMax.browseAttempts), checked on every
//     attempt including retries, in every budget mode; refused attempts make
//     the raw path hold the listing (no write), and the guard is removed before
//     any undisplaced tail target is scanned;
//   - structurally: no search, raw listings only (no graded lookup), at most
//     RAW_CONDITION_LOOKUP_PER_CARD condition requests per card, one retry per
//     request, at most PILOT_MAX_CARDS cards;
//   - the allocated run's own quota floor (checked before any work).
// SUPPORTED MODES: budget mode off/observe with a known quota reading. In
// enforce mode the worker loop can stop before reaching a displaced target, so
// its price lookup would not be definitely avoided; with an unknown quota
// reading the allocated floor check is skipped. The pilot is disabled there.
