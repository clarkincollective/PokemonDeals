...
// The SPREAD is deliberately modest, so no marketplace's long tail is
// starved relative to another's.
//
// FULL-ROTATION CADENCE, MEASURED 2026-09-23 rather than asserted. The
// previous wording here - "every marketplace still rotates its whole
// eligible pool well inside the CURRENT ~30-day cadence" - was wrong on
// both halves, and it misled a reader into quoting ~30 days for how long
// a stale card waits. Computed from budgetForRun() against the 8,430
// active `extended` watchlist entries, over the two allocated runs per
// country per day in vercel.json, counting ONLY the explore lane (step 2
// of allocateScanTargets - pure least-recently-searched, the guarantee
// that a card cannot be starved by poor yield):
//
//     EBAY_US  budget 200/run  explore 124  248/day  ~34 days
//     EBAY_GB         150            93     186      ~45 days
//     EBAY_AU         131            82     164      ~51 days
//     EBAY_CA         131            82     164      ~51 days
//     EBAY_DE         119            74     148      ~57 days
//     EBAY_IT         113            71     142      ~59 days
//
// So the real figure is 34-59 days depending on marketplace, not "well
// inside ~30". Hot and exploit picks revisit productive cards sooner;
// they do not speed up the tail, which is what this cadence describes.
//
// THE OTHER HALF OF THE OLD ERROR: a comment in app/api/refresh-deals
// still refers to "vercel.json's now-30 extended cron entries, one per
// country-chunk-day". THERE ARE NONE - vercel.json has 6 sweep and 6
// tier=allocated entries and no tier=extended at all. The chunked
// extended rotation that comment describes is the FALLBACK branch, which
// runs only when scan_target_state is unavailable or SCAN_ALLOCATOR=off.
// The live path is this allocator. 99.7% of the watchlist (8,430 of
// 8,456) is extended tier and reaches a scanner only through it.
//
// tests/scanner/scan-allocator-cadence.test.mjs pins these figures to
// what the code actually computes, so this comment cannot drift again.
