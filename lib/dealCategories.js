...
// UI audit 2026-09-20 - the chip label. The h1 ("Pokemon Card Deals Under
// $25") is a page title; twelve of them in a chip strip read as a wall of
// the same words. A chip says only what distinguishes the page. Falls back
// to the h1 for any slug not listed here.
...
function categoryShortLabel(slug) {
  return CATEGORY_SHORT[slug] ?? DEAL_CATEGORIES[slug]?.h1 ?? slug;
}
...
const DEAL_CATEGORIES = {
  ...
  // 2026-09-22 redesign: the fourth price band, added so the homepage's
  // budget modules cover the range a collector actually shops in rather
  // than stopping at $100. Same shape as its three siblings - a real
  // maxPrice filter, its own copy, and therefore its own indexable
  // landing page; nothing here is a decorative tile.
  "under-250": {
    filter: { maxPrice: 250 },
    h1: "Pokemon Card Deals Under $250",
    title: "Pokemon Cards Under $250 — Best Current Deals",
    description:
      "Browse tracked Pokemon card listings under US$250 on eBay, with matching market references where available for raw cards and graded slabs.",
    intro:
      "Tracked eBay listings for Pokemon cards under US$250 — the band most chase cards and graded slabs sit in. Filter for raw or graded, price or auctions.",
  },
  graded: {
    ...
    title: "Graded Pokemon Card Deals — PSA, CGC & BGS Slabs on eBay",
    description:
  ...
  vintage: {
    ...
    // A category lists tracked listings whether or not each has a market
    // comparison (R6) - the title names the era, never a saving.
    title: "Vintage Pokemon Card Deals (1998–2003) — WOTC & e-Card Era",
    description:
