...
export function removeRecent(card) {
  const key = cardKey(card);
  if (!key) return readRecent();
  const list = readRecent().filter((c) => c.key !== key);
  write(RECENT_KEY, list);
  return list;
}

export function removeSaved(card) {
  const key = cardKey(card);
  if (!key) return readSaved();
  const list = readSaved().filter((c) => c.key !== key);
