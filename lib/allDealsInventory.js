...
import listingIdentity from "./listingIdentity.js";
...
// FINDING 6 (2026-09-25). The listing-identity rule this module already had
// is now lib/listingIdentity.js, shared with the card hubs and the species
// counts, which had a DIFFERENT and wrong one (it included the marketplace,
// so one eBay listing stored for four regions read as four buying options).
// Nothing here changes: the same values and the same function, re-exported
// under the names this module already published.
export const ALL_DEALS_MARKETPLACES = listingIdentity.MARKETPLACE_PREFERENCE;
...
// statement and destination travel together. The rule itself now lives in
// lib/listingIdentity.js so every surface uses the same one.
export const chooseListingCopy = listingIdentity.chooseListingCopy;

const listingKey = listingIdentity.listingIdentityKey;
...
function matchesPlan(row, plan) {
  ...
  // 2026-09-19: the auction ending window bounds auction_end_at (an ISO
  // string) - compare timestamps as times, numbers as numbers; a row with
  // no parseable value never satisfies a bound.
  const asNumber = (v) => (typeof v === "string" && Number.isNaN(Number(v)) ? Date.parse(v) : Number(v));
  for (const [col, val] of Object.entries(plan.lte)) if (!(asNumber(row[col]) <= asNumber(val))) return false;
  for (const [col, val] of Object.entries(plan.gte)) if (!(asNumber(row[col]) >= asNumber(val))) return false;
  return true;
}
...
export function queryAllDeals(chunks, params = {}, { now = Date.now(), pageSize = ALL_DEALS_PAGE_SIZE } = {}) {
  ...
  const eligibleFor = (scope, narrow = true) => {
    ...
    if (narrow && params.narrowingSorts) r = r.filter((x) => keptBySortScope(x, sort, now));
    return r;
  };
  let rows = eligibleFor(country);
  // When a narrowing sort removed rows, say so: which restriction, and the
  // count for the same filters and marketplace without it (the page shows
  // "N of M" and a way back to every listing). null when nothing narrowed.
  const narrowing = params.narrowingSorts && (sort === "discount" || sort === "ending")
    ? { sort, kept: sort === "discount" ? "supported_saving" : "live_auction", totalWithoutRestriction: eligibleFor(country, false).length }
    : null;
  ...
  return {
    ...
    // graded-inventory-r1: totalCount and additionalOnOtherMarketplaces are
    // already restricted by this; see `narrowing` above
    narrowing,
