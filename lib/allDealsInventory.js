...
//   2. request: decode -> re-check the time-dependent gates (freshness TTL,
//      ended auction) -> dedup by exact eBay listing identity -> filters ->
...
//   marketplace EBAY_US 476 rows / 355 KB). Presentation outputs
...
export function encodeMarketplaceInventory(rows, { marketplace, readLimitHit = false, builtAt = new Date().toISOString() } = {}) {
  // English single cards only, like the reader's SQL pre-filter (Japanese
  // and sealed have their own pages); then the real display gate.
  const eligible = (rows ?? [])
    .filter((r) => r && r.marketplace === marketplace && r.card_language === "english" && isDisplayableDeal(r))
    .sort(byNewest);
...
export function queryAllDeals(chunks, params = {}, { now = Date.now(), pageSize = ALL_DEALS_PAGE_SIZE } = {}) {
  ...
  const inScope = (chunks ?? []).filter((c) => c && (!country || c.marketplace === country));
  let rows = inScope.flatMap(decodeMarketplaceInventory);

  // time-dependent gates can change between cache build and this request
  rows = rows.filter((r) => !isStale(r, now) && !auctionEnded(r, now));
  rows = dedupeByListing(rows);
  ...
  rows = rows.filter((r) => matchesPlan(r, plan));
  const q = typeof params.q === "string" ? params.q.trim().toLowerCase() : "";
  if (q.length >= 2) rows = rows.filter((r) => String(r.title ?? "").toLowerCase().includes(q));
  ...
  return {
    ...
    sort,
    country,
    filters: { type: plan.type, grader: plan.grader, grade: plan.grade, listing: plan.listing, minPrice: plan.minPrice, maxPrice: plan.maxPrice, q: q.length >= 2 ? q : null },
  };
}
