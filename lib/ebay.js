...
function mapItemSummary(item, marketplaceId) {
  ...
  return {
    ...
    // (OUT_OF_STOCK / 0 remaining), false when it reports stock, null when
    // the response carries no availability block (every search result).
...
//               network error / unparseable body: inconclusive, caller
//               must NOT retire the row on this
