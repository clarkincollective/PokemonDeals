...
const { recordBrowseCall, recordAnalyticsCall, recordGradedDetailCall, consumeBrowseAttempt, currentJobContext, recordBrowseOutcome, recordBrowseTransportFailure } = require("./ebayTelemetry");
const { reportRateLimitFailure } = require("./rateLimitDiagnostics");

// browse-budget-r1 - a Browse attempt the budget lease does not cover (enforce
// mode). Thrown only when NO request was sent for this call.
class BrowseBudgetExhaustedError extends Error {
  constructor(message = "Browse budget lease exhausted") {
    super(message);
    this.name = "BrowseBudgetExhaustedError";
  }
}
const { classifyItemLookup, soldOutFromItemBody } = require("./listingAvailability");
...
async function fetchWithRetry(url, options, { retries = 1, delayMs = 800 } = {}) {
  ...
  for (let attempt = 0; ; attempt++) {
    // browse-budget-r1 - every attempt (first try or retry) draws one unit
    // from this invocation's lease BEFORE it is sent. A refused retry returns
    // the response / error already in hand, so no uncovered request is made.
    if (!consumeBrowseAttempt()) {
      if (lastRes) return lastRes;
      if (lastErr) throw lastErr;
      throw new BrowseBudgetExhaustedError();
    }
    let res;
    try {
      ...
      recordBrowseCall();
      // browse-outcomes-r1: the class of the response we just counted. Same
      // attempt, same chokepoint, so the buckets always sum to browseCalls -
      // a retry is a second answered attempt and gets its own bucket. No
      // budget charge: the unit was drawn before the request was sent.
      recordBrowseOutcome(res.status);
    } catch (err) {
      // never answered: not a browseCall, not an outcome bucket, its own
      // counter so "no response" can never be read as a provider refusal
...
// but a false impression of attribution. See lib/affiliateAttribution.js and
...
function mapItemSummary(item, marketplaceId) {
  ...
  return {
    ...
    // Also single-item only: true when eBay reports the item sold out
    // (OUT_OF_STOCK / 0 remaining), false when it reports positive stock,
    // null when availability is absent or inconclusive (every search result).
    // The feed ingestion must not re-publish a sold-out item.
    soldOut: soldOutFromItemBody(item),
...
//     ACTIVE  - HTTP 200 and every estimatedAvailabilities entry is
//               IN_STOCK / LIMITED_STOCK (positive evidence required - a
//               200 with no availability data is UNKNOWN)
//     ENDED   - eBay returns 404/410 for this item IN THIS MARKETPLACE
//               (removed/ended, or not offered on this site - the
//               response does not say which, so it is never evidence for
//               another marketplace's row; the web /itm/ url would 301 to
//               an unrelated /p/ group - deal 12766 - which is exactly why
//               this uses the API, not the page)
//     SOLD    - item exists but estimatedAvailabilities says OUT_OF_STOCK
//               or 0 remaining (item-level quantity)
//     UNKNOWN - 429 / 401 / 403 / other non-2xx (5xx after one retry) /
//               network error / unparseable body / a 200 without
//               positive or sold-out availability: inconclusive, caller
//               must NOT retire or confirm the row on this
// Full mapping: lib/listingAvailability.classifyItemLookup.
async function getListingFreshness(legacyItemId, marketplaceId) {
  ...
  try {
    ...
    return { status: "UNKNOWN", calls: 1, evidence: "network_error" };
  }
  const verdict = await readItemLookup(res);
  return { status: verdict.status, calls: 1, evidence: verdict.evidence, httpStatus: res.status };
}

// Shared by getListingFreshness / getListingSnapshot: the response ->
// verdict mapping lives in lib/listingAvailability.classifyItemLookup.
async function readItemLookup(res) {
  if (!res.ok) return { ...classifyItemLookup({ httpStatus: res.status }), body: null };
  let body;
  try {
    body = await res.json();
  } catch {
    return { ...classifyItemLookup({ httpStatus: res.status, parseFailed: true }), body: null };
  }
  return { ...classifyItemLookup({ httpStatus: res.status, body }), body };
}
...
async function getListingSnapshot(legacyItemId, marketplaceId) {
  ...
  const verdict = await readItemLookup(res);
  if (verdict.status === "ENDED" || verdict.status === "UNKNOWN") {
    return { status: verdict.status, calls: 1, evidence: verdict.evidence, httpStatus: res.status };
  }
  ...
  return {
    status: verdict.status, // SOLD | ACTIVE (see lib/listingAvailability)
    calls: 1,
...
// Every failed exit now emits one structured line (lib/rateLimitDiagnostics)
// naming WHICH exit it was: token / fetch / http / parse / shape. Diagnosis
// only - the return value, the recordAnalyticsCall() accounting and every
// caller's fail-open / fail-closed behaviour are unchanged, no request is
// added and nothing is retried. See that module for what may be logged.
async function getBrowseRateLimit() {
  const job = currentJobContext();

  let token;
  try {
    token = await getAccessToken();
  } catch (error) {
    return reportRateLimitFailure("token", { error, job });
  }

  let res;
  try {
    res = await fetch(EBAY_RATE_LIMIT_URL, {
      headers: { Authorization: `Bearer ${token}` },
    });
  } catch (error) {
    // no response arrived, so no analytics call is recorded - unchanged
    return reportRateLimitFailure("fetch", { error, job });
  }
  ...
  // The body is read BEFORE the ok check purely to recover the provider's
  // own error code on a rejection. No extra request; nothing else changes.
  let body;
  try {
    body = await res.json();
  } catch (error) {
    return reportRateLimitFailure(res.ok ? "parse" : "http", { status: res.status, error, job });
  }

  if (!res.ok) return reportRateLimitFailure("http", { status: res.status, body, job });

  for (const group of body.rateLimits ?? []) {
    for (const resource of group.resources ?? []) {
      if (resource.name !== "buy.browse") continue;
      const rate = resource.rates?.[0];
      if (!rate) return reportRateLimitFailure("shape", { status: res.status, body, reason: "no_rate_object", job });
      // A rate object that omits `remaining` still returns - unchanged - but
      // the caller then reads a null remaining, which is indistinguishable
      // downstream from a failed meta-call. Say so here rather than let it
      // look like the same silence.
      if (rate.remaining == null) {
        reportRateLimitFailure("shape", { status: res.status, body, reason: "rate_missing_remaining", job });
      }
      return {
        limit: rate.limit ?? null,
        remaining: rate.remaining ?? null,
        reset: rate.reset ?? null,
        // browse-budget-r1 - the provider's own window length (seconds)
        // and when this reading was taken; see lib/browseBudget.js
        timeWindow: rate.timeWindow ?? null,
        readAt: new Date().toISOString(),
      };
    }
  }
  return reportRateLimitFailure("shape", { status: res.status, body, reason: "no_buy_browse_resource", job });
}

module.exports = {
  BrowseBudgetExhaustedError,
  fetchWithRetry,
  MARKETPLACES,
  searchListings,
  searchNewlyListed,
