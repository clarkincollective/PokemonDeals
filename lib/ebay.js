...
// Like getListingFreshness, but also returns the listing's LIVE price
// fields from the same single get_item_by_legacy_id call - so an auction
// deal row can be re-priced as bids come in (app/api/verify-deals) without
// a second request. For an auction, `price` is the current bid; for a
// fixed-price listing it is the item price. `currency` is the listing's
// own. Any inconclusive outcome returns status "UNKNOWN" with null price
// fields, and the caller MUST NOT re-price or retire on that.
//
//   { status: "ACTIVE"|"ENDED"|"SOLD"|"UNKNOWN", calls: 1,
