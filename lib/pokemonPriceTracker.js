...
async function fetchPPTPaced(url, { maxRetries = 6 } = {}) {
  for (let attempt = 0; ; attempt++) {
    ...
    await new Promise((r) => setTimeout(r, Math.min(waitMs, 45000)));
  }
}
...
async function getSealedPrice(tcgPlayerId, language = "english") {
  ...
  const body = await fetchPPT(url);
  const product = body.data?.[0];
  if (product?.unopenedPrice == null) return null;
