...
const { rawSaleMatchesPrinting } = require("./dealMatching");
...
async function getFullPriceAnalysis(tcgplayerId, { primaryGrader, primaryGrade, language = "english" } = {}) {
  ...
  // Japanese prints). Keep only sales whose title reads as this exact
  // raw printing (lib/dealMatching rawSaleMatchesPrinting). When a real
  // grader WAS requested (a graded deal-detail page) the list IS that
  // grade's bucket and is left exactly as-is.
  if (!primaryKey) {
    ...
    primarySoldListings = primarySoldListings.filter((s) => rawSaleMatchesPrinting(s.title, identity));
  }
