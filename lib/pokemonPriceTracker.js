...
const { wrapEbayAffiliateUrl } = require("./ebayLinks");
...
const { sortSoldListingsByDate } = require("./soldListingOrder");
// ppt-telemetry-r1 - counts each outbound attempt; never changes a request.
const { recordPptAttempt } = require("./pptTelemetry");
...
// op: the lib function making the request (telemetry label only).
async function fetchPPT(url, op = "fetchPPT") {
  const headers = { Authorization: `Bearer ${apiKey()}` }; // a missing key throws before any request, as before
  let res;
  try {
    res = await fetch(url, { headers });
  } catch (e) {
    await recordPptAttempt({ url, op, error: e });
    throw e;
  }
  if (!res.ok) {
    const text = await res.text();
    await recordPptAttempt({ url, op, res, bodyText: text });
    throw new Error(`PokemonPriceTracker request failed: ${res.status} ${text}`);
  }
  await recordPptAttempt({ url, op, res });
  return res.json();
}
...
async function fetchPPTPaced(url, { maxRetries = 6, retryAny429 = false, op = "fetchPPTPaced" } = {}) {
  for (let attempt = 0; ; attempt++) {
    ...
    try {
      ...
      await recordPptAttempt({ url, op, error: e, attempt });
      throw e;
    }
    if (res.ok) {
      await recordPptAttempt({ url, op, res, attempt });
      return res.json();
    }
    const text = await res.text();
    await recordPptAttempt({ url, op, res, attempt, bodyText: text });
...
async function searchCard(name, language = "english") {
  ...
  const body = await fetchPPT(url, "searchCard");
...
async function searchCards(query, { limit = 20, offset = 0, language = "english" } = {}) {
  ...
  const body = await fetchPPT(url, "searchCards");
  return {
    results: body.data ?? [],
...
// bigger).
//
// PRICE-CONDITION PROVENANCE (2026-09-11). The figure this picks is not
// always a Near Mint price: when the non-1st-Edition variant has no entry
// for the requested condition it takes the variant's FIRST entry, whatever
// condition that is. Verified live on Magneton 009/102 (tcgplayer 107003,
// "Base Set (Shadowless)"): PPT prices only "Lightly Played Unlimited
// Holofoil" $73.99 (no Near Mint anywhere), so $73.99 - a Lightly Played
// figure - became the card's reference and was then LABELLED Near Mint by
// every consumer. The number was fine (and an LP listing was correctly
// discounted against it); the label was not. pickMarketReference() is the
// same selection with its provenance attached; pickMarketPrice() stays the
// price-only wrapper so no caller's figure changes.
function pickMarketPrice(prices, condition = "Near Mint") {
  return pickMarketReference(prices, condition).price;
}

const NO_REFERENCE = Object.freeze({ price: null, condition: null, printing: null, exact: false });

// The condition tier a PPT price key names ("Lightly Played Unlimited
// Holofoil" -> "Lightly Played"), or null when the key names none.
function conditionFromKey(key) {
  const k = String(key ?? "").toLowerCase();
  return CONDITION_TIERS.find((c) => k.includes(c.toLowerCase())) ?? null;
}

// Condition provenance comes from exactly two places, both DIRECT:
//   * the key of the variant entry the figure was selected from
//     ("Lightly Played Unlimited Holofoil" -> Lightly Played);
//   * a prices.conditions["Near Mint"] entry read as such.
// A headline `prices.market` (and the CSV's aggregate marketPrice column)
// has NO documented condition semantics from PokemonPriceTracker, so its
// condition is null - "unknown". It is never inferred from equality with a
// per-condition figure (two tiers can share a price, and the provider's
// sparse responses omit tiers) and never assumed Near Mint.

// pickMarketPrice with provenance. Returns
//   { price, condition, printing, exact }
//   price     - exactly what pickMarketPrice returns for the same input
//   condition - the tier `price` is actually for ("Near Mint", "Lightly
//               Played", ...) or null when the provider doesn't say
//   printing  - the variant / primary printing it belongs to, or null
//   exact     - true only when `price` is a real entry for the REQUESTED
//               condition (Near Mint by default). false = the figure is
//               another condition's price, or an unattributed headline.
function pickMarketReference(prices, condition = "Near Mint") {
  if (!prices) return NO_REFERENCE;
  if (isSentinelPrice(prices.market)) return NO_REFERENCE;
  const primary = prices.primaryPrinting ?? null;
  const headline = () => {
    const price = prices.market ?? null;
    if (price == null) return NO_REFERENCE;
    return { price, condition: null, printing: primary, exact: false };
  };
  if (!/1st edition/i.test(primary ?? "")) return headline();
  ...
  if (nonFirstEditionVariants.length === 0) return headline();

  let best = null;
  for (const variantName of nonFirstEditionVariants) {
    const entries = Object.entries(prices.variants[variantName] ?? {});
    const found = entries.find(([key]) => key.toLowerCase().includes(condition.toLowerCase()));
    const [key, match] = found ?? entries[0] ?? [];
    const price = match?.price;
    if (price != null && (best == null || price < best.price)) {
      best = { price, condition: conditionFromKey(key), printing: variantName, exact: Boolean(found) };
    }
  }
...
async function getCatalogNmPrice(tcgplayerId, language = "english") {
  return (await getCatalogReference(tcgplayerId, language)).price;
}

// getCatalogNmPrice with provenance: { price, condition, printing, exact }
// (price null -> no usable figure). NOTE the name of the older wrapper is
// historical - for a card whose Unlimited variant has no Near Mint entry
// the figure is another condition's price, and `condition` says which.
async function getCatalogReference(tcgplayerId, language = "english") {
  const url = new URL(`${BASE_URL}/cards`);
  ...
  const body = await fetchPPTPaced(url, { retryAny429: true, maxRetries: 8, op: "getCatalogReference" });
  const prices = body.data?.prices;
  if (!prices) return NO_REFERENCE;
  const ref = pickMarketReference(prices, "Near Mint");
  const p = Number(ref.price);
  return Number.isFinite(p) && p > 0 && !isSentinelPrice(p) ? { ...ref, price: p } : NO_REFERENCE;
}
...
function pickCatalogMarketPrice(entries) {
  return pickCatalogMarketReference(entries).price;
}

// pickCatalogMarketPrice with provenance: { price, condition, printing,
// exact }. `nm` is the CSV's marketNearMint column; `price` its aggregate
// marketPrice column (the sync keeps the two SEPARATE - folding the
// aggregate into `nm` is exactly how a non-NM figure got stamped Near
// Mint). A figure taken from `nm` is Near Mint. One taken from the
// aggregate has no documented condition semantics: its condition is null
// ("market reference") - never inferred from a matching ladder figure.
function pickCatalogMarketReference(entries) {
  const usable = (v) => {
    const n = Number(v);
    return Number.isFinite(n) && n > 0 && !isSentinelPrice(n);
  };
  const priced = (entries ?? []).filter((e) => usable(e?.nm ?? e?.price));
  if (priced.length === 0) return NO_REFERENCE;
  const chosen = priced.find((e) => !/1st\s*edition/i.test(e.printing ?? "")) ?? priced[0];
  const fromNm = chosen.nm != null && usable(chosen.nm);
  const figure = Number(fromNm ? chosen.nm : chosen.price);
  const ladder = { "Lightly Played": chosen.lp, "Moderately Played": chosen.mp, "Heavily Played": chosen.hp, Damaged: chosen.dmg };
  if (ladderInverted(ladder, figure)) return NO_REFERENCE;
  const condition = fromNm ? "Near Mint" : null;
  return { price: figure, condition, printing: chosen.printing ?? null, exact: fromNm };
}
...
function catalogRawMarketPrice(prices) {
  return catalogRawMarketReference(prices).price;
}

// catalogRawMarketPrice with provenance: { price, condition, printing,
// exact } - see pickMarketReference. Only the prices.conditions["Near
// Mint"] path is a PROVEN Near Mint figure; the 1st-Edition path carries
// whatever condition the Unlimited variant actually had, and the
// prices.market fallback is an unattributed headline (condition null -
// never inferred).
function catalogRawMarketReference(prices) {
  if (!prices) return NO_REFERENCE;
  if (/1st\s*edition/i.test(prices.primaryPrinting ?? "")) return pickMarketReference(prices, "Near Mint");
  ...
  if (Number.isFinite(nm) && nm > 0 && !isSentinelPrice(nm)) {
    ...
    return inverted ? NO_REFERENCE : { price: nm, condition: "Near Mint", printing: prices.primaryPrinting ?? null, exact: true };
  }

  // prices.conditions is EMPTY in the shapes the site actually receives
  // (verified 2026-09-11 on plain and includeEbay responses: Magneton Base
  // Set 42433 -> conditions {}, variants { Holofoil: { "Near Mint
  // Holofoil": 36.36 } }). The per-condition entries live under the
  // primary printing's variant, and selecting from THAT keyed entry is
  // direct provenance: the requested Near Mint entry when it exists, else
  // the variant's first entry with the condition its key names - the same
  // rule the 1st-Edition branch (pickMarketReference) already applies to
  // the Unlimited variant.
  // When the variant has entries its verdict is final - a Near Mint entry
  // rejected by its own ladder means "no trustworthy reference", never a
  // fall-through to the unlabelled headline (a missing price beats a
  // false one, as in the prices.conditions branch above).
  const primary = prices.primaryPrinting ?? null;
  const variantEntries = prices.variants?.[primary];
  if (Object.keys(variantEntries ?? {}).length > 0) return variantReference(variantEntries, primary, "Near Mint");

  const m = Number(prices.market);
  if (!(Number.isFinite(m) && m > 0 && !isSentinelPrice(m))) return NO_REFERENCE;
  // an unattributed headline: condition unknown, never inferred
  return { price: m, condition: null, printing: primary, exact: false };
}

// The reference taken from ONE printing's keyed condition entries
// (prices.variants[printing]): the requested condition's entry when it
// exists (exact), else the first entry with the condition its key names.
// A Near Mint figure contradicted by its own variant ladder (a played
// price materially above it) is rejected, as elsewhere. No entries ->
// NO_REFERENCE.
function variantReference(entries, printing, condition = "Near Mint") {
  const list = Object.entries(entries ?? {}).filter(([, e]) => e?.price != null && !isSentinelPrice(e.price));
  if (list.length === 0) return NO_REFERENCE;
  const found = list.find(([key]) => key.toLowerCase().includes(condition.toLowerCase()));
  const [key, match] = found ?? list[0];
  const tier = conditionFromKey(key);
  if (tier === "Near Mint") {
    const ladder = {};
    for (const [k, e] of list) {
      const t = conditionFromKey(k);
      if (t && t !== "Near Mint") ladder[t] = e.price;
    }
    if (ladderInverted(ladder, Number(match.price))) return NO_REFERENCE;
  }
  return { price: match.price, condition: tier, printing: printing ?? null, exact: Boolean(found) };
}
...
async function getRawPrice(tcgplayerId, condition = "Near Mint", language = "english") {
  ...
  url.searchParams.set("tcgPlayerId", tcgplayerId);
  url.searchParams.set("language", language);
  const body = await fetchPPT(url, "getRawPrice");
...
// 17C.10 - the SAME selection as conditionPricesFromVariants, with the
// provenance the price-only map throws away. ADDED ALONGSIDE: the numeric
// map above is unchanged and every existing caller keeps its numbers.
// For each tier this returns the entry that produced that tier's number -
// same comparison (lowest wins), so `byConditionReference[tier].price`
// always equals `byCondition[tier]`.
function conditionReferencesFromVariants(prices) {
  if (!prices) return {};
  const isFirstEdition = /1st edition/i.test(prices.primaryPrinting ?? "");
  const byTier = {};
  ...
  for (const [variantName, entries] of Object.entries(prices.variants ?? {})) {
    ...
    for (const [key, entry] of Object.entries(entries ?? {})) {
      ...
      const tier = CONDITION_TIERS.find((c) => key.toLowerCase().includes(c.toLowerCase()));
      if (!tier) continue;
      if (byTier[tier] == null || entry.price < byTier[tier].price) {
        // printing = the variant this entry belongs to. DIRECT only: the
        // variant name it was filed under, never inferred from a price.
        byTier[tier] = { price: entry.price, condition: tier, printing: variantName ?? null };
      }
    }
  }
...
// THE UNCOLLAPSED MATRIX (deal 42127). The two functions above fold every
// printing into one per-condition map, which is exactly how a plain
// listing acquired a Reverse Holofoil reference: the tier was filled by
// whichever printing happened to have a price for it. This keeps the
// printing dimension, so the SCANNER can pick a printing using the
// listing's own evidence.
//
// It is returned ALONGSIDE the collapsed maps rather than replacing them:
// getConditionPrices is cached per CARD (one provider call serves every
// listing of that card), while printing choice is per LISTING. Selecting
// inside this module would have made the cache key listing-dependent and
// multiplied provider calls. Shape: { [printingName]: { [tier]: price } }.
function printingConditionMatrix(prices) {
  if (!prices) return {};
  const isFirstEdition = /1st edition/i.test(prices.primaryPrinting ?? "");
  const out = {};
  for (const [variantName, entries] of Object.entries(prices.variants ?? {})) {
    ...
    for (const [key, entry] of Object.entries(entries ?? {})) {
      ...
      out[name] ??= {};
      // Same "lowest wins" rule as the collapsed maps, but WITHIN one
      // printing, where comparing prices is meaningful.
      if (out[name][tier] == null || entry.price < out[name][tier]) out[name][tier] = entry.price;
    }
  }
  return out;
}
...
async function getConditionPrices(tcgplayerId, language = "english") {
  ...
  url.searchParams.set("tcgPlayerId", tcgplayerId);
  url.searchParams.set("language", language);
  const body = await fetchPPT(url, "getConditionPrices");
  const prices = body.data?.prices;
  if (!prices) return null;

  // fallbackReference = the same figure with its provenance (condition /
  // printing / exact). The scanner's discount maths is unchanged - a
  // listing with no condition signal is still priced against
  // fallbackPrice - but a consumer can now say what that figure was.
  const fallbackReference = pickMarketReference(prices, "Near Mint");
  return {
    byCondition: conditionPricesFromVariants(prices),
    fallbackPrice: fallbackReference.price,
    fallbackReference,
    // 17C.10 - the same tiers with their provenance, alongside (not
    // instead of) the numeric map. `lastUpdated` is the PROVIDER's own
    // as-of for these figures; it is not our sync time.
    byConditionReference: conditionReferencesFromVariants(prices),
    // deal 42127: the printing dimension the maps above discard.
    byPrintingCondition: printingConditionMatrix(prices),
...
async function getRawPriceHistory(tcgplayerId, condition = "Near Mint", days = 90, language = "english") {
  ...
  const body = await fetchPPT(url, "getRawPriceHistory");
...
async function listSets(language = "english") {
  ...
  url.searchParams.set("language", language);
  const body = await fetchPPT(url, "listSets");
  return body.data ?? [];
}
...
async function listSetCards(setId, language = "english") {
  ...
  url.searchParams.set("language", language);
  const body = await fetchPPT(url, "listSetCards");
  return body.data ?? [];
}
...
async function downloadPrintingsExport() {
  ...
  if (!res.ok) {
    ...
    throw new Error(`PokemonPriceTracker export failed: ${res.status} ${text}`);
  }
...
// FINDING 9: ordered newest-first HERE, at the single place every
// sold-listing list is built, because the provider supplies no guaranteed
// order (see lib/soldListingOrder.js for the measured census). Every
// consumer renders these top-down and slices the first N, so an unordered
// array meant the eight shown were not the eight most recent. The sort is
// display-only and cannot move a price: no caller reads these by array
// position, and no pricing, savings or index path consumes them at all.
function normalizeSoldListings(rawListings) {
  return sortSoldListingsByDate(
    (rawListings ?? []).map((l) => ({
...
async function getGradedPrice(tcgplayerId, grader, grade, language = "english") {
  ...
  const body = await fetchPPT(url, "getGradedPrice");
  const d = body.data;
  const gradeData = d?.ebay?.salesByGrade?.[key];
  if (!gradeData) return null;

  const price = gradeData.smartMarketPrice?.price ?? gradeData.medianPrice ?? null;
  if (price == null) return null;

  // Phase 12B - do NOT hand a live graded deal a reference from an
  // incoherent bucket (blended printings, thin/one-sale samples, a slab
  // grade priced below raw NM). Fail closed: a low-confidence bucket
  // returns null and the scanner simply doesn't publish that deal.
  const rawNmRef = (() => {
    const v = catalogRawMarketPrice(d?.prices);
    return isSentinelPrice(v) || !(Number(v) > 0) ? null : Number(v);
  })();
  const siblingPrices = Object.entries(d?.ebay?.salesByGrade ?? {})
    .filter(([k, s]) => k !== "ungraded" && k !== key && s?.count > 0)
    .map(([, s]) => Number(s.smartMarketPrice?.price ?? s.medianPrice))
    .filter((p) => p > 0);
  const conf = gradedTierConfidence(
    {
      key,
      currentPrice: price,
      minPrice: gradeData.minPrice ?? null,
      maxPrice: gradeData.maxPrice ?? null,
      saleCount: gradeData.count ?? 0,
      lastSaleDate: gradeData.lastSaleDate ?? null,
      providerLowConfidence:
        Boolean(d?.ebay?.smartPriceOutlierByGrade?.[key]) ||
        gradeData.smartMarketPrice?.confidence === "low",
    },
    { rawNm: rawNmRef, setName: d?.setName, cardName: d?.name, siblingPrices }
  );
  ...
  return {
    ...
    lastSaleDate: gradeData.lastSaleDate ?? null,
    confidence: conf.level, // "high" | "limited"
    history,
...
async function getRawSoldComps(tcgplayerId, language = "english") {
  ...
  const body = await fetchPPT(url, "getRawSoldComps");
...
async function getFullPriceAnalysis(
  ...
  const body = await fetchPPT(url, "getFullPriceAnalysis");
  const d = body.data;
  if (!d) return null;
  ...
  // The trustworthy RAW reference (from d.prices) with its provenance, used
  // below to sanity-check the graded tiers' scale and to LABEL the figure.
  // Its condition is Near Mint only when PPT priced Near Mint for the
  // canonical printing (rawRef.condition) - a played-condition or
  // unattributed reference is a LOWER anchor, so every "graded >= raw" /
  // "graded <= N x raw" check below stays conservative with it.
  const rawRef = catalogRawMarketReference(d.prices);
  const rawNmRef = (() => {
    const v = rawRef.price;
    return isSentinelPrice(v) || !(Number(v) > 0) ? null : Number(v);
  })();
  ...
  const coherent = coherentGradedTiers(gradedRaw, rawNmRef);
  // Phase 12B - graded integrity gate. Decide per (card, grader, grade)
  // whether the evidence is coherent enough to publish a price. Only
  // suppresses / annotates; never rewrites a price or forces a ladder.
  const siblingPrices = coherent.map((g) => Number(g.currentPrice)).filter((p) => p > 0);
  const gradedScored = coherent.map((g) => {
    const conf = gradedTierConfidence(
      { ...g, providerLowConfidence: g.isLowConfidence },
      {
        rawNm: rawNmRef,
        setName: d.setName,
        cardName: d.name,
        siblingPrices: siblingPrices.filter((p) => p !== Number(g.currentPrice)),
      }
    );
    return { ...g, confidence: conf.level, confidenceReasons: conf.reasons };
  });
  const graded = gradedScored.filter((g) => g.confidence !== "low");
  const gradedSuppressedCount = gradedScored.length - graded.length;
  ...
  return {
    ...
    // Phase 17B (card "worth" answer) - two provider facts, passed
    // through, never inferred:
    //   priceUpdatedAt       - the provider's own prices.lastUpdated
    //   firstEditionExcluded - the provider lists a 1st Edition printing
    //     AND a non-1st-Edition one, so raw.currentPrice (via
    //     pickMarketPrice) is the non-1st-Edition figure - the page can
    //     say 1st Edition copies are priced separately.
    priceUpdatedAt: d.prices?.lastUpdated ?? null,
    firstEditionExcluded:
      /1st\s*edition/i.test(d.prices?.primaryPrinting ?? "") &&
      Object.keys(d.prices?.variants ?? {}).some((v) => !/1st\s*edition/i.test(v)),
    raw: {
      currentPrice: rawRef.price,
      // Provenance of currentPrice (price-condition provenance fix):
      // which condition tier / printing the figure is really for, and
      // whether it is a genuine Near Mint entry. Labels are derived from
      // these (lib/referenceCondition.js) - never assumed.
      referenceCondition: rawRef.condition,
      referencePrinting: rawRef.printing,
      referenceExact: rawRef.exact,
    ...
    graded,
    // How many coherent-looking tiers were withheld for weak/incoherent
    // evidence - lets the card page show one restrained line instead of a
    // wall of "insufficient sales" rows.
    gradedSuppressedCount,
...
async function searchSealedProduct(name, language = "english") {
  ...
  url.searchParams.set("language", language);
  url.searchParams.set("limit", "1");
  const body = await fetchPPT(url, "searchSealedProduct");
...
async function searchSealedProducts(query, { limit = 20, offset = 0, language = "english" } = {}) {
  ...
  url.searchParams.set("language", language);
  if (offset) url.searchParams.set("offset", String(offset));
  const body = await fetchPPT(url, "searchSealedProducts");
  return {
    results: body.data ?? [],
    total: body.metadata?.total ?? null,
    hasMore: body.metadata?.hasMore ?? false,
...
async function listSealedProductsForSet(setName, { language = "english" } = {}) {
  ...
  for (let offset = 0; ; offset += LIMIT) {
    ...
    url.searchParams.set("limit", String(LIMIT));
    if (offset) url.searchParams.set("offset", String(offset));
    const body = await fetchPPTPaced(url, { op: "listSealedProductsForSet" });
...
async function getSealedPrice(tcgPlayerId, language = "english") {
  ...
  url.searchParams.set("tcgPlayerId", tcgPlayerId);
  url.searchParams.set("language", language);
  const body = await fetchPPT(url, "getSealedPrice");
  const product = body.data?.[0];
...
async function getSealedPriceHistory(tcgPlayerId, days = 90, language = "english") {
  ...
  url.searchParams.set("days", String(days));
  url.searchParams.set("language", language);
  const body = await fetchPPT(url, "getSealedPriceHistory");
  const product = body.data?.[0];
...
module.exports = {
  ...
  getRawPrice,
  getConditionPrices,
  conditionPricesFromVariants,
  conditionReferencesFromVariants,
  getRawPriceHistory,
  getRawSoldComps,
  listSets,
  listSetCards,
