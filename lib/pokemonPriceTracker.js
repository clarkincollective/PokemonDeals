...
// User-facing search: returns several candidate cards (unlike searchCard,
// which is limit=1 and only ever used to resolve one specific card while
// seeding the watchlist). A name like "Charizard" matches dozens of
// different prints/sets - the caller shows these as options for a person
// to pick the specific one they mean. Costs `limit` credits (PPT bills
// search= by the limit requested), so kept modest.
