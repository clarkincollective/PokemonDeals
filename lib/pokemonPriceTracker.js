...
// The condition behind a headline `prices.market` figure. PPT does not say
// which condition its aggregate is, so it is only ever INFERRED: the figure
// must equal exactly one condition among the provider's own per-condition
// entries (prices.conditions, then the primary printing's ladder - every
// ladder when no primary is named). Anything else -> null, "condition
// unknown" - never Near Mint by assumption.
function marketFigureCondition(prices, figure) {
  const n = Number(figure);
  if (!Number.isFinite(n)) return null;
  const same = (v) => Number.isFinite(Number(v)) && Math.abs(Number(v) - n) < 0.005;
  const hits = new Set();
  for (const [cond, info] of Object.entries(prices?.conditions ?? {})) {
    const tier = conditionFromKey(cond);
    if (tier && same(info?.price)) hits.add(tier);
  }
  const primary = prices?.primaryPrinting;
  const ladders = primary && prices?.variants?.[primary] ? [prices.variants[primary]] : Object.values(prices?.variants ?? {});
  for (const entries of ladders) {
    for (const [key, entry] of Object.entries(entries ?? {})) {
      const tier = conditionFromKey(key);
      if (tier && same(entry?.price)) hits.add(tier);
    }
  }
...
function pickMarketReference(prices, condition = "Near Mint") {
  ...
  const headline = () => {
    ...
    const cond = marketFigureCondition(prices, price);
    return { price, condition: cond, printing: primary, exact: cond === condition };
  };
...
// aggregate is Near Mint only if PPT... doesn't say, so it is inferred
// only when it equals exactly one of the row's own played-condition
// figures; otherwise its condition is null ("market reference").
function pickCatalogMarketReference(entries) {
  ...
  let condition = fromNm ? "Near Mint" : null;
  if (!fromNm) {
    const hits = Object.entries(ladder).filter(([, v]) => Number.isFinite(Number(v)) && Math.abs(Number(v) - figure) < 0.005);
    if (hits.length === 1) condition = hits[0][0];
  }
  return { price: figure, condition, printing: chosen.printing ?? null, exact: condition === "Near Mint" };
}
...
// prices.market fallback is an unattributed headline (condition inferred
// or null).
function catalogRawMarketReference(prices) {
  ...
  const cond = marketFigureCondition(prices, m);
  return { price: m, condition: cond, printing: prices.primaryPrinting ?? null, exact: cond === "Near Mint" };
}
