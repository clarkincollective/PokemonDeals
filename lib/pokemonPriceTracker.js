...
const { titleLooksGraded } = require("./dealMatching");
...
async function getFullPriceAnalysis(tcgplayerId, { primaryGrader, primaryGrade, language = "english" } = {}) {
  ...
  // RAW recent-sales integrity (SEO Phase 3 closeout): when no grader was
  // requested we're pulling the "ungraded" bucket to show as RAW sales -
  // but PokemonPriceTracker's own grade classification isn't perfect and
  // that bucket occasionally contains sales whose title plainly says
  // PSA/CGC/BGS/etc. Drop those so a raw-only list never shows a graded
  // slab. When a real grader WAS requested (a graded deal-detail page),
  // the list IS that grade's bucket and is left exactly as-is.
  if (!primaryKey) {
    primarySoldListings = primarySoldListings.filter((s) => !titleLooksGraded(s.title));
  }
