...
// Sealed products a guide may picture (2026-09-20). Each is a VERIFIED
// sealed_catalog identity (name + set + product type + TCGplayer product
// id, checked 2026-09-20) whose image is the TCGplayer product photo the
// site already shows on sealed deal cards and deal pages - the same source
// and the same usage as the card scans, never a third-party photograph.
// There is no per-product page, so every tile links the sealed listings
// hub. What a guide may SAY about a product is limited to what the
// catalogue records: its name, set and product type. No prices.
// AUDIT 2026-09-23, FINDING 4. Every named product in a guide used to link
// to "/sealed-deals" - the unfiltered catalogue - so a reader who clicked
// "151 Pokemon Center Elite Trainer Box" landed on 2,300 products and had
// to find it again. Confirmed on five guides.
//
// The registry already held the exact catalogue id for each product, and
// that id is thrown away by a bare "/sealed-deals". THE EDITION IS IN THE
// ID, NOT THE NAME: 151 standard ETB is 503313 and the Pokemon Center ETB
// is 501999; Prismatic is 593355 and 593324. A set-name or text link cannot
// tell those apart, so the destination selects by catalogue identity and
// never by a text approximation.
export const SEALED_SELECTION_PARAM = "product";
export function sealedProductHref(tcgplayerId) {
  const id = String(tcgplayerId ?? "").trim();
  return id ? `/sealed-deals?${SEALED_SELECTION_PARAM}=${encodeURIComponent(id)}` : "/sealed-deals";
}

function product({ name, set, productType, tcgplayerId, label }) {
  return Object.freeze({ name, set, productType, tcgplayerId, label, href: sealedProductHref(tcgplayerId) });
}

export const GUIDE_PRODUCTS = Object.freeze({
  // Booster box price guide - one product per pack count the guide
  // compares (36 / 6 / 1). Verified against sealed_catalog 2026-09-20;
  // each image returned 200 image/jpeg from the product CDN.
  evolvingSkiesBoosterBox: product({ name: "Evolving Skies Booster Box", set: "SWSH07: Evolving Skies", productType: "Booster Box", tcgplayerId: "242436", label: "Evolving Skies Booster Box" }),
  ascendedHeroesBoosterBundle: product({ name: "Ascended Heroes Booster Bundle", set: "ME: Ascended Heroes", productType: "Booster Bundle", tcgplayerId: "668541", label: "Ascended Heroes Booster Bundle" }),
  ascendedHeroesBoosterPack: product({ name: "Ascended Heroes Booster Pack", set: "ME: Ascended Heroes", productType: "Booster Pack", tcgplayerId: "672434", label: "Ascended Heroes Booster Pack" }),
  // Audit batch 2026-09-22. Every id below is a real sealed_catalog row,
  // read from the database that day. The THREE separate Prismatic ETBs
  // are the point of the format guide: a listing saying "Prismatic ETB"
  // can be any one of three different products.
  s151EliteTrainerBox: product({ name: "151 Elite Trainer Box", set: "SV: Scarlet & Violet 151", productType: "Elite Trainer Box", tcgplayerId: "503313", label: "151 Elite Trainer Box" }),
  s151PokemonCenterEtb: product({ name: "151 Pokemon Center Elite Trainer Box (Exclusive)", set: "SV: Scarlet & Violet 151", productType: "Elite Trainer Box", tcgplayerId: "501999", label: "151 Pokemon Center ETB" }),
  s151UltraPremium: product({ name: "151 Ultra-Premium Collection", set: "SV: Scarlet & Violet 151", productType: "Collection Box", tcgplayerId: "502005", label: "151 Ultra-Premium Collection" }),
  prismaticEliteTrainerBox: product({ name: "Prismatic Evolutions Elite Trainer Box", set: "SV: Prismatic Evolutions", productType: "Elite Trainer Box", tcgplayerId: "593355", label: "Prismatic Evolutions ETB" }),
  prismaticPokemonCenterEtb: product({ name: "Prismatic Evolutions Pokemon Center Elite Trainer Box (Exclusive)", set: "SV: Prismatic Evolutions", productType: "Elite Trainer Box", tcgplayerId: "593324", label: "Prismatic Pokemon Center ETB" }),
  prismaticBoosterBundle: product({ name: "Prismatic Evolutions Booster Bundle", set: "SV: Prismatic Evolutions", productType: "Booster Bundle", tcgplayerId: "600518", label: "Prismatic Evolutions Booster Bundle" }),
  crownZenithEliteTrainerBox: product({ name: "Crown Zenith Elite Trainer Box", set: "SWSH: Crown Zenith", productType: "Elite Trainer Box", tcgplayerId: "453470", label: "Crown Zenith Elite Trainer Box" }),
});
...
export const GUIDE_CARDS = Object.freeze({
  ...
  // Delta Reign guides (2026-09-20): the other three Pokemon the official
  // pages name, each shown as a card that exists in the catalogue TODAY -
  // never as a Delta Reign card (no English Delta Reign card is printed,
  // and no leaked or third-party image is used). Verified against
  // card_catalog and resolved live (200) on 2026-09-20.
  golurkBlackBolt: card({ name: "Golurk - 123/086", set: "SV: Black Bolt", cardNumber: "123/086", tcgplayerId: "642578", label: "Golurk #123/086 (Black Bolt)" }),
  malamarExPhantomForces: card({ name: "Malamar EX (115 Full Art)", set: "XY - Phantom Forces", cardNumber: "115/119", tcgplayerId: "94684", label: "Malamar EX #115/119 (Phantom Forces)" }),
  golisopodExParadoxRift: card({ name: "Golisopod ex - 246/182", set: "SV04: Paradox Rift", cardNumber: "246/182", tcgplayerId: "523927", label: "Golisopod ex #246/182 (Paradox Rift)" }),
  ...
  // CORRECTED 2026-09-23 (SEO/affiliate audit, finding 3). These two
  // carried the bare names "Mewtwo ex" / "Mew ex". The catalogue stores
  // them as "Mewtwo ex - 157/128" / "Mew ex - 158/128" - the set numbers
  // duplicate names, so the collector number is part of the stored name -
  // and href is DERIVED from that name. The bare name therefore derived
  // /cards/mew-ex-me-30th-celebration, which is a 404, and 18 anchors
  // across six guides pointed at it. The product ids were right all along;
  // only the names were wrong.
  c30MewtwoExFuturistic: card({ name: "Mewtwo ex - 157/128", set: "ME: 30th Celebration", cardNumber: "157/128", tcgplayerId: "696687", label: "Mewtwo ex #157/128" }),
  c30MewExFuturistic: card({ name: "Mew ex - 158/128", set: "ME: 30th Celebration", cardNumber: "158/128", tcgplayerId: "696688", label: "Mew ex #158/128" }),
  ...
  // Every id below was read from card_catalog on that date.
  //
  // PRISMATIC UMBREON. Three separate catalogue cards share the collector
  // number 059/131, and two more Umbreon ex cards sit at different
  // numbers. This is the single most useful thing the Prismatic guide can
  // show a buyer, and it is first-party: a listing that says "Umbreon
  // 059/131" has not yet said which of three cards it is.
  prismaticUmbreon059: card({ name: "Umbreon", set: "SV: Prismatic Evolutions", cardNumber: "059/131", tcgplayerId: "610414", label: "Umbreon 059/131" }),
  prismaticUmbreon059PokeBall: card({ name: "Umbreon (Poke Ball Pattern)", set: "SV: Prismatic Evolutions", cardNumber: "059/131", tcgplayerId: "610578", label: "Umbreon 059/131 (Poke Ball Pattern)" }),
  prismaticUmbreon059MasterBall: card({ name: "Umbreon (Master Ball Pattern)", set: "SV: Prismatic Evolutions", cardNumber: "059/131", tcgplayerId: "610679", label: "Umbreon 059/131 (Master Ball Pattern)" }),
  prismaticUmbreonEx060: card({ name: "Umbreon ex - 060/131", set: "SV: Prismatic Evolutions", cardNumber: "060/131", tcgplayerId: "610415", label: "Umbreon ex 060/131" }),
  prismaticUmbreonEx161: card({ name: "Umbreon ex - 161/131", set: "SV: Prismatic Evolutions", cardNumber: "161/131", tcgplayerId: "610516", label: "Umbreon ex 161/131 (Special Illustration Rare)" }),
  prismaticEspeon033: card({ name: "Espeon", set: "SV: Prismatic Evolutions", cardNumber: "033/131", tcgplayerId: "610388", label: "Espeon 033/131" }),
  prismaticEspeon033PokeBall: card({ name: "Espeon (Poke Ball Pattern)", set: "SV: Prismatic Evolutions", cardNumber: "033/131", tcgplayerId: "610557", label: "Espeon 033/131 (Poke Ball Pattern)" }),

  // SURGING SPARKS PIKACHU ex. Four separate cards, four numbers, four
  // rarities - "the Surging Sparks Pikachu" names none of them.
  surgingSparksPikachuEx057: card({ name: "Pikachu ex - 057/191", set: "SV08: Surging Sparks", cardNumber: "057/191", tcgplayerId: "590025", label: "Pikachu ex 057/191 (Double Rare)" }),
  surgingSparksPikachuEx219: card({ name: "Pikachu ex - 219/191", set: "SV08: Surging Sparks", cardNumber: "219/191", tcgplayerId: "590026", label: "Pikachu ex 219/191 (Ultra Rare)" }),
  surgingSparksPikachuEx238: card({ name: "Pikachu ex - 238/191", set: "SV08: Surging Sparks", cardNumber: "238/191", tcgplayerId: "590027", label: "Pikachu ex 238/191 (Special Illustration Rare)" }),
  surgingSparksPikachuEx247: card({ name: "Pikachu ex - 247/191", set: "SV08: Surging Sparks", cardNumber: "247/191", tcgplayerId: "593169", label: "Pikachu ex 247/191 (Hyper Rare)" }),

  // PRINTING-PATTERN examples for the holo / reverse-holo guide. Both are
  // catalogue rows whose recorded printing is a REVERSE variant, i.e. the
  // foil pattern is part of the card's identity, not a finish applied to
  // the one card.
  ascendedHeroesPawniardQuickBall: card({ name: "Pawniard (Quick Ball)", set: "ME: Ascended Heroes", cardNumber: "146/217", tcgplayerId: "676966", label: "Pawniard 146/217 (Quick Ball pattern)" }),

  // 151. Charizard ex is the same one-name-three-cards shape as the
  // Surging Sparks Pikachu: three separate records, three numbers, three
  // rarities. 183 and 199 sit above the printed set total of 165.
  s151CharizardEx006: card({ name: "Charizard ex - 006/165", set: "SV: Scarlet & Violet 151", cardNumber: "006/165", tcgplayerId: "502558", label: "Charizard ex 006/165 (Double Rare)" }),
  s151CharizardEx183: card({ name: "Charizard ex - 183/165", set: "SV: Scarlet & Violet 151", cardNumber: "183/165", tcgplayerId: "517017", label: "Charizard ex 183/165 (Ultra Rare)" }),
  s151CharizardEx199: card({ name: "Charizard ex - 199/165", set: "SV: Scarlet & Violet 151", cardNumber: "199/165", tcgplayerId: "517045", label: "Charizard ex 199/165 (Special Illustration Rare)" }),
  s151VenusaurEx198: card({ name: "Venusaur ex - 198/165", set: "SV: Scarlet & Violet 151", cardNumber: "198/165", tcgplayerId: "517044", label: "Venusaur ex 198/165 (Special Illustration Rare)" }),

  // Crown Zenith: one card from the main /159 run and one from the
  // GG##/GG70 subset, so the guide can show the two numbering schemes
  // side by side rather than describing them.
  crownZenithCharizardVstar: card({ name: "Charizard VSTAR", set: "SWSH: Crown Zenith", cardNumber: "019/159", tcgplayerId: "478094", label: "Charizard VSTAR 019/159 (main set)" }),
  crownZenithRadiantCharizard: card({ name: "Radiant Charizard", set: "SWSH: Crown Zenith", cardNumber: "020/159", tcgplayerId: "478098", label: "Radiant Charizard 020/159" }),
  crownZenithGgMew: card({ name: "Mew", set: "SWSH: Crown Zenith: Galarian Gallery", cardNumber: "GG10/GG70", tcgplayerId: "478027", label: "Mew GG10/GG70 (Galarian Gallery)" }),
  crownZenithGgMewtwoVstar: card({ name: "Mewtwo VSTAR", set: "SWSH: Crown Zenith: Galarian Gallery", cardNumber: "GG44/GG70", tcgplayerId: "477057", label: "Mewtwo VSTAR GG44/GG70 (Galarian Gallery)" }),
});
...
export const GUIDE_SETS = Object.freeze({
  ...
  // Promo SETS. The promo-numbers guide decodes what a promo number means;
  // these are the pages holding the actual per-era list. Each slug is derived
  // by set() and was checked live on 2026-09-17: all nine return 200 and
  // carry no noindex, with 38-343 catalogue cards each.
  promoWotc: set("WoTC Promo"),
  promoDiamondPearl: set("Diamond and Pearl Promos"),
  promoHgss: set("HGSS Promos"),
  promoBlackWhite: set("Black and White Promos"),
  promoXy: set("XY Promos"),
  promoSm: set("SM Promos"),
  promoSwsh: set("SWSH: Sword & Shield Promo Cards"),
  promoSv: set("SV: Scarlet & Violet Promo Cards"),
  promoMegaEvolution: set("ME: Mega Evolution Promo"),
  // Audit batch 2026-09-22 - the four sets the new buying guides send
  // readers to. Each is the exact catalogue `set` string, verified
  // against card_catalog that day (215 / 355 / 179 / 70 / 262 rows).
  scarletViolet151: set("SV: Scarlet & Violet 151"),
  prismaticEvolutions: set("SV: Prismatic Evolutions"),
  crownZenith: set("SWSH: Crown Zenith"),
  crownZenithGalarianGallery: set("SWSH: Crown Zenith: Galarian Gallery"),
  surgingSparks: set("SV08: Surging Sparks"),
});
