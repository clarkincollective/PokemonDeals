...
export async function runSealedVerifyLane({
  ...
  const out = {
    // `entered` distinguishes "the lane ran its selection and found no
    // eligible candidate" (entered: true, poolSize 0, used 0) from "the
    // lane never ran at all" (entered: false - no slots, migration not
    // applied, or no dependency). Both make zero provider calls, so
    // without this flag they are indistinguishable in telemetry.
    entered: false,
    used: 0,
    calls: 0,
  ...
  for (const r of [...recoveryRows, ...freshRows]) {
    ...
    if (status === "SOLD" || status === "ENDED") {
      ...
      const { error } = await db
        .from("sealed_deals")
        .update({ last_seen_at: checkedAt, exact_verified_at: checkedAt })
        .eq("id", r.id);
      if (error) out.writeErrors += 1;
    }
