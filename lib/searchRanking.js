...
function queryNumbers(q) {
  ...
  // Prefixed subset numbers ("RC3/RC32"). Without this the loop below
  // only yields the halves ("rc3", "rc32"), which never intersect the
  // card's own "rc3/rc32" form, so an exact prefixed-number match earned
  // no collector-number credit at all.
  for (const m of s.matchAll(/\b([a-z]{1,5})(\d{1,4})\s*\/\s*([a-z]{1,5})(\d{1,4})\b/gi)) {
    out.add(`${m[1].toLowerCase()}${stripZeros(m[2])}/${m[3].toLowerCase()}${stripZeros(m[4])}`);
  }
