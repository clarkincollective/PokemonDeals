...
//   referenceCondition - the condition the figure is REALLY for, from the
//                        reference's own provenance (analysis.raw
//                        .referenceCondition / card_catalog.market_condition):
//                        "Near Mint", "Lightly Played", ... or null when the
//                        provider doesn't state it. The answer says "Near
//                        Mint" only when this says so; unknown -> the figure
//                        is a "market reference price" for a raw copy.
...
//   liveListings       - { count, lowUsd, lowBasis, lowIsAuction } of active
//                        listings (lowBasis / lowIsAuction come from
//                        lib/cardSummaryOffer, so the floor is stated with
//                        the basis it really has), or null
...
export function cardWorthAnswer({
  ...
  const live =
    liveListings && Number(liveListings.count) > 0
      ? {
          count: Number(liveListings.count),
          lowUsd: isUsableUsdPrice(liveListings.lowUsd) ? Number(liveListings.lowUsd) : null,
          // Audit finding 2: the floor used to be stated as a bare "from $X
          // USD (asking prices, not sold)". lowUsd is the offer's stored
          // TOTAL, which includes a shipping charge only where one was
          // actually recorded (lib/offerPresentation), and on an auction it
          // is a current bid - neither of which "asking price" describes.
          // Both facts now travel with the figure and are stated.
          lowShippingText:
            liveListings.lowBasis === "delivered"
              ? "including shipping"
              : liveListings.lowBasis === "before_shipping"
                ? "before shipping"
                : "with no shipping breakdown recorded",
          lowKindText: liveListings.lowIsAuction ? "a current auction bid, not a sold price" : "asking prices, not sold",
        }
      : null;
  ...
  const cond = referenceConditionLabels(referenceCondition);
  return {
    status: "priced",
    question,
    subject,
    ...
    // "raw (ungraded), Near Mint" ONLY when the reference really is Near
    // Mint; another tier by name; "raw (ungraded)" when unknown.
    condition: cond.worth,
    referenceCondition: cond.condition,
    conditionKnown: cond.known,
    marketNoun: cond.noun,
