...
function beginJobRun(meta) {
  const ctx = {
    ...
    // sum to browseCalls. Transport failures are NOT here - a request that
...
const BROWSE_OUTCOME_KEYS = ["browse_ok", "browse_401", "browse_403", "browse_429", "browse_4xx", "browse_5xx", "browse_transport_failures"];
...
function recordBrowseOutcome(status) {
  ...
  // anything else (1xx/3xx) is left uncounted rather than silently bucketed,
  // so a reconciliation gap is visible instead of hidden.
}
