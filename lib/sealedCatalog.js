...
function sealedCatalogRecord(p, { language = "english" } = {}) {
  ...
  return {
    ...
    // Prefer the large derivative of the exact product image (see
    // lib/cardImage). upgradeCatalogImage bumps PPT's own _in_200x200
    // imageCdnUrl200 too; a non-TCGplayer imageUrl is left as-is.
    image_url:
      upgradeCatalogImage(p.imageCdnUrl200) ||
      upgradeCatalogImage(p.imageUrl) ||
      (id ? catalogImageUrl(id) : null),
...
// eBay Browse call volume (~194 products × 6 marketplaces ≈ 1.2k/day)
