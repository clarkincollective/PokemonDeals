...
// STORED classification only (Phase 17C.5): the card's catalogue set IS
// "Jumbo Cards" or "World Championship Decks". Used where the page states
// a COUNT of Jumbo / World Championship printings - a count must come from
// stored data, never from the name-marker fallback above.
export function isStoredSpecialtyCard(card) {
  return Boolean(card) && SPECIALTY_SETS.has(card.set);
}
