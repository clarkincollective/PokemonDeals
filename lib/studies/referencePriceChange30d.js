...
//
// WHERE THAT INPUT IS, AND WHAT THAT MEANS (recorded 2026-09-23 so the
// next person does not repeat the search). Neither `analysis-final.json`,
// the saved provider responses, nor `tools/jtcg-emit-artifact` has ever
// been in this repository: `git log --all` over every ref matches no path
// under tools/ and none containing "jtcg", and nothing matching them is
// in .local/. They were never deleted - the study ran in a separate
// working area and only this aggregate was imported, which is what the
// rule about keeping retained evidence outside source control implies.
//
// The consequence is worth stating plainly rather than leaving implied:
// THESE FIGURES CANNOT BE RE-DERIVED FROM ANYTHING IN THIS REPOSITORY.
// This file holds aggregates only - no per-record data - so the medians
// and shares below can be read but not recomputed. That is a weaker
// position than the shipping-cost study, which freezes and digests its
// row-level inputs before calculating (scripts/studies/
// freezeShippingCostInput.mjs) and can therefore be re-run offline.
//
// What IS checked: tests/scanner/reference-price-study.test.mjs verifies
// that every figure here reconciles with its own denominators - the
// variant funnel, the sample composition, each up/down/flat split, the
// era partition and both endpoint distributions. That catches a
// hand-edit, a partial regeneration or a figure copied from the wrong
// run. It is not reproducibility and must not be described as such.
//
// If this study is ever revised, freeze its inputs first.
//
