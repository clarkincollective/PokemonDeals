...
import { guideForSet, guideLinkLabel } from "./guides.js";
...
export function cardNextSteps({ species = null, speciesLive = null, set = null, setLive = null, marketUsd = null, setName = null } = {}) {
  ...
  // A guide written about this card's set, when one exists. It sits above
  // the generic price / era categories because it is about this exact card;
  // it is editorial, so it never carries a live-deal count or claim.
  const guide = guideForSet(setName ?? set?.name);
  if (guide) links.push({ key: "guide", href: `/guides/${guide.slug}`, label: guideLinkLabel(guide), live: null });
