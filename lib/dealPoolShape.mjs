...
export const DEAL_POOL_SELECT = [
  ...
  // reads. It is now required on EVERY set, not just the tracked releases,
  // so a row that reaches a render path without these columns shows no
  // saving at all. They are short scalars (an id, an amount, a currency, a
  // timestamp and the condition/printing or grader/grade the reference
  // priced), and the pool stays well under the 2 MB data-cache ceiling -
  // pinned by tests/scanner/infra-db-1.
  "reference_product_id", "reference_amount", "reference_currency",
  "reference_observed_at", "reference_condition", "reference_printing",
