...
// ONE stable title / H1 for an indexable /pokemon/[slug] page, whether
// deal-backed or catalogue-backed (SEO-2). The page renders the full
// per-species card index plus market references, and real queries reach
// it as "all <pokemon> cards" / "<pokemon> cards list" as often as
// "<pokemon> card prices" - so the phrasing says exactly that. Never a
// deal-state flip ("... & Deals"), never "for sale".
export function speciesPageTitle(speciesName) {
  return `${speciesName} Cards – Full List, Prices & Values`;
}

// SEO-2.3 PILOT TITLE. Used ONLY for species in SPECIES_PILOT; every other
// species keeps speciesPageTitle above, unchanged, as the experiment's
// control.
//
// Why the wording changed: the control title says "Full List", but the
// page's own FAQ says these are "the cards we price and monitor", "not
// necessarily the set's full printed checklist", and a species page openly
// reports how many of its cards have no reliable reference. "Full List" is
// a completeness claim the page contradicts, so the pilot drops it while
// keeping the three intents Search Console actually shows against these
// pages: entity/list ("all dragonite cards", "how many dragonite pokemon
// cards are there"), price, and value.
//
// DETERMINISTIC LENGTH LADDER, not hand-maintained exceptions. The first
// rung that fits `cap` wins; the last rung is returned whatever its length
// so a name can never produce an empty title. The longest real indexable
// species name is 12 characters, so rung 1 is 46 characters at worst and
// the lower rungs are defensive only - they exist so a future catalogue
// name cannot silently blow the cap.
export const SPECIES_TITLE_CAP = 65;

export function speciesPilotPageTitle(speciesName, cap = SPECIES_TITLE_CAP) {
  const n = String(speciesName ?? "").trim();
  if (!n) return "";
  const ladder = [
    `${n} Cards: Prices, Values & Card List`,
    `${n} Cards: Prices & Card List`,
    `${n} Pokemon Cards`,
  ];
  return ladder.find((t) => t.length <= cap) ?? ladder[ladder.length - 1];
}
