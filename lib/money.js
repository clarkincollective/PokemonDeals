...
// The USD-canonical asking-price for a deal, or null when it can't be
// trusted as USD. `deal.total_price` carries NO unit identity (it's the
// listing's native currency), so any region-agnostic string - a <meta>
// description, an OG/Twitter card, Web-Share text - must NOT prefix it
// with "$" and sit it next to the (USD) `market_price`: that renders e.g.
// a CAD listing as "$685" against a "$558" USD market price - a mixed,
// often self-contradictory comparison. Use this instead so both figures
// in that sentence are genuinely USD.
function dealTotalUsd(deal) {
  const usd = Number(deal?.total_price_usd);
  if (Number.isFinite(usd) && usd > 0) return usd;
  // Older rows without a stored USD total: only safe when the listing
  // itself is USD.
...
// AUCTION PRICE-DISPLAY CONTRACT (P0 auction-price-integrity fix).
//
// A deal row stores `price` (for an auction: the current BID alone),
// `shipping`, and `total_price` (= price + shipping, in the listing's
// native currency), plus `total_price_usd` (the landed total in USD).
// Deal qualification / the "% below market" figure are computed against
// the landed total - that methodology is unchanged. But the UI must never
// show that landed total under the words "current bid": for an auction the
// headline "current listing price" is the BID, with shipping and the
// estimated landed total shown as their own distinct lines.
//
// This splits a row into { bid, shipping, total }, each as
// { native, usd }, using the SCAN-TIME implied rate (total_price /
// total_price_usd) so all three convert with the exact rate the stored
// total was built from - no new FX call, and the three lines can never
// disagree with each other or with the headline. USD-native listings use
// rate 1. Returns null when the stored totals aren't usable (caller then
// keeps the single-figure landed-total rendering).
function auctionDisplayParts(deal) {
  const bidNative = Number(deal?.price);
  const shipNativeRaw = Number(deal?.shipping);
  const shipNative = Number.isFinite(shipNativeRaw) && shipNativeRaw > 0 ? shipNativeRaw : 0;
  const totalNative = Number(deal?.total_price);
  const totalUsd = Number(deal?.total_price_usd);
  const currency = currencyForDeal(deal);

  if (!Number.isFinite(bidNative) || bidNative <= 0) return null;
  if (!Number.isFinite(totalNative) || totalNative <= 0) return null;

  // native units per 1 USD, from the stored pair. USD listing -> 1.
  let rate = 1;
  if (currency !== "USD") {
    if (!Number.isFinite(totalUsd) || totalUsd <= 0) return null;
    rate = totalNative / totalUsd;
    if (!Number.isFinite(rate) || rate <= 0) return null;
  }
  const toUsdAt = (n) => (currency === "USD" ? n : n / rate);

  return {
    currency,
    bid: { native: bidNative, usd: toUsdAt(bidNative) },
    shipping: { native: shipNative, usd: toUsdAt(shipNative) },
    total: {
      native: totalNative,
      usd: Number.isFinite(totalUsd) && totalUsd > 0 ? totalUsd : toUsdAt(totalNative),
    },
  };
}
