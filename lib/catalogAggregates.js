...
export function computeAggregates(rows) {
  ...
  // FINDING 6 FAIL-SAFE. listingCount is only meaningful if the rows
  // actually carry listing identity. On 2026-09-24 a caller's select
  // omitted `listing_id`, every row keyed the same, and every listingCount
  // was published as 1 - a wrong number is worse than a missing one,
  // because the tile treats 1 as real and shows it. When no row carries an
  // identity the field is omitted entirely and every consumer falls back
  // to `count`, which is the honest answer for "we could not compute it".
  const identityKnown = rows.some((r) => hasKnownListingIdentity(r));
  const withListingCount = (obj, size) => (identityKnown ? { ...obj, listingCount: size } : obj);
  // --- sets: count per set name ---
  const setCounts = new Map();
  const setListings = new Map();
  for (const row of rows) {
    const set = row.watchlist?.set;
    if (!set) continue;
    setCounts.set(set, (setCounts.get(set) ?? 0) + 1);
    if (!setListings.has(set)) setListings.set(set, new Set());
    setListings.get(set).add(listingIdentityKey(row));
  }
  const sets = Array.from(setCounts.entries())
    .filter(([, count]) => count >= SET_MIN_LISTINGS) // thin-content rule - see lib/indexability.js
    .map(([set, count]) => withListingCount({ set, slug: slugifySet(set), count }, setListings.get(set).size))
  ...
  for (const w of byCard.values()) {
    ...
    const { listings, ...rest } = w;
    cardHubs.push(withListingCount({ ...rest, slug }, listings.size));
  }
