...
import { listingIdentityKey } from "@/lib/listingIdentity";
...
export function computeAggregates(rows) {
  ...
  const sets = Array.from(setCounts.entries())
    ...
    .map(([set, count]) => ({ set, slug: slugifySet(set), count, listingCount: setListings.get(set).size }))
  ...
  for (const w of byCard.values()) {
    ...
    cardHubs.push({ ...rest, listingCount: listings.size, slug });
  }
