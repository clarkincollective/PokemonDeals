...
import { preReleaseListing } from "@/lib/dealQuality";
...
export function computeAggregates(rows) {
  // 17C.7: listings for an officially upcoming expansion are not deals yet
  // (lib/dealQuality preReleaseListing), so they never feed a set badge,
  // hub count or "from $X". They return on the first refresh after
  // release day.
  rows = (rows ?? []).filter((row) => !preReleaseListing(row));
