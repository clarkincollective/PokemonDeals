...
import { savingsClaimTrusted, isOfferCountable } from "@/lib/dealQuality";
...
export function computeAggregates(rows) {
  // 17C.7: these counts are "qualifying deals", so a listing that may not
  // claim savings (no evidenced reference for the exact product and
  // condition - lib/dealQuality savingsClaimTrusted) never feeds a set
  // badge, hub count or "from $X". It returns on the first refresh after
  // the evidence exists.
  ...
  rows = (rows ?? []).filter((row) => isOfferCountable(row) && savingsClaimTrusted(row));
