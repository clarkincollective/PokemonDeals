...
export function parseSearchIntent(raw, { knownSets = [] } = {}) {
  ...
  // ---- 9. era / set phrase
  {
    const setHit = matchSetPhrase(work) || matchSetPhraseFromKnown(work, knownSets);
    if (setHit) {
...
// Set phrase from a DB-provided list [{name, slug, setId}] - longest name
// first. Used by the API route which passes the real card_catalog set index.
function matchSetPhraseFromKnown(work, knownSets) {
  ...
  const sorted = knownSets
    .filter((s) => s && s.name)
    .sort((a, b) => b.name.length - a.name.length);
  for (const s of sorted) {
    const phrase = s.name.toLowerCase();
    if (phrase.length >= 4 && hay.includes(` ${phrase} `)) {
      return { canonical: s.name, phrase, setId: s.setId ?? s.set_id ?? null };
    }
  }
  return null;
}
