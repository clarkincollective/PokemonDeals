...
// Subset/promo numbers that carry the SAME alphabetic prefix on both
// sides: "RC3/RC32", "SV49/SV94", "TG22/TG30", "GG34/GG70", "H11/H32".
// 532 english card_catalog rows across 6 families (surveyed 2026-09-12);
// in every family the two prefixes are identical, so differing prefixes
// are NOT claimed as one number. Must be tested BEFORE HASH/PREFIXED:
// those two match only the first half and leave "/sv94" behind as name
// text, which is worse than not parsing it at all.
const PREFIXED_SLASH_NUMBER_RE = /#?\s*\b([a-z]{1,5})(\d{1,4})\s*\/\s*([a-z]{1,5})(\d{1,4})\b/i;
...
export function collectorNumberVariants(num) {
  ...
  } else if (/^([a-z]{1,5})(\d{1,4})\/([a-z]{1,5})(\d{1,4})$/.test(raw)) {
    // Prefixed subset number ("rc3/rc32"). Two things matter here:
    //  * CASE - createSupabaseLookup builds `card_number.eq.<v>`, which is
    //    case-sensitive in Postgres, and all 532 stored rows use an
    //    UPPERCASE prefix. Lowercase-only variants match the in-memory
    //    fixture lookup and silently miss in production, so both cases are
    //    emitted here rather than changing the query shape.
    //  * leading zeros - same equivalence the numeric branch gives.
    const [, p1, a, p2, b] = raw.match(/^([a-z]{1,5})(\d{1,4})\/([a-z]{1,5})(\d{1,4})$/);
    const A = stripZeros(a);
    const B = stripZeros(b);
    // BOTH halves are padded in real data: 162 of the 532 stored rows carry
    // a zero-padded half - "H05/H32", "GG02/GG70", and "SV094/SV122" which
    // pads the denominator too. Padding only the first half would still
    // miss those, so both sides get the same 1/2/3-digit treatment.
    const pads = (n) => [n, n.padStart(2, "0"), n.padStart(3, "0")];
    for (const pa of pads(A)) {
      for (const pb of pads(B)) {
        out.add(`${p1}${pa}/${p2}${pb}`);
        out.add(`${p1.toUpperCase()}${pa}/${p2.toUpperCase()}${pb}`);
      }
    }
  } else {
...
export function parseSearchIntent(raw, { knownSets = [] } = {}) {
  ...
  let collector = null;
  {
    const pslash = work.match(PREFIXED_SLASH_NUMBER_RE);
    const slash = work.match(SLASH_NUMBER_RE);
    const prefixed = work.match(PREFIXED_NUMBER_RE);
    const hash = work.match(HASH_NUMBER_RE);
    // Same prefix on both halves, or we don't treat it as one number. The
    // whole token is compared downstream - never the "/RC32" tail alone,
    // which RC3/RC5/RC17 all share.
    if (pslash && pslash[1].toLowerCase() === pslash[3].toLowerCase()) {
      const p = pslash[1].toLowerCase();
      collector = `${p}${stripZeros(pslash[2])}/${p}${stripZeros(pslash[4])}`;
      consume(PREFIXED_SLASH_NUMBER_RE, "collector_number");
    } else if (slash) {
