...
export const OFFICIAL_RELEASES = Object.freeze([
  ...
  },
  {
    ...
    // Released on one calendar date in every region (the official pages
    // say so, and `regional` below records it). Its release day is judged
    // in WORLDWIDE_RELEASE_TIME_ZONE, not the US-first default - see there.
    worldwide: true,
...
// A SIMULTANEOUS WORLDWIDE release (record.worldwide) is the exception to
// the US-first rule. Its official date is the same calendar day in every
// region, so the day is judged where it begins first among the big
// English-language markets - Australia - rather than where it begins last.
// Holding a worldwide release "upcoming" until midnight in Los Angeles
// meant that on release day itself, for every viewer east of California,
// the site hid real, active, eligible listings for a product already on
// sale where they live. That is the opposite of the gate's purpose.
// Staggered releases are unaffected and keep the conservative zone.
export const WORLDWIDE_RELEASE_TIME_ZONE = "Australia/Sydney";

// The zone an official record's release day is judged in.
export function releaseZoneFor(record) {
  return record?.worldwide ? WORLDWIDE_RELEASE_TIME_ZONE : RELEASE_TIME_ZONE;
}
...
// Formatters per zone, built once. `zone` defaults to the US-first zone
// everywhere; a worldwide record passes its own (releaseZoneFor).
const DAY_FMTS = new Map();
const PARTS_FMTS = new Map();
function dayFmt(zone) {
  if (!DAY_FMTS.has(zone)) DAY_FMTS.set(zone, new Intl.DateTimeFormat("en-CA", { timeZone: zone, year: "numeric", month: "2-digit", day: "2-digit" }));
  return DAY_FMTS.get(zone);
}
function partsFmt(zone) {
  if (!PARTS_FMTS.has(zone)) {
    PARTS_FMTS.set(zone, new Intl.DateTimeFormat("en-US", {
      timeZone: zone, hourCycle: "h23", year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit",
    }));
  }
  return PARTS_FMTS.get(zone);
}
const ISO_DAY = /^\d{4}-\d{2}-\d{2}$/;

function zoneOffsetMs(ms, zone = RELEASE_TIME_ZONE) {
  const p = Object.fromEntries(partsFmt(zone).formatToParts(new Date(ms)).map((x) => [x.type, x.value]));
  return Date.UTC(+p.year, +p.month - 1, +p.day, +p.hour, +p.minute, +p.second) - ms;
}

// The instant an ISO release day begins in `zone` (default RELEASE_TIME_ZONE).
export function releaseDayStart(isoDay, zone = RELEASE_TIME_ZONE) {
  const guess = Date.parse(`${isoDay}T00:00:00Z`);
  return new Date(guess - zoneOffsetMs(guess, zone));
}

// The instant an official record's release day begins, in its own zone.
export function releaseStartFor(record) {
  return releaseDayStart(record.released, releaseZoneFor(record));
}

// Has the record's release day begun at `clock`? Judged in the record's
// own zone, so a worldwide release flips at midnight in Sydney and a
// staggered one at midnight in Los Angeles.
export function releasedAt(record, clock) {
  return record.released <= releaseDayOf(clock, releaseZoneFor(record));
}
...
// The current release day ("YYYY-MM-DD") in `zone` (default RELEASE_TIME_ZONE).
export function releaseDayOf(clock, zone = RELEASE_TIME_ZONE) {
  return dayFmt(zone).format(new Date(instantOf(clock)));
}
...
export function expansionReleaseStatus(setName, clock) {
  ...
  if (!r) return null;
  return releasedAt(r, clock) ? "released" : "upcoming";
}
...
export function latestReleasedExpansions(clock, limit = 3) {
  return OFFICIAL_RELEASES.filter((r) => releasedAt(r, clock))
...
export function nextReleaseBoundary(clock) {
  ...
  const starts = OFFICIAL_RELEASES.map((r) => releaseStartFor(r).getTime()).filter((t) => t > now).sort((a, b) => a - b);
...
export function upcomingExpansionForListing({ set = null, title = "" } = {}, clock) {
  ...
  if (bySet && !releasedAt(bySet, clock)) return bySet;
  for (const r of OFFICIAL_RELEASES) {
    if (!releasedAt(r, clock) && r.titlePattern?.test(String(title ?? ""))) return r;
  }
  return null;
}
...
export function expansionReleaseLabel(setOrRecord, clock) {
  const r = typeof setOrRecord === "string" ? officialReleaseForSet(setOrRecord) : setOrRecord;
  if (!r?.released || releasedAt(r, clock)) return null;
...
// DISPLAY ONLY. The catalogue's set name carries a release code prefix
// ("ME: 30th Celebration", "SWSH07: Evolving Skies") that people never use
// when they talk about or search for a set. This strips it for headings and
// link text.
//
// It never touches the canonical set name: slugs, catalogue lookups, the
// sitemap and every /sets/<slug> URL still derive from the stored name, so
// no URL changes and nothing is re-resolved by the display string.
const SET_CODE_PREFIX = /^[A-Z]{2,6}\d{0,2}:\s*/;

export function setDisplayName(setName) {
  const s = String(setName ?? "").trim();
  if (!s) return "";
  const stripped = s.replace(SET_CODE_PREFIX, "").trim();
  // Never strip down to nothing, and never leave a name so short it stops
  // identifying the set.
