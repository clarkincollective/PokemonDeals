...
function releaseRecordForSet(setName) {
...
export function upcomingExpansionForListing({ set = null, title = "" } = {}, clock) {
  const day = releaseDayOf(clock);
  const bySet = releaseRecordForSet(set);
...
// "Upcoming — releases 16 September" while an expansion is upcoming (the
// year is added when it differs from the current one); null once released.
// Worded as a release date, never as local availability.
export function expansionReleaseLabel(setOrRecord, clock) {
  const r = typeof setOrRecord === "string" ? releaseRecordForSet(setOrRecord) : setOrRecord;
  if (!r?.released) return null;
  const day = releaseDayOf(clock);
  if (r.released <= day) return null;
  const [y, m, d] = r.released.split("-").map(Number);
  return `Upcoming — releases ${d} ${MONTHS[m - 1]}${String(y) === day.slice(0, 4) ? "" : ` ${y}`}`;
}
