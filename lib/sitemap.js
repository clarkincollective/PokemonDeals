...
import { fetchSets, fetchCatalogSets, fetchCardHubs, fetchSpeciesHubs, fetchCatalogCardSitemapRows, fetchCatalogSpecies, fetchCatalogSyncedAt } from "@/lib/deals";
...
import { CARD_SITEMAP_SEGMENTS, NO_REFERENCE_SHARD, isCardSitemapSegment, assignCardShards, cardSitemapSnapshotId, sanitizeLastmodMap, selectSitemapCards, selectBulkShardCards } from "@/lib/cardSitemap";
...
async function fetchActiveDealIdsUncached(table) {
  ...
  // R3: card listings with unusable native totals are also noindex; select
  // that field below so sitemap projection preserves the page decision.
  const pageIndexable = sealed
    ? (r) => isDisplayableSealedDeal(r) && savingsClaimTrusted(r)
    : (r) => isDisplayableDeal(r) && savingsClaimTrusted(r) && hasPrice(r.total_price);
  ...
  // SEO-4: savingsClaimTrusted now requires stored reference evidence on
  // EVERY set, not only the tracked releases, so both selects must carry
  // the evidence columns it reads. Omitting them makes every row untrusted
  // and empties the deal sitemaps - the identical failure the catalogue
  // snapshot hit at 17C.7 (lib/deals.js SAVINGS_EVIDENCE_COLUMNS) and the
  // social pool hit here (lib/social/db.mjs). Pinned by
  // tests/scanner/r3-sitemap-price-parity.test.mjs, which projects ONLY
  // the columns named here.
  const evidenceCols =
    "reference_product_id, reference_amount, reference_currency, reference_fx_rate, " +
    "reference_fx_asof, reference_observed_at";
  // AUDIT 2026-09-23 FINDING 1: the sealed list had the very gap the
  // paragraph above warns about, and it is why nine quarantined rows were
  // still advertised after they stopped rendering. It omitted
  // `disqualified_reason` (so isDisplayableSealedDeal's first check read
  // undefined and never fired) and, worse, selected the embedded product
  // as `(set, tcgplayer_id)` with NO `name` - and sealedRowMatchesItsProduct
  // begins `if (!product?.name) return true`, so the product-identity
  // decision was silently skipped for every sealed sitemap row. The page
  // selects "*" and hid them correctly; only the sitemap leaked.
  // `exact_verified_at` stays out deliberately: without it
  // isPositiveActiveConfirmation is false, so an early sealed listing is
  // hidden from the sitemap - stricter than the page, never looser.
  const cols = sealed
    ? "id, is_active, disqualified_reason, last_seen_at, first_seen_at, title, listing_id, listing_url, affiliate_url, listing_type, auction_end_at, " +
      `market_price, ${evidenceCols}, ` +
      "sealed_watchlist:sealed_watchlist_id (name, set, tcgplayer_id)"
    : "id, last_seen_at, first_seen_at, exact_verified_at, is_active, is_graded, condition, title, card_name, card_set, card_language, " +
      ...
      "grade, grader, image_count, returns_accepted, seller_feedback_score, total_price, " +
      `card_tcgplayer_id, ${evidenceCols}, reference_condition, reference_printing, reference_grader, reference_grade`;
...
// Cards a guide links to by name. These are editorially chosen, verified
// catalogue identities that we actively point readers at, so they stay in
// the sitemap even when their reference price has never moved. Derived
// from the same GUIDE_CARDS hrefs the guides render (never hand-typed), so
// adding a card to a guide adds it here automatically.
export const EDITORIAL_CARD_SLUGS = new Set(
  Object.values(GUIDE_CARDS)
    .map((c) => String(c?.href ?? "").replace(/^\/cards\//, ""))
    .filter(Boolean)
);
...
export async function fetchCardSitemapDataset() {
  ...
  // SEO-4 substance gate. Advertise a card only when there is something to
  // recrawl: live listings, a proven price change, or an editorial link.
  // See lib/cardSitemap.js selectSitemapCards for the full rationale and
  // the fail-open rule - the gate is skipped entirely unless the lastmod
  // map came from a healthy RPC, so a missing/failing function can never
  // shrink the sitemap.
  const { kept, dropped, enforced } = selectSitemapCards(cards, {
    lastmodByTcg,
    editorialSlugs: EDITORIAL_CARD_SLUGS,
    enforce: lastmod.source === "rpc",
  });
  const { shards, noReference } = assignCardShards(kept, lastmodByTcg);
  const out = {};
  for (const [key, entries] of shards) out[key] = entries.map((e) => ({ ...e, hub: hubSlugs.has(e.slug) }));
  // Bulk shard rule (2026-09-20, lib/cardSitemap selectBulkShardCards): the
  // lowest band lists live-deal hubs and article-linked cards only; a
  // sub-$5 card whose only signal is a lastmod is not advertised. Applied
  // to the shard's entries after banding, so band membership itself is
  // unchanged and every other shard is exactly as before.
  const bulk = selectBulkShardCards(out[NO_REFERENCE_SHARD] ?? [], { editorialSlugs: EDITORIAL_CARD_SLUGS });
  out[NO_REFERENCE_SHARD] = bulk.kept;
  shards.set(NO_REFERENCE_SHARD, bulk.kept);
  ...
  return {
    shards: out,
    noReference,
    lastmodSource: lastmod.source,
    cardCount: kept.length - bulk.dropped.length,
    eligibleCount: cards.length,
    droppedCount: dropped.length,
    bulkDroppedCount: bulk.dropped.length,
    bulkKeptCount: bulk.kept.length,
    substanceEnforced: enforced,
    snapshotId,
  };
}
...
const STATIC_ROUTES = [
  ...
  // A guide is a fixed, dated editorial page, so its own published /
  // updated date IS its last modification - the same truthful mapping the
  // news items below already use. 24 of 28 guides carry a real date;
  // GUIDES_PUBLISHED is NOT substituted for the rest, because a shared
  // launch date is not a per-page modification date. Before 2026-09-21 no
  // guide carried a lastmod at all, so the editorial pages we most want
  // recrawled gave Google nothing to schedule on.
  ...GUIDES.map((g) => {
    const lastmod = g.updated ?? g.published ?? null;
    return {
  ...
  // Dated news. The hub changes whenever an item is published; an item
  // itself is a fixed, dated page and carries its publish date as lastmod.
  { loc: `${SITE_URL}/news`, changefreq: "weekly", priority: 0.7 },
  ...NEWS.map((n) => ({
    loc: `${SITE_URL}/news/${n.slug}`,
    changefreq: "monthly",
    priority: 0.6,
    lastmod: n.updated ?? n.published,
  })),
  { loc: `${SITE_URL}/how-it-works`, changefreq: "monthly", priority: 0.5 },
  { loc: `${SITE_URL}/methodology`, changefreq: "monthly", priority: 0.5 },
  { loc: `${SITE_URL}/integrity`, changefreq: "daily", priority: 0.5 },
  ...
  // Still no <lastmod>, deliberately. The news items and guides above DO
  // carry one (their own publish/update date is a real modification date),
  // but this study's observation window is NOT a modification date -
  // stamping 2026-09-11 here would assert something untrue about when the
  // page last changed. The date belongs in temporalCoverage on the
  // Dataset, which is where it already is.
  {
    loc: `${SITE_URL}/market-data/pokemon-reference-price-changes`,
    changefreq: "yearly",
    priority: 0.6,
  },
  // The same treatment for the same reason: a fixed, dated snapshot
  // (observed 2026-09-22) rendered from a frozen artifact, so its figures
  // never move and its observation date is not a modification date. No
  // <lastmod>.
  {
    loc: `${SITE_URL}/market-data/pokemon-shipping-cost-study`,
    changefreq: "yearly",
    priority: 0.6,
  },
];
...
export async function segmentEntries(segment) {
  ...
  switch (segment) {
    ...
    case "sets": {
      ...
      const [{ sets }, { sets: catalogSets }, syncedAt] = await Promise.all([
        fetchSets({ language: "english" }),
        fetchCatalogSets(),
        fetchCatalogSyncedAt(),
      ]);
      // SEO audit 2026-09-20: <lastmod> = the catalogue sync date (the
      // reference prices on every set page come from it); omitted when unknown
      const lastmod = syncedAt ? { lastmod: syncedAt } : {};
      ...
      for (const s of catalogSets) {
        ...
        entries.push({ loc: `${SITE_URL}/sets/${s.slug}`, changefreq: "weekly", priority: 0.6, ...lastmod });
      }
      return entries;
    }
    ...
    case "pokemon": {
      ...
      const [{ species }, { species: catalogSpecies }, syncedAt] = await Promise.all([
        fetchSpeciesHubs({ language: "english" }),
        fetchCatalogSpecies(),
        fetchCatalogSyncedAt(),
      ]);
      const lastmod = syncedAt ? { lastmod: syncedAt } : {}; // see the sets case
      ...
      for (const s of catalogSpecies) {
        if (dealSlugs.has(s.slug)) continue;
        entries.push({ loc: `${SITE_URL}/pokemon/${s.slug}`, changefreq: "weekly", priority: 0.6, ...lastmod });
      }
      return entries;
    }
...
export async function segmentXml(segment) {
  ...
  if (isCardSitemapSegment(segment)) {
    const dataset = await fetchCardSitemapDataset();
    const substance = dataset.substanceEnforced
      ? `substance-gate on (${dataset.cardCount}/${dataset.eligibleCount} advertised, ${dataset.droppedCount} unchanged cards not listed)`
      : "substance-gate off (lastmod unavailable)";
    const bulkRule =
      segment === NO_REFERENCE_SHARD
        ? ` bulk-rule on (live-deal hubs and article-linked cards only: ${dataset.bulkKeptCount} listed, ${dataset.bulkDroppedCount} sub-$5 lastmod-only cards not listed)`
        : "";
    return urlsetXml(entries, {
      comment: `card-sitemap snapshot ${dataset.snapshotId} lastmod-source ${dataset.lastmodSource} ${substance}${bulkRule}`,
    });
  }
  return urlsetXml(entries);
}
...
// Crawl-budget decision, 2026-09-19 → resolved 2026-09-20.
// 19 Sep (Search Console Page indexing: 24,952 "Discovered - currently not
// indexed" against 1,813 indexed): the index stopped listing cards-bulk.
// 20 Sep inspection: the shard still served 6,050 URLs, Search Console
// still held a direct submission of it (15 Sep), and six live-deal hubs
// plus 41 guide-linked cards were in that unadvertised file - against the
// "a hub must be in a sitemap" rule. Resolution moved to the shard itself
// (lib/cardSitemap selectBulkShardCards): cards-bulk now lists only hubs
// and article-linked cards, so it is advertised again and the direct
// submission is consistent with the index. Sub-$5 lastmod-only cards stay
// live, canonical, indexable and internally linked - just not advertised.
// This set stays as the mechanism; it is empty.
// Re-read the report on 2026-10-04.
export const UNADVERTISED_SEGMENTS = new Set([]);
...
export function indexXml() {
  ...
  const rows = SITEMAP_SEGMENTS.filter((id) => !UNADVERTISED_SEGMENTS.has(id)).map(
