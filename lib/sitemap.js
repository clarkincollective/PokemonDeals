...
async function fetchActiveDealIdsUncached(table) {
  ...
  const cols = sealed
    ...
    : "id, last_seen_at, is_active, is_graded, condition, title, card_name, card_set, card_language, " +
...
export function cacheControlForSegment(segment) {
  return EPHEMERAL_SEGMENTS.has(segment) ? SITEMAP_CACHE_CONTROL_EPHEMERAL : SITEMAP_CACHE_CONTROL;
}
