...
import { CARD_SITEMAP_SEGMENTS, isCardSitemapSegment, assignCardShards, cardSitemapSnapshotId, sanitizeLastmodMap, selectSitemapCards } from "@/lib/cardSitemap";
...
export async function segmentXml(segment) {
  ...
  if (isCardSitemapSegment(segment)) {
    ...
    return urlsetXml(entries, {
      comment: `card-sitemap snapshot ${dataset.snapshotId} lastmod-source ${dataset.lastmodSource} ${substance}`,
    });
  }
  return urlsetXml(entries);
}
...
// Crawl-budget decision, 2026-09-20 (Search Console Page indexing report:
// 24,952 URLs "Discovered - currently not indexed" against 1,813 indexed).
// Google knows every advertised URL but spends its crawl on a fraction, so
// the sitemap index no longer advertises the cards-bulk shard - 6,051 card
// pages whose reference is under $5 or absent, the least-searched pages
// on the site. They stay live, canonical, indexable and internally linked
// (species, set and card pages all reach them); they are simply not put
// in front of the crawler ahead of sets, Pokemon, guides and priced cards.
// The shard's own /sitemaps/cards-bulk.xml still serves (nothing 404s),
// and the shard rules in lib/cardSitemap are untouched, so this is one
// Set to change back. Re-read the report on 2026-10-04.
export const UNADVERTISED_SEGMENTS = new Set(["cards-bulk"]);
