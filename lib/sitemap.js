...
import { fetchSets, fetchCardHubs, fetchSpeciesHubs, fetchCatalogCardSlugs, fetchCatalogSpecies } from "@/lib/deals";
...
async function fetchActiveDealIdsUncached(table) {
  ...
  for (let from = 0; from < MAX_DEAL_URLS; from += PAGE_SIZE) {
    ...
    const { data } = await supabase
      ...
      // Only live deals - an expired /deals/[id] or /sealed-deals/[id]
      // renders noindex, so it must not be in the sitemap (the function
      // name always implied this filter; it was missing, which put
      // freshly-expired deal URLs in the sitemap until the 900s cache
      // refreshed - caught by tests/seo/sitemap.test.mjs).
      .eq("is_active", true)
