...
const { upcomingExpansionForListing, expansionIdentityConflict, expansionReleaseLabel } = require("./pokemonSets.js");
...
// PRE-RELEASE GATE (17C.7). We have no preorder handling yet: nothing can
// label a listing as a preorder, state when it will ship, or back a
// discount against a set that has no post-release market. So a listing for
// an officially UPCOMING expansion is kept off every deal surface (grids,
// premium slots, social, search, sitemap, deal pages) until release day.
// Its catalogue pages are unaffected. Display-time only and
// time-dependent, so it is deliberately NOT a disqualificationReason and
// is never persisted. Once release day begins (US-first policy,
// lib/pokemonSets RELEASE_TIME_ZONE), the row is judged by the normal
// gates again as caches refresh. Returns { set, officialName, released,
// label } or null.
function listingCatalogueSet(row) {
  return row?.card_set ?? row?.watchlist?.set ?? row?.sealed_watchlist?.set ?? null;
}
function preReleaseListing(row, now = Date.now()) {
  if (!row) return null;
  const r = upcomingExpansionForListing({ set: listingCatalogueSet(row), title: row.title ?? "" }, now);
  return r ? { set: r.set, officialName: r.officialName, released: r.released, label: expansionReleaseLabel(r, now) } : null;
}
...
function isDisplayableDeal(row) {
  ...
  if (preReleaseListing(row)) return false;
...
// pre-release and expansion-identity rules as cards. Sealed preorders are
// the common case, and a presale of a new product must not be priced
// against an older product of a similar name.
function isDisplayableSealedDeal(row, now = Date.now()) {
  if (!row) return false;
  if (!isExactEbayDealDestination(row)) return false;
  if (auctionEnded(row, now)) return false;
  if (listingNamesDifferentExpansion(row)) return false;
  if (preReleaseListing(row, now)) return false;
  return true;
}
