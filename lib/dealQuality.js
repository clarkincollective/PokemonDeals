...
// tiered. Auctions are governed by their end time first (auctionEnded);
// their last_seen age only matters once it is very stale (a cancelled /
// relisted auction whose stored end time never moved).
const FRESHNESS_TTL_HOURS = {
  ...
  low: 168,
  // a future-dated auction: its end time is the real signal, so only a
  // very stale last_seen (never re-swept) drops it.
  auction: 168,
};

function freshnessTierTtl(row) {
  if (row.listing_type === "AUCTION" && row.auction_end_at && Date.parse(row.auction_end_at) > Date.now()) {
    return FRESHNESS_TTL_HOURS.auction;
  }
