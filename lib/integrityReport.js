...
// SEEN AGAIN in eBay search results in the last 24 hours (`checked24h`
// counts last_seen_at, i.e. a scan refresh - NOT a per-listing
// availability re-check, which is `exact_verified_at` and is what the
// card's own "Checked N ago" row reads; measured 2026-09-22, the two
// were 1,021 and 172) and - once supabase/integrity_migration.sql
...
import { countDisplayableActiveDeals } from "@/lib/deals";
...
export async function computeIntegrityReport(now = Date.now()) {
  ...
  try {
    // "Shown" is the SAME row-level displayable count the homepage feed
    // prints ("N live deals"), so the two pages never cite two figures.
