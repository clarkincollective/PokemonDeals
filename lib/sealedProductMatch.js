...
const KIND_RULES = [
  ...
  //
  // AUDIT 2026-09-23 FINDING 1 - the markers above missed the single
  // commonest shape of all: a GRADED PROMO from the box, whose title is
  // the box's name plus the slab's label. Confirmed cases /sealed-deals
  // /2435 (a PSA 9 "N's Zekrom #031" rendered as an Ascended Heroes ETB)
  // and /961 (a PSA 9 "Shining Ho-Oh SM70" rendered as a Shining Legends
  // ETB), both verified against the listing photograph.
  //
  //   GRADER TOKEN. The old `\b(?:psa|cgc|bgs)\s*\d` needs a digit
  //   IMMEDIATELY after the grader, but real slab titles write the grade
  //   in words first - "CGC Mint 9", "CGC NM/Mint 8", "PSA Grade 4",
  //   "CGC come nuovo 9", or (as in /961) omit it entirely. Measured on
  //   the 202 stored rows displayable on 2026-09-24: the bare grader
  //   token appears in 34 of them, across 9 distinct titles, and every
  //   one is a graded single. No genuine sealed box in the corpus names
  //   a grader. This is why it is the grader's NAME and not a "promo"
  //   keyword: a real ETB legitimately advertises its promo card
  //   ("... ETB Sealed Charizard Promo", "... ETB Zorua Promo 9 Packs"),
  //   and all of those survive.
  //   SM PROMO CODES are three digits (SM210); SM SET codes are one or
  //   two (SM6, SM12), so three digits cannot collide with a set.
  //   BSP is "Black Star Promo"; required next to its number so the
  //   German abbreviation "bsp." cannot trip it.
  //   A PARENTHESISED COLLECTOR NUMBER WITH A LEADING ZERO - "(031)",
  //   "(087)". A year never starts with 0 and a quantity is never
  //   written this way, so this cannot collide with the "(2021)" that
  //   an earlier lot rule already had to be narrowed for.
  [
    "single_card",
    /\b(?:psa|cgc|bgs|sgc)\b|\bgraded\b|\bslab(?:bed)?\b|#?\s?swsh\s?\d{3}\b|\bsm\s?\d{3}\b|\bbsp\s*#?\s?\d{2,3}\b|\(\s*0\d{2}\s*\)|\bgold star\b|\bblack star promos?\b|\bsingle card\b/i,
  ],
  // AN ACCESSORY sold FOR the product - a protector, an acrylic case,
  // sleeves. The PREPOSITION is what makes the accessory the subject
  // ("Protectors for <ETB>", "Acrylic Case fits <ETB>"), so a genuine box
  // sold WITH one ("... ETB SEALED WITH ACRYLIC", "... Elite Trainer Box
  // + Schutzbox") is untouched - both are in the corpus and both survive.
  [
    "accessory",
    /\b(?:protectors?|protezioni|protecteurs?|schutzh[uü]lle|acrylic case|display case|hard case|magnetic case)\b[^|]{0,24}?\b(?:for|fits|per|f[uü]r|pour)\b|\b(?:deck shield sleeves?|card sleeves?|outer sleeve)\b/i,
  ],
  // AN EMPTY BOX - the packaging with its contents removed. Stated
  // plainly by the seller in every corpus case ("TCG-no Cards. All
  // Extras!", "| NO BOOSTER PACKS or PROMO CARDS").
  //
  // STAGE 4 (2026-09-24): the rule was English-only, so six corpus rows
  // saying the same thing in German or Italian were classified by a later
  // rule instead - "Case leer", "LEER OHNE KARTEN", "VUOTO NO CARTE".
  // Every one of them is already refused, so this changes the RECORDED
  // REASON and nothing a visitor sees; it is here so the next such title
  // is refused for being empty rather than by luck.
  [
    "empty_box",
    /\bno\s+(?:cards?|booster packs?|packs?|promos?)\b|\bempty box\b|\bbox only\b|\bwithout\s+(?:the\s+)?(?:cards?|packs?|promos?)\b|\bleer\b|\bohne\s+karten\b|\bvuoto\b|\b(?:no|senza)\s+carte\b|\bvide\b|\bsans\s+cartes\b|\bincomplete\b/i,
  ],
  ...
  // STAGE 4 (2026-09-24): "Top-Trainer-Box" is the OFFICIAL GERMAN retail
  // name for the Elite Trainer Box. Without it, `collection` matched the
  // bare "box" further down and a genuine sealed German ETB was refused -
  // deal 937, confirmed from the listing photograph (a sealed Pokemon GO
  // box whose front panel reads "Top-Trainer-Box"). A product alias, not a
  // loosening: it names one more way sellers write the same product.
  ["etb", /\belite trainer box\b|\betb\b|\btop[\s-]?trainer[\s-]?box\b/i],
...
//
// STAGE 4 (2026-09-24) adds the ACCOMPANIMENT form. The adjective list
// above only recognised protective packaging that was named: a seller who
// writes "Elite Trainer Box Englisch OVP sealed + Case" names no adjective
// at all, and the `case` rule (which sits above `etb`) claimed the title.
// Deal 221 is exactly that, and the listing photograph shows a sealed
// Champion's Path ETB inside a clear protective case - a genuine box
// refused as a case SKU.
//
// The new alternative requires an ACCOMPANIMENT MARKER before the word:
// "+ case", "with case", "incl. case", "and case". That is what separates
// a box sold WITH a case from a case sold on its own - "ETB … Case leer"
// (German: empty) and "Sealed Case of 4 ETBs" carry no such marker and are
// still read as cases, which the corpus rows 930/931 and the factory-case
// rule below both depend on.
const PROTECTIVE_CASE =
  /\b(?:acrylic|display|protector|protective|magnetic|storage)\s+case\b|\bcase\s+(?:protector|only)\b|(?:\+|\bwith\b|\bw\/|\binc(?:luding|l)?\.?|\band\b|\bplus\b)\s*(?:an?\s+)?(?:hard\s+|soft\s+|clear\s+|acrylic\s+|protective\s+)?case\b/i;
...
// Kinds a WATCHED CATALOGUE PRODUCT can never be. We sell sealed product;
// we do not sell single cards, box protectors or empty packaging, so these
// are only ever read off a listing TITLE - and a title carrying one is
// evidence the listing is not the product, whatever else it names.
export const NOT_A_SEALED_PRODUCT = Object.freeze(["single_card", "accessory", "empty_box"]);
...
export function productKindOfProduct({ name, productType } = {}) {
  ...
  for (const [kind, re] of KIND_RULES) {
    if (NOT_A_SEALED_PRODUCT.includes(kind)) continue;
    if (re.test(s)) return kind;
  }
  return null;
}
...
export function sealedListingDecision(title, product) {
  ...
  // 4. A single card, graded promo, accessory or empty box that merely
  //    names a product is not that product. Rejected on the title's own
  //    evidence, without needing the product's kind to be readable - the
  //    embedded sealed_watchlist carries no product_type, so productKind
  //    is often null and rule 5 would not fire.
  if (NOT_A_SEALED_PRODUCT.includes(titleKind)) return { ok: false, reason: `kind_mismatch:${titleKind}` };
