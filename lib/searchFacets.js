...
// Phase 13B.6.2 - a deterministic, order-independent key for the
// EFFECTIVE search URL state. The server render computes it for the
// initial deep-link result; SearchClient computes it from window.location
// after hydration and skips the redundant /api/card-search call when they
// match. Filters go through normalizeDealFilters() so a contradictory /
// malformed URL produces the SAME key on both sides. No `page` here on
// purpose - catalogue pagination is a client-only concern and the server
// only ever seeds page 1. Never includes tracking params.
export function searchStateKey({ q, type, grader, grade, minPrice, maxPrice, listing, country, sort } = {}) {
  const n = normalizeDealFilters({ type, grader, grade, minPrice, maxPrice, listing });
  return [
    `q:${(q ?? "").trim().toLowerCase().replace(/\s+/g, " ")}`,
    `type:${n.type === "all" ? "" : n.type}`,
    `grader:${n.grader ?? ""}`,
    `grade:${n.grade ?? ""}`,
    `listing:${n.listing === "all" ? "" : n.listing}`,
    `min:${n.minPrice ?? ""}`,
    `max:${n.maxPrice ?? ""}`,
