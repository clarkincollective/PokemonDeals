...
// crossmatch-price-pilot-r1 - what the database can and cannot guarantee. The
// only uniqueness is deals_unique_listing (source, marketplace, listing_id):
// ON CONFLICT DO NOTHING stops a same-marketplace duplicate atomically, but
// nothing stops another writer creating a copy of the same eBay listing on a
// DIFFERENT marketplace between insertNewSighting's recheck and its insert.
// So immediately after a pilot insert, every other copy is read. If any copy
// is matched to a different (or unreadable) catalogue identity, or carries a
// non-availability exclusion (identity quarantine, review hold), the pilot's
// OWN new row is quarantined - hidden by the display gate and never recovered.
// Other writers' rows are never touched. A same-identity copy leaves the new
// row as it is, reported as a marketplace copy, not new supply. A failed read
// quarantines the new row (fail closed).
const PILOT_COPY_CONFLICT_REASON = "identity:crossmatch_copy_conflict";
async function checkPilotInsertCopies(db, { id, listingId, cardId, language }) {
  ...
  const others = error ? null : (data ?? []).filter((r) => String(r.id) !== String(id));
  const conflicting =
    others == null ||
    others.some(
      (o) =>
  ...
  if (!conflicting) return { otherCopies: others.length, quarantined: false };
  const q = await db.from("deals").update({ is_active: false, disqualified_reason: PILOT_COPY_CONFLICT_REASON }).eq("id", id).is("disqualified_reason", null).select("id");
  return { otherCopies: others?.length ?? null, quarantined: !q.error && (q.data ?? []).length === 1, quarantineError: q.error?.message ?? null, readError: error?.message ?? null };
}
