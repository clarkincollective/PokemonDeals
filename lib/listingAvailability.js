...
const { recordObservation } = require("./listingObservations");
...
async function writeDiscoverySighting(db, core) {
  ...
  await recordObservation(db, core).then(
    (r) => { lastObservationOutcome = r.outcome; },
    () => { lastObservationOutcome = "error"; }
  );
  return result;
}

// Last observation outcome, exposed for the scan routes' telemetry only.
// Not state anything depends on; purely so a run can report how many
// observations were written, suppressed as same-day duplicates, or skipped
// because the table is not migrated yet.
let lastObservationOutcome = null;
function takeLastObservationOutcome() {
  const v = lastObservationOutcome;
