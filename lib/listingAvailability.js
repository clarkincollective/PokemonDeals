...
// Called by sweep-stale-deals: expire every queued tag once, then delete only
// the queue rows it read. A row whose delete fails is expired again next run
...
async function drainCacheInvalidationQueue(db, revalidate, { limit = 200 } = {}) {
  try {
    ...
    if (error) return { rows: 0, expired: 0, errors: [error.message] };
    const rows = data ?? [];
    if (rows.length === 0) return { rows: 0, expired: 0, errors: [] };
    const tags = [...new Set(rows.flatMap((r) => (Array.isArray(r?.data?.tags) ? r.data.tags : [])).filter((t) => typeof t === "string" && t.length > 0 && t.length <= 256))];
    const out = expireTags(revalidate, tags);
    const { error: delError } = await db.from("catalog_snapshot").delete().in("kind", rows.map((r) => r.kind));
    return { rows: rows.length, expired: out.expired, errors: [...out.errors, ...(delError ? [delError.message] : [])] };
  } catch (e) {
    return { rows: 0, expired: 0, errors: [e?.message ?? String(e)] };
  }
}
