...
async function writeGuardedSighting(db, table, core, { insertHold = null } = {}) {
  ...
  // a NEW row only: the insert may carry a review hold (never the update, which
  // never sends the reason column)
  const heldReason = insertHold ? await insertHold(db, core) : null;
  const ins = await db
    .from(table)
    .upsert(heldReason ? { ...core, disqualified_reason: heldReason } : core, { onConflict: "source,marketplace,listing_id", ignoreDuplicates: true })
    .select("id");
  if (ins.error) return { outcome: "error", error: ins.error, markedSeenAgain: false };
  if ((ins.data ?? []).length > 0) return { outcome: "inserted", error: null, markedSeenAgain: false, heldReason };
...
// integrity-copy-hold (owner-reported counterfeits, deals 38616-38618,
// 15 Sep 2026). An authenticity disposition lives on ONE marketplace row, but
// the same eBay item is surfaced on several marketplaces. Copies of listings
// already flagged on EBAY_US - a stored authenticity:proxy_or_counterfeit
// (4220, 4247) and a vision COUNTERFEIT_MISMATCH verdict (38096) - were
// inserted on EBAY_GB as fresh rows with no reason and no verdict, and were
// displayable at once: visual screening runs out of band, and an unscreened
// row is not hidden. A new copy of a listing whose other copy is
// authenticity-held now inserts under a review hold instead. That is not an
// authenticity determination about the new row. Like every review hold it is
// never cleared by a re-sighting and never released automatically.
// Identity matching is separate: a correct card match says nothing about
// authenticity.
const COPY_HOLD_REASON = "review:copy_of_authenticity_hold";
const COPY_CHECK_FAILED_REASON = "review:copy_check_failed";
const OWNER_REPORTED_COUNTERFEIT_REASON = "review:owner_reported_counterfeit";
const COUNTERFEIT_VISUAL_VERDICTS = Object.freeze(["COUNTERFEIT_MISMATCH", "MISMATCH"]);
function authenticityHeld(row) {
  const r = String(row?.disqualified_reason ?? "");
  return (
    r.startsWith("authenticity:") ||
    r === OWNER_REPORTED_COUNTERFEIT_REASON ||
    r === COPY_HOLD_REASON ||
    COUNTERFEIT_VISUAL_VERDICTS.includes(row?.visual_authenticity_status)
  );
}
// integrity-copy-hold-2 (row 38096). The screener used to persist ONLY the
// verdict; the hold existed at display time (dealQuality.visualAuthenticityReason)
// and only where a reader loaded visual_authenticity_status. Readers that
// select a column subset (species prints, hub aggregates) could not see it
// and kept counting the row as a live deal, and the row itself carried
// nothing a copy or a later reader could inherit. A blocking verdict now also
// persists the same reason the gate derives, on the original row, when it
// carries no reason yet (a stronger existing reason is kept). Normal
// re-sightings never send the reason column, and it is not an availability
// reason, so nothing clears it automatically.
const { recordObservation, newObservationTally, countObservation } = require("./listingObservations");

const VERDICT_HOLD_REASON = "authenticity:proxy_or_counterfeit";
function authenticityVerdictHold(verdict, row) {
  if (!COUNTERFEIT_VISUAL_VERDICTS.includes(verdict?.status)) return null;
  if (row?.disqualified_reason != null) return null;
  return { disqualified_reason: VERDICT_HOLD_REASON };
}

// the hold reason for a NEW deals row, or null. Fails closed: copies that
// cannot be read are not assumed clean.
async function copyAuthenticityHold(db, core) {
  const { data, error } = await db.from("deals").select("id, marketplace, disqualified_reason, visual_authenticity_status").eq("listing_id", core.listing_id);
  if (error) return COPY_CHECK_FAILED_REASON;
  return (data ?? []).some((r) => r.marketplace !== core.marketplace && authenticityHeld(r)) ? COPY_HOLD_REASON : null;
}

async function writeDiscoverySighting(db, core) {
  const result = await writeGuardedSighting(db, "deals", core, { insertHold: copyAuthenticityHold });
  // SEO-2.5 observation log. THE canonical lifecycle boundary for a card
  // listing - both discovery callers (refresh-deals and ingest-feed) come
  // through here with a fully normalised `core`, after identity, pricing,
  // reference, condition and grading are settled. Logging once here beats
  // instrumenting every caller.
  //
  // Deliberately AFTER the sighting write and deliberately ignoring its own
  // result: this is a non-authoritative record of what the system concluded,
  // it must never change `result`, and recordObservation never throws. A
  // "blocked" or "error" sighting is still a real observation of the
  // listing's state, so it is logged too.
  await logObservation(db, core);
  return result;
}

// SEO-2.5.1 - the ONE place an observation is recorded, used by every
// discovery path. Swallows everything: a failure here counts, it never
// propagates, and it never touches the caller's sighting result.
async function logObservation(db, core) {
  try {
    countObservation(observationTally, await recordObservation(db, core));
  } catch (e) {
    countObservation(observationTally, { outcome: "error", error: e });
  }
}

// In-process tally for the current scan run. The defect this repairs was
// invisible - the scanner reported success while 100% of observation writes
// failed - so a run can now report what happened to its observations.
// Nothing reads this to make a decision.
let observationTally = newObservationTally();
function takeObservationTally() {
  const v = observationTally;
  observationTally = newObservationTally();
  return v;
}
...
async function insertNewSighting(db, core) {
  ...
  // SEO-2.5.1 - the cross-match pricing pilot is the OTHER discovery path
  // into `deals`, and it was not instrumented, so listings entering through
  // it were recording nothing. It goes through the same canonical observation
  // writer as writeDiscoverySighting: no duplicated pricing, identity,
  // screening or reference logic, just the state this path concluded.
  //
  // A listing that enters through BOTH paths cannot produce two research
  // observations for the same day - the partial unique index is authoritative
  // and the second write is reported as `duplicate`.
  //
  // Logged regardless of the insert's outcome (inserted / exists / error):
  // the observation records what was seen, not whether the row was ours.
  await logObservation(db, core);
  if (ins.error) return { outcome: "error", error: ins.error, where: null };
  if ((ins.data ?? []).length > 0) return { outcome: "inserted", error: null, where: null, id: ins.data[0].id };
  return { outcome: "exists", error: null, where: "concurrent_insert" };
}

// crossmatch-price-pilot-r1 - PUBLICATION IN TWO STEPS.
// What the database guarantees: the only uniqueness is deals_unique_listing
// (source, marketplace, listing_id), so ON CONFLICT DO NOTHING stops a
// same-marketplace duplicate atomically. Nothing stops another writer creating
// a copy of the same eBay listing on a DIFFERENT marketplace around the
// pilot's insert. So a pilot row is INSERTED HIDDEN, carrying the temporary
// reason PILOT_PENDING_REASON (any non-null disqualified_reason hides a row
// from every surface), and becomes visible only through finalisePilotInsert:
//   1. read every other copy of the listing;
//   2. a copy matched to a different or unreadable catalogue identity, or held
//      by a non-availability reason -> the pilot row's pending reason is
//      replaced by PILOT_COPY_CONFLICT_REASON (hidden for good);
//   3. otherwise -> clear the reason, but ONLY where the row still carries
//      exactly PILOT_PENDING_REASON and still belongs to the pilot's card
//      (a normal sighting may legitimately update a review-held row's fields;
//      such a row is never published by the pilot).
// Anything else - a failed read, a failed or unmatched update, a crash between
// the steps - leaves the row hidden with the pending reason. Such rows are
// reported (in the run, and by later runs from the table) and never released
// automatically. Other writers' rows and reasons are never touched.
// RESIDUAL (not solved here): a conflicting copy created on another
// marketplace AFTER finalisation is not detected by the pilot - the same
// exposure normal cross-marketplace writers already have. All deals withholds
// eligible listings whose copies disagree on identity; per-listing surfaces
// rely on the per-row display gate. This is not global atomic identity
// protection.
const PILOT_PENDING_REASON = "review:crossmatch_pending";
const PILOT_COPY_CONFLICT_REASON = "identity:crossmatch_copy_conflict";
async function finalisePilotInsert(db, { id, listingId, watchlistId, cardId, language }) {
  const { data, error } = await db.from("deals").select("id, marketplace, card_tcgplayer_id, card_language, disqualified_reason").eq("listing_id", listingId);
  if (error) return { published: false, state: "pending_copy_read_failed", otherCopies: null };
  const others = (data ?? []).filter((r) => String(r.id) !== String(id));
  const conflicting = others.some(
    (o) =>
      o.card_tcgplayer_id == null ||
      String(o.card_tcgplayer_id) !== String(cardId) ||
      (o.card_language ?? null) !== (language ?? null) ||
      (o.disqualified_reason != null && !String(o.disqualified_reason).startsWith(AVAILABILITY_REASON_PREFIX))
  );
  if (conflicting) {
    const q = await db.from("deals").update({ is_active: false, disqualified_reason: PILOT_COPY_CONFLICT_REASON }).eq("id", id).eq("disqualified_reason", PILOT_PENDING_REASON).select("id");
    if (q.error) return { published: false, state: "pending_quarantine_failed", otherCopies: others.length };
    return { published: false, state: (q.data ?? []).length === 1 ? "quarantined" : "pending_reason_replaced", otherCopies: others.length };
  }
  const f = await db.from("deals").update({ disqualified_reason: null }).eq("id", id).eq("disqualified_reason", PILOT_PENDING_REASON).eq("watchlist_id", watchlistId).select("id");
  if (f.error) return { published: false, state: "pending_finalise_failed", otherCopies: others.length };
  if ((f.data ?? []).length !== 1) return { published: false, state: "pending_reason_replaced", otherCopies: others.length };
  return { published: true, state: "published", otherCopies: others.length };
}

// Pending pilot rows older than `olderThanMs` (crashed or failed finalisation,
// possibly in an earlier invocation). Read-only; bounded.
async function abandonedPilotPendingRows(db, { olderThanMs = 15 * 60_000, now = Date.now(), limit = 50 } = {}) {
  const { data, error } = await db.from("deals").select("id, listing_id, marketplace, first_seen_at").eq("disqualified_reason", PILOT_PENDING_REASON).lt("first_seen_at", new Date(now - olderThanMs).toISOString()).limit(limit);
  if (error) return { rows: null, error: error.message };
  return { rows: (data ?? []).map((r) => ({ id: r.id, listingId: r.listing_id, marketplace: r.marketplace, firstSeenAt: r.first_seen_at })), error: null };
}
...
// catalog-price-freshness-r1. The catalogue-price surfaces (the most-valuable
// ranking and the composition/snapshot line beside it) are derived from
// card_catalog.market_price, which app/api/sync-card-catalog rewrites daily -
// including to NULL when the provider stops pricing a printing. Their caches
// carried no tag, so nothing could invalidate them and each expired on its own
// 6h TTL, independently of the other. That let the page pair a FRESH snapshot
// date with STALE ranking rows, so the dateline certified prices the sync had
// already withdrawn. One tag, expired by the sync that writes the prices.
const CATALOG_PRICES_TAG = "catalog-prices";
...
module.exports = {
  ...
  copyAuthenticityHold,
  authenticityVerdictHold,
  VERDICT_HOLD_REASON,
  authenticityHeld,
  COPY_HOLD_REASON,
  COPY_CHECK_FAILED_REASON,
  OWNER_REPORTED_COUNTERFEIT_REASON,
  COUNTERFEIT_VISUAL_VERDICTS,
  insertNewSighting,
  finalisePilotInsert,
  abandonedPilotPendingRows,
  PILOT_PENDING_REASON,
  PILOT_COPY_CONFLICT_REASON,
