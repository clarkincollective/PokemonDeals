...
export async function runOutreach({ env = process.env, now = Date.now(), dryRun = false, oidcToken = null, alert = async () => null, store = outreachStore(), client = instantlyClient({ apiKey: env.INSTANTLY_API_KEY }), provider = getProvider(), fetchImpl = fetch } = {}) {
  ...
  if (!caps.leads_read && !control.paused && env.INSTANTLY_API_KEY && !dryRun && /^(401|402|403)/.test(String(caps.detail.leads)) && !/Invalid scope/i.test(String(caps.detail.leads))) {
    // owner rule 2026-09-15: if Instantly sending becomes unavailable (e.g. the free trial
    // ended) pause outreach AND prospect generation; records and the queue are preserved
    await store.putJson("state/control.json", { paused: true, pausedReason: `Instantly access unavailable (${caps.detail.leads}) - no paid plan authorised`, pausedAt: new Date(now).toISOString() });
    await alert({ subject: "outreach paused: Instantly sending unavailable", lines: [`Instantly lead access now returns ${caps.detail.leads}.`, "Outreach and prospect discovery are paused; records and the queue are preserved.", "No upgrade was made. Resume only if the account can send again: npm run outreach -- resume"] });
    report.blockers.push("Instantly access unavailable - outreach auto-paused");
  }
  if (report.blockers.length && !dryRun) { report.outcome = "BLOCKED"; return report; }
  // owner rule 2026-09-15: new contact + discovery stop at control.newContactStopsAt (before
  // the free trial ends); Unibox reminders and send detection keep running
  const newContactStopped = Boolean(control.newContactStopsAt && now >= Date.parse(control.newContactStopsAt));
  ...
  if (!newContactStopped && (!disc.lastRunAt || now - Date.parse(disc.lastRunAt) >= 24 * 3_600_000)) {
  ...
  if (newContactStopped) report.skipped.push({ reason: `new contact stopped since ${control.newContactStopsAt}: ${control.newContactStopReason ?? ""}` });
  else if (!inSendWindow(now)) report.skipped.push({ reason: "outside the weekday send window" });
