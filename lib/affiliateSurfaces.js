// SUPERSEDED 2026-09-25 BY lib/affiliateAttribution.js — AUDIT FINDING 5.
//
// This module is the HISTORICAL record of the single-token `customid`
// vocabulary EPN recorded before that date. It is kept, unchanged, so the
// meaning of an archived EPN row stays readable, and so the migration
// table in lib/affiliateAttribution.js has something to point at.
//
// It is no longer wired to any outbound link. Nothing under app/ or
// components/ may import it — enforced by
// tests/scanner/affiliate-attribution-2026-09-25.test.mjs, because a
// second live mapping is exactly how the two would drift apart.
//
// WHY IT WAS REPLACED. The enum below has no token for the sealed
// catalogue, a guide, /japanese-cards or /latest-releases, so 83.2% of
// sampled outbound hrefs resolved to "other"; and it fuses "which page"
// with "which control on it" into one string, leaving no way to tell the
// sealed browse grid from the sealed selected-product panel.
//
