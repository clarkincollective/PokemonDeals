...
// The /sets directory's "Collection checklists" view advertises a set only
// when its page renders the interactive ownership checklist. This is the
// same three-part rule app/sets/[slug]/page.js applies to the same cached
// fetchSetCatalog payload (allowlist, identity guard, not browse-capped);
// tests pin both call sites to it.
export function checklistEligible({ setName, cards, truncated }) {
  return !truncated && isChecklistSet(setName) && checklistIdentityCheck(cards).ok;
}
...
export function buildChecklistRows(cards) {
  ...
  return [...list].sort(compareCollectorNumber).map((c) => {
    ...
    // 17C.11 visual: the card's own catalogue artwork, keyed on the stored
    // product id - the stored image_url first, else the id-derived CDN URL
    // from the existing helper. Never matched by name. Artwork identifies
    // the card; it says nothing about a copy's condition or every foil.
    const stored = c.image ?? c.imageUrl ?? c.image_url ?? null;
    const image = stored ? upgradeCatalogImage(stored) : c.tcgplayerId ? catalogImageUrl(c.tcgplayerId) : null;
    return {
