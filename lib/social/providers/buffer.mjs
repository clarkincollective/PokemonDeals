...
async function bufferGraphQL(apiKey, query, variables) {
  // SOCIAL-NEWSROOM-14C: a network failure, DNS error, or the 30s abort
  // timeout firing all reject fetch() - previously uncaught here, which
  // propagated all the way up through createPost()/scheduleOne() as an
  // unhandled exception. The genuinely dangerous case is a client-side
  // timeout that fires AFTER Buffer already processed the request
  // server-side - the caller cannot tell "definitely never happened" from
  // "definitely happened, we just didn't hear back" from a thrown error
  // alone. Converting this to the SAME {ok:false, reason, detail} shape
  // every other failure already uses lets callers treat it as the
  // ambiguous/unknown outcome it is (never a safe-to-blind-retry reject).
  let res;
  try {
    res = await fetch(BUFFER_GRAPHQL, {
      method: "POST",
    ...
    return { ok: false, reason: isTimeout ? "buffer_network_timeout" : "buffer_network_exception", detail: String(e?.message ?? e).slice(0, 300) };
  }
  ...
  return {
    ...
    async createPost(msg) {
      ...
      const assets = (msg.assets ?? []).map((a) =>
        // SOCIAL-LIVE-2: Buffer rejects video thumbnailUrl ("social networks do
        // not accept custom video thumbnail images") - never send it
        a.type === "video"
          ? { video: { url: a.url } }
      ...
      if (msg.platform === "instagram") {
        ...
        // SOCIAL-LIVE-2: Buffer requires a category for YouTube posts
        // (YouTube category id; 24 = Entertainment)
        metadata.youtube = { ...(msg.youtubeTitle ? { title: msg.youtubeTitle } : {}), categoryId: String(msg.youtubeCategoryId ?? "24"), privacy: "public", madeForKids: false, notifySubscribers: false };
      }
      const input = {
    ...
    // ---- SOCIAL-NEWSROOM-2D: delete ONE unpublished post/draft --------
    // Operator-only recovery for an invalid backlog draft (a placement
    // whose exact artifact later failed the Layer-5 QA invariant). It
    // REFUSES to touch a post that has already sent. Returns
    // { ok, deleted, statusRaw, reason }.
    async deletePost(id) {
      if (!apiKey) return { ok: false, reason: "buffer_not_configured" };
      // guard: never delete a sent post
      const st = await this.getPostStatus(id);
      if (st.ok && st.published) return { ok: false, deleted: false, reason: "refusing_to_delete_sent_post", statusRaw: st.statusRaw };
      // DeletePostPayload (verified live 2026-09-08 by introspection) is a
      // union: DeletePostSuccess { id } | VoidMutationError { message }.
      ...
      const r = await bufferGraphQL(apiKey, mutation, { input: { id } });
      if (!r.ok) return { ok: false, deleted: false, reason: r.reason, detail: r.detail };
      const d = r.data?.deletePost;
      if (d?.__typename === "DeletePostSuccess") return { ok: true, deleted: true, id: d.id, statusRaw: "deleted" };
      return { ok: false, deleted: false, reason: "buffer_delete_error", detail: String(d?.message ?? JSON.stringify(r.data)).slice(0, 300) };
    },
