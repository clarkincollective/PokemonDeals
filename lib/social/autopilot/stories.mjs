...
export const GUIDE_STORIES = Object.freeze([
  ...
  {
    // Evergreen buying-safety story for the counterfeit guide. Deliberately
    // makes no detection claim: the note (which is also the X caption's only
    // body line for a tip layout) says plainly that a photo and a low price
    // cannot prove a card genuine. The card is catalogue artwork of a
    // printing, labelled as such in its detail line so the creative never
    // reads as a photo of a seller's card.
    key: "guide:ask-for-photos",
    eyebrow: "Buying safely",
    headline: "Before you buy a card online, ask for these photos.",
    sub: "A listing photo tells you what to ask next. It cannot tell you a card is genuine.",
    layout: "tip",
    cards: [{ ...g(GUIDE_CARDS.arcanineShadowless), detail: "Catalogue artwork · Base Set (Shadowless) 023/102" }],
    points: [
      { title: "Front and back", text: "Of the actual card you would receive, in good light - not a stock image." },
      { title: "The exact printing", text: "Set, collector number and variant should match the card you think you are buying." },
      { title: "Still unsure", text: "Ask someone experienced to look, or buy a copy already graded." },
    ],
    note: "A photograph and a low price cannot establish authenticity on their own.",
    // X-only body: the shared headline + note + link overruns the 275 guard
    // (the attributed URL alone is 161 chars), which would drop the caveat and
    // leave a bare headline. This keeps hook + one actionable point + the
    // caveat, and drops the hashtags for room. The link is appended by the
    // caption builder, so tracking parameters are untouched.
    xText: [
      "Buying a card online? Ask for the actual card, front and back.",
      "A photo alone cannot prove it is genuine.",
    ].join("\n"),
    href: "/guides/spotting-fake-pokemon-cards-in-listings",
    hashtags: ["#pokemontcg", "#pokemoncards", "#cardcollecting"],
  },
  {
