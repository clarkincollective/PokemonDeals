...
function renderMoverSlide(s, { spec, variant, background, cardArtwork }) {
  ...
  // Fail closed: no premium creative without BOTH the real card and the
  // real chart.
  if (!chart || !hasCard) {
  ...
  if (variant === "B") {
    // card stacked above the chart - still card + chart, never chart-only
    return shell(
      `${brand}${headline}<div style="height:${S.md}px"></div>${metric}
