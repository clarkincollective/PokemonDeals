...
import { TOKENS, resolveCreativeSpec, resolveAccent, familyForContentType, selectCarouselHook } from "./creativeSpec.mjs";
...
<html><head><meta charset="utf-8"><style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  /* CTA: website-first. A strong text intent + an arrow, then the real
     on-site destination - never a filled tappable-looking button, and the
     visitor lands on PokemonDealFinder.com (the site owns the eBay hop). */
  .cta{margin-top:auto;display:flex;flex-direction:column;gap:6px;padding-top:${S.xl}px}
  .cta .cta-line{display:flex;align-items:center;gap:${S.sm}px;font-size:${T.cta.size}px;font-weight:${T.cta.weight};color:${C.ink}}
  .cta .cta-line .u{border-bottom:3px solid ${C.brand};padding-bottom:6px;text-transform:uppercase;letter-spacing:0.01em}
  .cta .cta-line svg{flex:none}
  .cta .url{font-size:${T.fine.size}px;font-weight:600;letter-spacing:0.02em;color:${C.brand}}

  /* the hook line - the scroll-stopper. Uppercase, tight, the biggest
     text on a Deal Drop after the card itself (13E.11A: up from 56px, and
     bounded to <= 3 lines so a long hook can never wall the composition). */
  .hook-line{font-size:64px;font-weight:800;letter-spacing:-0.03em;line-height:1.04;text-transform:uppercase;color:${C.ink};text-wrap:balance;max-height:3.4em;overflow:hidden}
  .hook-line .em{color:${C.brand}}

  /* Market Mover editorial: name huge, % huge, period a quiet label; the
     chart is a clean line over the ground, NOT a boxed dashboard widget. */
  .mover-name{font-size:${T.title.size}px;font-weight:800;letter-spacing:-0.03em;line-height:1.02;color:${C.ink}}
  .mover-name .set{display:block;font-size:${T.fine.size}px;font-weight:600;letter-spacing:0.04em;text-transform:uppercase;color:${C.inkFaint};margin-top:6px}
  /* 13E.11A: restrained - a price move is not a "saving", so no big
     green/red trading-app figure. Neutral ink, stepped down from 150px. */
  .mover-move{font-size:112px;font-weight:800;letter-spacing:-0.04em;line-height:1;font-variant-numeric:tabular-nums;color:${C.ink}}
  .mover-period{font-size:${T.label.size}px;font-weight:${T.label.weight};letter-spacing:${T.label.tracking};text-transform:uppercase;color:${C.inkSub};margin-top:2px}
  .chart.editorial{background:none;border:0;padding:0;flex:1;min-height:0}
  .chart.editorial .cap{display:none}

  /* Hook Carousel cover: real fanned card artwork as supporting imagery.
     overflow:hidden clips the fan cleanly at the container edge so a tall
     card can never ride up over the hook. */
  .cover-fan{position:relative;flex:1;min-height:0;margin:${S.xl}px 0 0;overflow:hidden}
  .cover-fan .card-frame{position:absolute;left:50%;bottom:0;border-radius:14px}
  .cover-fan .card-frame .card-art{height:470px;width:auto;max-width:none;max-height:none}

  /* Brand Ad: 3 supported benefits as quiet chips + a LARGE screenshot */
  .benefits{display:flex;gap:${S.sm}px;flex-wrap:wrap;margin:${S.md}px 0 0}
  .benefit{font-size:${T.fine.size}px;font-weight:700;letter-spacing:0.04em;text-transform:uppercase;color:${C.inkSub};
    border:1px solid ${C.hair};border-radius:${TOKENS.radius.pill}px;padding:10px 18px}
  .device-frame.big{margin-top:${S.lg}px;flex:1;min-height:0;display:flex;flex-direction:column}
  .device-frame.big .bar{flex:none}
  .device-frame.big img{flex:1;min-height:0;height:auto;object-fit:cover;object-position:top}
  ...
  /* 13E.11A: the standalone % figure steps DOWN below the hook so the
     (experiment-varied) hook stays the hero and the % reads as a
     consistent secondary cue - identical in every E1 variant. */
  .hero-split .rail .metric .fig{font-size:84px}
  .hero-split .rail .metric.compact .fig{font-size:56px}
  ...
  /* 13E.11A: the card is the hero of a Deal Drop - larger occupancy. */
  .hero-split .prod .card-art{max-height:700px;max-width:100%}
  .stack .card-track .card-art{max-height:640px;max-width:80%}
  .mover-stack-card{display:flex;align-items:center;justify-content:center;margin:${S.md}px 0}
  .mover-stack-card .card-art{max-height:480px;max-width:56%}
...
// `cta` is a resolved { label, url } from creativeSpec.resolveCta, or a
// bare string (older callers / raw test slides). Website-first: the
// destination line is always a real on-site PokemonDealFinder.com route.
function zCta(cta, { plain = false } = {}) {
  const label = typeof cta === "string" ? cta : cta?.label ?? "See today's deals";
  const url = typeof cta === "string" ? null : cta?.url ?? null;
  const line = plain
    ? `<div class="cta-line"><span class="u">${esc(label)}</span></div>`
    : `<div class="cta-line"><span class="u">${esc(label)}</span>${arrow(C.brand, 30)}</div>`;
  return `<div class="cta">${line}${url ? `<div class="url">${esc(url)}</div>` : ""}</div>`;
}
...
function zChart({ series, accent, deltaLabel, editorial = false }) {
  ...
  const col = accent?.color ?? C.neutral;
  const cap = editorial ? "" : `<div class="cap"><span style="color:${col}">${esc(deltaLabel)}</span></div>`;
  return `<div class="chart${editorial ? " editorial" : ""}">
    ${cap}
    <svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
      <path d="${area}" fill="${col}" fill-opacity="${editorial ? 0.1 : 0.12}"/>
      <path d="${d}" fill="none" stroke="${col}" stroke-width="${editorial ? 5 : 4}" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke"/>
      <circle cx="${x(pts.length - 1).toFixed(1)}" cy="${y(vs[vs.length - 1]).toFixed(1)}" r="${editorial ? 7 : 6}" fill="${col}"/>
    </svg>
  </div>`;
}
...
function dealSlideData(payload, { carousel = null } = {}) {
  ...
  return {
    ...
    carousel,
    accent,
    // 13E.3D - the scroll-stopping hook, chosen from the strongest
    // truthful angle by creativeSpec.selectDealHook (via the payload).
    hook: payload.hook?.text ?? null,
    metricValue: `${Math.round(d.discount_pct * 100)}%`,
    metricLabel: "BELOW RECENT MARKET",
    name: d.card_name,
    set: d.card_set,
    ...
    cta: payload.cta ?? "See today's deals",
  };
}
...
function moverSlideData(payload) {
  ...
  return {
    ...
    accent,
    metricValue: pct,
    // 13E.3D - period is its own quiet label under the % figure.
    periodLabel: m?.windowLabel ? `${String(m.windowLabel).toUpperCase()} TREND` : "PRICE MOVEMENT",
    ...
    freshnessLabel: "Canonical price history. Card prices can move — not investment advice.",
    cta: payload.cta ?? "Full price history",
  };
}

// 13E.3C - `opts.distinctCount` is the TRUTHFUL number of distinct card
// identities the carousel will show (from creativeSpec.buildCarouselSequence).
// The hook states that number; the slide count is distinctCount + 2 (cover
// + N + close). Falls back to deal_data.length only when no count is given
// (older callers / the market_snapshot shape).
function coverSlideData(payload, opts = {}) {
  const fallbackN = Array.isArray(payload.deal_data) ? payload.deal_data.length : 0;
  const n = Number.isInteger(opts.distinctCount) ? opts.distinctCount : fallbackN;
  const total = Number.isInteger(opts.totalSlides) ? opts.totalSlides : n + 2;
  // 13E.3D - a truthful social-native hook generated from the real
  // distinct count; deterministic variant per (count, day).
  const hook = selectCarouselHook({ count: n, rotationKey: String(payload.generated_at ?? "").slice(0, 10) });
  return {
    kind: "cover",
    family: "hook_carousel",
    carousel: { position: 1, total },
    hookText: hook.text,
    hookVariant: hook.variant,
    count: n,
    // 2-3 real canonical card fileUrls from THIS carousel's own sequence.
    coverCards: Array.isArray(opts.coverCards) ? opts.coverCards.filter(Boolean).slice(0, 3) : [],
    supporting: "Every one checked against a real market reference. Swipe →",
    freshnessLabel: payload.freshness.label,
    cta: payload.cta ?? "See all live finds",
  };
}

function closeSlideData(payload, opts = {}) {
  const fallbackN = Array.isArray(payload.deal_data) ? payload.deal_data.length : 0;
  const n = Number.isInteger(opts.distinctCount) ? opts.distinctCount : fallbackN;
  const total = Number.isInteger(opts.totalSlides) ? opts.totalSlides : n + 2;
  // 13E.3D - a real reason to click: if the qualifying pool is larger
  // than the carousel showed, say so truthfully. `opts.moreCount` is a
  // real integer count of additional qualifying deals (0/undefined -> no
  // claim).
  const more = Number.isInteger(opts.moreCount) && opts.moreCount > 0 ? opts.moreCount : 0;
  return {
    kind: "close",
    family: "hook_carousel",
    carousel: { position: total, total },
    headline: "PokemonDealFinder",
    supporting: more
      ? `Free. We scan live eBay listings against a real market reference. ${more} more under-market find${more === 1 ? "" : "s"} on the site right now.`
      : "Free. We scan live eBay listings against a real market reference — so you see the ones priced below it.",
    freshnessLabel: payload.freshness.label,
    cta: payload.cta ?? "See today's deals",
  };
}
...
export function buildCoverSlideContent(payload, opts = {}) {
  return coverSlideData(payload, opts);
}
export function buildCloseSlideContent(payload, opts = {}) {
  return closeSlideData(payload, opts);
}
...
function renderDealSlide(s, { spec, variant, background, cardArtwork }) {
  ...
  // 13E.3D - the hook leads. Then card, saving, listing price, CTA.
  const hook = s.hook ? `<div class="hook-line">${esc(s.hook)}</div>` : "";
  // 13E.11A: when a hook is present it IS the framing line (the hero), so
  // the standalone % steps DOWN to its compact form (56px < the 64px hook)
  // and reads as a secondary data cue, not a competing headline (§2/§5).
  // It is rendered IDENTICALLY in every experiment (E1) variant - same
  // figure, same size, same accent - so PRICE_CONTRAST vs PERCENT_GAP
  // differ only in the hook text (§26).
  const metric = zMetric({ figure: s.metricValue, phrase: s.metricLabel, accent: acc, compact: variant === "B" || Boolean(s.hook) });
  ...
  // 13E.11A: ONE quiet factual line under the price - it still carries the
  // source (eBay + marketplace) and the market-reference basis, combined so
  // the footer never stacks 2-3 competing grey lines under the card (§11).
  const mk = s.marketplace ? `<span class="mk">${esc(s.marketplace)}</span>` : "";
  const context = zContext([
    s.marketplace
      ? `Live on eBay ${mk} · <span class="k">Market ref</span> recent sold prices`
      : `<span class="k">Market ref</span> — recent sold prices for this exact printing`,
  ]);
  const cta = zCta(s.cta || "See today's deals");
  ...
  if (cardArtwork && cardArtwork.card) {
    ...
    if (variant === "B") {
      return shell(
        `${brand}${hook}<div class="stack">${headline}<div class="card-track">${frame}</div>
         <div class="stack-facts">${metric}${price}</div></div>${context}${cta}${disc}`,
        { spec, background }
      );
    }
    const side = spec?.composition?.productSide === "left" ? "" : " right";
    const pw = spec?.composition?.productWidthPct ?? 54; // 13E.11A: card-forward split
    return shell(
      `${brand}${hook}<div class="hero-split${side}"><div class="prod" style="width:${pw}%">${frame}</div>
  ...
  const body =
    variant === "B"
      ? `${brand}${hook}<div class="stack-mid">${headline}${metric}</div>${price}${context}${cta}${disc}`
      : `${brand}${hook}<div class="stack-mid">${metric}${headline}</div>${price}${context}${cta}${disc}`;
  return shell(body, { spec, background });
}

// Market Mover: real card on a narrow left rail, identity + the movement
// figure, and the real price-history chart. 13E.3C: a normal Market Mover
// creative ALWAYS includes the real canonical card artwork - the card is
// part of the visual identity. There is no deliberate chart-only variant.
//   * card + chart present -> variant A = card on a left rail + chart;
//     variant B = card stacked above the chart. Both include the card.
//   * card OR chart missing -> FAIL CLOSED to a minimal identity + figure
//     slide (no premium chart-only creative). The daily/render caller
//     only ever produces a Market Mover post when BOTH resolve, so this
//     branch is a defensive floor, not a shipped format.
function renderMoverSlide(s, { spec, variant, background, cardArtwork }) {
  const acc = accentOf(s);
  // 13E.11A: a Market Mover uses NO directional colour on the move figure -
  // a price rise is not a "saving" (§9) and a big green/red number is the
  // financial-trading aesthetic (§15). Neutral ink; the chart line is a
  // single restrained neutral stroke, not a green/red trend.
  const neutralAcc = { color: C.inkSub, allowed: false };
  // 13E.3D - editorial hierarchy: NAME, then the MOVE %, then a quiet
  // PERIOD label; the chart is a clean line, not a boxed dashboard widget.
  const name = `<div class="mover-name">${esc(safeText(s.name, 30))}${s.set ? `<span class="set">${esc(safeText(s.set, 34))}</span>` : ""}</div>`;
  const move = `<div class="mover-move">${esc(s.metricValue)}</div><div class="mover-period">${esc(s.periodLabel)}</div>`;
  const chart = zChart({ series: s.series, accent: neutralAcc, editorial: true });
  const context = zContext([`<span class="k">Source</span> canonical price history · recent sold references`]);
  const cta = zCta(s.cta || "Full price history");
  const disc = zDisclosure({ freshnessLabel: s.freshnessLabel });
  const brand = zBrand();
  const geo = spec?.card ?? { rotationDeg: -3 };
  const hasCard = Boolean(cardArtwork && cardArtwork.card);

  // Fail closed: no premium creative without BOTH the real card and the real chart.
  if (!chart || !hasCard) {
    return shell(`${brand}<div class="stack-mid">${name}${move}</div>${context}${cta}${disc}`, { spec, background });
  }
  ...
  if (variant === "B") {
    return shell(
      `${brand}${name}<div style="height:${S.md}px"></div>${move}
       <div class="mover-stack-card">${frame}</div>${chart}${context}${cta}${disc}`,
      { spec, background }
    );
  }
  const pw = spec?.composition?.productWidthPct ?? 42;
  return shell(
    `${brand}<div class="hero-split"><div class="prod" style="width:${pw}%">${frame}</div>
     <div class="rail" style="justify-content:flex-start;padding-top:${S.md}px;gap:${S.sm}px">${name}<div style="height:${S.xs}px"></div>${move}<div style="height:${S.md}px"></div>${chart}</div></div>
     ${context}${cta}${disc}`,
    { spec, background }
  );
}
...
function renderCoverSlide(s, { spec, background }) {
  const brand = zBrand({ dots: s.carousel ? { total: s.carousel.total, position: s.carousel.position } : null });
  // 13E.3D - real canonical card artwork from THIS carousel's own
  // sequence, layered/fanned as supporting imagery under the hook.
  const cards = (s.coverCards || []).filter(Boolean).slice(0, 3);
  const rot = [-11, -1, 9];
  const dx = [-320, -50, 220]; // px, around the centred anchor
  const z = [0, 2, 1];
  const fan = cards.length
    ? `<div class="cover-fan">${cards
        .map(
          (url, i) =>
            `<div class="card-frame" style="transform:translateX(calc(-50% + ${dx[i] ?? -50}px)) rotate(${rot[i] ?? 0}deg);z-index:${z[i] ?? i};filter:drop-shadow(0 34px 70px rgba(0,0,0,0.62))"><img class="card-art" src="${url}" alt=""></div>`
        )
        .join("")}</div>`
    : `<div class="fill"></div>`;
  const body = `${brand}<div style="height:${S.md}px"></div>
    ${zHook(esc(s.hookText), 88)}
    <div style="height:${S.xs}px"></div>
    <div class="subhead">${esc(s.supporting)}</div>
    ${fan}
    ${zCta(s.cta, { plain: true })}
...
function renderCloseSlide(s, { spec, background }) {
  ...
  const body = `${brand}<div class="fill"></div>
    ...
    <div class="subhead" style="max-width:28ch;font-size:${T.body.size}px">${esc(s.supporting)}</div>
    <div class="fill"></div>
    ${zCta(s.cta)}
    ${zDisclosure({ freshnessLabel: s.freshnessLabel })}`;
  return shell(body, { spec, background });
...
// 13E.3D - hierarchy fix: the real screenshot is LARGE (fills the middle
// ~55% of the composition), a max of 3 supported benefits, and a clear
// website-first CTA. No affiliation implied.
function renderBrandAdSlide({ spec, background = null, screenshot, headline, sub, cta, benefits, freshnessLabel, urlLabel = "pokemondealfinder.com" }) {
  const brand = zBrand();
  // 13E.11A: lead with the product statement, not a generic "stop
  // overpaying" line - this is collectible discovery, not a SaaS pitch.
  const hookHtml = headline
    ? `<div class="hook" style="font-size:80px">${esc(headline)}</div>`
    : `<div class="hook" style="font-size:80px"><span class="em">Pokemon cards</span> listed below recent market prices</div>`;
  const B = (Array.isArray(benefits) && benefits.length ? benefits : ["Live eBay listings", "Real price history", "Below-market finds"]).slice(0, 3);
  const benefitRow = `<div class="benefits">${B.map((b) => `<span class="benefit">${esc(b)}</span>`).join("")}</div>`;
  const ctaResolved = cta || { label: "SEE TODAY'S DEALS", url: "PokemonDealFinder.com/deals" };
  const body = `${brand}
    <div style="height:${S.md}px"></div>
    ${hookHtml}
    <div style="height:${S.xs}px"></div>
    <div class="subhead" style="max-width:34ch;font-size:${T.body.size}px">${esc(sub || spec?.subhead || "PokemonDealFinder scans live eBay listings and compares each one to a real market reference — so you see the ones priced below it.")}</div>
    <div class="device-frame big">
