...
<html><head><meta charset="utf-8"><style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  /* the hook line - the scroll-stopper. Uppercase, tight, the biggest
     text on a Deal Drop after the card itself. */
  .hook-line{font-size:56px;font-weight:800;letter-spacing:-0.03em;line-height:1.04;text-transform:uppercase;color:${C.ink};text-wrap:balance}
  ...
  .mover-move{font-size:150px;font-weight:800;letter-spacing:-0.04em;line-height:1;font-variant-numeric:tabular-nums}
  ...
  .hero-split .prod .card-art{max-height:640px;max-width:100%}
  .stack .card-track .card-art{max-height:600px;max-width:76%}
  .mover-stack-card{display:flex;align-items:center;justify-content:center;margin:${S.md}px 0}
  .mover-stack-card .card-art{max-height:420px;max-width:52%}
...
function renderDealSlide(s, { spec, variant, background, cardArtwork }) {
  ...
  // two quiet lines max: the market-reference basis, then grade + where it's buyable
  ...
  const context = zContext([
    ...
    s.graded && s.marketplace ? `${esc(s.graded)} · eBay ${mk}` : s.marketplace ? `Live on eBay ${mk}` : s.graded && s.graded !== "Raw" ? esc(s.graded) : null,
  ]);
...
function renderMoverSlide(s, { spec, variant, background, cardArtwork }) {
  ...
  const move = `<div class="mover-move" style="color:${col}">${esc(s.metricValue)}</div><div class="mover-period">${esc(s.periodLabel)}</div>`;
  const chart = zChart({ series: s.series, accent: acc, editorial: true });
...
function renderBrandAdSlide({ spec, background = null, screenshot, headline, sub, cta, benefits, freshnessLabel, urlLabel = "pokemondealfinder.com" }) {
  ...
  const hookHtml = headline
    ...
    : `<div class="hook" style="font-size:80px"><span class="em">Stop overpaying</span> for Pokemon cards</div>`;
