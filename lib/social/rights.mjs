...
export const RIGHTS_STATE = Object.freeze({
  ...
  // Card artwork for social use: CLEARED by the owner (Phase 13E.2.1,
  // 2026-09-06). This permits compositing the REAL canonical catalogue
  // card image (TCGplayer product CDN, the same artwork the website
  // already shows for the exact matched printing) into a branded social
  // creative - "Version C". It does NOT permit AI-generated card
  // replicas, invented Pokemon, or a species-level generic image when an
  // exact-printing image exists (fail closed). It is a DISTINCT question
  // from ebay_seller_images below, which is NOT covered by this
  // clearance. Versions A/B (background only, no card image) are
  // unaffected. See docs/social-card-artwork.md.
  card_image: "CLEARED",
...
export const RIGHTS_STATE_REASON = Object.freeze({
  ...
  card_image: "Canonical catalogue card artwork cleared by owner (Phase 13E.2.1, 2026-09-06) - real exact-printing image only, fail closed on wrong/missing print; NOT eBay seller photos",
  ebay_seller_images: "eBay listing photos still NOT cleared for branded compositing - unresolved EPN 'visually isolated' clause; separate from the canonical card_image clearance",
