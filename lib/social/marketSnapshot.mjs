...
import { resolveCta, contentGoalFor, buildCreativeIdentifiers } from "./creativeSpec.mjs";
...
export function buildMarketSnapshotPayload({ candidate, now = Date.now() }) {
  ...
  const cta = resolveCta({ family: "market_snapshot", contentType: "market_snapshot", route: payload.destination.route });
  payload.cta = cta;
  payload.content_goal = contentGoalFor("market_snapshot"); // TRUST
  payload.creative = buildCreativeIdentifiers({
    family: "market_snapshot", contentType: "market_snapshot", subject: payload.market_snapshot.biggest_gap_card ?? "market",
    generatedAt: payload.generated_at, variant: "A", hookVariant: "biggest_gap", ctaVariant: cta.variant,
  });
  return payload;
}
