...
//
// Nothing in this module does I/O, imports live data, or touches OpenAI.
// It is evergreen STRUCTURE. The deterministic renderer overlays 100% of
// the real facts; the OpenAI background (Layer 1) only ever receives the
// data-free {family,style,zone} enums defined in assetPrompts.mjs; the
// real canonical card artwork (Layer 2) is the exact matched printing
// resolved by cardArtwork.mjs. See docs/social-creative-system.md.
...
export function buildCarouselSequence(deals, { minSlides, maxSlides } = FAMILY_SPECS.hook_carousel.sequence) {
  const cards = Array.isArray(deals) ? deals.filter(Boolean) : [];
  const maxCardSlides = Math.max(1, (maxSlides ?? 6) - 2); // reserve cover + close
  const chosen = cards.slice(0, maxCardSlides);
  const slides = [
    { kind: "cover", index: 0 },
    ...chosen.map((deal, i) => ({ kind: "card", index: i + 1, deal })),
    { kind: "close", index: chosen.length + 1 },
  ];
  const ok = slides.length >= (minSlides ?? 3);
  return {
    ok,
    slides,
    count: slides.length,
    reason: ok ? null : `only ${chosen.length} card slide(s) - a carousel needs at least ${(minSlides ?? 3) - 2}`,
  };
}
