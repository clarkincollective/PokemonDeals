...
// The public freshness line. A payload built from a fixture whose exact
// verification has aged past the social preview ceiling carries an
// internal diagnostic string ("...not eligible for preview.") - never
// surface that as creative copy. Fall back to a truthful "Checked <the
// listing's real verification time> UTC" line, matching the exact wording
// the static system uses.
function publicFreshnessLabel(payload, dealArr) {
  const raw = payload?.freshness?.label ?? null;
  const looksDiagnostic = raw && /not eligible|outside .*threshold/i.test(raw);
  if (raw && !looksDiagnostic) return raw;
