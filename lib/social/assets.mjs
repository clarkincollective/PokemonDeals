...
// Phase 13E.3 - ASSET STORAGE POLICY (docs/social-asset-library.md SS22).
//   assets/social/generated/  = the working area. Raw OpenAI output lands
//        here as `<category>/<id>.png`. The PNGs are GITIGNORED
//        (assets/social/generated/**/*.png) - they are pending-review
//        scratch, like .social-preview/, and a `git add -A` can never
//        sweep them in. Only social-assets.json (the manifest) is tracked.
//   assets/social/approved/   = the shipped area. `npm run social:assets
//        approve <id>` COPIES the reviewed PNG here as `<category>/<id>.png`
//        and repoints the manifest `file` at it. These ARE committed - they
//        are what `social:daily` and any deployed runtime read, so the
//        repo only ever carries the small set of human-approved assets.
// The manifest `file` field always points at whichever of the two a given
// asset currently lives in; approvedAssetsForCategory() below only trusts
// an approved entry whose `file` exists on disk (fail closed).
export const GENERATED_ASSET_DIR = path.join("assets", "social", "generated");
export const APPROVED_ASSET_DIR = path.join("assets", "social", "approved");
...
export const ASSET_CATEGORY_FOR_CONTENT_TYPE = Object.freeze({
  ...
  market_mover: "market_watch", // 13E.3 - Market Mover uses the market-watch background family
