...
export function platformCaptions(story, { cta = story.cta_intensity ?? "BRAND_ONLY" } = {}) {
  ...
  for (const platform of ["instagram", "x", "youtube"]) {
    ...
    if (platform === "instagram") {
      ...
      // SOCIAL-LIVE-2: lead with the hook (the scroll-stopping line) when the
      // body doesn't already say it
      const lead = hook && !body.includes(hook) ? hook : null;
      text = [[lead, body].filter(Boolean).join("\n"), link ? link : brandLine ? brandLine : null, tags.slice(0, 2).join(" ")].filter(Boolean).join("\n\n");
