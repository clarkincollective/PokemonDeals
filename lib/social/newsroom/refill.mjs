...
import { manualPlatforms } from "../autopilot/slots.mjs";
...
export function planRefill({ candidateSeries = [], placements = [], now = Date.now(), initial = false, platforms = EDITORIAL_REFILL_PLATFORMS, env = process.env } = {}) {
  const health = backlogHealth(placements, { now });
  const needs = refillNeeds(health); // [{ platform, state, days_covered, need_slots }]
  // A feed the owner writes by hand is never below target from this job's
  // point of view: it asks for no refill slots and raises no coverage need.
  // Reported as its own state so it is not confused with the SS5 scope rule.
  const manual = manualPlatforms(env);

  const safeSeries = candidateSeries.filter((s) => refillConsiderable(s) && getSeries(s));
  const perPlatform = [];
  for (const n of needs) {
    if (manual.has(n.platform)) {
      perPlatform.push({ platform: n.platform, state: "MANUALLY_PRODUCED", refill: 0 });
      continue;
    }
