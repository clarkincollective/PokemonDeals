...
const MEDIA_KIND = Object.freeze({
  instagram: { LIVE: "reel", SHORT: "carousel", EDITORIAL: "carousel", EVERGREEN: "carousel" },
...
export function xAllowed(story) {
  ...
  return ["MARKET", "COMPARISON", "EDUCATION", "STORY", "BEHIND_THE_FINDER"].includes(def.pillar);
}
...
export function placementsForStory(story, { platforms = null } = {}) {
  ...
  for (const platform of wanted) {
    // platform must carry a compatible family (reuse existing roles)
    const roleFamilies = PLATFORM_ROLES[platform]?.families ?? [];
    // map pillar -> nearest existing family for the role check
    const famGuess =
