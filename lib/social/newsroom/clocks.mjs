...
export function shelfWindow(shelfClass, { capturedAt, anchorAt = null, now = Date.now() } = {}) {
  const cls = String(shelfClass || "").toUpperCase();
  const capMs = Date.parse(capturedAt ?? "");
  // EVERGREEN uses a FIXED identity epoch as captured_at (persist.mjs) so
  // rebuilds collapse to one row - but its publish window must be anchored
  // to render time, not 2020, or it is always "expired". LIVE/SHORT/
  // EDITORIAL captured_at values are real recent dates and stay as-is.
  const from = cls === "EVERGREEN" ? now : Number.isFinite(capMs) ? capMs : now;
