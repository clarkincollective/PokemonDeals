...
// same pattern as the refill lock: one append-only `social_qa_runs`
// snapshot row per change (qa_type BACKLOG_CIRCUIT), newest wins. This is
// the only sanctioned write path for it (lib/social/newsroom/backlogCircuit
// holds the rules, never the I/O). Errors are THROWN so the caller can fail
// closed - a circuit that cannot be read must never look CLOSED.
export const BACKLOG_CIRCUIT_QA_TYPE = "BACKLOG_CIRCUIT";

export async function loadBacklogCircuitRow(db = supabaseAdmin()) {
  const { data, error } = await db
    .from("social_qa_runs")
    .select("checked_at, detail")
    .eq("qa_type", BACKLOG_CIRCUIT_QA_TYPE)
    .order("checked_at", { ascending: false })
    .limit(1);
  if (error) throw new Error(`backlog circuit read failed: ${error.message}`);
  const row = data?.[0];
  if (!row) return null; // never written = CLOSED
  const c = row.detail?.circuit;
  if (!c || typeof c !== "object" || typeof c.state !== "string") throw new Error("backlog circuit row has an unexpected shape");
  return c;
}

export async function saveBacklogCircuitRow(circuit, db = supabaseAdmin()) {
  const { error } = await db.from("social_qa_runs").insert({
    qa_type: BACKLOG_CIRCUIT_QA_TYPE,
    result: circuit.state === "CLOSED" ? "PASS" : "FAIL",
    detail: { circuit, at: new Date().toISOString() },
  });
