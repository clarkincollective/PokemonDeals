...
// The quantised captured_at as a REAL ISO timestamp (for the timestamptz
// DB column). Same window -> same instant, so it is still idempotent.
//   EVERGREEN  -> a fixed canonical epoch
//   EDITORIAL  -> Monday 00:00Z of the ISO week
//   SHORT      -> that UTC day 00:00Z
//   LIVE       -> the exact verification instant (or the day, as fallback)
export const EVERGREEN_EPOCH_ISO = "2020-01-01T00:00:00.000Z";
export function quantizedCapturedAtIso(series, { now = Date.now(), anchorAt = null } = {}) {
  const key = stableCapturedAt(series, { now, anchorAt });
  if (key === "evergreen") return EVERGREEN_EPOCH_ISO;
  ...
  if (m) {
    ...
    monday.setUTCDate(jan4.getUTCDate() - dow + (w - 1) * 7);
    monday.setUTCHours(0, 0, 0, 0);
    return monday.toISOString();
  }
  if (/^\d{4}-\d{2}-\d{2}$/.test(key)) return `${key}T00:00:00.000Z`;
  const t = Date.parse(key);
  return Number.isFinite(t) ? new Date(t).toISOString() : new Date(now).toISOString();
}
