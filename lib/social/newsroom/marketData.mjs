...
// SOCIAL-CREATIVE-4A (§4) - EXACT_PRINTING_MATTERS only qualifies when the
// pair teaches a real printing/variant lesson (not just a price gap).
import { printingComparisonRelevance } from "../../newsroom/editorial/printingRelevance.mjs";
...
// 17C.7 - a story sample is a savings claim, so it needs an evidenced
// reference for the exact product and condition (lib/dealQuality
// savingsClaimTrusted, which also rejects a listing priced as a different
// product).
import dealQuality from "../../dealQuality.js";
// SOCIAL-LIVE-1 (2026-09-14): a social post's "real asking price" / "standout
// deal" / budget pick is a public claim about a live listing, so every
// deal-sourced fact must ALSO pass the site's full display gate - never a
// quarantined (identity:*), review-held, stale, ended or condition-excluded
// row that the website itself hides. Full rows are selected so the gate
// sees every column it reads.
// SOCIAL-LIVE-2: a World Championship deck / jumbo / proxy copy listed
// against a standard printing's market reference is a different (far
// cheaper) product - e.g. "Solgaleo GX SM104 2019 World Championships"
// matched to the SM104 promo at $16. Social never features that as a deal.
const SPECIALTY_COPY_RX = /world champ|\bwcs?\b|championship deck|\bjumbo\b|oversized?|\bproxy\b|\breplica\b|\bcustom\b|\bfan ?made\b/i;
export function specialtyCopyMismatch(d) {
  const title = String(d?.title ?? "");
  const set = String(d?.card_set ?? "");
  return SPECIALTY_COPY_RX.test(title) && !SPECIALTY_COPY_RX.test(set);
}
const notPreReleaseOrMismatched = (d) => dealQuality.isDisplayableDeal(d) && dealQuality.savingsClaimTrusted(d) && !specialtyCopyMismatch(d);
export const socialDealUsable = notPreReleaseOrMismatched;
...
export const SOLD_OBSERVATIONS_STORED = false;
export async function resolveAskingVsSold({ minDiscount = 0.35, minMarket = 20 } = {}) {
  ...
  const pick = (data ?? []).filter(notPreReleaseOrMismatched).find((d) => /^\d+$/.test(String(d.card_tcgplayer_id ?? "").trim()) && Number(d.total_price_usd) > 0);
  if (!pick) return UNDER("no active BIN deal with a real market reference + canonical printing id");
  // SOCIAL-LIVE-2: the story's premise is completed sales. No per-listing
  // sold observations are stored, and a market reference must not stand in
  // for one, so the series is withheld until genuine sold evidence exists.
  if (!SOLD_OBSERVATIONS_STORED) return UNDER("no stored sold observations - a market reference is not a completed sale");
  ...
  return {
    ...
    data: {
      ...
      // SOCIAL-LIVE-2 (2026-09-14): this used to synthesise three
      // "illustrative recent sold" points (market ref x 0.96 / 1.00 / 1.04)
      // that templates, briefs and charts then showed as completed sales.
      // No sold observations are stored per listing, so there are none:
      // the claim is omitted and the market reference keeps its own label.
      sold_points: [],
...
export async function resolveDealHeroSamples({ n = 8 } = {}) {
  ...
  const { data, error } = await db
    ...
    .limit(300);
  if (error) return UNDER(`db: ${error.message}`);
  const ok = (data ?? []).filter(notPreReleaseOrMismatched).filter(
    (d) =>
...
export async function resolveBidVsTotalSamples({ n = 8 } = {}) {
  ...
  const { data, error } = await db
    ...
    .limit(300);
  if (error) return UNDER(`db: ${error.message}`);
  const ok = (data ?? []).filter(notPreReleaseOrMismatched).filter((d) => {
...
export async function resolveBidVsTotal() {
  ...
  const { data, error } = await db
    ...
    .limit(40);
  if (error) return UNDER(`db: ${error.message}`);
  const pick = (data ?? []).filter(notPreReleaseOrMismatched).find(
...
// SOCIAL-LIVE-1 (2026-09-14): the two market references must describe the
// same condition, and neither may come from a multi-edition product whose
// catalogue image can show a different edition than the priced printing
// (found live: "Base Set (Shadowless)" priced as Unlimited Holofoil /
// Lightly Played against a Near Mint card, while its image shows a 1st
// Edition stamp - an unlike-for-like "2.5x").
export function printingPairComparable(high, low) {
  const cond = (r) => String(r?.market_condition ?? "").trim();
  const editionAmbiguous = (r) => /unlimited|1st edition|first edition/i.test(String(r?.market_printing ?? ""));
  if (!cond(high) || !cond(low)) return { ok: false, reason: "market_condition missing on one side" };
  if (cond(high) !== cond(low)) return { ok: false, reason: `condition mismatch: ${cond(high)} vs ${cond(low)}` };
  if (editionAmbiguous(high) || editionAmbiguous(low)) return { ok: false, reason: "edition-specific price on a multi-edition product (image may show another edition)" };
  return { ok: true };
}
...
export async function resolvePrintingPair({ species = null, minRatio = 1.6 } = {}) {
  ...
  const { data, error } = await db
    .from("card_catalog")
    .select("tcgplayer_id, name, set, card_number, rarity, market_price, market_condition, market_printing, image_url")
    .gt("market_price", 8)
  ...
  for (const sp of wanted) {
    const rows = (bySpecies[sp] ?? [])
      ...
      .slice(0, 40); // price-desc; bound the O(n^2) scan for common species
    if (rows.length < 2) continue;
    // §4 - scan high x low candidates; take the first pair that clears
    // PRINTING_COMPARISON_RELEVANCE_CHECK at a worthwhile spread. The old
    // logic ("most expensive card vs any cheaper different-set card")
    // produced unrelated-era pairs (e.g. two unrelated Umbreons) that
    // taught nothing.
    for (let i = 0; i < rows.length - 1; i++) {
      const high = rows[i];
      for (let j = i + 1; j < rows.length; j++) {
        const low = rows[j];
        const ratio = Number(high.market_price) / Number(low.market_price);
        if (!Number.isFinite(ratio) || ratio < minRatio) continue;
        sawRatioPair = true;
        if (!printingPairComparable(high, low).ok) continue;
        const rel = printingComparisonRelevance(high, low);
        if (rel.verdict !== "MEANINGFUL") continue;
        return {
          ok: true,
          data: {
            species: sp,
            ...
            printing_lesson: rel.lesson,
            relevance: { verdict: rel.verdict, axis: rel.axis, reason: rel.reason },
          },
          cards: [
  ...
  return UNDER(
    sawRatioPair
      ? "candidate printing pairs existed but none passed PRINTING_COMPARISON_RELEVANCE_CHECK (real printing/variant lesson, not a price gap)"
      : "no species with two well-priced printings at a >=1.6x spread"
  );
}
...
export async function resolveMarketShape() {
  ...
  // SOCIAL-LIVE-2: the example card illustrates the statistic itself - a
  // recognisable catalogue printing whose market reference is under $25 -
  // not a live listing (which went stale in the queue and leaned on the
  // withheld asking-vs-sold resolver).
  const feat = await resolveUnder25Example();
  return {
    ok: true,
    data: {
      priced_cards: priced,
      ...
      featured: feat ? feat.data : null,
    },
    cards: feat ? feat.cards : [],
  };
}

export const UNDER_25_EXAMPLE_SPECIES = Object.freeze(["Pikachu", "Eevee", "Umbreon", "Gengar", "Charizard", "Mewtwo", "Snorlax", "Lucario", "Gardevoir", "Sylveon", "Greninja", "Rayquaza", "Lugia", "Dragonite", "Mew"]);

// pure: choose one example printing, rotating weekly among the strongest
export function pickUnder25Example(rows = [], { now = Date.now() } = {}) {
  const ok = rows.filter((r) =>
    /^\d+$/.test(String(r.tcgplayer_id ?? "").trim()) &&
    r.image_url &&
    Number(r.market_price) >= 10 && Number(r.market_price) < 25 &&
    String(r.market_condition ?? "").trim() === "Near Mint" &&
    !/unlimited|1st edition|first edition/i.test(String(r.market_printing ?? "")) &&
    (!r.language || /^en(glish)?$/i.test(String(r.language))) &&
    UNDER_25_EXAMPLE_SPECIES.some((s) => new RegExp(`^${s}\\b`, "i").test(String(r.name ?? "")))
  );
  if (!ok.length) return null;
  const ranked = [...ok].sort((a, b) => Number(b.market_price) - Number(a.market_price) || String(a.tcgplayer_id).localeCompare(String(b.tcgplayer_id))).slice(0, 12);
  const week = Math.floor(now / (7 * 86_400_000));
  return ranked[week % ranked.length];
}

async function resolveUnder25Example() {
  const db = supabaseAdmin();
  const { data, error } = await db
    .from("card_catalog")
    .select("tcgplayer_id,name,set,card_number,rarity,language,image_url,market_price,market_condition,market_printing")
    .gte("market_price", 10)
    .lt("market_price", 25)
    .not("image_url", "is", null)
    .or(UNDER_25_EXAMPLE_SPECIES.map((s) => `name.ilike.${s}*`).join(","))
    .limit(500);
  if (error) return null;
  const row = pickUnder25Example(data ?? []);
  if (!row) return null;
  const id = String(row.tcgplayer_id).trim();
  return {
    data: { kind: "catalog_example", tcgplayerId: id, card_name: row.name, card_set: row.set, card_number: row.card_number ?? null, market_ref_usd: Number(row.market_price), market_condition: row.market_condition, asking_usd: null },
    cards: [{ tcgplayerId: id, catalog: row }],
  };
}
...
export async function resolveThreeUnder({ cap = 25, n = 3 } = {}) {
  ...
  const seen = new Set();
  const picks = [];
  for (const d of (data ?? []).filter(notPreReleaseOrMismatched)) {
...
export async function resolveThreeUnderSamples({ cap = 25, stories = 12, perStory = 3 } = {}) {
  ...
  // SOCIAL-LIVE-2: a caller asking for ONE story (the renderer) needs one,
  // not the three the sample packs need
  if (out.length < Math.min(3, stories)) return UNDER(`only ${out.length} distinct 3-card stories available under $${cap}`);
  return {
    ok: true,
