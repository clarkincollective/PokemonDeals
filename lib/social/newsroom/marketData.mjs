...
// 17C.7 - story samples follow the site's pre-release and expansion-
// identity rules (lib/dealQuality). A listing for an upcoming expansion,
// or one priced as a different product, is never a promoted deal.
import dealQuality from "../../dealQuality.js";
const notPreReleaseOrMismatched = (d) => !dealQuality.preReleaseListing(d) && !dealQuality.listingNamesDifferentExpansion(d);

const UNDER = (detail) => ({ ok: false, reason: "VISUALLY_UNDERPOWERED_DATA", detail });

const DEAL_COLS =
  "id, title, card_name, card_set, card_tcgplayer_id, listing_type, marketplace, " +
