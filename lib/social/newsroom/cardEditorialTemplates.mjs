...
// SOCIAL-CREATIVE-4B.1 (§9) - deterministic text-fit so a hero price /
// saving label can never run off the right rail.
import { fitHeroNumber, fitText } from "../../newsroom/hybrid/textFit.mjs";
...
// SOCIAL-LIVE-2: cents always show two digits ($24.40, never $24.4)
const usd = (n) => {
  const v = Number(n);
  const cents = v < 100 && !Number.isInteger(v);
...
// "Charizard GX - SM195" + number "SM195" -> "Charizard GX" (the number
...
export function dealHero({ target = "ig_45", cardArt = {}, card = {}, priceUsd, marketUsd, discountPct } = {}) {
  ...
  const t = CARD_TARGETS[target] ?? CARD_TARGETS.ig_45;
  // Right rail width = frame inner - card width - row gap. Card is
  // height:H, width:auto; a standard TCG card renders ~0.717 * H wide.
  const cardH = sh ? 1040 : 820;
  const cardW = Math.round(cardH * 0.717);
  const rowGap = sh ? 48 : 44;
  const railW = (t.w - 2 * t.pad) - cardW - rowGap;
  // §9 - fit each hero string to the rail; never crop, never overflow.
  const priceFit = fitHeroNumber(usd(priceUsd), railW, { minPx: sh ? 64 : 52, maxPx: sh ? 172 : 148, tracking: -0.03 });
  const marketFit = fitHeroNumber(usd(marketUsd), railW, { minPx: 40, maxPx: sh ? 78 : 64, tracking: -0.02 });
  const savingTxt = `${savedPct}% below market`;
  // the pill has ~48px horizontal padding; fit its text to rail - padding
  const savingFit = fitText(savingTxt, { boxW: railW - 52, minPx: 24, maxPx: sh ? 52 : 44, maxLines: 2, tracking: -0.01, lineHeight: 1.06 });
  const nameFit = fitText(esc(card.name ?? ""), { boxW: railW, minPx: 26, maxPx: sh ? 44 : 38, maxLines: 2, tracking: -0.02, lineHeight: 1.06 });
  return shell({ target, inner: `
    ...
    <div style="display:flex;gap:${rowGap}px;align-items:center;flex:1;margin-top:20px">
      ${cardImg(cardArt, card.tcgplayerId, `height:${cardH}px;width:auto;flex:0 0 auto`)}
      <div style="flex:1;min-width:0;display:flex;flex-direction:column;gap:${sh ? 8 : 6}px">
        <div style="font-size:${nameFit.px}px;font-weight:800;line-height:1.06;letter-spacing:-0.02em">${esc(card.name)}</div>
        <div style="font-size:${S.fine}px;color:${C.inkSub};margin-bottom:${sh ? 26 : 22}px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(card.set ?? "")}</div>

        <div class="eyebrow" style="color:${C.inkFaint}">Recent market</div>
        <div class="bignum" style="font-size:${marketFit.px}px;color:${C.inkSub};text-decoration:line-through;margin-bottom:${sh ? 20 : 16}px">${usd(marketUsd)}</div>

        <div class="eyebrow" style="color:${C.up}">Deal price</div>
        <div class="bignum" style="font-size:${priceFit.px}px;color:${C.up}">${usd(priceUsd)}</div>

        <div style="margin-top:${sh ? 24 : 20}px;align-self:flex-start;max-width:100%;background:${C.up};color:#04150c;font-weight:800;font-size:${savingFit.px}px;letter-spacing:-0.01em;line-height:1.06;border-radius:12px;padding:10px 24px">${savingTxt}</div>
      </div>
    </div>
...
// SOCIAL-NEWSROOM-14B: the ig_45 card height was tuned much shorter
// (proportionally) than short_916's, so the centered row left a visibly
// larger dead band above/below the card on the 4:5 canvas than on 9:16
// for the exact same layout - a real composition weakness (L5 WATCH,
// core_score ~81, all 5 calls WATCH), not reviewer variance. Bringing the
// two proportions in line closes most of that gap without touching the
// data or the 9:16 composition that already PASSes.
export function askingVsSold({ target = "ig_45", cardArt = {}, card = {}, askingUsd, soldPoints = [], marketRefUsd } = {}) {
  return shell({ target, inner: `
    ...
    <div style="display:flex;gap:40px;align-items:center;flex:1;margin-top:18px">
      ${cardImg(cardArt, card.tcgplayerId, `height:${target === "short_916" ? 860 : 720}px;width:auto;flex:0 0 auto`)}
      <div style="flex:1;display:flex;flex-direction:column;gap:26px">
        <div>
          <div class="eyebrow" style="color:${C.inkFaint}">This listing asks</div>
          <div class="mono" style="font-size:${target === "short_916" ? 120 : 100}px;font-weight:800;line-height:.9;color:${C.inkSub}">${usd(askingUsd)}</div>
        </div>
        <div>
          <div class="eyebrow" style="color:${C.up}">Market reference</div>
          <div class="mono" style="font-size:${target === "short_916" ? 84 : 68}px;font-weight:800;color:${C.up};line-height:1">${usd(marketRefUsd)}</div>
        </div>
        ${Array.isArray(soldPoints) && soldPoints.length ? `<div>
          <div class="eyebrow" style="color:${C.inkFaint}">Recent sold</div>
          <div class="mono" style="font-size:${target === "short_916" ? 60 : 48}px;font-weight:800;color:${C.inkSub};line-height:1">${soldPoints.map((p) => usd(p)).join("  ")}</div>
        </div>` : ""}
        <div style="font-size:${S.fine + 4}px;color:${C.inkSub};line-height:1.35;max-width:24ch">Check an asking price against the market reference &mdash; not the highest ask.</div>
      </div>
    </div>
...
export function marketShape({ target = "ig_45", cardArt = {}, pricedCards, under25Pct, over100Pct, featured = null } = {}) {
  ...
  return shell({ target, inner: `
    ...
    <h1 style="font-size:${target === "short_916" ? 64 : 56}px;font-weight:800;line-height:1.04;margin-top:14px;max-width:24ch;text-wrap:balance">Most Pokemon cards cost less than people think.</h1>
    <div style="display:flex;align-items:center;gap:24px;margin-top:22px">
      <div class="bignum" style="font-size:${target === "short_916" ? 150 : 136}px;line-height:.85;color:${C.up};flex:0 0 auto">${under25Pct}%</div>
      <div style="font-size:${S.body + 2}px;line-height:1.25;color:${C.ink};max-width:17ch">of ${Number(pricedCards).toLocaleString("en-US")} tracked singles have a market price <span style="color:${C.up};font-weight:800">under $25</span></div>
    </div>
    <div style="margin-top:36px">
      <div style="display:flex;height:64px;border-radius:14px;overflow:hidden;border:1px solid ${C.hair}">
    ...
    ${featured
      ? `<div style="display:flex;gap:30px;align-items:center;background:${C.surface};border:1px solid ${C.hair};border-radius:16px;padding:24px">
          ${cardImg(cardArt, featured.tcgplayerId, `height:${target === "short_916" ? 560 : 470}px;width:auto;flex:0 0 auto`)}
          ${featured.asking_usd == null
            ? `<div style="flex:1;min-width:0">
            <div class="eyebrow" style="color:${C.up}">One example under $25</div>
            <div style="font-size:${S.title}px;font-weight:800;margin-top:8px;line-height:1.05">${esc(stripNumberSuffix(featured.card_name, featured.card_number))}</div>
            <div style="font-size:${S.fine + 2}px;color:${C.inkSub};margin-top:6px">${esc(featured.card_set ?? "")}${featured.card_number ? ` &middot; ${esc(featured.card_number)}` : ""}</div>
            <div class="bignum" style="font-size:${S.metric}px;color:${C.up};margin-top:10px">${usd(featured.market_ref_usd)}</div>
            <div style="font-size:${S.fine}px;color:${C.inkFaint};margin-top:4px">market reference${featured.market_condition ? ` &middot; ${esc(featured.market_condition)}` : ""}</div>
          </div>`
            : `<div style="flex:1">
    ...
    <div style="display:flex;justify-content:space-between;align-items:baseline;margin-top:${target === "short_916" ? 26 : 20}px;font-size:${S.fine}px;color:${C.inkFaint}">
      <span>${featured ? "Market references &middot; catalogue snapshot" : ""}</span><span style="color:${C.ink};font-weight:800">pokemondealfinder.com</span>
    </div>
  ` });
}
...
export function threeUp({ target = "ig_45", cardArt = {}, cap, items = [] } = {}) {
  ...
  // SOCIAL-LIVE-2: size each column from the frame. A fixed card HEIGHT
  // (480px -> ~344px wide x3 + gaps) overran the 928px ig_45 frame, so the
  // third card and its price were cut off the right edge.
  const t = CARD_TARGETS[target] ?? CARD_TARGETS.ig_45;
  const colGap = sh ? 36 : 28;
  const cellW = Math.floor((t.w - 2 * t.pad - 2 * colGap) / 3);
  const cardH = Math.round(cellW / 0.717);
  const cell = (it) => {
    const priceFit = fitHeroNumber(usd(it.price_usd), cellW, { minPx: 40, maxPx: sh ? 84 : 72, tracking: -0.03 });
    return `<div style="flex:0 0 ${cellW}px;width:${cellW}px;min-width:0;display:flex;flex-direction:column;align-items:center;gap:${sh ? 16 : 12}px">
      ${cardImg(cardArt, it.tcgplayerId, `width:${cellW}px;height:${cardH}px;object-fit:contain;border-bottom:4px solid ${C.up}`)}
      <div style="font-size:${sh ? 28 : 24}px;font-weight:700;letter-spacing:-0.01em;color:${C.ink};text-align:center;max-width:100%;line-height:1.2">${esc(it.card_name)}</div>
      <div class="bignum" style="font-size:${priceFit.px}px;color:${C.up};white-space:nowrap">${usd(it.price_usd)}</div>
  ...
  return shell({ target, inner: `
    ...
    <div style="display:flex;gap:${colGap}px;align-items:center;justify-content:center;flex:1;margin-top:${sh ? 30 : 24}px">${shown.map(cell).join("")}</div>
    <div style="border-top:1px solid ${C.hair};padding-top:${sh ? 22 : 18}px;margin-bottom:${sh ? 22 : 18}px;font-size:${S.fine + 2}px;line-height:1.35;color:${C.inkSub}">Live Buy It Now totals, each checked against that exact printing&rsquo;s market reference. Listings can sell at any time.</div>
    ${ctaPill("Find more deals at pokemondealfinder.com")}
  ` });
}
