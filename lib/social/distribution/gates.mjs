...
export function readinessGates(args) {
  ...
  // The "live switches" the owner flips at launch time - not structural
  // readiness concerns. A batch can be a valid, approvable launch plan
  // before these are set; they are all still enforced at send.
  const IGNORE = new Set(["publish_switch", "live_mode", "owner_approval", "epn_compliance"]);
