...
export function runAllGates({ row, variant, flags, providerConfigured, ledger = [], force = false } = {}) {
  ...
  // G10 - the media is present. A text-only X post has no file; the
  // caption gate covers it. Everything else must have real files on disk.
  const textOnly = rowMedia.kind === "text_only";
  gates.push(
    gate(
      "media_present",
      textOnly ? String(row?.caption ?? "").trim().length > 0 : rowMedia.filesExist === true && (rowMedia.files?.length ?? 0) > 0,
      textOnly ? "text-only post (no media file expected)" : `files=${(rowMedia.files ?? []).length}, exist=${rowMedia.filesExist}`
    )
  );
