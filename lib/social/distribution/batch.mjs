...
export function approvalChecksum(batch) {
  const placements = [...(batch.placements ?? [])]
    .map((p) => ({
      ...
      // 13E.10A - the conversion experiment is part of what the owner signs
      // off; any post-approval change to it invalidates the checksum.
      experiment_id: p.experiment?.experiment_id ?? null,
      variant_id: p.experiment?.variant_id ?? null,
      hook_variant: p.experiment?.hook_variant ?? null,
      cta_variant: p.experiment?.cta_variant ?? null,
    }))
...
export function buildBatch({ content_id, rows, sendOrder = DEFAULT_SEND_ORDER, scheduleMode = "PUBLISH_NOW", scheduledFor = null, sourceCommit = null }) {
  ...
  const batch = {
    ...
    placements: ordered.map((r) => ({
      ...
      // 13E.10A - frozen conversion experiment (deterministic; null when none)
      experiment: {
        experiment_id: r.experiment_id ?? null,
        variant_id: r.variant_id ?? r.experiment_variant ?? null,
        hook_variant: r.hook_variant ?? null,
        cta_variant: r.cta_variant ?? null,
        hypothesis: r.experiment_hypothesis ?? null,
      },
    })),
