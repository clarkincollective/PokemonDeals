...
import { attributedCtaUrl } from "./attribution.mjs";
import { RIGHTS_STATE } from "../rights.mjs";

// 13E.7A - the URL embedded in X post text / YouTube description must carry
// first-party attribution so a click is traceable to CONTENT -> PLATFORM
// POST. Deterministic; falls back to the bare URL if it can't be attributed.
function attributedFor(platform, baseUrl, { contentGoal, contentId, contentFamily }) {
  if (!baseUrl) return baseUrl;
  try {
    return attributedCtaUrl({ baseUrl, platform, contentGoal, contentId, contentFamily });
  } catch {
    return baseUrl;
  }
}
...
export function readDailyArtifact(contentType) {
  ...
  const attrCtx = { contentGoal, contentId: ident.content_id, contentFamily: family };
  const xFacts = { ...facts, ctaUrl: attributedFor("x_post", ctaUrl, attrCtx) };
  const ytFacts = { ...facts, ctaUrl: attributedFor("youtube_short", ctaUrl, attrCtx) };
  ...
  return {
    ...
    // 13E.7A §2 identity contract - reserved; null until an experiment sets them.
    hook_variant: ident.hook_variant ?? null,
    cta_variant: ident.cta_variant ?? null,
    stale,
    variants: [
      {
        creative_variant: "A",
        ...
        facts,
        x: xPostText(xFacts),
        youtube: youtubeShortsMeta(ytFacts),
...
export function readVideoArtifacts() {
  ...
  for (const fam of m.families ?? []) {
    ...
    for (const [cut, v] of Object.entries(fam.platforms ?? {})) {
      ...
      const vAttrCtx = { contentGoal: v.content_goal ?? null, contentId: v.content_id ?? null, contentFamily: fam.family };
      const xFacts = { ...facts, ctaUrl: attributedFor("x_post", ctaUrl, vAttrCtx) };
      const ytFacts = { ...facts, ctaUrl: attributedFor("youtube_short", ctaUrl, vAttrCtx) };
      variants.push({
