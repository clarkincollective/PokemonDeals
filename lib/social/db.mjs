...
const SELECT_COLUMNS =
  ...
  // SEO-4 (16 Sep 2026): the stored reference evidence savingsClaimTrusted
  // reads. A social post that states a saving must be able to evidence it,
  // and a select that omits these columns makes EVERY row untrusted - the
  // same silent failure the catalogue snapshot hit when it omitted them
  // (lib/deals.js SAVINGS_EVIDENCE_COLUMNS). Kept in sync with
  // lib/dealQuality storedReferenceEvidence.
  "reference_product_id, reference_amount, reference_currency, reference_fx_rate, " +
  "reference_fx_asof, reference_observed_at, reference_condition, reference_printing, " +
  "reference_grader, reference_grade, " +
...
// Phase 13E.2.1 - one bounded read of card_catalog rows for a SMALL,
// explicit set of tcgplayer ids (only the ids of the day's selected
// social candidates - never a catalogue-wide scan). Used by
// lib/social/cardArtwork.mjs to cross-check that the canonical image we
// are about to composite really is the deal's exact matched printing
// (name / set / collector number). Read-only.
export async function fetchCatalogRows(tcgplayerIds = []) {
  const ids = [...new Set((tcgplayerIds ?? []).map((v) => String(v).trim()).filter(Boolean))];
  if (!ids.length) return { rows: [], byId: {}, error: null };
  const db = supabaseAdmin();
  const { data, error } = await db
    .from("card_catalog")
    .select("tcgplayer_id, name, set, card_number, rarity, image_url, market_price")
    .in("tcgplayer_id", ids);
  if (error) return { rows: [], byId: {}, error: error.message };
  const byId = {};
  for (const r of data ?? []) byId[String(r.tcgplayer_id).trim()] = r;
  return { rows: data ?? [], byId, error: null };
}
