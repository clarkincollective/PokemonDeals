...
function shell(timeline, bodyHtml, { background = null } = {}) {
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  /* 13E.11A: the hook is the hero of a 9:16 frame - bounded to <= 3 lines
     so a long hook can never wall the card. */
  .vhook{font-weight:800;letter-spacing:-0.04em;line-height:1.02;text-transform:uppercase;color:${C.ink};text-wrap:balance;max-height:3.2em;overflow:hidden}
...
function dealDropDoc(timeline, content, { background, cardArtwork }) {
  ...
  const body = `<div class="safe">
    ${zBrand()}
    <div class="vhook" style="font-size:78px;margin-top:26px;${enter("mask_up", cue.hook.start, cue.hook.dur)}">${esc(content.hookText)}</div>
    ${cardUrl
      ? `<div class="vband" style="flex-basis:560px;margin:16px 0">
           <div class="vcard" style="--cah:560px;max-width:74%;${enter("slide_up", cue.card.start, cue.card.dur)}">${cardFrameV(cardUrl, { rotationDeg: geo.rotationDeg })}</div>
         </div>`
      : `<div class="vspring"></div>`}
    <div class="vmetric" style="font-size:64px;color:${acc};${enter("scale_in", cue.metric.start, cue.metric.dur)}">${esc(content.metricValue)}</div>
...
function marketMoverDoc(timeline, content, { background, cardArtwork }) {
  const cue = timeline.cues;
  // 13E.11A: no directional colour on a Market Mover - a price move is not
  // a saving (§9); the chart is a single restrained neutral line (§15/§16).
  const acc = C.inkSub;
  const cardUrl = cardArtwork?.card?.fileUrl;
  ...
  const body = `<div class="safe">
    ...
    <div class="vmetric" style="font-size:104px;color:${C.ink};margin-top:10px;${enter("scale_in", cue.move.start, cue.move.dur)}">${esc(content.moveValue)}</div>
    <div class="vmetric-label" style="${enter("rise", cue.move.start + 120, 400)}">${esc(content.periodLabel)}</div>
    <div class="vband" style="flex-basis:480px;gap:40px;margin:18px 0">
      ${cardUrl ? `<div class="vcard" style="--cah:460px;flex:0 0 42%;display:flex;align-items:center;justify-content:center;${enter("slide_right", cue.card.start, cue.card.dur)}">${cardFrameV(cardUrl, { rotationDeg: -3 })}</div>` : ""}
...
function brandDoc(timeline, content, { background, screenshot }) {
  ...
  const bh = String(timeline.facts.hook_text || "Pokemon cards listed below recent market prices").split(" ");
  const body = `<div class="safe">
    ${zBrand()}
