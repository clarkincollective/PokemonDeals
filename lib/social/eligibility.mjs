...
// 13D.4.1: human-readable, still exact (not vague/relative-only) - a raw
// ISO string ("2026-09-05T01:26:56.666Z") reads as a developer log line,
// not collector-market copy. `checkedAt` (the payload/audit field) stays
// full ISO precision; only this display LABEL is reformatted.
...
function humanChecked(at) {
  return `${UTC_FORMAT.format(at)} UTC`;
}
...
export function socialFreshnessLine(row, { at = new Date() } = {}) {
  ...
  if (!Number.isFinite(hrs) || hrs > SOCIAL_FRESHNESS_MAX_AGE_HOURS) {
    ...
    return { label: "Verification age outside social freshness threshold - not eligible for preview.", shortLabel: "Not eligible", checkedAt: null };
  }
  const checkedAt = at.toISOString();
  return {
    label: `Checked ${humanChecked(at)}. Availability can change.`,
    shortLabel: `Checked ${humanChecked(at)}`,
    checkedAt,
  };
}
