...
function candidateCard(entry) {
  const { family, payload, hashtags = [], captions = {}, reasonSelected = "", thumb } = entry;
  ...
  <section class="candidate" data-state-key="${esc(stateKey)}">
    ...
    <div class="cand-body">
      <img class="thumb" src="${esc(thumb)}" alt="${esc(label)} creative preview" />
      <div class="details">
...
<style>
  ...
  .thumb { width: 280px; height: 350px; object-fit: cover; border-radius: 12px; border: 1px solid #E4E4E7; flex-shrink: 0; }
...
<body>
  ...
  <div class="publish-banner">REVIEW ONLY — PUBLISHING DISABLED. Nothing on this page is published, scheduled, or connected to Instagram, TikTok, or any platform. Approve/Reject below is local workflow state only; the owner publishes selected content manually.</div>
