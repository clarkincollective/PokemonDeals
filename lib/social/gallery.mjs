...
function warningFlags(payload) {
  ...
  const flags = [...fails];
  if (payload.rights_state.card_image !== "NOT_CLEARED") flags.push("card_image rights state changed - stop");
  return flags;
}

// Phase 13E.2 - the side-by-side creative block. A = deterministic Mode B
// (always). B = the same overlay over an approved generated background
// (only when one is in rotation for this family). The AI version is never
// pre-selected - the owner picks A or B below.
function mediaBlock(entry) {
  const { family, thumb, enhanced } = entry;
  ...
  if (!enhanced) return `<div class="media media-solo">${a}</div>`;
  const b = `
      <figure class="shot">
        <img class="thumb" src="${esc(enhanced.thumb)}" alt="${esc(label)} — background-enhanced creative" />
        <figcaption><strong>B</strong> — background-enhanced<br><span class="note">asset ${esc(enhanced.assetId)} · ${esc(enhanced.style ?? "")}/${esc(enhanced.zone ?? "")}</span></figcaption>
      </figure>`;
  return `<div class="media">${a}${b}</div>`;
}

function assetReviewBlock(entry) {
  const { family, payload, enhanced } = entry;
  if (!enhanced) return "";
  const dateKey = String(payload.generated_at).slice(0, 10);
  const assetKey = `pdf-social-asset-review::${dateKey}::${family}`;
  const rows = ASSET_REVIEW_CHECKS.map(
    ([k, lbl]) => `
        <div class="acheck" data-check="${k}">
          <span class="acheck-label">${lbl}</span>
          <button type="button" class="abtn" data-v="PASS">PASS</button>
          <button type="button" class="abtn" data-v="REJECT">REJECT</button>
        </div>`
  ).join("");
  return `
        <h3>Generated background — human review <span class="note">(SS17 · local only)</span></h3>
        <p class="note">Source asset <code>${esc(enhanced.sourceFile ?? enhanced.assetId)}</code>. Image generation received no card / price / listing / user data — it is an evergreen background; every fact on the slide is the deterministic overlay.</p>
        <div class="asset-checks" data-asset-key="${esc(assetKey)}">
          ${rows}
          <div class="apublish">
            <span class="acheck-label">PUBLISH WHICH VERSION?</span>
            <button type="button" class="abtn" data-pub="A">A — deterministic</button>
            <button type="button" class="abtn" data-pub="B">B — enhanced</button>
          </div>
        </div>
        <p class="note">The AI-enhanced version is never auto-preferred. Only assets whose 5 checks are all PASS in the manifest enter <code>social:daily</code> rotation; the toggles above are your per-post record.</p>`;
}
  ...
  <section class="candidate" data-state-key="${esc(stateKey)}">
    ...
    <div class="cand-body">
      ...
      <div class="details">
        ...
        <p class="note">Seller-image evidence: <code>${esc(payload.rights_state.ebay_seller_images)}</code> — Mode B creative, no card/listing image is used.</p>
...
<style>
  ...
  .acheck, .apublish { display: flex; align-items: center; gap: 8px; font-size: 12px; }
...
<body>
  ...
  <div class="publish-banner">REVIEW ONLY — PUBLISHING DISABLED. Nothing on this page is published, scheduled, or connected to Instagram, TikTok, or any platform. Approve/Reject below is local workflow state only; the owner publishes selected content manually. Where a version B is shown it is the same deterministic overlay over an approved, evergreen, AI-generated background — never auto-preferred; the owner chooses A or B.</div>
...
(function () {
  ...
  // Phase 13E.2 - the 5 generated-background checks + A/B publish pick.
  document.querySelectorAll(".asset-checks").forEach(function (box) {
    ...
    function paint() {
      ...
      box.querySelectorAll(".apublish .abtn").forEach(function (b) {
