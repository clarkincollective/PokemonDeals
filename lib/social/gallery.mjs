...
import { familyForContentType, resolveCreativeSpec, FAMILY_SPECS } from "./creativeSpec.mjs";

const FAMILY_LABELS = {
  "deal-of-day": "Deal Drop · Deal of the Day",
  "just-found": "Deal Drop · Just Found",
  "market-mover": "Market Mover",
  "pokemon-spotlight": "Hook Carousel · Pokemon Spotlight",
  "set-spotlight": "Hook Carousel · Set Spotlight",
  "market-snapshot": "Market Snapshot",
  "best-deals": "Hook Carousel · Best Deals Found Today",
};

// The 13E.3 creative family + its composition, resolved from the payload's
// content type, for the review header.
function creativeMeta(payload) {
  const fam = familyForContentType(payload.content_type);
  if (!fam) return null;
  try {
    const spec = resolveCreativeSpec({ family: fam, variant: "A" });
    return { family: FAMILY_SPECS[fam].label, composition: spec.compositionKey, oneSecond: spec.oneSecondRead.join(" → ") };
  } catch {
    return { family: FAMILY_SPECS[fam]?.label ?? fam, composition: "—", oneSecond: "—" };
  }
}
...
function candidateCard(entry) {
  ...
  const cm = creativeMeta(payload);
  return `
  <section class="candidate" data-state-key="${esc(stateKey)}">
    <div class="cand-header">
      <div><span class="cand-family">${esc(label)}</span> <span class="cand-type">${esc(payload.content_type)}</span>${
        cm ? `<span class="cand-comp">${esc(cm.family)} · composition <code>${esc(cm.composition)}</code> · 1-sec read: ${esc(cm.oneSecond)}</span>` : ""
      }</div>
...
<style>
  ...
  .cand-comp { display: block; color: #52525B; font-size: 11px; margin-top: 3px; }
