...
export const PROMPT_SPEC_VERSION = "13e2-v1";
...
export const SHARED_NEGATIVE =
  "MUST NOT contain: any Poke Ball or sphere-with-band shape; the Pokemon logo or wordmark; any Pokemon creature, character, or recognizable Pokemon silhouette; " +
  ...
  "Generic card-like objects are allowed ONLY as clearly original blank forms: a plain rounded rectangle, an anonymous clear slab shape, a fanned stack of blank cards, an out-of-focus abstract foil surface - with NO readable markings.";
...
const STYLE_CLAUSES = Object.freeze({
  ...
  collector_desk:
    "STYLE - Collector Desk: an original overhead or three-quarter view of a tidy research desk - neutral mat, blank card sleeves, a plain slab shape, " +
    "a simple line-art magnifying glass, a folded neutral cloth - all generic props, soft daylight, shallow depth of field, nothing branded, nothing readable.",
  abstract_market:
    "STYLE - Abstract Market: geometric abstraction of price movement and signal - layered translucent rectangles like stacked cards, ascending step lines, " +
    "scatter dots, a faint scanner sweep, gentle parallax depth, red/charcoal/white only.",
});
...
const FAMILY_SPEC = Object.freeze({
  deal_intelligence: {
    ...
    defaultZone: "C",
    motif:
      "MOTIF - Deal Intelligence: a single blank card-shaped rectangle catching soft light, sitting slightly below an implied reference line, " +
      "a subtle downward price step and a thin bracket suggesting a gap, one confident red highlight. Sense of a spotted opportunity, measured not hyped.",
  },
  just_found: {
    label: "Just Found",
    usedBy: "just-found",
    ...
    defaultZone: "A",
    motif:
      "MOTIF - Just Found: a fresh blank card sliding into frame, a faint scanner sweep line just passing over it, a soft 'new' pulse ring, " +
      "a light motion trail. Sense of something just surfaced by a monitor, clean and immediate.",
  },
  market_watch: {
    label: "Market Watch",
    ...
    defaultZone: "D",
    motif:
      "MOTIF - Market Watch: an aggregate view - many faint blank card rectangles arranged as a quiet grid or histogram, a calm median line running across, " +
      "a few cells lifted in red to imply outliers. Sense of surveying a whole market at a glance.",
  },
  pokemon_watch: {
    label: "Pokemon Watch",
    ...
    defaultZone: "B",
    motif:
      "MOTIF - Pokemon Watch: a small cluster of blank cards of one implied 'subject' held together by a soft focus ring or bracket, a line-art magnifying glass hovering near, " +
      "the rest of the desk falling away. Sense of tracking one subject across many listings. NO actual creature, NO silhouette.",
  },
  set_watch: {
    label: "Set Watch",
    ...
    defaultZone: "B",
    motif:
      "MOTIF - Set Watch: a neat fanned arc or shelf row of blank cards of uniform size implying a complete run, one card gently pulled forward, " +
      "a faint completion progress arc. Sense of watching a whole set. NO set symbols, NO numbering.",
  },
  raw_vs_graded: {
    label: "Raw vs Graded",
    ...
    defaultZone: "C",
    motif:
      "MOTIF - Raw vs Graded: two blank card-shaped forms side by side - one a plain open card, one sealed inside an anonymous clear slab shape (no label text, no grade number, no company mark) - " +
      "a thin divider between them, balanced neutral lighting. Sense of a like-for-like comparison.",
  },
  auction_watch: {
    label: "Auction Watch",
    ...
    defaultZone: "D",
    motif:
      "MOTIF - Auction Watch: a blank card under a soft countdown arc or thinning ring of light, a gentle pulse, a faint ascending bid-step line, " +
      "restrained tension. NO clock numerals, NO gavel, NO hammer.",
  },
  collector_education: {
    ...
    defaultZone: "A",
    motif:
      "MOTIF - Collector Education: an exploded tidy diagram of generic blank card anatomy - corners, edges, surface, an anonymous slab - arranged as calm labelled callout positions (positions only, NO label text), " +
      "soft daylight, textbook clarity.",
  },
  trust_verification: {
    ...
    defaultZone: "C",
    motif:
      "MOTIF - Trust / Verification: a blank card passing through a soft check ring or a clean shield-less tick motif, a faint verified glow, a precise crosshair align mark, " +
      "calm reassurance. NO real checkmark-brand styling, NO badge logos.",
  },
  search_discovery: {
    ...
    defaultZone: "B",
    motif:
      "MOTIF - Search / Discovery: a large original line-art magnifying glass sweeping across a faint field of blank card rectangles, one card resolving into sharp focus inside the lens, " +
      "a light scanner grid. Sense of finding the one that matters.",
  },
});
...
const LIVE_DATA_SIGNATURES = [
  ...
  /\bcard_name\b|\bcard_set\b|\bseller\b|\bsearch query\b|\bdistinct_id\b/i,
