...
// 13e3-v1: the brand direction moved to a PREMIUM DARK foundation
// (charcoal / near-black surfaces, one red accent, the real card supplies
// the colour). Every style clause below now describes a dark ground; the
// data-free / no-card / no-text / reserved-hero-zone guarantees are
// unchanged from 13e2.1.
export const PROMPT_SPEC_VERSION = "13e3-v1";
...
const SHARED_INTRO =
  ...
  "Premium, restrained, editorial, PREMIUM DARK. Portrait 4:5 aspect ratio (1080x1350), designed as a social-media background that will have bright text and data laid over it later.";

const SHARED_BRAND =
  "Ground is a PREMIUM DARK charcoal / near-black surface (#0B0B0D to #161619) - deep, matte, not flat black. " +
  "Brand palette ONLY on that dark ground: one restrained signal red accent (#F0322E), cool near-white highlights, and soft dark greys. " +
  "Bright white is a small accent, never a field. Feel: modern marketplace intelligence at night, a collector's research desk under a single lamp, financial-data calm. " +
  "NOT childish, NOT a toy advert, NOT a gaming meme, NOT casino, NOT crypto-trading, NOT a loud discount blowout, NOT a bright/white background.";
...
const STYLE_CLAUSES = Object.freeze({
  clean_editorial:
    "STYLE - Dark Editorial: a deep charcoal / near-black ground with generous negative space, one or two precise red accents, magazine-cover minimalism, " +
    "a single crisp geometric focal element, thin luminous hairline rules, soft directional light from the top-left. Restrained, expensive, quiet.",
  dark_market_intelligence:
    ...
    "analytics-dashboard calm at night, deep matte finish, one restrained bright accent.",
  collector_desk:
    "STYLE - Collector Desk at Night: an original overhead or three-quarter view of a tidy research desk lit by a single warm lamp against a dark room - dark desk mat, a simple line-art magnifying glass, a folded dark cloth, " +
    "a soft pool of lamplight, shallow depth of field, deep shadows. Generic desk props only - NO cards, NO sleeves-with-cards, NO slabs. Nothing branded, nothing readable.",
  abstract_market:
    "STYLE - Abstract Market: geometric abstraction of price movement and signal on a dark ground - ascending step lines, a calm baseline, scatter dots, a faint scanner sweep, translucent overlapping dark planes and soft red light shafts, " +
    "gentle parallax depth, red / charcoal / near-white only. NO card shapes, NO rectangles that could read as cards.",
});
