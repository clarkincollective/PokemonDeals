...
import { assignExperimentForPlacement } from "../experiments/index.mjs";
...
export function buildPlan({
  ...
  for (const pc of placementCands) {
    ...
    // 13E.10A - deterministic conversion-experiment assignment. Runs AFTER
    // every quality / freshness / rights / QA / eligibility decision above
    // and only chooses among already-eligible CREATIVE variants (§19).
    const exp = assignExperimentForPlacement(c, pc.platform);
    if (exp.experiment_id) whyBits.push(`experiment ${exp.experiment_id} → ${exp.variant_id}`);
    ...
    entries.push({
      ...
      // 13E.10A conversion experiment (deterministic; null when none applies)
      experiment_id: exp.experiment_id,
      variant_id: exp.variant_id,
      hook_variant: exp.hook_variant,
      cta_variant: exp.cta_variant,
      experiment_hypothesis: exp.hypothesis,
      experiment_dimension: exp.dimension,
      simulate,
    });
  }
