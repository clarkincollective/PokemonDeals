...
// available via the older `social:preview` command. `market-mover`
// (Phase 13E.3) is resolved by scripts/socialDaily.mjs BEFORE the batch
// (it needs a price_history read per candidate, and dailyMix stays pure)
// and appended if a real confident movement exists - it is listed here so
// `social:daily record market-mover` validates.
export const DAILY_FAMILIES = ["deal-of-day", "just-found", "pokemon-spotlight", "set-spotlight", "market-snapshot", "market-mover"];

// The families buildDailyBatch actually resolves from the pool (pure, no
// I/O). market-mover is intentionally excluded here - it is resolved
// asynchronously in scripts/socialDaily.mjs and appended.
export const MIX_FAMILIES = ["deal-of-day", "just-found", "pokemon-spotlight", "set-spotlight", "market-snapshot"];
...
// The rights gate for a daily candidate. Every family always renders a
// Version A (deterministic Mode B, no card image) - the only hard
// requirement there is that publishing stays a local-review flow.
// market_snapshot additionally needs PPT social data cleared. The
// canonical-card-artwork clearance (card_image, 13E.2.1) is NOT gated
// here: it only unlocks the optional Version C, which
// scripts/socialDaily.mjs resolves per-candidate and fails closed on if
// the exact-printing image can't be verified. GenAI state is irrelevant:
// nothing here touches an LLM.
...
export function buildDailyBatch(rows, { history = [], now = Date.now() } = {}) {
  ...
  return {
    selected,
    rejected,
    considered: MIX_FAMILIES.length,
