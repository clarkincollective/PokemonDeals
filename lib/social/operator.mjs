...
// 13E.10A §21 - experiment visibility rows for the dashboard. PURE.
//   planEntries - today's planner entries (carry experiment_id / variant_id)
//   reports     - reportAll() output (one per experiment; carries state/leader)
// Before anything is published a report's state is "NOT_AVAILABLE_YET" -
// never a fake 0. Rows are ordered by experiment_id for stable output.
export function experimentDashboardRows(planEntries = [], reports = []) {
  const repById = new Map(reports.map((r) => [r.experiment_id, r]));
  const plannedByExp = {};
  for (const e of planEntries) {
    if (!e.experiment_id) continue;
    const b = (plannedByExp[e.experiment_id] ??= { A: 0, B: 0 });
    if (e.variant_id === "A") b.A += 1;
    else if (e.variant_id === "B") b.B += 1;
  }
  const ids = [...new Set([...Object.keys(plannedByExp), ...repById.keys()])].sort();
  return ids.map((id) => {
    const rep = repById.get(id) ?? null;
    const planned = plannedByExp[id] ?? { A: 0, B: 0 };
    return {
      experiment_id: id,
      dimension: rep?.dimension ?? null,
      planned_today: { A: planned.A, B: planned.B },
      published: { A: rep?.variants?.A?.n ?? 0, B: rep?.variants?.B?.n ?? 0 },
      state: rep?.state ?? "NOT_AVAILABLE_YET",
      leader: rep?.leader ?? "n/a",
    };
  });
}
