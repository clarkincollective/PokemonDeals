...
// The 13B.1 tiebreak for one collector number across several sets is now
// applied per-row in `cleanFirst` (resolveSearchIntent) - era match, then
// plain-set-beats-"(Shadowless)", then cleanliness, then older set, then
// a stable tcgplayer_id tiebreak.
...
export async function resolveSearchIntent(intent, { lookup }) {
  ...
  // 13B.6.3 - score each row's sort keys ONCE (cleanliness/paren/era/rank
  // are pure per-row) instead of recomputing them inside every O(n log n)
  // comparison. Byte-identical ordering to the old makeComparator sort -
  // a common-species list (~500 rows) went from ~8k cleanliness() calls
  // to ~500.
  const wantEra = intent.era || null;
  const cleanFirst = (rows) => {
    const scored = [];
    for (const r of rows) {
      if (!catalogCardResolvable(r)) continue;
      scored.push({
        r,
        era: wantEra ? (eraForSetName(r.set) === wantEra ? 0 : 1) : 0,
        paren: hasParenQualifier(r.set) ? 1 : 0,
        clean: cleanliness(r, wantName),
        rank: setReleaseRank(r.set),
        tid: String(r.tcgplayer_id),
      });
    }
    scored.sort(
      (a, b) =>
