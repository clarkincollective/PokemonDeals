...
export async function resolveSearchIntent(intent, { lookup }) {
  ...
  if (s.set && numVariants.length) {
    const rows = cleanFirst(await lookup.bySetAndNumber(s.set, numVariants, { language }));
    if (rows.length === 1) return { ...applyExact(intent, rows[0], { confidence: "high", resolvedVia: "set+number" }), candidates: rows };
    if (rows.length > 1) {
      intent.ambiguities.push(`number ${s.collector_number} in set ${s.set} has ${rows.length} rows`);
      return { ...applyExact(intent, rows[0], { confidence: "high", resolvedVia: "set+number" }), candidates: rows.slice(0, 8) };
    }
  }

  // ---- 2. collector number + name/species (no confident set) -------
  if (numVariants.length) {
    let rows = await lookup.byNumber(numVariants, { language });
    if (wantName) {
      const wn = norm(wantName);
      const filtered = rows.filter(
        (r) => norm(r.name).includes(wn) || (s.species && r.species === s.species)
      );
      if (filtered.length) rows = filtered;
    }
    rows = cleanFirst(rows);
    if (rows.length >= 1) {
      const top = rows[0];
  ...
  if (s.set && wantName && !numVariants.length) {
    const rows = (await lookup.byName(wantName, { language, limit: 60 }))
      .filter((r) => norm(r.set) === norm(s.set) && slugifySet(r.name) === slugifySet(wantName));
  ...
  // ---- 4. exact card name (unique) ------------------------------
  if (wantName && !numVariants.length) {
  ...
  if (s.species) {
    ...
    if (rows.length === 1) {
      return { ...applyExact(intent, rows[0], { confidence: "high", resolvedVia: "species_single" }), candidates: rows.map(toExact) };
    }
    s.kind = "species";
