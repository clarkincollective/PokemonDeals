...
export function organizationNode({ sameAs = [] } = {}) {
  return {
    ...
    url: `${SITE_URL}/`,
    description:
      "Pokemon Deal Finder is a free, independent tool that lists live eBay Pokemon card listings, after card-identity, condition, availability and image-authenticity checks. A below-market saving is claimed only where the printing and condition are matched to a documented market reference. It holds no stock and runs no paid placement.",
    logo: { "@id": IDS.logo },
    image: { "@id": IDS.logo },
